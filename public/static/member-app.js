...
function openSidebar() {const s = document.getElementById('sidebar'),o = document.getElementById('sidebar-overlay');if (s) s.classList.remove('-translate-x-full');if (o) {o.classList.remove('hidden');document.body.style.overflow = 'hidden';}}function closeSidebar() {const s = document.getElementById('sidebar'),o = document.getElementById('sidebar-overlay');if (s) s.classList.add('-translate-x-full');if (o) {o.classList.add('hidden');document.body.style.overflow = '';}}function toggleSidebar() {const isMobile = window.innerWidth < 768;if (isMobile) {const s = document.getElementById('sidebar');const isOpen = s && !s.classList.contains('-translate-x-full');if (isOpen) {closeSidebar();} else {openSidebar();}} else {const e = document.getElementById('sidebar');if (e) {e.classList.toggle('w-64');e.classList.toggle('w-16');}}}async function renderDashboard(e) {e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';try {const t = await api("GET", "/members/dashboard"),n = t.member,a = t.wallet,s = t.pendingWalletTotal || 0;t.nextEligible, document.getElementById("header-balance").innerHTML = "<span class=\"text-gray-400\">Solde : </span><span class=\"text-rouge-400 font-bold\">".concat(fmt$(a?.balance), "</span>");const _licPrice = fmtPrice(t.license_price || 97);e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n      <div>\n\n        <h1 class=\"text-2xl font-bold text-white\">Bonjour, ".concat(
      ...
      );}return;}const c = e._x + 77,x = e._y;if (null !== i && a.push("<path d=\"".concat(svgBezier(i, l + 108, c, x), "\" stroke=\"#374151\" stroke-width=\"1.5\" fill=\"none\"/>")), null !== d) {const e = "L" === d ? "#60A5FA" : "#34D399";s.push("<text x=\"".concat(c, "\" y=\"").concat(x - 14, "\" text-anchor=\"middle\" fill=\"").concat(e, "\" font-size=\"9\" font-weight=\"700\" font-family=\"sans-serif\">").concat("L" === d ? "G" : "D", "</text>"));}const p = e.in_holding_tank,m = { captain: "#791E15", commander: "#93C5FD", captain: "#67E8F9", leader: "#86EFAC", mentor: "#5EEAD4", superviseur: "#FDE047", executive: "#FDBA74", president: "#FCD34D", director: "#FCA5A5", boss: "#F9A8D4", visionary: "#F59E0B" },u = p ? "#FB923C" : m[e.current_rank] || "#374151",g = p ? "#431407" : "#1F2937",f = "".concat((e.first_name || "?")[0]).concat((e.last_name || "?")[0]).toUpperCase(),b = { captain: "Capt.", commander: "Cmdr.", administrator: "Admin.", manager: "Mgr.", captain: "Capt.", leader: "Leader", mentor: "Mentor", superviseur: "Super.", executive: "Exec.", president: "Prés.", director: "Dir.", boss: "Boss", visionary: "Vision.", membre: "Mbr." }[e.current_rank] || e.current_rank || "",v = m[e.current_rank] || "#9CA3AF";s.push("\n\n    <g class=\"svg-node-member\" title=\"".concat(
    e.unique_id, "\">\n\n      <rect x=\"").concat(
    ...
    e._x + 8, "\" y=\"").concat(x + 102, "\" fill=\"#60A5FA\" font-size=\"8\" font-family=\"sans-serif\">G:").concat(fmtBV(e.left_bv_total), "</text>\n\n      <text x=\"").concat(
    e._x + 154 - 8, "\" y=\"").concat(x + 102, "\" text-anchor=\"end\" fill=\"#34D399\" font-size=\"8\" font-family=\"sans-serif\">D:").concat(fmtBV(e.right_bv_total), "</text>\n\n      <text x=\"").concat(e._x + 8, "\" y=\"").concat(x + 112, "\" fill=\"#4B5563\" font-size=\"7\" font-family=\"sans-serif\">m G:").concat(fmtBV(e.left_bv_monthly), "</text><text x=\"").concat(e._x + 154 - 8, "\" y=\"").concat(x + 112, "\" text-anchor=\"end\" fill=\"#4B5563\" font-size=\"7\" font-family=\"sans-serif\">D:").concat(fmtBV(e.right_bv_monthly), "</text>\n\n      ").concat(
    ...
    e, "\"</strong>\n\n      </div>"),
  void t.classList.remove("hidden");const a = { administrator: "text-slate-300", manager: "text-blue-300", captain: "text-cyan-300", leader: "text-green-300", mentor: "text-teal-300", superviseur: "text-yellow-300", executive: "text-orange-300", president: "text-amber-300", director: "text-red-300", boss: "text-pink-300", visionary: "text-gold-400" };t.innerHTML = n.map((e, t) => {const n = a[e.current_rank] || "text-rouge-400",s = "".concat((e.first_name || "?")[0]).concat((e.last_name || "?")[0]).toUpperCase(),r = e.id === _classicRoot?.id,i = fmtBV(e.left_bv_monthly),l = fmtBV(e.right_bv_monthly);return "\n\n    <div class=\"tree-search-result flex items-center gap-3 px-4 py-2.5 hover:bg-dark-700 cursor-pointer transition-colors\n\n                ".concat(
  ...
  }).join(""), t.classList.remove("hidden");}function initTreeSearch() {const e = document.getElementById("tree-search-input"),t = document.getElementById("tree-search-dropdown");e && t && (e.addEventListener("input", (e) => renderTreeSearchResults(e.target.value)), e.addEventListener("focus", (e) => {e.target.value && renderTreeSearchResults(e.target.value);}), e.addEventListener("blur", () => setTimeout(() => t.classList.add("hidden"), 150)), e.addEventListener("keydown", (n) => {if ("Escape" === n.key && (t.classList.add("hidden"), e.blur()), "Enter" === n.key) {const e = t.querySelector(".tree-search-result");e && e.click();}}), document.addEventListener("keydown", (t) => {"/" !== t.key || document.activeElement === e || ["INPUT", "TEXTAREA"].includes(document.activeElement?.tagName) || (t.preventDefault(), e.focus());}));}function findNodeById(e, t) {return e ? e.id === t ? e : findNodeById(e.left, t) || findNodeById(e.right, t) : null;}async function classicNavigateTo(e) {const t = findNodeById(_fullTree, e);if (t && (t.left || t.right)) {_classicNavStack.push(_classicFocus);_classicFocus = t;renderClassicTreeInner();return;}// Nœud en limite de profondeur → recharger depuis l'API à partir de ce nœud
const wrap = document.getElementById("member-tree-wrap");if (wrap) wrap.innerHTML = '<div class="flex justify-center py-16"><div class="loader"></div></div>';try {const s = await api("GET", "/members/tree?depth=12&root=" + e);if (s.tree) {const prev = _classicFocus || _classicRoot;_classicNavStack.push(prev);_fullTree = s.tree;_classicRoot = s.tree;_classicFocus = s.tree;renderClassicTreeInner();}} catch(err) {if (wrap) wrap.innerHTML = '<div class="text-red-400 p-4">Erreur chargement arbre</div>';}}function classicGoBack() {0 !== _classicNavStack.length && (_classicFocus = _classicNavStack.pop(), renderClassicTreeInner());}function classicGoHome() {_classicNavStack = [], _classicFocus = _classicRoot, renderClassicTreeInner();}function renderClassicNode(e, t, n = !1) {if (!e) return "";
// Nœud ROOT système : affichage spécial non-cliquable
if (e.unique_id === "ROOT") {const rootCard = '<div class="tree-node-card cursor-default border-dark-500 opacity-70"><div class="w-9 h-9 rounded-full bg-dark-900 flex items-center justify-center mx-auto mb-2 text-xs font-bold text-gray-500 border border-dark-600"><i class="fas fa-network-wired text-gray-600"></i></div><div class="text-xs font-semibold text-gray-400 truncate">Réseau</div><div class="text-[10px] text-gray-600 font-mono">ROOT</div></div>';if (t >= 4) return '<div class="tree-node">'+rootCard+'</div>';const childrenHTML='<div class="tree-children"><div class="flex flex-col items-center"><div class="text-[10px] text-blue-400 mb-1 font-medium">Gauche</div>'+(e.left?renderClassicNode(e.left,t+1):'<div class="tree-node"><div class="tree-node-card empty opacity-30"><div class="text-gray-600 text-[10px]">—</div></div></div>')+'</div><div class="flex flex-col items-center"><div class="text-[10px] text-green-400 mb-1 font-medium">Droite</div>'+(e.right?renderClassicNode(e.right,t+1):'<div class="tree-node"><div class="tree-node-card empty opacity-30"><div class="text-gray-600 text-[10px]">—</div></div></div>')+'</div></div>';return '<div class="tree-node"><div class="tree-node-wrap">'+rootCard+childrenHTML+'</div></div>';}
const a = n,s = e.in_holding_tank ? '<div class="text-[9px] text-orange-400 mt-0.5"><i class="fas fa-clock mr-0.5"></i>Holding</div>' : "",r = !a,i = "\n\n    tree-node-card ".concat(
      { administrator: "border-slate-400/60 ring-slate-400/20", manager: "border-blue-500/60 ring-blue-500/20", captain: "border-cyan-500/60 ring-cyan-500/20", leader: "border-green-500/60 ring-green-500/20", mentor: "border-teal-500/60 ring-teal-500/20", superviseur: "border-yellow-500/60 ring-yellow-500/20", executive: "border-orange-500/60 ring-orange-500/20", president: "border-amber-500/60 ring-amber-500/20", director: "border-red-500/60 ring-red-500/20", boss: "border-pink-500/60 ring-pink-500/20", visionary: "border-gold-400/60 ring-gold-400/20" }[e.current_rank] || "border-dark-500 ring-dark-500/0", "\n\n    ").concat(
    ...
    l = "\n\n    <div class=\"w-9 h-9 rounded-full bg-dark-900 flex items-center justify-center mx-auto mb-2 text-xs font-bold ".concat(
      { administrator: "text-slate-300", manager: "text-blue-300", captain: "text-cyan-300", leader: "text-green-300", mentor: "text-teal-300", superviseur: "text-yellow-300", executive: "text-orange-300", president: "text-amber-300", director: "text-red-300", boss: "text-pink-300", visionary: "text-gold-400" }[e.current_rank] || "text-rouge-400", " ").concat(a ? "ring-2 ring-gold-500/40" : "", "\">\n\n      ").concat(
      ...
      fmtBV(e.left_bv_total), "</span>\n\n      <span class=\"text-green-400\">D:").concat(
      fmtBV(e.right_bv_total), "</span>\n\n    </div>\n\n    <div class=\"mt-0.5 flex justify-center gap-2 text-[8px] text-gray-500\">\n\n      <span>mois G:").concat(fmtBV(e.left_bv_monthly), "</span><span class=\"text-gray-600\">|</span><span>D:").concat(fmtBV(e.right_bv_monthly), "</span>\n\n    </div>\n\n    ").concat(
...
async function renderReservoir(e) {loading("page-content");try {const t = await api("GET", "/members/holding-tank"),n = !0 === t.license_active,a = n && t.my_unique_id ? "".concat(window.location.origin, "/register/").concat(t.my_unique_id) : "",s = t.pending || [],r = t.placed || [];e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n      <div class=\"flex items-center justify-between flex-wrap gap-2\">\n\n        <h2 class=\"text-xl font-bold\">R\xE9servoir \xE0 placement</h2>\n\n        <div class=\"flex items-center gap-2\">\n\n          <span class=\"bg-orange-500/20 text-orange-400 text-xs px-3 py-1 rounded-full font-medium\">\n\n            <i class=\"fas fa-clock mr-1\"></i>".concat(
    ...
    r.join("\n"), "\n\n    </g>\n\n  </svg>");
      ...
      { administrator: "text-slate-300", manager: "text-blue-300", captain: "text-cyan-300", leader: "text-green-300", mentor: "text-teal-300", superviseur: "text-yellow-300", executive: "text-orange-300", president: "text-amber-300", director: "text-red-300", boss: "text-pink-300", visionary: "text-gold-400" }[e.current_rank] || "text-rouge-400", "\">\n\n      ").concat(
      (e.first_name || "?")[0]).concat((e.last_name || "?")[0], "\n\n    </div>\n\n    <div class=\"text-xs font-semibold text-white truncate\">").concat(
