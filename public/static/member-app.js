...
async function _drawCardDesignA(ctx, W, H, member, config, qrImg, logoImg) {
  ...
  if (member.city) {
    ...
    ctx.fillText('\u{1F4CD} ' + member.city, 26, 244);
  }
  if (qrImg) {
    var qs = 108;
...
async function _drawCardDesignB(ctx, W, H, member, config, qrImg, logoImg) {
  ...
  if (member.city) {
    ...
    ctx.fillText('\u{1F4CD} ' + member.city, 26, 236);
  }
  if (qrImg) {
    var qs = 108;
    ctx.save(); ctx.fillStyle = '#fff';
