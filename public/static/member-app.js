...
async function openWalletDetail(type) {
  const labels = {
    principal: "Portefeuille Principal",
    pending: "Wallet en Attente",
    cc: "Crédit de Croissance",
  ...
  try {
    ...
    if (type === "pending") {
      body.innerHTML = "<div class=\"space-y-3\">" + rows.map(e => {
        const remaining = Math.max(0, (e.total_amount || 0) - (e.amount_per_day || 0) * (e.days_paid || 0));
        const daysLeft = Math.max(0, (e.days_in_month || 0) - (e.days_paid || 0));
        const progress = e.days_in_month > 0 ? Math.round(e.days_paid / e.days_in_month * 100) : 0;
        const baseLabel = LABELS[e.commission_type] || e.commission_type;
        const rankForLabel = e.commission_type === "prime_leadership" ? e.commission_rank || e.source_rank || null : null;
        const typeLabel = rankForLabel ? "" + baseLabel + " " + rankForLabel : baseLabel;
        const rankLabels = {
          "none": "—",
        ...
        const sourceHtml = e.source_name ? "<span class=\"text-gray-300\"> \xB7 " + e.source_name + "</span> <span class=\"text-gray-500 text-xs\">" + (e.source_unique_id || "") + "</span>" : "";
        const releaseHtml = e.status === "pending_release" ? "<span class=\"text-yellow-400\">Lib\xE9ration le " + fmtD(e.eligible_date) + "</span>" : e.status === "active" ? "<span class=\"text-green-400\">" + daysLeft + "j restants \xB7 " + (e.amount_per_day || 0).toFixed(2) + "$/j</span>" : "<span class=\"text-gray-400\">Lib\xE9r\xE9</span>";
        return "<div class=\"bg-dark-700 rounded-xl p-3\"><div class=\"flex justify-between items-start mb-1\"><div class=\"text-sm font-medium text-white\">" + typeLabel + sourceHtml + "</div><div class=\"text-orange-300 font-bold ml-3 flex-shrink-0\">" + fmt$(remaining) + "</div></div><div class=\"text-xs mb-2\">" + releaseHtml + " \xB7 P\xE9riode " + (e.period || "—") + "</div><div class=\"h-1.5 bg-dark-600 rounded-full\"><div class=\"h-1.5 bg-orange-500/60 rounded-full\" style=\"width:" + progress + "%\"></div></div><div class=\"flex justify-between text-xs text-gray-600 mt-1\"><span>" + (e.days_paid || 0) + "j vers\xE9s</span><span>" + daysLeft + "j restants</span></div></div>";
      }).join("") + "</div>";
    } else {
      const fmtA = v => v === undefined || v === null ? "—" : v < 0 ? "<span class=\"text-red-400\">-$" + Math.abs(v).toLocaleString("fr-FR", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      }) + "</span>" : "<span class=\"text-green-400\">+$" + v.toLocaleString("fr-FR", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      }) + "</span>";
      body.innerHTML = "<div class=\"space-y-2\">" + rows.map(tx => "<div class=\"flex items-center justify-between py-2 border-b border-dark-700\"><div class=\"flex-1 min-w-0\"><div class=\"text-sm text-white truncate\">" + (tx.type_label || tx.label || tx.description || tx.transaction_type || "Transaction") + "</div><div class=\"text-xs text-gray-500\">" + fmtD(tx.created_at) + "</div></div><div class=\"text-right ml-3\">" + fmtA(tx.amount) + "</div></div>").join("") + "</div>";
    }
  } catch (e) {
    body.innerHTML = "<p class=\"text-red-400 text-center py-8\">Erreur : " + (e.message || e.error || "inconnue") + "</p>";
  }
}
const WALLET_COMMISSION_LABELS = {
  "binary": "Commission Binaire",
  "fast_start": "Fast Start",
  "fast_start_contribution": "Contribution Fast Start",
  "unilevel": "Unilevel",
  "rayonnement": "Rayonnement",
  "prime_leadership": "Prime Leadership",
  "credit_croissance": "Crédit de Croissance",
  "reserve_strategique": "Réserve Stratégique",
  "dreamiles": "Dreamiles Luxia",
  "matching": "Matching Bonus",
  "rank_bonus": "Bonus de Rang",
  "admin_fee": "Frais Admin",
  "manual": "Ajustement manuel"
};
let memberToken = window.__IMPERSONATE_TOKEN__ || localStorage.getItem("leader_member_token"),
  currentMember = null,
...
function fmtDate(e) {
  return e ? new Date(e).toLocaleDateString("fr-FR", {
    day: "2-digit",
    month: "short",
    year: "numeric"
  }) : "—";
...
function fmtDatetime(e) {
  return e ? new Date(e).toLocaleString("fr-FR", {
    day: "2-digit",
    month: "short",
    year: "numeric",
    hour: "2-digit",
...
function showToast(e, t = "success") {
  const n = document.getElementById("toast"),
    a = document.getElementById("toast-inner");
  a.textContent = e, a.className = "px-6 py-3 rounded-xl shadow-lg text-sm font-medium " + ("success" === t ? "bg-green-600 text-white" : "bg-red-600 text-white"), n.classList.remove("hidden"), n.classList.add("show"), setTimeout(() => {
    n.classList.add("hidden"), n.classList.remove("show");
  }, 3500);
}
function renderPagination(containerId, total, page, perPage, onPageChange, onPerPageChange) {
  ...
  var cid = containerId;
  window['_cbPg_' + cid] = typeof onPageChange === 'function' ? onPageChange : new Function('p', onPageChange.replace(/^\s*\(function\s*\([^)]*\)\s*\{/, '').replace(/\}\s*\)\s*$/, '').replace(/^\s*p\s*=>\s*/, ''));
  window['_cbPp_' + cid] = typeof onPerPageChange === 'function' ? onPerPageChange : new Function('pp', 'p', onPerPageChange.replace(/^\s*\(function\s*\([^)]*\)\s*\{/, '').replace(/\}\s*\)\s*$/, ''));
  window['_pg_' + cid] = function (p) {
    window['_cbPg_' + cid](p);
  };
  window['_pp_' + cid] = function (pp, p) {
    window['_cbPp_' + cid](pp, p);
  };
  var k = '_pg_' + cid;
  var kp = '_pp_' + cid;
  var selectHtml = '<select class="pagination-perpage" onchange="window[this.dataset.kp](parseInt(this.value),1)" data-kp="' + kp + '">' + '<option value="10"' + (perPage === 10 ? ' selected' : '') + '>10 / page</option>' + '<option value="25"' + (perPage === 25 ? ' selected' : '') + '>25 / page</option>' + '<option value="50"' + (perPage === 50 ? ' selected' : '') + '>50 / page</option>' + '</select>';
  ...
  if (start > 1) {
    buttons += '<button class="pagination-btn" onclick="window[this.dataset.k](1)" data-k="' + k + '">1</button>';
  ...
  var prevBtn = '<button class="pagination-btn"' + (page <= 1 ? ' disabled' : '') + ' onclick="if(' + page + '>1)window[this.dataset.k](' + (page - 1) + ')" data-k="' + k + '">&#8592;</button>';
  var nextBtn = '<button class="pagination-btn"' + (page >= totalPages ? ' disabled' : '') + ' onclick="if(' + page + '<' + totalPages + ')window[this.dataset.k](' + (page + 1) + ')" data-k="' + k + '">&#8594;</button>';
  el.innerHTML = '<div class="pagination-bar">' + info + selectHtml + '<div class="pagination-pages">' + prevBtn + buttons + nextBtn + '</div></div>';
}
function rankBadge(e) {
  const t = (e || "none").toLowerCase().replace(" ", "");
  return "<span class=\"rank-" + t + " text-xs px-2 py-0.5 rounded-full font-medium\">" + ({
    none: "—",
    administrator: "Administrator",
    manager: "Manager",
    captain: "Captain",
...
function statusBadge(e) {
  return "<span class=\"" + ({
    AMI: "status-ami",
    Partenaire: "status-partner",
    Client: "status-client",
    Membre: "status-member"
  }[e] || "status-member") + " text-xs px-2 py-0.5 rounded-full font-medium\">" + e + "</span>";
}
function commTypeFr(e) {
  return {
    prime_leadership: "Prime Leadership",
    bonus_influence: "Bonus Influence",
    bonus_rayonnement: "Bonus Rayonnement",
    valorisation_recommandation: "Val. Recommandation",
    fast_start: "Fast Start",
    credit_croissance: "Crédit Croissance",
    reserve_strategique: "Réserve Stratégique"
  }[e] || e;
}
const DREAMILES_LOGO_SM = '<svg width="20" height="20" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">\n  <circle cx="16" cy="16" r="16" fill="#EAB308" opacity="0.15"/>\n  <text x="16" y="22" font-family="Georgia,serif" font-size="18" font-weight="bold"\n        fill="#EAB308" text-anchor="middle" dominant-baseline="auto">D</text>\n</svg>';
function loading(e) {
  document.getElementById(e).innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
}
function showModal(e, t = !1) {
  const n = document.getElementById("member-modal-overlay"),
    a = document.getElementById("member-modal-content");
  n && a && (a.className = "bg-dark-800 rounded-2xl shadow-2xl border border-dark-600 w-full " + (t ? "max-w-4xl" : "max-w-lg") + " max-h-[92vh] overflow-y-auto relative", a.innerHTML = e, n.classList.remove("hidden"), n.onclick = e => {
    e.target === n && closeModal();
  });
}
function closeModal() {
  const e = document.getElementById("member-modal-overlay");
  e && e.classList.add("hidden");
}
function showLogin() {
  document.getElementById("login-form").classList.remove("hidden"), document.getElementById("register-form").classList.add("hidden"), document.getElementById("pin-confirmation").classList.add("hidden");
}
function showRegister() {
  document.getElementById("register-form").classList.remove("hidden"), document.getElementById("login-form").classList.add("hidden"), document.getElementById("pin-confirmation").classList.add("hidden");
}
const REG_COUNTRIES = [{
  code: 'AF',
  name: 'Afghanistan',
  ...
  code: 'IO',
  name: 'Territoire brit. Océan Indien',
  flag: '🇮🇴'
}, {
  code: 'TH',
  name: 'Thaïlande',
...
function regCheckPasswordStrength(pwd) {
  ...
  if (lbl) {
    lbl.textContent = pwd.length > 0 ? labels[score - 1] || '' : '';
    lbl.className = 'text-xs mt-1 ' + (score > 0 ? labelColors[score - 1] : 'text-gray-500');
  }
}
...
function regNextStep(target) {
  ...
  if (target === 2) {
    ...
    if (!fn || !ln) {
      if (err1) {
        err1.textContent = 'Prénom et nom sont obligatoires.';
    ...
    if (!bd) {
      if (err1) {
        err1.textContent = 'La date de naissance est obligatoire.';
        err1.classList.remove('hidden');
      }
      return;
    }
  ...
  if (target === 3) {
    ...
    if (!ph) {
      if (err2) {
        err2.textContent = 'Le téléphone est obligatoire.';
    ...
    if (!co) {
      if (err2) {
        err2.textContent = 'Le pays est obligatoire.';
        err2.classList.remove('hidden');
      }
      return;
    }
    if (!nat) {
      if (err2) {
        err2.textContent = 'La nationalité est obligatoire.';
        err2.classList.remove('hidden');
      }
      return;
    }
    if (!addr) {
      if (err2) {
        err2.textContent = 'L\'adresse est obligatoire.';
        err2.classList.remove('hidden');
      }
      return;
    }
    if (!city) {
      if (err2) {
        err2.textContent = 'La ville est obligatoire.';
        err2.classList.remove('hidden');
      }
      return;
    }
    if (!postal) {
      if (err2) {
        err2.textContent = 'Le code postal est obligatoire.';
        err2.classList.remove('hidden');
      }
      return;
    }
  ...
  [1, 2, 3].forEach(s => {
    ...
    if (s < target) {
      ...
      dot.textContent = s;
      if (lbl) {
        lbl.className = 'text-xs mt-1 text-rouge-400 font-medium';
      }
    } else {
      ...
      dot.textContent = s;
      if (lbl) {
        lbl.className = 'text-xs mt-1 text-gray-500';
      }
    }
  });
...
async function initRegistrationMode() {
  const e = window.__REGISTRATION__ || {},
    t = e.mode || "M1";
  if ("M1" === t) return document.getElementById("reg-title").textContent = "Créer un compte", void document.getElementById("reg-sponsor-field").classList.remove("hidden");
  if ("M2" === t) {
    document.getElementById("reg-title").textContent = "Inscription — Placement direct", document.getElementById("reg-sponsor-field").classList.add("hidden");
    const t = document.getElementById("slot-error-banner"),
      n = document.getElementById("sponsor-banner"),
      a = document.getElementById("reg-submit-btn");
    try {
      const s = e.parent || e.sponsor,
        r = (await axios.get("/api/auth/validate-slot?sponsor=" + encodeURIComponent(e.sponsor) + "&parent=" + encodeURIComponent(s) + "&position=" + encodeURIComponent(e.position))).data;
      if (r.valid) {
        document.getElementById("reg-sponsor").value = r.sponsor_id, document.getElementById("reg-binary-parent").value = r.parent_id, n.classList.remove("hidden");
        const t = r.parent_unique_id !== r.sponsor_unique_id ? "Parrain\xE9(e) par " + r.sponsor_name + " \u2014 plac\xE9(e) sous " + r.parent_name + " (" + r.parent_unique_id + "), jambe " + ("L" === e.position ? "GAUCHE" : "DROITE") : "Vous serez plac\xE9(e) c\xF4t\xE9 " + ("L" === e.position ? "GAUCHE" : "DROITE") + " de " + r.sponsor_name + " (" + r.sponsor_unique_id + ")";
        document.getElementById("sponsor-banner-text").textContent = t;
      } else t.classList.remove("hidden"), document.getElementById("slot-error-text").textContent = r.error || "Slot invalide", a.disabled = !0;
    } catch (e) {
      const n = e.response?.data?.error || "Lien de parrainage invalide ou expiré.";
      t.classList.remove("hidden"), document.getElementById("slot-error-text").textContent = n, a.disabled = !0;
    }
    return;
  }
  if ("M3" === t) {
    document.getElementById("reg-title").textContent = "Inscription — Via lien parrain", document.getElementById("reg-sponsor-field").classList.add("hidden");
    const t = document.getElementById("sponsor-banner"),
      n = document.getElementById("slot-error-banner"),
      a = document.getElementById("reg-submit-btn"),
      s = e.sponsorUid || "";
    if (s) try {
      const e = (await axios.get("/api/auth/sponsor/" + encodeURIComponent(s))).data;
      e.found ? (document.getElementById("reg-sponsor").value = e.id, t.classList.remove("hidden"), document.getElementById("sponsor-banner-text").textContent = "Parrain\xE9(e) par : " + e.name + " (" + e.unique_id + ")") : "no_license" === e.reason ? (n.classList.remove("hidden"), document.getElementById("slot-error-text").textContent = e.message || "Ce lien de parrainage est inactif (licence du parrain expirée ou non activée).", a.disabled = !0) : (t.classList.remove("hidden"), document.getElementById("sponsor-banner-text").textContent = "Inscription sans parrain (lien introuvable)");
    } catch (e) {
      const t = e.response?.data?.error || "Lien de parrainage invalide.";
      n.classList.remove("hidden"), document.getElementById("slot-error-text").textContent = t, a.disabled = !0;
    }
  }
}
async function doRegister() {
  ...
  try {
    const cfg = await api("GET", "/auth/reg-required-fields");
    if (cfg && Array.isArray(cfg.fields)) reqFields = cfg.fields;
  } catch (err) {}
  const fieldMap = {
    country: {
      val: i,
  ...
  for (const fk of reqFields) {
    if (fieldMap[fk] && !fieldMap[fk].val) return c.textContent = "Le champ «" + fieldMap[fk].label + "» est obligatoire.", void c.classList.remove("hidden");
  }
  const m = {
    first_name: n,
    last_name: a,
    ...
    postal_code: regPostal || null,
    city: regCity || null
  };
  if ("M1" === t) {
    m.registration_method = "M1";
    const e = document.getElementById("reg-sponsor").value.trim();
    if (e) try {
      const t = await axios.get("/api/auth/sponsor/" + encodeURIComponent(e.toUpperCase()));
      t.data.found && (m.sponsor_id = t.data.id);
    } catch {}
  }
...
async function doLogin() {
  ...
  try {
    const n = await api("POST", "/auth/login", {
      email: e,
      password: t
    });
    memberToken = n.token, localStorage.setItem("leader_member_token", n.token), currentMember = n.member, initApp();
  } catch (e) {
    n.textContent = e.error || "Erreur de connexion", n.classList.remove("hidden");
  }
}
function doLogout() {
  localStorage.removeItem("leader_member_token"), memberToken = null, currentMember = null, document.getElementById("app").classList.add("hidden"), document.getElementById("auth-screen").classList.remove("hidden"), showLogin();
}
async function initApp() {
  if (memberToken) try {
    const e = await api("GET", "/members/me");
    currentMember = e.member, updateSidebar(), document.getElementById("auth-screen").classList.add("hidden"), document.getElementById("app").classList.remove("hidden"), showPage("dashboard"), loadNotifCount(), loadReservoirCount(), loadSupportUnread();
  } catch {
    doLogout();
  } else document.getElementById("auth-screen").classList.remove("hidden");
}
function updateSidebar() {
  const e = currentMember;
  if (!e) return;
  const t = ((e.first_name || "")[0] + (e.last_name || "")[0]).toUpperCase();
  document.getElementById("sidebar-avatar").textContent = t, document.getElementById("sidebar-name").textContent = "" + e.first_name + " " + e.last_name, document.getElementById("sidebar-uid").textContent = e.unique_id;
  const n = document.getElementById("sidebar-status");
  n.textContent = e.member_status, n.className = "text-xs px-2 py-0.5 rounded-full font-medium " + ({
  ...
  }[e.member_status] || ""), document.getElementById("sidebar-rank").textContent = "none" === e.current_rank ? "—" : e.current_rank;
}
async function loadNotifCount() {
  try {
    const e = ((await api("GET", "/members/notifications?per_page=100")).notifications || []).filter(e => !e.is_read).length,
      t = document.getElementById("notif-badge"),
      n = document.getElementById("header-notif-badge");
    e > 0 ? (t.textContent = e, t.classList.remove("hidden"), n.textContent = e, n.classList.remove("hidden")) : (t.classList.add("hidden"), n.classList.add("hidden"));
  } catch {}
}
async function loadReservoirCount() {
  try {
    const e = await api("GET", "/members/holding-tank"),
      t = document.getElementById("reservoir-badge");
    if (!t) return;
    (e.pending_count || 0) > 0 ? (t.textContent = e.pending_count, t.classList.remove("hidden")) : t.classList.add("hidden");
  } catch {}
}
function showPage(e) {
  window._prevPage = (typeof currentPage !== 'undefined' && currentPage !== e ? currentPage : window._prevPage) || 'wallet';
  currentPage = e, document.querySelectorAll(".nav-btn").forEach(t => {
    t.classList.toggle("active", t.dataset.page === e);
  });
  const t = document.getElementById("page-content");
  t.className = "p-4 md:p-6 fade-in";
  const n = {
    ...
    "cc-wallet": renderCCWallet,
    "reserve-strategique": renderReserveStrategique,
    "commissions-faq": renderCommissionsFAQ,
    support: window.renderSupport,
    earnings: window.renderEarnings,
    "member-card": window.renderMemberCard,
    services: renderServices,
    campus: showCampusPage
  };
  n[e] ? (n[e](t), setTimeout(_applyMemberMobileFixes, 300)) : t.innerHTML = "<p class=\"text-gray-400\">Page \"" + e + "\" non disponible</p>";
}
...
function toggleSidebar() {
  ...
  if (isMobile) {
    const s = document.getElementById('sidebar');
    const isOpen = s && !s.classList.contains('-translate-x-full');
    if (isOpen) {
      closeSidebar();
    } else {
      openSidebar();
    ...
    if (e) {
      e.classList.toggle('w-64');
      e.classList.toggle('w-16');
    }
  }
}
async function renderDashboard(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const t = await api("GET", "/members/dashboard"),
      n = t.member,
      a = t.wallet,
      s = t.pendingWalletTotal || 0;
    t.nextEligible, document.getElementById("header-balance").innerHTML = "<span class=\"text-gray-400\">Solde : </span><span class=\"text-rouge-400 font-bold\">" + fmt$(a?.balance) + "</span>";
    ...
    e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n      <div>\n\n        <h1 class=\"text-2xl font-bold text-white\">Bonjour, " + n.first_name + " ! </h1>\n\n        <p class=\"text-gray-400 mt-1\">Voici votre tableau de bord LEADER</p>\n\n      </div>\n\n\n\n      <!-- Widget Fast Start Bonus \u2014 EN \xC9VIDENCE, dispara\xEEt uniquement \xE0 la fin des 30j -->\n\n      " + (() => {
      ...
      return "\n\n        <div class=\"rounded-xl border " + f + " overflow-hidden relative\"\n\n             style=\"background: linear-gradient(135deg, #0f1a14 0%, #0d1a16 100%); box-shadow: 0 0 24px " + b + "\">\n\n\n\n          <!-- Bande sup\xE9rieure color\xE9e -->\n\n          <div class=\"h-0.5 w-full\" style=\"background:linear-gradient(90deg," +
        ...
        (x ? x.paid ? "<span class=\"text-[10px] bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 px-1.5 py-0.5 rounded-full font-semibold\"><i class=\"fas fa-wallet mr-0.5\"></i>Bonus " + x.bv_threshold + " BV vers\xE9 !</span>" : "<span class=\"text-[10px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded-full font-semibold\"><i class=\"fas fa-check mr-0.5\"></i>Palier " +
        x.bv_threshold + " BV atteint !</span>" : "") + "\n\n                </div>\n\n                <div class=\"text-[11px] text-gray-400 mt-0.5\">Filleuls directs avant le <span class=\"font-medium text-white\">" + w + "</span></div>\n\n              </div>\n\n            </div>\n\n            <div class=\"flex flex-col items-end\">\n\n              <span class=\"" +
        v + " font-black text-xl leading-none\">" + l + "</span>\n\n              <span class=\"text-[10px] text-gray-400\">jour" + (1 !== l ? "s" : "") + " restant" + (1 !== l ? "s" : "") + "</span>\n\n            </div>\n\n          </div>\n\n\n\n          <!-- Corps -->\n\n          <div class=\"px-4 pb-3 space-y-2.5\">\n\n\n\n            <!-- Barre de temps -->\n\n            <div>\n\n              <div class=\"flex justify-between text-[11px] text-gray-400 mb-1\">\n\n                <span>Temps \xE9coul\xE9</span>\n\n                <span class=\"" +
        v + " font-semibold\">" + d + "% \u2014 " + r + "j / " + i + "j</span>\n\n              </div>\n\n              <div class=\"h-1.5 rounded-full bg-dark-700 overflow-hidden\">\n\n                <div class=\"h-full rounded-full transition-all\" style=\"width:" +
        d + "%; background:linear-gradient(90deg," + y + ")\"></div>\n\n              </div>\n\n            </div>\n\n\n\n            <!-- BV + progression -->\n\n            <div>\n\n              <div class=\"flex justify-between text-[11px] text-gray-400 mb-1\">\n\n                <span>BV filleuls directs accumul\xE9s</span>\n\n                <span class=\"text-emerald-400 font-bold\">" +
        fmtBV(o) + "</span>\n\n              </div>\n\n              <div class=\"h-1.5 rounded-full bg-dark-700 overflow-hidden\">\n\n                <div class=\"h-full rounded-full transition-all\" style=\"width:" + m + "%; background:linear-gradient(90deg,#10b981,#059669)\"></div>\n\n              </div>\n\n              <div class=\"flex justify-between text-[10px] text-gray-500 mt-1\">\n\n                " +
        (p ? "<span>" + fmtBV(o) + " / " + fmtBV(p.bv_threshold) + " BV pour d\xE9crocher <span class=\"text-emerald-400 font-bold\">" + fmt$(p.bonus_amount) + "</span></span><span class=\"text-emerald-400\">" + m + "%</span>" : x ? x.paid ? "<span class=\"text-indigo-400 font-semibold\"><i class=\"fas fa-wallet mr-1\"></i>Palier max atteint \u2014 bonus " +
        fmt$(x.bonus_amount) + " vers\xE9 dans votre wallet</span><span></span>" : "<span class=\"text-emerald-400 font-semibold\"><i class=\"fas fa-check-circle mr-1\"></i>Palier max atteint \u2014 bonus " + fmt$(x.bonus_amount) + " en cours de versement</span><span></span>" : "<span>Premier palier : " +
        (c[0] ? fmtBV(c[0].bv_threshold) + " BV" : "—") + "</span><span></span>") + "\n\n              </div>\n\n            </div>\n\n\n\n            <!-- Paliers -->\n\n            <div class=\"grid grid-cols-2 sm:grid-cols-4 gap-1.5\">\n\n              " + c.map(e => {
        const t = o >= e.bv_threshold,
          ...
          i = n ? '<div class="mt-0.5 text-[9px] text-indigo-400 font-semibold"><i class="fas fa-wallet mr-0.5"></i>Versé ✓</div>' : t ? '<div class="mt-0.5 text-[9px] text-emerald-500 font-semibold"><i class="fas fa-check-circle mr-0.5"></i>Atteint</div>' : "<div class=\"mt-0.5 text-[9px] text-gray-600\">" + fmtBV(e.bv_threshold - o) + " manquants</div>";
      ...
      if (drt <= 0) return '';
      const rows = drb.map(x => "<div class=\"flex justify-between text-xs py-0.5\"><span class=\"text-gray-400\">" + x.label + "</span><span class=\"text-emerald-300 font-medium\">+" + fmt$(x.amount_per_day) + "/j</span></div>").join('');
      ...
      return e ? "\n\n              <div class=\"absolute inset-0 opacity-5 pointer-events-none\"\n\n                   style=\"background: radial-gradient(ellipse at 50% 0%, var(--rank-glow-color, #d4af37) 0%, transparent 70%)\"></div>\n\n              <div class=\"flex items-start justify-between mb-5\">\n\n                <h3 class=\"font-semibold text-white text-sm uppercase tracking-wider\">Mon Rang Actuel</h3>\n\n                " +
        ...
        t.progressToNext + "%; background: linear-gradient(90deg, " + (t.progressToNext >= 80 ? "#22c55e,#16a34a" : t.progressToNext >= 50 ? "#eab308,#ca8a04" : "#d4af37,#b8860b") + ")\"></div>\n\n                </div>\n\n                <div class=\"text-xs text-gray-500 mt-1.5 flex justify-between\">\n\n                  <span>" +
        fmtBV(t.progressMinLeg) + " / " + fmtBV(t.nextRankConfig.min_bv_leg) + " BV jambe faible</span>\n\n                  <span class=\"" + ("total" === t.progressBVMode ? "text-rouge-400/70" : "text-blue-400/70") + "\">" + ("total" === t.progressBVMode ? "∞ à vie" : "mensuel") +
      ...
      "\n            </div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">↳ Jambe gauche</span><span class=\"text-rouge-400 font-bold\">") + fmtBV(n.left_bv_total) + "</span></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">\u21B3 Jambe droite</span><span class=\"text-rouge-400 font-bold\">" +
      fmtBV(n.right_bv_total) + "</span></div>\n\n                        <div class=\"border-t border-dark-600 pt-2 mt-2 text-xs text-blue-400 font-semibold uppercase tracking-wider mb-1 flex items-center gap-1\">\n\n              <i class=\"fas fa-calendar-alt text-[10px]\"></i> BV Mensuel <span class=\"text-gray-500 font-normal normal-case\">(commissions \u2014 reset le 1er)</span>\n\n            </div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">\u21B3 Jambe gauche</span><span class=\"text-blue-300\">" +
      fmtBV(n.left_bv_monthly) + "</span></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">\u21B3 Jambe droite</span><span class=\"text-blue-300\">" + fmtBV(n.right_bv_monthly) + "</span></div>\n\n                        <div class=\"border-t border-dark-600 pt-2 mt-2\"></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Total gagn\xE9</span><span class=\"text-green-400\">" +
      fmt$(a?.total_earned) + "</span></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Total retir\xE9</span><span class=\"text-red-400\">" + fmt$(a?.total_withdrawn) + "</span></div>\n\n            " + ((t.ccBalance?.total || 0) > 0 ? "<div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Cr\xE9dit Croissance</span><span class=\"text-pink-400\">" +
      fmt$(t.ccBalance.total) + "</span></div>" : "") + "\n\n            " + (n.reserve_strategique > 0 ? "<div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">R\xE9serve Strat\xE9gique</span><span class=\"text-cyan-400\">" + fmt$(n.reserve_strategique) +
      ...
      "justify-between text-sm\"><span class=\"text-gray-400\">Directs actifs validés</span><span class=\"text-rouge-400 font-bold\">") + (t.directActiveCount || 0) + "</span></div>\n\n            " + (t.nextRankConfig ? "<div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Requis prochain rang (" +
      (t.nextRankConfig.rank_name || '') + ")</span><span class=\"text-blue-400\">" + (t.nextRankConfig.min_directs || 0) + " direct(s) \u2265 " + fmt$(+(t.nextRankConfig.min_monthly_package_value || 0)) + "</span></div>" : "") + ("\n\n            <div class=\"border-t border-dark-600 pt-2 mt-2\">\n\n              <p class=\"text-xs text-gray-500\">Directs avec package validé ≥ au seuil configuré. Valeurs lues depuis la configuration admin.</p>\n\n            </div>\n\n          </div>\n\n        </div>\n\n        <!-- Bloc B : Prime mensuelle de Leadership -->\n\n        <div class=\"stat-card\" style=\"align-self:start\">\n\n          <h3 class=\"font-semibold text-white mb-3 flex items-center gap-2\"><i class=\"fas fa-trophy text-rouge-400\"></i> Prime Mensuelle de Leadership</h3>\n" +
      "\n          <div class=\"space-y-2\">\n\n            <div class=\"text-xs text-rouge-400 font-semibold uppercase tracking-wider mb-1\">BV Jambe Faible (petite jambe)</div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Ce mois</span><span class=\"text-rouge-400 font-bold\">") +
      fmtBV(t.monthlyBvLegCurrent || 0) + "</span></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Requis" + (t.rankConfig ? " (" + t.rankConfig.rank_name + ")" : "") + "</span><span class=\"text-blue-400\">" + fmtBV(t.monthlyBvLegRequired || 0) +
      "</span></div>\n\n            <div class=\"w-full bg-dark-600 rounded-full h-1.5 my-1\"><div class=\"bg-rouge-500 h-1.5 rounded-full\" style=\"width:" + (t.monthlyBvLegRequired > 0 ? Math.min(100, Math.round(t.monthlyBvLegCurrent / t.monthlyBvLegRequired * 100)) : 0) +
      ...
      (t.monthlyRecruitsCurrent || 0) + "</span></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Requis" + (t.rankConfig ? " (" + t.rankConfig.rank_name + ")" : "") + "</span><span class=\"text-blue-400\">" + (t.monthlyRecruitsRequired || 0) +
      " recrue(s) \u2265 " + fmt$(+(t.monthlyPkgMinValue || 0)) + "</span></div>\n\n            <div class=\"border-t border-dark-600 pt-1 mt-1\"></div>\n\n            <div class=\"flex justify-between text-sm\"><span class=\"text-gray-400\">Plancher rang</span><span class=\"text-yellow-400\">" +
      (t.monthlyFloorRanks || 0) + " rang(s) en dessous accept\xE9(s)</span></div>\n\n            <div class=\"flex justify-between text-sm font-semibold\"><span class=\"text-gray-300\">Rang effectif</span><span class=\"" + (t.monthlyEffectiveRank && t.monthlyEffectiveRank !== 'none' ? 'text-green-400' : 'text-red-400') +
      "\">" + (t.monthlyEffectiveRank && t.monthlyEffectiveRank !== 'none' ? t.monthlyEffectiveRank : 'Non qualifié') + "</span></div>\n\n          </div>\n\n        </div>\n\n      </div>\n\n\n\n      <!-- Bloc Licence Finstrategia \u2014 CDC v2 A7 : 4 \xE9tats -->\n\n      " +
      (() => {
      const e = new Date(),
    ...
    console.error("renderDashboard ERROR:", t, JSON.stringify(t));
    e.innerHTML = "<div class=\"text-red-400\">Erreur : " + (t?.error || t?.message || JSON.stringify(t) || "JS runtime error") + "</div>";
  }
}
async function renderTeam(e) {
  e.innerHTML = "\n  <div class=\"space-y-4\">\n    <div class=\"flex items-center justify-between flex-wrap gap-3\">\n      <h2 class=\"text-xl font-bold\">Mon Équipe</h2>\n    </div>\n\n    <!-- Onglets -->\n    <div class=\"flex gap-1 bg-dark-800 border border-dark-600 rounded-xl p-1 w-fit\">\n      <button id=\"team-tab-directs\" onclick=\"switchTeamTab('directs')\"\n        class=\"px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-dark-600 text-white\">\n        <i class=\"fas fa-users mr-1.5\"></i>Filleuls directs\n      </button>\n" +
    "      <button id=\"team-tab-downline\" onclick=\"switchTeamTab('downline')\"\n        class=\"px-4 py-2 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white\">\n        <i class=\"fas fa-sitemap mr-1.5\"></i>Arbre de parrainage\n      </button>\n    </div>\n\n    <!-- Panneau filleuls directs -->\n    <div id=\"team-panel-directs\" class=\"bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden\">\n      <div class=\"p-4 border-b border-dark-600 flex gap-3\">\n        <input id=\"team-search\" type=\"text\" placeholder=\"Rechercher un membre...\" class=\"form-input flex-1\"\n" +
    "          oninput=\"loadTeam(this.value)\">\n      </div>\n      <div id=\"team-table-container\"></div>\n    </div>\n\n    <!-- Panneau arbre de parrainage -->\n    <div id=\"team-panel-downline\" class=\"hidden space-y-3\">\n      <div id=\"team-downline-container\">\n        <div class=\"flex justify-center py-12\"><div class=\"loader\"></div></div>\n      </div>\n    </div>\n  </div>", switchTeamTab("directs"), loadTeam();
}
function switchTeamTab(e) {
  ["directs", "downline"].forEach(t => {
    const n = document.getElementById("team-tab-" + t),
      a = document.getElementById("team-panel-" + t);
    if (!n || !a) return;
    const s = t === e;
    n.className = s ? "px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-dark-600 text-white" : "px-4 py-2 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white", a.classList.toggle("hidden", !s);
  }), "downline" === e && loadDownline();
}
async function loadTeam(e = "", page, perPage) {
  page = page || window._teamPage || 1;
  perPage = perPage || window._teamPerPage || 25;
  window._teamPage = page;
  window._teamPerPage = perPage;
  window._teamSearch = e !== undefined ? e : window._teamSearch || "";
  e = window._teamSearch;
  const t = document.getElementById("team-table-container");
  if (t) {
    t.innerHTML = '<div class="flex justify-center py-8"><div class="loader"></div></div>';
    try {
      const n = await api("GET", "/members/team?search=" + encodeURIComponent(e) + "&page=" + page + "&per_page=" + perPage);
      t.innerHTML = "\n\n      <div class=\"overflow-x-auto\">\n\n        <table class=\"data-table\">\n\n          <thead><tr>\n\n            <th>ID</th><th>Nom</th><th class=\"text-blue-400\"><i class=\"fas fa-arrow-left mr-1\"></i>BV Gauche</th><th class=\"text-green-400\"><i class=\"fas fa-arrow-right mr-1\"></i>BV Droite</th><th>Statut</th><th>Rang</th>\n\n            <th title=\"Nombre de filleuls directs\">Filleuls</th><th>Inscription</th>\n\n          </tr></thead>\n\n          <tbody>\n\n            " +
        ...
        fmtBV(e.left_bv_monthly) + "</td><td class=\"text-green-400 font-semibold text-center\">" + fmtBV(e.right_bv_monthly) + "</td>\n\n              <td>" + statusBadge(e.member_status) + "</td>\n\n              <td>" + rankBadge(e.current_rank) + "</td>\n\n              <td class=\"text-center text-sm\">" +
        (e.direct_count || 0) + "</td>\n\n              <td class=\"text-gray-500 text-xs\">" + fmtDate(e.created_at) + "</td>\n\n            </tr>").join("") || '<tr><td colspan="9" class="text-center text-gray-500 py-8">Aucun filleul direct</td></tr>') + "\n\n          </tbody>\n\n        </table>\n\n      </div>\n\n      <div class=\"px-6 py-3 text-sm text-gray-500 border-t border-dark-600 flex items-center justify-between gap-4\"><span>" +
        (n.total || 0) + " filleul(s) direct(s)</span><div id=\"team-pagination\"></div></div>";
      renderPagination("team-pagination", n.total || 0, page, perPage, "(function(p){loadTeam(window._teamSearch,p,window._teamPerPage)})", "(function(pp,p){loadTeam(window._teamSearch,p,pp)})");
    } catch (e) {
      t.innerHTML = "<div class=\"p-4 text-red-400\">Erreur : " + e.error + "</div>";
    }
  }
}
document.addEventListener("keydown", e => {
  "Enter" === e.key && (document.getElementById("login-form").classList.contains("hidden") || doLogin());
});
let _downlineCache = null;
async function loadDownline(e = !1) {
  const t = document.getElementById("team-downline-container");
  if (t) if (!_downlineCache || e) {
    t.innerHTML = '<div class="flex justify-center py-12"><div class="loader"></div></div>';
    try {
      const s = await api("GET", "/members/downline");
      if (!s.tree || 0 === s.tree.length) return _downlineCache = '<div class="bg-dark-800 rounded-2xl border border-dark-600 p-12 text-center">\n        <i class="fas fa-sitemap text-4xl text-gray-600 mb-4 block"></i>\n        <p class="text-gray-400 font-medium">Votre réseau de parrainage est vide</p>\n        <p class="text-gray-600 text-sm mt-1">Les personnes que vous parrainez apparaîtront ici, ainsi que leurs propres filleuls.</p>\n      </div>', void (t.innerHTML = _downlineCache);
      function n(e, t) {
        const s = 24 * t,
          r = 0 === t,
          i = {
            ...
            ambassador: "#F97316",
            diamond: "#38BDF8",
            triple_diamond: "#E879F9",
            crown: "#EF4444"
          }[e.current_rank] || "#6B7280",
          l = ("" + (e.first_name || "?")[0] + (e.last_name || "?")[0]).toUpperCase(),
          d = e.children && e.children.length > 0,
        ...
        return "\n\n      <div class=\"downline-node\" style=\"margin-left:" + s + "px\">\n\n        <!-- Connecteur vertical -->\n\n        " + (t > 0 ? "<div class=\"downline-connector\" style=\"margin-left:" + -s + "px;padding-left:" + s + "px\"></div>" : "") + "\n\n        <div class=\"flex items-center gap-2.5 py-1.5 group\">\n\n          <!-- Fil de branche -->\n\n          " +
          ...
          i + "22;border:1.5px solid " + i + ";color:" + i + "\">\n\n            " + l + "\n\n          </div>\n\n\n\n          <!-- Infos membre -->\n\n          <div class=\"flex-1 min-w-0\">\n\n            <div class=\"flex items-center gap-2 flex-wrap\">\n\n              <span class=\"font-semibold text-sm text-white\">" +
      ...
      function a(e) {
        return e.children && e.children.length ? e.children.length + e.children.reduce((e, t) => e + a(t), 0) : 0;
      }
      const r = "\n\n    <div class=\"grid grid-cols-3 gap-3 mb-4\">\n\n      <div class=\"bg-dark-800 border border-dark-600 rounded-xl p-3 text-center\">\n\n        <div class=\"text-2xl font-black text-white\">" + s.totalCount + "</div>\n\n        <div class=\"text-xs text-gray-500 mt-0.5\">Membres dans le r\xE9seau</div>\n\n      </div>\n\n      <div class=\"bg-dark-800 border border-dark-600 rounded-xl p-3 text-center\">\n\n        <div class=\"text-2xl font-black text-green-400\">" +
        ...
        i = "\n\n    <div class=\"bg-dark-800 border border-dark-600 rounded-2xl p-5\">\n\n      <div class=\"flex items-center justify-between mb-4\">\n\n        <div class=\"flex items-center gap-2\">\n\n          <i class=\"fas fa-sitemap text-indigo-400\"></i>\n\n          <span class=\"font-semibold text-white\">Arbre de parrainage</span>\n\n          <span class=\"text-xs text-gray-500\">— qui a parrainé qui dans votre réseau</span>\n\n        </div>\n\n        <button onclick=\"loadDownline(true)\" class=\"text-xs text-gray-400 hover:text-white transition px-2 py-1 rounded bg-dark-700 border border-dark-600\">\n" +
          ...
          "\n\n      </div>\n\n    </div>";
      _downlineCache = r + i, t.innerHTML = _downlineCache;
    } catch (l) {
      t.innerHTML = "<div class=\"p-4 text-red-400\">Erreur : " + (l.error || l.message) + "</div>";
    }
  } else t.innerHTML = _downlineCache;
}
function toggleDownlineNode(e) {
  const t = document.getElementById("" + e + "-children"),
    n = document.getElementById("" + e + "-icon");
  if (!t) return;
  const a = "none" === t.style.display;
  t.style.display = a ? "" : "none", n && (n.className = a ? "fas fa-chevron-down" : "fas fa-chevron-right");
}
const SVG_NW = 154,
  SVG_NH = 108,
  SVG_HGAP = 36,
...
function treeMaxDepth(e) {
  return e ? 1 + Math.max(treeMaxDepth(e.left), treeMaxDepth(e.right)) : 0;
}
function svgAssignPositions(e, t, n) {
  if (!e) return 190;
  const a = e.left ? svgAssignPositions(e.left, t + 1, n) : 190,
    s = a + (e.right ? svgAssignPositions(e.right, t + 1, n + a) : 190);
  return e._x = n + a - 95, e._y = 188 * t, e.left && svgAssignPositions(e.left, t + 1, n), e.right && svgAssignPositions(e.right, t + 1, n + a), s;
}
...
function svgCollectElements(e, t, n, a, s, r, i, l, d, o) {
  if (!e) {
    if (null !== i && null !== d) {
      const e = i - 77,
        t = l + 108 + 40;
      a.push("<path d=\"" + svgBezier(i, l + 108, i, t) + "\" stroke=\"#374151\" stroke-width=\"1.5\" fill=\"none\" stroke-dasharray=\"5,4\"/>");
      ...
      r.push("\n\n        <a href=\"" + s + "\" target=\"_blank\" title=\"S'inscrire ici (" + ("L" === d ? "Gauche" : "Droite") + ")\">\n\n          <rect x=\"" + e + "\" y=\"" + t + "\" width=\"154\" height=\"108\" rx=\"12\"\n\n                fill=\"#111827\" stroke=\"#374151\" stroke-width=\"1.5\" stroke-dasharray=\"6,4\"\n\n                class=\"svg-slot-view\"/>\n\n          <text x=\"" +
        (e + 77) + "\" y=\"" + (t + 36) + "\" text-anchor=\"middle\" fill=\"#4B5563\" font-size=\"20\" font-family=\"sans-serif\">+</text>\n\n          <text x=\"" + (e + 77) + "\" y=\"" + (t + 58) + "\" text-anchor=\"middle\" fill=\"#6B7280\" font-size=\"10\" font-family=\"sans-serif\">Position libre</text>\n\n          <text x=\"" +
        (e + 77) + "\" y=\"" + (t + 72) + "\" text-anchor=\"middle\" fill=\"#4B5563\" font-size=\"9\" font-family=\"sans-serif\">Cliquer pour s'inscrire</text>\n\n        </a>");
    }
    return;
  }
  const c = e._x + 77,
  ...
  if (null !== i && a.push("<path d=\"" + svgBezier(i, l + 108, c, x) + "\" stroke=\"#374151\" stroke-width=\"1.5\" fill=\"none\"/>"), null !== d) {
    ...
    s.push("<text x=\"" + c + "\" y=\"" + (x - 14) + "\" text-anchor=\"middle\" fill=\"" + e + "\" font-size=\"9\" font-weight=\"700\" font-family=\"sans-serif\">" + ("L" === d ? "G" : "D") + "</text>");
  }
  const p = e.in_holding_tank,
    ...
    u = p ? "#FB923C" : m[e.current_rank] || "#374151",
    g = p ? "#431407" : "#1F2937",
    f = ("" + (e.first_name || "?")[0] + (e.last_name || "?")[0]).toUpperCase(),
    b = {
      captain: "Capt.",
      commander: "Cmdr.",
      admiral: "Adm.",
      ...
      ambassador: "Ambas.",
      diamond: "Diam.",
      triple_diamond: "3xDiam.",
      crown: "Crown",
      membre: "Mbr."
    }[e.current_rank] || e.current_rank || "",
  ...
  s.push("\n\n    <g class=\"svg-node-member\" title=\"" + e.unique_id + "\">\n\n      <rect x=\"" + e._x + "\" y=\"" + x + "\" width=\"154\" height=\"108\" rx=\"12\"\n\n            fill=\"" + g + "\" stroke=\"" + u + "\" stroke-width=\"" + (p ? 2 : 1.5) + "\"/>\n\n      <circle cx=\"" +
    c + "\" cy=\"" + (x + 28) + "\" r=\"18\" fill=\"#111827\" stroke=\"" + u + "\" stroke-width=\"1.5\"/>\n\n      <text x=\"" + c + "\" y=\"" + (x + 33) + "\" text-anchor=\"middle\" fill=\"" + v + "\" font-size=\"11\" font-weight=\"700\" font-family=\"sans-serif\">" +
    ...
    (x + 68) + "\" text-anchor=\"middle\" fill=\"#9CA3AF\" font-size=\"8.5\" font-family=\"monospace\">" + e.unique_id + "</text>\n\n      <rect x=\"" + (c - 28) + "\" y=\"" + (x + 73) + "\" width=\"56\" height=\"13\" rx=\"6\" fill=\"" + v + "22\"/>\n\n      <text x=\"" +
    c + "\" y=\"" + (x + 83) + "\" text-anchor=\"middle\" fill=\"" + v + "\" font-size=\"8\" font-weight=\"700\" font-family=\"sans-serif\">" + b + "</text>\n\n      <text x=\"" + c + "\" y=\"" + (x + 93) + "\" text-anchor=\"middle\" fill=\"#6B7280\" font-size=\"8\" font-family=\"sans-serif\">" +
    (e.member_status || "") + "</text>\n\n      <text x=\"" + (e._x + 8) + "\" y=\"" + (x + 102) + "\" fill=\"#60A5FA\" font-size=\"8\" font-family=\"sans-serif\">G:" + fmtBV(e.left_bv_monthly) + "</text>\n\n      <text x=\"" + (e._x + 154 - 8) + "\" y=\"" + (x + 102) +
    "\" text-anchor=\"end\" fill=\"#34D399\" font-size=\"8\" font-family=\"sans-serif\">D:" + fmtBV(e.right_bv_monthly) + "</text>\n\n      " + (p ? "<text x=\"" + c + "\" y=\"" + (x + 8) + "\" text-anchor=\"middle\" fill=\"#FB923C\" font-size=\"7.5\" font-weight=\"700\" font-family=\"sans-serif\">HOLDING</text>" : "") +
    "\n\n    </g>"), t < n && (svgCollectElements(e.left, t + 1, n, a, s, r, c, x, "L", e.unique_id), svgCollectElements(e.right, t + 1, n, a, s, r, c, x, "R", e.unique_id));
}
function renderTreeSVG(e) {
  const t = Math.min(treeMaxDepth(e), 5),
    n = svgAssignPositions(e, 0, 0),
    a = 188 * (t + 1) + 40,
    s = [],
    r = [],
    i = [];
  return svgCollectElements(e, 0, t, s, r, i, null, null, null, null), "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"" + Math.max(n + 36, 400) + "\" height=\"" + a + "\" style=\"display:block;overflow:visible\">\n\n    <style>\n\n      .svg-slot-view rect { transition: stroke 0.2s, fill 0.2s; }\n\n      .svg-slot-view:hover rect { stroke: #5A1510 !important; fill: #1C1003 !important; }\n\n      .svg-slot-view:hover text { fill: #791E15 !important; }\n\n      .svg-node-member rect { transition: stroke 0.15s; }\n\n    </style>\n\n    <g transform=\"translate(18, 20)\">\n\n      " + s.join("\n") + "\n\n      " + i.join("\n") + "\n\n      " + r.join("\n") + "\n\n    </g>\n\n  </svg>";
}
...
function svgInitPanZoom(e) {
  const t = document.getElementById(e + "-vp"),
    n = document.getElementById(e + "-canvas");
  if (!t || !n) return;
  const a = {
    tx: 0,
    ty: 0,
  ...
  function s() {
    n.style.transform = "translate(" + a.tx + "px," + a.ty + "px) scale(" + a.scale + ")";
  }
  window._svgPZ[e] = a;
  let r = !1,
    i = 0,
  ...
  t.addEventListener("mousedown", e => {
    0 === e.button && (r = !0, i = e.clientX - a.tx, l = e.clientY - a.ty, t.style.cursor = "grabbing", e.preventDefault());
  }), window.addEventListener("mousemove", e => {
    r && (a.tx = e.clientX - i, a.ty = e.clientY - l, s());
  }), window.addEventListener("mouseup", () => {
    r = !1, t.style.cursor = "grab";
  });
  let d = 0,
    o = 0;
  t.addEventListener("touchstart", e => {
    1 === e.touches.length && (d = e.touches[0].clientX, o = e.touches[0].clientY);
  }, {
    passive: !0
  }), t.addEventListener("touchmove", e => {
    1 === e.touches.length && (a.tx += e.touches[0].clientX - d, a.ty += e.touches[0].clientY - o, d = e.touches[0].clientX, o = e.touches[0].clientY, s());
  }, {
    passive: !0
    ...
    const n = t.getBoundingClientRect(),
      r = e.clientX - n.left,
      i = e.clientY - n.top,
      l = e.deltaY > 0 ? .88 : 1.14,
      d = Math.min(3, Math.max(.15, a.scale * l));
    a.tx = r - (r - a.tx) * (d / a.scale), a.ty = i - (i - a.ty) * (d / a.scale), a.scale = d, s();
  }, {
    passive: !1
  });
}
function svgZoom(e, t) {
  const n = window._svgPZ[e];
  if (!n) return;
  const a = document.getElementById(e + "-vp");
  if (!a) return;
  const s = document.getElementById(e + "-canvas"),
    r = a.getBoundingClientRect(),
    i = r.width / 2,
    l = r.height / 2,
    d = Math.min(3, Math.max(.15, n.scale * t));
  n.tx = i - (i - n.tx) * (d / n.scale), n.ty = l - (l - n.ty) * (d / n.scale), n.scale = d, s.style.transform = "translate(" + n.tx + "px," + n.ty + "px) scale(" + n.scale + ")";
}
function svgReset(e) {
  const t = window._svgPZ[e];
  if (!t) return;
  t.tx = 0, t.ty = 0, t.scale = 1;
  const n = document.getElementById(e + "-canvas");
  n && (n.style.transform = "translate(0px,0px) scale(1)");
}
window._svgPZ = {};
let _fullTree = null,
  _classicRoot = null,
  _classicFocus = null,
  _classicNavStack = [];
const CLASSIC_DEPTH = 4;
function collectAllNodes(e, t = []) {
  return e ? (t.push(e), collectAllNodes(e.left, t), collectAllNodes(e.right, t), t) : t;
}
function findNodesByQuery(e) {
  if (!_fullTree || !e || e.trim().length < 1) return [];
  const t = e.trim().toLowerCase(),
    n = collectAllNodes(_fullTree),
    a = [];
  for (const e of n) {
    const n = ("" + (e.first_name || "") + " " + (e.last_name || "")).toLowerCase(),
      s = (e.unique_id || "").toLowerCase(),
      r = (e.first_name || "").toLowerCase(),
      i = (e.last_name || "").toLowerCase();
    if ((s.includes(t) || n.includes(t) || r.startsWith(t) || i.startsWith(t)) && (a.push(e), a.length >= 8)) break;
  }
  return a;
}
function treeSearchNavigateTo(e) {
  const t = findNodeById(_fullTree, e);
  if (!t) return;
  const n = document.getElementById("tree-search-dropdown"),
    a = document.getElementById("tree-search-input");
  if (n && n.classList.add("hidden"), a && (a.value = ""), _classicNavStack.push(_classicFocus), _classicFocus = t, "svg" === getTreeMode()) {
    const e = document.getElementById("member-tree-wrap");
    if (!e) return;
    const n = renderTreeSVG(t);
    e.innerHTML = svgPanZoomWrapper(n, "member-tree"), svgInitPanZoom("member-tree");
  } else renderClassicTreeInner();
}
function renderTreeSearchResults(e) {
  const t = document.getElementById("tree-search-dropdown");
  if (!t) return;
  if (!e || e.trim().length < 1) return void t.classList.add("hidden");
  const n = findNodesByQuery(e);
  if (0 === n.length) return t.innerHTML = "\n\n      <div class=\"px-4 py-3 text-sm text-gray-500 flex items-center gap-2\">\n\n        <i class=\"fas fa-search-minus text-gray-600\"></i>\n\n        Aucun r\xE9sultat pour <strong class=\"text-gray-400 ml-1\">\"" + e + "\"</strong>\n\n      </div>", void t.classList.remove("hidden");
  const a = {
  ...
  t.innerHTML = n.map((e, t) => {
    const n = a[e.current_rank] || "text-rouge-400",
      s = ("" + (e.first_name || "?")[0] + (e.last_name || "?")[0]).toUpperCase(),
      r = e.id === _classicRoot?.id,
...
function initTreeSearch() {
  const e = document.getElementById("tree-search-input"),
    t = document.getElementById("tree-search-dropdown");
  e && t && (e.addEventListener("input", e => renderTreeSearchResults(e.target.value)), e.addEventListener("focus", e => {
    e.target.value && renderTreeSearchResults(e.target.value);
  }), e.addEventListener("blur", () => setTimeout(() => t.classList.add("hidden"), 150)), e.addEventListener("keydown", n => {
    if ("Escape" === n.key && (t.classList.add("hidden"), e.blur()), "Enter" === n.key) {
      const e = t.querySelector(".tree-search-result");
      e && e.click();
    }
  }), document.addEventListener("keydown", t => {
    "/" !== t.key || document.activeElement === e || ["INPUT", "TEXTAREA"].includes(document.activeElement?.tagName) || (t.preventDefault(), e.focus());
  }));
}
function findNodeById(e, t) {
  return e ? e.id === t ? e : findNodeById(e.left, t) || findNodeById(e.right, t) : null;
}
function classicNavigateTo(e) {
  const t = findNodeById(_fullTree, e);
  t && (_classicNavStack.push(_classicFocus), _classicFocus = t, renderClassicTreeInner());
}
function classicGoBack() {
  0 !== _classicNavStack.length && (_classicFocus = _classicNavStack.pop(), renderClassicTreeInner());
}
function classicGoHome() {
  _classicNavStack = [], _classicFocus = _classicRoot, renderClassicTreeInner();
}
function renderClassicNode(e, t, n = !1) {
  if (!e) return "";
  const a = n,
    s = e.in_holding_tank ? '<div class="text-[9px] text-orange-400 mt-0.5"><i class="fas fa-clock mr-0.5"></i>Holding</div>' : "",
    r = !a,
    i = "\n\n    tree-node-card " + ({
      captain: "border-yellow-500/60 ring-yellow-500/20",
      commander: "border-purple-500/60 ring-purple-500/20",
      admiral: "border-blue-400/60 ring-blue-400/20",
      vice_admiral: "border-green-400/60 ring-green-400/20",
      ambassador: "border-orange-500/60 ring-orange-500/20",
      diamond: "border-sky-400/60 ring-sky-400/20",
      triple_diamond: "border-fuchsia-500/60 ring-fuchsia-500/20",
      crown: "border-red-500/60 ring-red-500/20"
    }[e.current_rank] || "border-dark-500 ring-dark-500/0") + "\n\n    " + (a ? "ring-2 bg-dark-700 scale-105 shadow-lg shadow-black/40" : "") + "\n\n    " + (r ? "cursor-pointer hover:border-rouge-500/60 hover:bg-dark-700 transition-all" : "cursor-default") + "\n\n  ",
    l = "\n\n    <div class=\"w-9 h-9 rounded-full bg-dark-900 flex items-center justify-center mx-auto mb-2 text-xs font-bold " + ({
      ...
      triple_diamond: "text-fuchsia-400",
      crown: "text-red-400"
    }[e.current_rank] || "text-rouge-400") + " " + (a ? "ring-2 ring-gold-500/40" : "") + "\">\n\n      " + (e.first_name || "?")[0] + (e.last_name || "?")[0] + "\n\n    </div>\n\n    <div class=\"text-xs font-semibold text-white truncate\">" + e.first_name + " " + e.last_name +
      "</div>\n\n    <div class=\"text-[10px] text-gray-400 font-mono\">" + e.unique_id + "</div>\n\n    <div class=\"mt-1\">" + rankBadge(e.current_rank) + "</div>\n\n    " + s + "\n\n    " + (e.member_status ? "<div class=\"text-[9px] text-gray-400 mt-0.5\">" + e.member_status +
    ...
    d = r ? "<div class=\"" + i + "\" onclick=\"classicNavigateTo('" + e.id + "')\" title=\"Voir l'arbre depuis " + e.first_name + "\">" + l + "</div>" : "<div class=\"" + i + "\">" + l + "</div>";
  if (t >= 3) return "<div class=\"tree-node\">" + d + "</div>";
  const o = (e, t) => {
    const n = currentMember && currentMember.unique_id ? currentMember.unique_id : e;
...
function renderClassicTreeInner() {
  const e = document.getElementById("member-tree-wrap");
  if (!e || !_classicFocus) return;
  const t = _classicFocus.id === _classicRoot.id,
    n = _classicNavStack.length > 0,
    a = _classicNavStack.length > 0 ? _classicNavStack.map((e, t) => "\n\n        <span class=\"text-gray-500 text-xs\">" + (0 === t ? '<i class="fas fa-home text-[10px]"></i>' : "") + " " + e.first_name + "</span>\n\n        <span class=\"text-gray-600 text-xs\">\u203A</span>").join("") : "";
...
function setTreeMode(e) {
  localStorage.setItem("tree_mode", e);
}
function toggleTreeMode() {
  const e = "classic" === getTreeMode() ? "svg" : "classic";
  setTreeMode(e), _renderTreeWithMode(e), _updateTreeToggleBtn(e);
}
function _updateTreeToggleBtn(e) {
  const t = document.getElementById("tree-toggle-btn");
  t && ("classic" === e ? (t.innerHTML = '<i class="fas fa-diagram-project mr-1.5"></i>Vue SVG', t.title = "Passer en vue SVG (pan/zoom)") : (t.innerHTML = '<i class="fas fa-sitemap mr-1.5"></i>Vue Classique', t.title = "Passer en vue classique (navigation par clic)"));
}
function _renderTreeWithMode(e) {
  if (_fullTree) if ("classic" === e) _classicFocus || (_classicFocus = _classicRoot, _classicNavStack = []), renderClassicTreeInner();else {
    const e = document.getElementById("member-tree-wrap");
    if (!e) return;
    e.innerHTML = "";
    const t = renderTreeSVG(_fullTree);
    e.innerHTML = svgPanZoomWrapper(t, "member-tree"), svgInitPanZoom("member-tree");
  }
}
async function renderTree(e) {
  const t = getTreeMode();
  e.innerHTML = "\n  <div class=\"space-y-3\">\n    <!-- Header : titre + recherche + toggle -->\n    <div class=\"flex items-center gap-3 flex-wrap\">\n      <h2 class=\"text-xl font-bold flex-shrink-0\">Arbre Binaire</h2>\n\n      <!-- Barre de recherche premium -->\n      <div class=\"relative flex-1 min-w-[200px] max-w-sm\" id=\"tree-search-box\">\n        <div class=\"relative\">\n          <i class=\"fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-xs pointer-events-none\"></i>\n          <input id=\"tree-search-input\" type=\"text\" autocomplete=\"off\"\n" +
  ...
  try {
    const e = await api("GET", "/members/tree?depth=12");
    if (!e.tree) return void (document.getElementById("member-tree-wrap").innerHTML = '\n        <div class="bg-dark-800 rounded-2xl border border-dark-600 p-10 text-center text-gray-400">\n          <i class="fas fa-tree text-4xl text-gray-600 mb-4 block"></i>\n          Vous n\'êtes pas encore placé dans l\'arbre binaire.\n        </div>');
    _fullTree = e.tree, _classicRoot = e.tree, _classicFocus = e.tree, _classicNavStack = [], initTreeSearch(), _renderTreeWithMode(t);
  } catch (e) {
    document.getElementById("member-tree-wrap").innerHTML = "<div class=\"bg-dark-800 rounded-2xl border border-dark-600 p-6 text-red-400\">Erreur : " + (e.error || e.message) + "</div>";
  }
}
async function renderBV(e, page, perPage) {
  page = page || window._bvPage || 1;
  perPage = perPage || window._bvPerPage || 25;
  window._bvPage = page;
  window._bvPerPage = perPage;
  e.innerHTML = '<div class="space-y-4"><h2 class="text-xl font-bold">Journal BV</h2><div class="bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden"><div id="bv-table"></div></div></div>';
  try {
    const d = await api("GET", "/members/bv-journal?page=" + page + "&per_page=" + perPage);
    function bvBadge(v) {
      var lbl = {
        personal: "Personnel",
        propagated: "Propag\u00e9",
        fast_start_contribution: "Fast Start"
      };
      var col = {
        personal: "bg-emerald-500/20 text-emerald-400",
        propagated: "bg-blue-500/20 text-blue-400",
        fast_start_contribution: "bg-orange-500/20 text-orange-400"
      };
      return '<span class="text-xs px-2 py-0.5 rounded-full ' + (col[v] || "bg-gray-500/20 text-gray-400") + '">' + (lbl[v] || v) + "</span>";
    }
    function srcRank(r) {
      return r && r !== "none" ? rankBadge(r) : '<span class="text-gray-600 text-xs">—</span>';
    }
    function bvCell(v, cls) {
      return v != null ? '<span class="' + cls + ' font-semibold">' + fmtBV(v) + "</span>" : '<span class="text-gray-600">—</span>';
    }
    var rows = (d.logs || []).map(function (l) {
      var src = l.first_name ? '<div class="font-medium text-sm text-white">' + l.first_name + " " + l.last_name + '</div><div class="text-xs text-rouge-400/70 font-mono">' + l.source_unique_id + "</div>" : '<span class="text-gray-600">—</span>';
      return "<tr>" + "<td>" + bvBadge(l.bv_type) + "</td>" + "<td class=\"text-rouge-400 font-bold\">" + fmtBV(l.bv_amount) + "</td>" + "<td>" + src + "</td>" + "<td>" + srcRank(l.source_rank) + "</td>" + "<td>" + bvCell(l.source_left_bv, "text-blue-400") + "</td>" + "<td>" + bvCell(l.source_right_bv, "text-green-400") + "</td>" + "<td class=\"text-gray-400\">" + (l.period || "—") + "</td>" + "<td class=\"text-gray-500 text-xs\">" + fmtDate(l.created_at) + "</td>" + "</tr>";
    }).join("") || '<tr><td colspan="8" class="text-center text-gray-500 py-8">Aucun BV enregistr\u00e9</td></tr>';
    document.getElementById("bv-table").innerHTML = '<div class="overflow-x-auto"><table class="data-table"><thead><tr>' + '<th>Type</th><th>Montant BV</th><th>Source</th><th>Rang</th>' + '<th class="text-blue-400"><i class="fas fa-arrow-left mr-1"></i>BV Gauche</th>' + '<th class="text-green-400"><i class="fas fa-arrow-right mr-1"></i>BV Droite</th>' + '<th>P\u00e9riode</th><th>Date</th>' + '</tr></thead><tbody>' + rows + '</tbody></table></div>' + '<div class="px-6 py-3 text-sm text-gray-500 border-t border-dark-600">' + (d.total || 0) + ' entr\u00e9e(s)</div>' + '<div id="bv-pagination" class="px-6 py-3 border-t border-dark-600"></div>';
    renderPagination("bv-pagination", d.total || 0, page, perPage, "(function(p){renderBV(document.getElementById('page-content'),p,window._bvPerPage)})", "(function(pp,p){renderBV(document.getElementById('page-content'),p,pp)})");
  } catch (err) {
    e.innerHTML = '<div class="p-4 text-red-400">Erreur : ' + (err.error || err.message || "Inconnue") + "</div>";
  }
}
async function renderCommissions(e, page, perPage) {
  ...
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const [commData, pweData] = await Promise.all([api('GET', "/members/commissions?page=" + page + "&per_page=" + perPage), api('GET', '/members/transactions?type=pending&per_page=100')]);
    ...
    function pweCard(p) {
      const tc = typeColors[p.commission_type] || {
        icon: 'fa-coins',
        color: 'gold'
      };
      ...
      return "\n      <div class=\"rounded-xl border " + cc.border + " " + cc.bg + " overflow-hidden\">\n        <!-- En-t\xEAte -->\n        <div class=\"px-4 py-3 flex items-center justify-between gap-2 flex-wrap\">\n          <div class=\"flex items-center gap-2.5\">\n            <div class=\"w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 " +
        ...
        fmt$(dejaLiberé) + "</div>\n          </div>\n          <div class=\"bg-dark-800/60 rounded-lg p-2\">\n            <div class=\"text-[10px] text-gray-400 mb-0.5\">Reste \xE0 lib\xE9rer</div>\n            <div class=\"font-bold " + (aLiberér > 0 ? 'text-orange-400' : 'text-gray-500') +
        " text-sm\">" + fmt$(aLiberér) + "</div>\n          </div>\n        </div>\n\n        <!-- Barre de progression -->\n        <div class=\"px-4 py-2\">\n          " + progressBar(pct, tc.color) + "\n          <div class=\"flex justify-between text-[10px] text-gray-500 mt-1\">\n            <span>" +
        (p.days_paid || 0) + " jour" + ((p.days_paid || 0) > 1 ? 's' : '') + " vers\xE9" + ((p.days_paid || 0) > 1 ? 's' : '') + "</span>\n            <span>" + daysLeft + " jour" + (daysLeft > 1 ? 's' : '') + " restant" + (daysLeft > 1 ? 's' : '') + " / " + (p.days_in_month || 30) +
        ...
        fmtDate(p.eligible_date) + "</strong>, tous les jours \xE9coul\xE9s seront vers\xE9s d'un coup (rattrapage),\n          puis une prime journali\xE8re de <strong>" + fmt$(p.amount_per_day) + "</strong> sera vers\xE9e chaque jour jusqu'\xE0 la fin du mois.\n        </div>" : '') +
...
async function showCommissionCalendar(pweId) {
  ...
  try {
    ...
    const rows = d.calendar.map(day => "\n      <div class=\"flex items-center gap-2 py-1 border-b border-dark-600/40 last:border-0\">\n        <div class=\"w-6 text-[10px] text-gray-500 text-right flex-shrink-0\">" + day.day_index + "</div>\n        <div class=\"text-xs text-gray-400 w-20 flex-shrink-0\">" +
      day.date + "</div>\n        <div class=\"flex-1 text-xs text-gray-300\">" + (day.is_catchup ? '<span class="text-orange-300 font-medium">Rattrapage</span>' : day.status === 'waiting' ? '<span class="text-orange-400">Délai 14j</span>' : day.status === 'paid' ? '<span class="text-emerald-400">Versé</span>' : '<span class="text-blue-400">À venir</span>') +
...
async function renderWallet(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const t = await api("GET", "/members/wallet"),
      n = t.wallet,
      a = t.member,
      s = t.pendingWithdrawals || [];
    const _wdCfg = t.withdrawalConfig || {};
    const i = _wdCfg.is_today_allowed || false;
    const o = _wdCfg.next_allowed_date ? new Date(_wdCfg.next_allowed_date + "T12:00:00").toLocaleDateString("fr-FR", {
        weekday: "long",
        day: "numeric",
        month: "long"
      }) : _wdCfg.allowed_days_labels && _wdCfg.allowed_days_labels.length ? _wdCfg.allowed_days_labels.join(", ") : "—",
      c = e => ({
        ...
        completed: '<span class="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full">✓ Complété</span>',
        rejected: '<span class="text-xs bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full">✗ Rejeté</span>'
      })[e] || "<span class=\"text-xs bg-gray-500/20 text-gray-400 px-2 py-0.5 rounded-full\">" + e + "</span>",
      x = t.pendingWallet || {},
      p = x.total || 0,
      m = x.entries || [],
      u = x.nextEligible || null;
    let g = null;
    try {
      g = await api("GET", "/members/dreamiles");
    } catch (e) {}
    const f = g?.wallet?.dreamiles || 0,
      b = g?.credited_this_month || 0,
      v = t.ccBalance || {
        total: 0,
        available: 0,
        held: 0
    ...
    e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n\n\n      <!-- En-t\xEAte -->\n\n      <div class=\"flex items-center justify-between\">\n\n        <div>\n\n          <h2 class=\"text-xl font-bold text-white\">Mon Portefeuille</h2>\n\n          <p class=\"text-xs text-gray-500 mt-0.5\">Cliquez sur un wallet pour voir l'historique des mouvements</p>\n\n        </div>\n\n        <div class=\"text-right\">\n\n          <div class=\"text-xs text-gray-500\">Patrimoine total</div>\n\n          <div class=\"text-2xl font-bold text-white\">" +
      ...
      return "<div>\n\n                <div class=\"flex justify-between text-xs\"><span class=\"text-gray-400 truncate\">" + (WALLET_COMMISSION_LABELS[e.commission_type] || e.commission_type) + (e.source_name ? " \xB7 <span class=\"text-gray-300\">" + e.source_name + "</span> <span class=\"text-gray-500\">" + (e.source_unique_id || "") + "</span>" : "") + "</span><span class=\"text-orange-300 ml-2 flex-shrink-0\">" + fmt$(t) + "</span></div>\n\n                <div class=\"text-xs mt-0.5\">" + (e.status === "pending_release" ? "<span class=\"text-yellow-500/70\">Lib\xE9ration le " + (e.eligible_date ? new Date(e.eligible_date).toLocaleDateString("fr-FR", {
        day: "2-digit",
        month: "short",
        year: "numeric"
      }) : "") + "</span>" : e.status === "active" ? "<span class=\"text-green-500/70\">" + Math.max(0, (e.days_in_month || 0) - (e.days_paid || 0)) + "j restants \xB7 " + (e.amount_per_day || 0).toFixed(2) + "$/j</span>" : "<span class=\"text-gray-500\">Lib\xE9r\xE9</span>") + "</div>\n\n                <div class=\"h-1 bg-dark-600 rounded-full mt-0.5\"><div class=\"h-1 bg-orange-500/60 rounded-full\" style=\"width:" + n + "%\"></div></div>\n\n              </div>";
      ...
      fmt$(p) + "</span>\n\n            </div>\n\n            <div class=\"flex justify-between items-center\">\n\n              <span class=\"text-xs text-gray-500\">Cr\xE9dit Croissance</span>\n\n              <span class=\"text-sm font-bold text-pink-400\">" + fmt$(y) +
      "</span>\n\n            </div>\n\n            <div class=\"flex justify-between items-center\">\n\n              <span class=\"text-xs text-gray-500\">R\xE9serve Strat\xE9gique</span>\n\n              <span class=\"text-sm font-bold text-cyan-400\">" + fmt$(a?.reserve_strategique) +
      "</span>\n\n            </div>\n\n            <div class=\"flex justify-between items-center\">\n\n              <span class=\"text-xs text-gray-500\">Dreamiles Luxia</span>\n\n              <span class=\"text-sm font-bold text-yellow-400\">" + f.toLocaleString() +
...
async function renderWithdraw(el, page, perPage) {
  page = page || window._wdPage || 1;
  perPage = perPage || window._wdPerPage || 25;
  window._wdPage = page;
  window._wdPerPage = perPage;
  el.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const walletData = await api("GET", "/members/wallet");
    const t = walletData.member;
    const wdConfig = walletData.withdrawalConfig || {
      min_withdrawal: 50,
      kyc_level: 0,
      kyc_max_withdrawal: 0,
      enabled: true,
      allowed_days: [3],
      is_today_allowed: false,
    ...
    const minWd = wdConfig.min_withdrawal || 50;
    const kycMax = wdConfig.kyc_max_withdrawal || 0;
    const wdResp = await api("GET", "/members/withdrawals?page=" + page + "&per_page=" + perPage);
    window._wdTotal = wdResp.total || 0;
    const n = wdResp.withdrawals || [];
    const now = new Date();
    const isTodayAllowed = wdConfig.is_today_allowed !== undefined ? wdConfig.is_today_allowed : wdConfig.allowed_days.includes(now.getDay());
    const allowedDaysLabels = wdConfig.allowed_days_labels && wdConfig.allowed_days_labels.length ? wdConfig.allowed_days_labels : [];
    const nextAllowedStr = wdConfig.next_allowed_date ? new Date(wdConfig.next_allowed_date + "T12:00:00").toLocaleDateString("fr-FR", {
      weekday: "long",
      day: "numeric",
      month: "long"
    }) : "";
    const wdEnabled = wdConfig.enabled !== false;
    const hasPending = (walletData.pendingWithdrawals || []).some(w => ["pending", "processing"].includes(w.status));
    const canWithdraw = "verified" === t?.kyc_status && isTodayAllowed && wdEnabled && t?.wallet_balance >= minWd && !hasPending;
    const wdStatusBadge = s => ({
      pending: '<span class="inline-flex items-center gap-1 text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full"><i class="fas fa-clock text-[10px]"></i>En attente</span>',
      processing: '<span class="inline-flex items-center gap-1 text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full"><i class="fas fa-spinner fa-spin text-[10px]"></i>En cours</span>',
      completed: '<span class="inline-flex items-center gap-1 text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full"><i class="fas fa-check text-[10px]"></i>Confirmé</span>',
      rejected: '<span class="inline-flex items-center gap-1 text-xs bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full"><i class="fas fa-times text-[10px]"></i>Rejeté</span>'
    ...
    el.innerHTML = "\n    <div class=\"max-w-2xl space-y-6\">\n      <div class=\"flex items-center justify-between\">\n        <div>\n          <h2 class=\"text-xl font-bold text-white flex items-center gap-2\"><i class=\"fas fa-paper-plane text-rouge-400\"></i> Demande de Retrait</h2>\n          <p class=\"text-xs text-gray-500 mt-0.5\">Portefeuille principal uniquement — Traitement sous 24-48h</p>\n        </div>\n        <button onclick=\"showPage('wallet')\" class=\"text-xs text-gray-400 hover:text-white transition flex items-center gap-1\"><i class=\"fas fa-arrow-left text-[10px]\"></i>Retour</button>\n" +
      ...
      ">\n            </div>\n            <div>\n              <label class=\"form-label\">Num\xE9ro de compte</label>\n              <input id=\"wd-bank-account\" type=\"text\" placeholder=\"Num\xE9ro de compte (optionnel)\" class=\"form-input\" " + (canWithdraw ? "" : "disabled") +
      ">\n            </div>\n            <div class=\"sm:col-span-2\">\n              <label class=\"form-label\">Adresse de la banque</label>\n              <input id=\"wd-bank-address\" type=\"text\" placeholder=\"Adresse compl\xE8te (optionnel)\" class=\"form-input\" " +
      ...
      (canWithdraw ? "" : "disabled") + ">\n            </div>\n            <div>\n              <label class=\"form-label\">Devise <span class=\"text-red-400\">*</span></label>\n              <input id=\"wd-crypto-currency\" type=\"text\" placeholder=\"USDT, BTC...\" class=\"form-input\" " +
    ...
    setTimeout(function () {
      if (typeof wdInitForm === 'function') wdInitForm();
    }, 50);
    renderPagination("wd-pagination", window._wdTotal || 0, page, perPage, function (p) {
      renderWithdraw(el, p, window._wdPerPage);
    }, function (pp, p) {
      renderWithdraw(el, p, pp);
    });
  } catch (err) {
    el.innerHTML = "<div class=\"p-4 text-red-400 flex items-center gap-2\"><i class=\"fas fa-triangle-exclamation\"></i>Erreur : " + (err.error || err.message) + "</div>";
  }
}
function wdUpdatePreview() {
  const cfg = window._wdWithdrawConfig || {};
  const amount = parseFloat(document.getElementById("wd-amount")?.value) || 0;
  const paypal = document.getElementById("wd-paypal")?.value || cfg.paypal || "—";
  const preview = document.getElementById("wd-preview");
  if (!preview) return;
  if (amount > 0) {
    preview.classList.remove("hidden");
    document.getElementById("prev-amount").textContent = fmt$(amount);
    document.getElementById("prev-net").textContent = fmt$(amount);
    document.getElementById("prev-paypal").textContent = paypal || "—";
  } else {
    preview.classList.add("hidden");
  }
}
function doWithdrawConfirm() {
  const cfg = window._wdWithdrawConfig || {
    minWd: 50,
    kycMax: 0,
    balance: 0
  ...
  const method = window._wdActiveMethod || "paypal";
  const errEl = document.getElementById("wd-error");
  const errMsg = document.getElementById("wd-error-msg");
  ...
  hideErr();
  if (!amount || amount <= 0) return showErr("Veuillez saisir un montant valide.");
  if (amount < cfg.minWd) return showErr("Montant minimum : " + fmt$(cfg.minWd));
  if (amount > cfg.balance) return showErr("Solde insuffisant. Disponible : " + fmt$(cfg.balance));
  if (cfg.kycMax > 0 && amount > cfg.kycMax) return showErr("Votre niveau KYC limite les retraits \xE0 " + fmt$(cfg.kycMax) + " par transaction.");
  if (!pin || pin.length < 4) return showErr("Veuillez saisir votre PIN (4 ou 6 chiffres).");
  let payload = {
    amount,
    pin,
  ...
  let destSummary = "";
  if (method === "paypal") {
    const email = document.getElementById("wd-paypal")?.value?.trim() || "";
    if (!email || !email.includes("@")) return showErr("Email PayPal invalide.");
    payload.paypal_email = email;
    methodLabel = '<i class="fab fa-paypal mr-1 text-blue-400"></i>Vers PayPal';
    destSummary = email;
  } else if (method === "bank_transfer") {
    const holder = document.getElementById("wd-bank-holder")?.value?.trim() || "";
    const iban = document.getElementById("wd-bank-iban")?.value?.trim() || "";
    const bic = document.getElementById("wd-bank-bic")?.value?.trim() || "";
    if (!holder) return showErr("Titulaire du compte requis.");
    if (!iban) return showErr("IBAN requis.");
    if (!bic) return showErr("BIC requis.");
    payload.payout_bank_holder = holder;
    payload.payout_bank_iban = iban;
    payload.payout_bank_bic = bic;
    payload.payout_bank_name = document.getElementById("wd-bank-name")?.value?.trim() || "";
    payload.payout_bank_account = document.getElementById("wd-bank-account")?.value?.trim() || "";
    payload.payout_bank_address = document.getElementById("wd-bank-address")?.value?.trim() || "";
    payload.payout_bank_city = document.getElementById("wd-bank-city")?.value?.trim() || "";
    payload.payout_bank_country = document.getElementById("wd-bank-country")?.value?.trim() || "";
    payload.payout_bank_swift = document.getElementById("wd-bank-swift")?.value?.trim() || "";
    methodLabel = '<i class="fas fa-university mr-1 text-green-400"></i>Virement bancaire';
    destSummary = "" + iban + " \u2014 " + holder;
    ...
    const currency = document.getElementById("wd-crypto-currency")?.value?.trim() || "";
    if (!addr) return showErr("Adresse wallet requise.");
    if (!network) return showErr("Réseau blockchain requis.");
    if (!currency) return showErr("Devise crypto requise.");
    payload.payout_crypto_address = addr;
    payload.payout_crypto_network = network;
    payload.payout_crypto_currency = currency;
    methodLabel = '<i class="fas fa-coins mr-1 text-yellow-400"></i>Cryptomonnaie';
    destSummary = "" + currency + " (" + network + ") \u2014 " + addr.slice(0, 12) + "...";
...
async function doWithdraw() {
  const p = window._wdCurrentPayload;
  if (!p) return;
  const btn = document.getElementById("wd-confirm-btn");
  const errEl = document.getElementById("wd-confirm-error");
  if (btn) {
    btn.disabled = true;
  ...
  try {
    await api("POST", "/members/withdraw", p);
    closeModal();
    showToast("Demande de retrait soumise ! Traitement sous 24-48h.", "success");
    showPage("wallet");
  } catch (e) {
    if (errEl) {
      errEl.textContent = e.error || "Erreur lors de la soumission";
      errEl.classList.remove("hidden");
    }
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-check mr-2"></i>Valider';
    }
  }
}
let _wdActiveMethod = "paypal";
window._wdActiveMethod = "paypal";
function wdSwitchMethod(method) {
  window._wdActiveMethod = method;
  _wdActiveMethod = method;
  const methods = ["paypal", "bank_transfer", "crypto"];
  methods.forEach(m => {
    const tab = document.getElementById("wd-tab-" + m);
    const panel = document.getElementById("wd-panel-" + m);
    if (tab) {
      if (m === method) {
        tab.classList.add("bg-rouge-500", "text-dark-900", "border-rouge-500");
        tab.classList.remove("bg-dark-700", "text-gray-400", "border-dark-600");
      } else {
        tab.classList.remove("bg-rouge-500", "text-dark-900", "border-rouge-500");
        tab.classList.add("bg-dark-700", "text-gray-400", "border-dark-600");
      }
    }
    if (panel) {
...
function wdInitForm() {
  const info = window._memberPayoutInfo || {};
  const method = info.payout_method || "paypal";
  wdSwitchMethod(method);
  const setVal = (id, v) => {
    if (v) {
      const el = document.getElementById(id);
      if (el) el.value = v;
  ...
  setVal("wd-paypal", info.payout_paypal_email);
  setVal("wd-bank-holder", info.payout_bank_holder);
  setVal("wd-bank-iban", info.payout_bank_iban);
  setVal("wd-bank-bic", info.payout_bank_bic);
  setVal("wd-bank-name", info.payout_bank_name);
  setVal("wd-bank-account", info.payout_bank_account);
  setVal("wd-bank-address", info.payout_bank_address);
  setVal("wd-bank-city", info.payout_bank_city);
  setVal("wd-bank-country", info.payout_bank_country);
  setVal("wd-bank-swift", info.payout_bank_swift);
  setVal("wd-crypto-address", info.payout_crypto_address);
  setVal("wd-crypto-network", info.payout_crypto_network);
  setVal("wd-crypto-currency", info.payout_crypto_currency);
}
async function doConvertDreamiles() {
  let e = null;
  try {
    e = await api("GET", "/members/dreamiles");
  } catch (e) {
    return void showToast("Impossible de charger le solde Dreamiles", "error");
  }
  const t = e?.wallet?.dreamiles || 0;
  t <= 0 ? showToast("Aucun Dreamile disponible à convertir", "error") : showModal("\n\n  <div class=\"p-6 space-y-5 max-w-sm mx-auto\">\n\n    <div class=\"flex justify-between items-center\">\n\n      <div class=\"flex items-center gap-3\">\n\n        <div class=\"w-10 h-10 rounded-xl bg-yellow-500/20 flex items-center justify-center\">\n\n          <i class=\"fas fa-sync text-yellow-400\"></i>\n\n        </div>\n\n        <div>\n\n          <h3 class=\"font-bold text-white\">Convertir des Dreamiles</h3>\n\n          <p class=\"text-xs text-gray-500\">1 Dreamile = 1 $ dans votre wallet principal</p>\n\n        </div>\n" +
...
async function confirmConvertDreamiles(e) {
  const t = parseInt(document.getElementById("drm-convert-amount").value),
    n = document.getElementById("drm-convert-error");
  if (n.classList.add("hidden"), !t || t <= 0) return n.textContent = "Montant invalide", void n.classList.remove("hidden");
  if (t > e) return n.textContent = "Maximum disponible : " + e + " DRM", void n.classList.remove("hidden");
  try {
    const e = await api("POST", "/members/dreamiles/convert", {
      dreamiles: t
    });
    closeModal(), showToast("" + t.toLocaleString() + " Dreamiles convertis \u2192 " + fmt$(e.usd_credited) + " cr\xE9dit\xE9s dans votre wallet !"), showPage("wallet");
  } catch (e) {
    n.textContent = e.error || "Erreur lors de la conversion", n.classList.remove("hidden");
  }
}
const TX_TYPE_CONFIG = {
    ...
    valorisation_recommandation_daily: {
      label: "Valorisation Reco.",
      badge: "bg-emerald-500/15 text-emerald-400",
      icon: "fa-handshake",
      group: "Versement journalier"
    },
    bonus_influence_daily: {
      label: "Bonus Influence",
      badge: "bg-purple-500/15 text-purple-400",
      icon: "fa-sitemap",
      group: "Versement journalier"
    },
    bonus_rayonnement_daily: {
      label: "Bonus Rayonnement",
      badge: "bg-cyan-500/15 text-cyan-400",
      icon: "fa-sun",
      group: "Versement journalier"
    },
    credit_daily: {
      label: "Versement journalier",
      badge: "bg-emerald-500/15 text-emerald-400",
    ...
    prime_leadership: {
      label: "Prime Leadership",
      badge: "bg-rouge-500/15 text-rouge-400",
      icon: "fa-crown",
      group: "Commission créditée"
    },
    fast_start: {
    ...
    bonus_influence: {
      label: "Bonus d'Influence",
      badge: "bg-purple-500/15 text-purple-400",
      icon: "fa-sitemap",
      group: "Commission créditée"
    },
    bonus_rayonnement: {
      label: "Bonus de Rayonnement",
      badge: "bg-cyan-500/15 text-cyan-400",
      icon: "fa-sun",
      group: "Commission créditée"
    },
    ...
    dreamiles_conversion: {
      label: "Conversion Luxia",
  ...
  TX_DETAIL_LABELS = {
    prime_leadership: {
      title: "Prime de Leadership",
      color: "text-rouge-400",
      icon: "fa-crown",
      desc: "Prime mensuelle basée sur le volume de réseau (BV) généré par les deux jambes."
    },
    fast_start: {
      title: "Fast Start Bonus",
      color: "text-green-400",
      icon: "fa-rocket",
      desc: "Bonus de démarrage rapide attribué selon le BV accumulé dans les premiers jours."
    },
    valorisation_recommandation: {
      title: "Valorisation Recommandation",
      color: "text-blue-400",
      icon: "fa-handshake",
      desc: "Bonus versé pour chaque parrainage direct ayant activé un package."
    },
    bonus_influence: {
      title: "Bonus d'Influence",
      color: "text-purple-400",
      icon: "fa-sitemap",
      desc: "Bonus calculé sur la Prime de Leadership d'un membre de la downline (chaîne sponsor, 5 niveaux)."
    },
    bonus_rayonnement: {
      title: "Bonus de Rayonnement",
      color: "text-cyan-400",
      icon: "fa-sun",
      desc: "10% de la Prime de Leadership de chaque filleul direct ayant validé les conditions du mois."
    },
    reserve_strategique: {
      title: "Réserve Stratégique",
      color: "text-amber-400",
      icon: "fa-piggy-bank",
      desc: "Épargne constituée sur 3 ans, abondée par le réseau. Libération progressive au terme."
    },
    credit_croissance: {
      title: "Crédit de Croissance",
      color: "text-teal-400",
      icon: "fa-chart-line",
      desc: "Crédit différé utilisable pour l'activation ou le renouvellement de packages."
    },
    campus_purchase: {
      title: "Achat Formation Campus",
      color: "text-violet-400",
      icon: "fa-graduation-cap",
      desc: "Achat d'une formation via le Campus LEADER."
    }
  },
  INFLUENCE_LEVEL_PCT = {
    1: 5,
...
function _parseTxInfluenceLevel(e) {
  if (!e) return null;
  const t = e.match(/Niveau\s+(\d)/i);
  return t ? parseInt(t[1]) : null;
}
function _parseTxRayonnementName(e) {
  if (!e) return null;
  const t = e.match(/Bonus de Rayonnement\s+—\s+(.+?)\s+—/);
  return t ? t[1].trim() : null;
}
function _buildSourceSection(e) {
  const t = e.transaction_type || e.tx_type || "";
  if (!["bonus_influence", "bonus_rayonnement", "valorisation_recommandation"].includes(t)) return "";
  let n = e.source_name || null,
    a = e.source_unique_id || null,
    s = e.source_rank || null;
  n || "bonus_rayonnement" !== t || (n = _parseTxRayonnementName(e.description));
  let r = "",
    i = "",
    l = "";
  if ("bonus_influence" === t) {
    const t = _parseTxInfluenceLevel(e.description),
      n = t && INFLUENCE_LEVEL_PCT[t] || null;
    t && (r = "<span class=\"inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-300 text-xs font-semibold\">\n\n                      <i class=\"fas fa-layer-group text-[10px]\"></i> Niveau " + t + "\n\n                    </span>"), n && (i = "<span class=\"inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 text-xs font-medium\">\n\n                    " +
  ...
  }[t] || "text-gray-400 bg-dark-600") + " flex items-center justify-center text-sm font-bold flex-shrink-0\">\n\n          " + ("—" !== d ? d.charAt(0).toUpperCase() : "?") + "\n\n        </div>\n\n        <div class=\"flex-1 min-w-0\">\n\n          <div class=\"font-semibold text-white text-sm truncate\">" +
    d + "</div>\n\n          <div class=\"text-xs text-gray-400 mt-0.5 flex items-center gap-2 flex-wrap\">\n\n            " + (a ? "<span class=\"font-mono\">" + a + "</span>" : "") + "\n\n            " + (s ? "<span class=\"inline-flex items-center gap-1\"><i class=\"fas fa-medal text-[10px] text-rouge-400\"></i>" +
    s + "</span>" : "") + "\n\n          </div>\n\n        </div>\n\n        <div class=\"flex flex-col items-end gap-1 flex-shrink-0\">\n\n          " + r + "\n\n          " + i + "\n\n        </div>\n\n      </div>\n\n      " + l + "\n\n    </div>";
}
let _txModalData = null;
function showTxDetail(e) {
  const t = "string" == typeof e ? JSON.parse(e) : e;
  _txModalData = t;
  const n = TX_TYPE_CONFIG[t.transaction_type || t.tx_type] || TX_TYPE_CONFIG.commission,
    a = TX_DETAIL_LABELS[t.transaction_type] || null,
    s = (t.amount ?? 0) >= 0,
    r = _buildSourceSection(t),
  ...
  l.id = "tx-detail-modal", l.className = "fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4", l.innerHTML = "\n\n    <!-- Overlay -->\n\n    <div class=\"absolute inset-0 bg-black/70 backdrop-blur-sm\" onclick=\"closeTxModal()\"></div>\n\n    <!-- Panel -->\n\n    <div class=\"relative w-full max-w-md bg-dark-800 border border-dark-600 rounded-2xl shadow-2xl overflow-hidden animate-slide-up\">\n\n      <!-- Header coloré selon type -->\n\n      <div class=\"px-5 py-4 border-b border-dark-600 flex items-center justify-between gap-3\">\n\n        <div class=\"flex items-center gap-3\">\n\n          <div class=\"w-10 h-10 " +
    ...
    a.icon + " " + a.color + " text-sm\"></i>\n\n            <span class=\"text-xs font-semibold " + a.color + "\">" + a.title + "</span>\n\n          </div>\n\n          <p class=\"text-xs text-gray-400 leading-relaxed\">" + a.desc + "</p>\n\n        </div>" : "") + "\n\n      </div>\n\n\n\n      <!-- Footer -->\n\n      <div class=\"px-5 pb-5 pt-3 border-t border-dark-600\">\n\n        <button onclick=\"closeTxModal()\" class=\"w-full bg-dark-700 hover:bg-dark-600 text-gray-300 font-semibold py-3 rounded-xl transition text-sm\">\n\n          Fermer\n\n        </button>\n\n      </div>\n\n    </div>", document.body.appendChild(l);
}
function closeTxModal() {
  document.getElementById("tx-detail-modal")?.remove(), _txModalData = null;
}
async function renderTransactions(e, page, perPage) {
  page = page || window._txPage || 1;
  perPage = perPage || window._txPerPage || 25;
  window._txPage = page;
  window._txPerPage = perPage;
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const resp = await api("GET", "/members/transactions?page=" + page + "&per_page=" + perPage);
    const t = resp.transactions || [],
      total = resp.total || t.length;
    const statusBadgeTx = function (s) {
      const map = {
      ...
      return '<span class="text-[10px] px-1.5 py-0.5 rounded-full ' + (map[s] || "bg-gray-500/15 text-gray-400") + '">' + (lbl[s] || s || "—") + "</span>";
    };
    let rows = "";
    for (const row of t) {
      const isCredit = (row.amount ?? 0) >= 0;
      const txType = row.tx_type || row.transaction_type || "";
      const cfg = TX_TYPE_CONFIG[txType] || TX_TYPE_CONFIG.commission;
      const desc = row.description || "—";
      const balAfter = null != row.balance_after ? fmt$(Math.abs(row.balance_after)) : "—";
      const srcLine = row.source_name ? '<div class="text-[10px] text-gray-500 mt-0.5 truncate max-w-[180px]">de ' + row.source_name + "</div>" : " ";
      const pkgLine = row.package_name ? '<div class="text-[10px] text-indigo-400/70 mt-0.5">' + row.package_name + "</div>" : " ";
      const encoded = encodeURIComponent(JSON.stringify(row));
      rows += '<tr class="cursor-pointer hover:bg-dark-700/60 transition-colors group" onclick="showTxDetail(decodeURIComponent(\x27' + encoded + '\x27))"><td class="text-gray-400 text-xs whitespace-nowrap">' + fmtDate(row.created_at) + '</td><td><span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ' +
        (cfg?.badge || "bg-gray-500/15 text-gray-400") + '"><i class="fas ' + (cfg?.icon || "fa-circle") + ' text-[10px]"></i>' + (cfg?.label || txType || "—") + "</span></td>" + '<td class="text-gray-300 text-sm max-w-[220px]"><div class="truncate" title="' + desc + '">' +
        desc + "</div>" + srcLine + pkgLine + "</td>" + "<td>" + statusBadgeTx(row.status) + "</td>" + '<td class="text-right font-bold whitespace-nowrap ' + (isCredit ? "text-green-400" : "text-red-400") + '">' + (isCredit ? "+" : "−") + fmt$(Math.abs(row.amount ?? 0)) +
        "</td>" + '<td class="text-right font-medium text-white text-sm whitespace-nowrap">' + balAfter + '</td><td class="text-center"><i class="fas fa-chevron-right text-gray-600 group-hover:text-gray-400 text-[10px] transition-colors"></i></td></tr>';
    }
    const emptyState = '<div class="bg-dark-800 border border-dark-600 rounded-2xl p-12 text-center"><div class="w-16 h-16 rounded-full bg-dark-700 flex items-center justify-center mx-auto mb-4"><i class="fas fa-receipt text-gray-600 text-2xl"></i></div><h3 class="text-gray-400 font-medium mb-2">Aucune transaction pour le moment</h3><p class="text-gray-600 text-sm">Vos bonus, achats de packages, licences et retraits apparaîtront ici.</p></div>';
    const tableHtml = 0 === t.length ? emptyState : '<div class="text-xs text-gray-500 flex items-center gap-2 mb-2"><i class="fas fa-hand-pointer text-gray-600"></i>Cliquez sur une ligne pour voir le détail</div><div class="bg-dark-800 border border-dark-600 rounded-2xl overflow-hidden"><div class="overflow-x-auto"><table class="data-table w-full"><thead><tr><th>Date</th><th>Type</th><th>Description</th><th>Statut</th><th class="text-right">Montant</th><th class="text-right">Solde après</th><th class="text-center w-8"></th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
    e.innerHTML = '<div class="space-y-5"><div class="flex items-center justify-between flex-wrap gap-2"><div><h2 class="text-xl font-bold text-white">Toutes les transactions</h2><p class="text-xs text-gray-500 mt-0.5">' + total + ' opération' + (total > 1 ? "s" : "") + " au total</p></div>" + '<button onclick="showPage(\'wallet\')" class="text-xs text-rouge-400 hover:underline"><i class="fas fa-wallet mr-1"></i>Voir le portefeuille</button></div>' + tableHtml + '</div><div id="tx-pagination" class="px-4 py-3 border-t border-dark-600"></div>';
    renderPagination("tx-pagination", total, page, perPage, "(function(p){renderTransactions(document.getElementById(\'page-content\'),p,window._txPerPage)})", "(function(pp,p){renderTransactions(document.getElementById(\'page-content\'),p,pp)})");
  } catch (err) {
    e.innerHTML = '<div class="p-4 text-red-400"><i class="fas fa-exclamation-circle mr-2"></i>Erreur : ' + (err.error || err.message || String(err) || 'Erreur inconnue') + "</div>";
  }
}
...
async function _topupConfirmAmount() {
  ...
  if (!amount || amount < 10) {
    if (errEl) {
      errEl.textContent = 'Montant minimum : $10';
      errEl.classList.remove('hidden');
    }
    return;
  }
  try {
    ...
    if (amount < minAmt) {
      if (errEl) {
        errEl.textContent = "Montant minimum de recharge : $" + minAmt;
        errEl.classList.remove('hidden');
      }
      return;
    }
...
async function _showTransferModal() {
  ...
  let walletBalance = 0;
  try {
    const [cfgResp, walletResp] = await Promise.all([api("GET", "/payment/transfer/config"), api("GET", "/members/wallet")]);
    transferCfg = cfgResp || transferCfg;
    walletBalance = walletResp?.wallet?.balance || walletResp?.member?.wallet_balance || 0;
  } catch (e) {}
  const fee = transferCfg.fee_type === 'percent' ? "" + transferCfg.fee_value + "%" : transferCfg.fee_value > 0 ? "$" + transferCfg.fee_value + " fixe" : 'Aucun frais';
  showModal("\n  <div class=\"p-6 space-y-5 max-w-md mx-auto\">\n    <div class=\"flex items-center justify-between\">\n      <div class=\"flex items-center gap-3\">\n        <div class=\"w-10 h-10 rounded-xl bg-blue-500/20 flex items-center justify-center\">\n          <i class=\"fas fa-exchange-alt text-blue-400\"></i>\n        </div>\n        <div>\n          <h3 class=\"font-bold text-white\">Transfert vers un membre</h3>\n          <p class=\"text-xs text-gray-400\">Solde disponible : <span class=\"text-green-400 " + "font-semibold\">$" + Number(walletBalance).toLocaleString('en-US', {
    minimumFractionDigits: 2
...
async function _transferLookup() {
  ...
  if (!uid || uid.length < 4) {
    if (infoEl) infoEl.innerHTML = '<p class="text-red-400 text-xs">Identifiant requis (min 4 caractères)</p>';
    return;
  }
  if (infoEl) infoEl.innerHTML = '<div class="flex items-center gap-2 text-xs text-gray-400"><div class="loader-sm"></div>Recherche…</div>';
  try {
    const r = await api("POST", "/payment/transfer/lookup", {
      unique_id: uid
    });
...
async function _doTransfer() {
  ...
  try {
    const r = await api("POST", "/payment/transfer/send", {
      recipient_unique_id: uid,
      amount,
      pin,
      note: note || undefined
    });
    closeModal();
    showToast("$" + Number(r.amount_sent || amount).toLocaleString('en-US', {
      minimumFractionDigits: 2
    }) + " envoy\xE9s avec succ\xE8s !", 'success');
    setTimeout(() => {
      const el = document.getElementById('page-content');
      if (el) renderWallet(el);
    }, 800);
  } catch (e) {
    if (btn) {
      btn.disabled = false;
...
async function _walletPayOrder() {
  ...
  try {
    const r = await api("POST", "/payment/wallet/pay-order", {
      order_id: orderId
    });
    if (r.new_balance !== undefined) {
      ...
      if (hb) {
        hb.innerHTML = '<span class="text-gray-400">Solde : </span><span class="text-rouge-400 font-bold">$' + Number(r.new_balance).toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        }) + '</span>';
      }
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-wallet text-sm"></i> Confirmer le paiement';
    }
...
let _wizardData = {};
let cp_enabled = !1,
  _cpCoins = "USDT.TRC20,USDT.ERC20,BTC,ETH";
function _wizardReset() {
  var keys = Object.keys(_wizardData);
  for (var i = 0; i < keys.length; i++) {
    delete _wizardData[keys[i]];
  }
}
window._wizardReset = function () {
  _wizardReset();
};
window._wizardOpenForCourse = function (courseId, title, priceUsd) {
  var _now = Date.now();
  if (window._wizardOpenLock && _now - window._wizardOpenLock < 3000) {
    console.log('[Wizard] déjà en cours, ignoré');
    return;
  }
  ...
  setTimeout(function () {
    if (window._wizardStep4Running) {
      console.log('[Wizard] wizardStep4 déjà en cours (setTimeout)');
      return;
    }
...
let _gw = {},
  ...
  _ppSdkLoaded = !1,
  _ppSdkLoading = !1;
async function _loadPayPalSdk() {
  if (!_ppClientId) try {
    const e = await api("GET", "/members/paypal/config");
    if (!e.enabled) return !1;
    _ppClientId = e.client_id;
  } catch (e) {
    return !1;
  }
  if (!_ppClientId) return !1;
  if (_ppSdkLoaded) return !0;
  if (_ppSdkLoading) {
    for (let e = 0; e < 80; e++) if (await new Promise(e => setTimeout(e, 100)), _ppSdkLoaded) return !0;
    return !1;
  }
  return _ppSdkLoading = !0, new Promise(e => {
    const t = document.createElement("script");
    t.src = "https://www.paypal.com/sdk/js?client-id=" + _ppClientId + "&currency=USD&components=buttons&enable-funding=card,paylater&disable-funding=venmo", t.onload = () => {
      _ppSdkLoaded = !0, _ppSdkLoading = !1, e(!0);
    }, t.onerror = () => {
      _ppSdkLoading = !1, e(!1);
    }, document.head.appendChild(t);
  });
}
async function _renderPayPalButtons(e, t, n, a) {
  (await _loadPayPalSdk()) && "undefined" != typeof paypal ? paypal.Buttons({
    style: {
      layout: "vertical",
      color: "gold",
      shape: "rect",
    ...
    createOrder: t,
    onApprove: async t => {
      document.getElementById(e).innerHTML = '<div class="flex flex-col items-center gap-3 py-4"><div class="loader"></div><p class="text-gray-400 text-sm">Confirmation du paiement…</p></div>';
      try {
        const e = await api("POST", "/members/paypal/capture-order", {
          paypal_order_id: t.orderID
        });
        n(e);
      } catch (e) {
        a(e?.error || e?.message || "Erreur lors de la capture du paiement");
      }
    },
    onError: e => {
      a("Une erreur PayPal est survenue. Réessayez ou choisissez une autre méthode.");
    },
    onCancel: () => {
      document.getElementById(e).innerHTML = '<div class="text-yellow-400 text-sm text-center py-2"><i class="fas fa-times-circle mr-1"></i>Paiement annulé. Cliquez sur « Retour » pour recommencer.</div>';
    }
  }).render("#" + e) : document.getElementById(e).innerHTML = '<div class="text-red-400 text-sm text-center py-2"><i class="fas fa-exclamation-triangle mr-1"></i>SDK PayPal indisponible. Rechargez ou utilisez une autre méthode.</div>';
}
function _buildGw(e) {
  return {
    paypal_email: e.paypal?.email || "",
    paypal_instructions: e.paypal?.instructions || "",
    paypal_active: "true" === e.paypal?.active,
    bank_name: e.bank?.name || "",
    bank_beneficiary: e.bank?.beneficiary || "",
    bank_iban: e.bank?.iban || "",
    bank_swift: e.bank?.swift || "",
    bank_instructions: e.bank?.instructions || "",
    bank_active: "true" === e.bank?.active,
    crypto_address: e.crypto?.address || "",
    crypto_network: e.crypto?.network || "",
    crypto_instructions: e.crypto?.instructions || "",
    crypto_active: "true" === e.crypto?.active,
    wallet_active: "true" === e.wallet?.active,
    wallet_balance: 0,
    instructions: e.manual?.instructions || "",
    contact_email: e.manual?.contact_email || "",
    contact_phone: e.manual?.contact_phone || "",
    manual_active: "true" === e.manual?.active
  };
}
const CAT_THEME = {
    "cat-synex": {
      gradient: "from-blue-950 via-dark-900 to-dark-900",
      border: "border-blue-500/30",
      ...
      headerBg: "from-blue-900/50 to-dark-900",
      gridCols: "xl:grid-cols-4",
      catCardBg: "from-blue-950/80 to-dark-900",
      catCardHover: "hover:border-blue-400/60 hover:shadow-blue-900/40"
    },
    "cat-ezra": {
      gradient: "from-emerald-950 via-dark-900 to-dark-900",
      border: "border-emerald-500/30",
      ...
      headerBg: "from-emerald-900/50 to-dark-900",
      gridCols: "xl:grid-cols-3",
      catCardBg: "from-emerald-950/80 to-dark-900",
      catCardHover: "hover:border-emerald-400/60 hover:shadow-emerald-900/40"
    },
    "cat-luxia": {
      gradient: "from-amber-950 via-dark-900 to-dark-900",
      ...
      headerBg: "from-amber-900/50 to-dark-900",
      gridCols: "xl:grid-cols-2",
      catCardBg: "from-amber-950/80 to-dark-900",
      catCardHover: "hover:border-amber-400/60 hover:shadow-amber-900/40"
    },
    "cat-club": {
      gradient: "from-indigo-950 via-dark-900 to-dark-900",
      ...
      headerBg: "from-indigo-900/50 to-dark-900",
      gridCols: "xl:grid-cols-2",
      catCardBg: "from-indigo-950/80 to-dark-900",
      catCardHover: "hover:border-indigo-400/60 hover:shadow-indigo-900/40"
    },
    "cat-synex-triomarkets": {
      gradient: "from-orange-950 via-dark-900 to-dark-900",
      ...
      headerBg: "from-orange-900/50 to-dark-900",
      gridCols: "xl:grid-cols-4",
      catCardBg: "from-orange-950/80 to-dark-900",
      catCardHover: "hover:border-orange-400/60 hover:shadow-orange-900/40"
    },
    _none: {
      gradient: "from-dark-800 to-dark-900",
      border: "border-dark-600",
      cardBorder: "border-dark-600",
      accent: "text-rouge-400",
      accentBg: "bg-rouge-500/10",
      priceColor: "text-rouge-400",
      btnClass: "bg-rouge-600 hover:bg-rouge-500",
      badgeBg: "bg-rouge-500/15 border-rouge-500/25 text-rouge-400",
      checkColor: "text-rouge-400",
      headerBg: "from-dark-700 to-dark-900",
      gridCols: "xl:grid-cols-3",
      catCardBg: "from-dark-800 to-dark-900",
      catCardHover: "hover:border-rouge-500/30"
    }
  },
  CAT_META_FALLBACK = {
    _none: {
      icon: "[+]",
      desc: "Autres packages disponibles.",
      tag: ""
    }
  };
function getCatMeta(e, t) {
  const n = t[e] || {},
    a = CAT_META_FALLBACK[e] || CAT_META_FALLBACK._none;
  return {
    icon: n.icon || a.icon,
    desc: n.desc || a.desc,
    tag: n.tag || a.tag
...
function renderPkgCard(e, t, n) {
  let a = [];
  try {
    a = JSON.parse(e.features || "[]");
  } catch {}
  const s = "monthly" === e.pricing_type || "subscription" === e.payment_mode,
    r = "" + ("EUR" === e.currency ? "€" : "$") + Number(e.price_usd).toLocaleString("fr-FR"),
    i = s ? '<span class="text-base font-normal text-gray-400">/mois</span>' : "",
    l = e.dreamiles_per_payment || 0,
    d = [];
  let o = null;
  for (const e of a) {
    const t = e.match(/^([A-ZÀÂÉÈÊËÎÏÔÙÛÜÇ ''&]+)\s*—\s*(.+)/u);
    if (t) {
      const e = t[1].trim();
      o && o.label === e || (o = {
        label: e,
        items: []
      }, d.push(o)), o.items.push(t[2].trim());
    } else o || (o = {
      label: "",
      items: []
    }, d.push(o)), o.items.push(e);
  }
  const c = {
      "S'ÉDUQUER": "fa-book-open",
      "CRÉER": "fa-hammer",
      "PROTÉGER": "fa-shield-halved",
      MULTIPLIER: "fa-arrow-trend-up",
      PROFITER: "fa-star",
      "IDÉAL POUR": "fa-bullseye"
    },
  ...
  return "\n\n  <div class=\"rounded-2xl border " + t.cardBorder + " bg-gradient-to-b " + t.gradient + " flex flex-col overflow-hidden hover:shadow-2xl hover:-translate-y-0.5 transition-all duration-300\">\n\n    <!-- En-t\xEAte prix -->\n\n    <div class=\"bg-gradient-to-b " +
    ...
    const sameOwned = n.find(o => o.package_id === e.id && (o.status === 'active' || o.status === 'validated'));
    const superiorInCat = e.category_id && n.find(o => (o.status === 'active' || o.status === 'validated') && o.category === e.category_id && o.package_id !== e.id && Number(o.bv_value) >= e.bv_value);
    const activeOrder = e.category_id && n.find(o => (o.status === 'active' || o.status === 'validated') && o.category === e.category_id && o.package_id !== e.id && Number(o.bv_value) < e.bv_value);
    if (e.payment_mode === 'broker') {
      var _brRegs = window._brokerRegs || [];
      var _brReg = _brRegs.find(function (r) {
        return r.package_id === e.id;
      });
    ...
    if (sameOwned) {
      return '<div class="w-full text-center py-3.5 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-sm font-semibold flex items-center justify-center gap-2"><i class="fas fa-check-circle"></i>Package actif</div>';
    }
    if (superiorInCat) {
      return '<div class="w-full text-center py-3.5 rounded-xl bg-gray-700/50 border border-gray-600/40 text-gray-500 text-sm font-semibold flex items-center justify-center gap-2 cursor-not-allowed select-none"><i class="fas fa-lock mr-1"></i>Niveau d\'abord atteint — Upgrade requis</div>';
    }
    if (activeOrder) {
      return "<button onclick=\"wizardStart('" + e.id + "','" + encodeURIComponent(e.name) + "'," + e.price_usd + "," + e.bv_value + ",true," + (activeOrder.price_usd || 0) + "," + (activeOrder.bv_value || 0) + ")\" class=\"w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl transition-all duration-200 text-sm shadow-lg flex items-center justify-center gap-2\"><i class=\"fas fa-arrow-up\"></i>Upgrader depuis " + (activeOrder.package_name || 'mon package') + "</button>";
    }
...
async function renderPackages(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const [t, _bsRes] = await Promise.all([api("GET", "/members/packages"), api("GET", "/broker/status").catch(() => ({
      registrations: []
    }))]);
    window._brokerRegs = _bsRes.registrations || [];
    _gw = _buildGw(t.gateways || {}), _adminFeeInfo = t.adminFee || {
      amount: 0,
      active: !1,
      paid: !0
    };
    _licPriceForWizard = t.licensePrice || 97;
    const n = {},
      a = {};
    for (const e of t.packages || []) {
      const t = e.category_id || "_none";
      n[t] || (n[t] = [], a[t] = {
        name: e.category_name || "Packages",
        icon: e.category_icon || "[+]",
        desc: e.category_description || "",
        tag: e.category_tag || ""
      }), n[t].push(e);
    }
    window._pkgData = {
    ...
    const s = ["cat-synex", "cat-ezra", "cat-luxia", "cat-club", "cat-synex-triomarkets"].filter(e => n[e]),
      r = Object.keys(n).filter(e => !s.includes(e)),
      i = [...s, ...r].map(e => {
        const t = n[e],
          s = a[e],
          r = CAT_THEME[e] || CAT_THEME._none,
          i = getCatMeta(e, a),
          l = Math.min(...t.map(e => e.price_usd)),
          d = Math.max(...t.map(e => e.price_usd)),
          o = t.some(e => "monthly" === e.pricing_type || "subscription" === e.payment_mode),
          c = "EUR" === t[0]?.currency ? "€" : "$";
        return l === d ? Number(l).toLocaleString("fr-FR") : (Number(l).toLocaleString("fr-FR"), Number(d).toLocaleString("fr-FR")), "\n\n      <div onclick=\"showPackageCategory('" + e + "')\"\n\n        class=\"group cursor-pointer rounded-2xl border " + r.border + " " +
          r.catCardHover + "\n\n               bg-gradient-to-br " + r.catCardBg + " p-7\n\n               hover:shadow-2xl hover:-translate-y-1 transition-all duration-300\">\n\n\n\n        <!-- Ic\xF4ne + nom -->\n\n        <div class=\"flex items-center gap-4 mb-4\">\n\n          <div class=\"w-14 h-14 rounded-2xl " +
          r.accentBg + " border " + r.border + "\n\n                      flex items-center justify-center text-2xl shrink-0\n\n                      group-hover:scale-110 transition-transform duration-300\">\n\n            " + i.icon + "\n\n          </div>\n\n          <div class=\"min-w-0\">\n\n            <h3 class=\"font-extrabold text-white text-base leading-tight truncate\">" +
...
function showPackageCategory(e) {
  const {
    pkgByCategory: t,
    categoryMeta: n
  } = window._pkgData || {};
  ...
  const a = t[e],
    ...
    r = CAT_THEME[e] || CAT_THEME._none,
    i = getCatMeta(e, n),
    l = r.gridCols || "xl:grid-cols-3";
  window.scrollTo({
    top: 0,
    behavior: "smooth"
  }), document.getElementById("pkg-view-categories").classList.add("hidden");
...
function backToCategories() {
  document.getElementById("pkg-view-categories").classList.remove("hidden"), document.getElementById("pkg-view-detail").classList.add("hidden"), window.scrollTo({
    top: 0,
    behavior: "smooth"
  });
}
async function brokerRedirect(pkgId, pkgNameEnc, evt) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Redirection en cours…';
  }
  try {
    var r = await api('POST', '/broker/redirect', {
      package_id: pkgId
    });
    if (r && r.url) {
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-check mr-2"></i>Redirection ouverte';
      }
    } else {
      showToast('Erreur : URL de redirection manquante', 'error');
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-external-link-alt mr-2"></i>S&#39;inscrire chez Triomarkets';
      }
    }
  } catch (err) {
...
function renderMyOrders(e) {
  ...
  const t = {
      ...
      proof_submitted: "Preuve soumise — en vérification",
      validated: "Validé ✓",
      rejected: "Rejeté",
      active: "Actif"
    },
    n = {
      ...
      proof_submitted: "text-blue-400 bg-blue-500/10 border-blue-500/30",
...
function _wizardProgress(e, t) {
  ...
  return "\n\n  <div class=\"px-6 pt-5 pb-3 border-b border-dark-600\">\n\n    <div class=\"flex items-center justify-between mb-2\">\n\n      <span class=\"text-xs font-semibold text-rouge-400 uppercase tracking-wider\">\xC9tape " + e + "/" + t + " \u2014 " + (["Package", "Licence", "Récap.", "Paiement", "Preuve", "Confirmation", "Terminé"][e - 1] || "") + "</span>\n\n      <button onclick=\"wizardClose()\" class=\"text-gray-400 hover:text-white w-7 h-7 flex items-center justify-center rounded-lg hover:bg-dark-700 text-lg\">\xD7</button>\n\n    </div>\n\n    <div class=\"w-full bg-dark-700 rounded-full h-1.5\">\n\n      <div class=\"bg-rouge-500 h-1.5 rounded-full transition-all\" style=\"width:" + n + "%\"></div>\n\n    </div>\n\n  </div>";
}
function wizardStart(e, t, n, a, s, r, i) {
  _wizardReset(), _wizardData.pkgId = e, _wizardData.pkgName = decodeURIComponent(t), _wizardData.price = Number(n), _wizardData.bv = Number(a), _wizardData.isUpgrade = !!s, _wizardData.currentPrice = Number(r || 0), _wizardData.currentBV = Number(i || 0), _wizardData.diffPrice = _wizardData.isUpgrade ? Math.max(0, _wizardData.price - _wizardData.currentPrice) : _wizardData.price, _wizardData.diffBV = _wizardData.isUpgrade ? Math.max(0, _wizardData.bv - _wizardData.currentBV) : _wizardData.bv, _wizardData.addLicense = !1, _wizardData.licensePrice = typeof _licPriceForWizard !== "undefined" ? _licPriceForWizard : 97;
  const l = _adminFeeInfo.mode || "once",
    d = _adminFeeInfo.active && ("multiple" === l || !_adminFeeInfo.paid),
    o = d ? Number(_adminFeeInfo.amount) : 0,
    c = _wizardData.isUpgrade ? "\n\n    <div class=\"bg-green-900/20 border border-green-500/30 rounded-lg p-3 text-sm space-y-1\">\n\n      <div class=\"flex justify-between text-gray-400\">\n\n        <span><i class=\"fas fa-minus-circle text-gray-500 mr-1\"></i>Package actuel (d\xE9j\xE0 pay\xE9)</span>\n\n        <span>$" + _wizardData.currentPrice.toLocaleString("en-US") + " \xB7 " + _wizardData.currentBV + " BV</span>\n\n      </div>\n\n      <div class=\"flex justify-between text-green-300 font-semibold\">\n\n        <span><i class=\"fas fa-arrow-up text-green-400 mr-1\"></i>Vous payez la diff\xE9rence</span>\n\n        <span>$" + _wizardData.diffPrice.toLocaleString("en-US") + " \xB7 +" + _wizardData.diffBV + " BV</span>\n\n      </div>\n\n    </div>" : "";
...
function wizardStep2() {
  ...
  _wizardShow("\n\n  " + _wizardProgress(2, 7) + ("\n\n  <div class=\"p-6 space-y-5\">\n\n    <div>\n\n      <h3 class=\"text-lg font-bold text-white\">Ajouter la Licence Finstrategia ?</h3>\n\n      <p class=\"text-sm text-gray-400 mt-1\">La licence annuelle est nécessaire pour être éligible aux commissions LEADER.</p>\n\n    </div>\n\n    <div class=\"space-y-3\">\n\n      <button onclick=\"wizardSetLicense(true)\" id=\"btn-yes-lic\"\n\n        class=\"w-full border-2 border-dark-500 rounded-xl p-4 text-left hover:border-rouge-500 transition group\">\n\n        <div class=\"flex items-start gap-4\">\n" +
    ...
    "\n            <div class=\"text-sm text-gray-400 mt-1\">Vous pourrez activer la licence plus tard depuis votre espace membre.</div>\n\n          </div>\n\n        </div>\n\n      </button>\n\n    </div>\n\n    <button onclick=\"wizardStart('") + _wizardData.pkgId +
    "','" + encodeURIComponent(_wizardData.pkgName) + "'," + _wizardData.price + "," + _wizardData.bv + "," + _wizardData.isUpgrade + "," + _wizardData.currentPrice + "," + _wizardData.currentBV + ")\"\n\n      class=\"text-sm text-gray-500 hover:text-gray-300 transition\"><i class=\"fas fa-arrow-left mr-1\"></i>Retour</button>\n\n  </div>");
}
function wizardSetLicense(e) {
  _wizardData.addLicense = e, wizardStep3();
}
function wizardStep3() {
  const e = _adminFeeInfo.mode || "once",
    t = _adminFeeInfo.active && ("multiple" === e || !_adminFeeInfo.paid),
    n = t ? Number(_adminFeeInfo.amount) : 0,
    a = _wizardData.addLicense ? _wizardData.licensePrice || 97 : 0,
    s = (_wizardData.isUpgrade ? _wizardData.diffPrice : _wizardData.price) + a + n;
  _wizardData.totalAmt = s;
  const r = _wizardData.isUpgrade ? "\n\n      <div class=\"flex justify-between text-xs text-gray-500 pb-2 border-b border-dark-600\">\n\n        <span>Package actuel (d\xE9j\xE0 pay\xE9)</span>\n\n        <span>$" + _wizardData.currentPrice.toLocaleString("en-US") + " \xB7 " +
    _wizardData.currentBV + " BV</span>\n\n      </div>\n\n      <div class=\"flex justify-between\">\n\n        <span class=\"text-gray-400\">Upgrade \u2192 " + _wizardData.pkgName + " <span class=\"text-xs text-green-400\">(diff\xE9rentiel)</span></span>\n\n        <span class=\"text-white font-medium\">$" +
    _wizardData.diffPrice.toLocaleString("en-US") + "</span>\n\n      </div>" : "\n\n      <div class=\"flex justify-between\">\n\n        <span class=\"text-gray-400\">Package " + _wizardData.pkgName + "</span>\n\n        <span class=\"text-white font-medium\">$" + _wizardData.price.toLocaleString("en-US") +
    "</span>\n\n      </div>";
...
async function wizardStep4() {
  if (window._wizardStep4Running) {
    console.log('[Wizard] wizardStep4 déjà en cours');
    return;
  }
  window._wizardStep4Running = true;
  setTimeout(function () {
    window._wizardStep4Running = false;
  }, 5000);
  const e = _wizardData.totalAmt;
  if (!_gw || !Object.keys(_gw).length) {
    _wizardShow('<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Chargement des moyens de paiement…</p></div>');
    try {
      const e = await api("GET", "/members/packages");
      _gw = _buildGw(e.gateways || {}), _adminFeeInfo = e.adminFee || {
        amount: 0,
        active: !1,
        paid: !0
      };
    } catch (e) {
      return void _wizardShow("<div class=\"p-8 text-center space-y-4\">\n\n        <i class=\"fas fa-exclamation-circle text-red-400 text-3xl\"></i>\n\n        <p class=\"text-red-400\">Impossible de charger les modes de paiement.<br><span class=\"text-xs text-gray-500\">" + (e.error || e.message || "") + "</span></p>\n\n        <button onclick=\"wizardClose()\" class=\"py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm\">Fermer</button>\n\n      </div>");
    }
  }
  const t = _gw,
    n = !(!t.paypal_email || !t.paypal_active),
    a = !(!t.bank_iban && !t.bank_name) && t.bank_active,
    s = !(!t.crypto_address || !t.crypto_active),
    r = !(!t.instructions || !t.manual_active);
  let i = !1;
  try {
    const e = await api("GET", "/members/stripe/config");
    i = !(!e || !e.enabled), i && (window._stripePublicKey = e.public_key);
  } catch (e) {
    i = !1;
  }
  cp_enabled = !1;
  _cpCoins = "USDT.TRC20,LTC,ETH,BTC";
  try {
    const e = await api("GET", "/members/coinpayments/config");
    cp_enabled = !(!e || !e.enabled), cp_enabled && (_cpCoins = e.coins || "USDT.TRC20,USDT.ERC20,BTC,ETH");
  } catch (e) {
    cp_enabled = !1;
  }
  let _v2Methods = [];
  try {
    const _v2r = await api("GET", "/payment/methods");
    const _v2all = Object.values(_v2r.methods || {}).flat();
    _v2Methods = _v2all.filter(m => m.is_automatic && m.is_active && !["stripe", "paypal", "coinpayments", "wallet", "internal_wallet", "internal", "leader_wallet"].includes(m.provider));
  } catch (_v2e) {
    _v2Methods = [];
  }
  const _v2Btns = _v2Methods.map(m => {
    const _ico = m.provider === "mollie" ? "fas fa-credit-card" : m.logo_emoji || "[CB]";
    const _col = m.provider === "mollie" ? "indigo" : "gray";
    return "\n\n  <button onclick=\"wizardSelectPaymentV2('" + m.id + "','" + m.provider + "','" + (m.display_name || m.provider).replace(/'/g, "") + "')\"\n    class=\"w-full border-2 border-" + _col + "-500/40 rounded-xl p-4 text-left hover:border-" + _col + "-400 transition flex items-center gap-4 bg-" +
      ...
      (m.description || "Paiement sécurisé") + "</div>\n    </div>\n    <span class=\"ml-auto text-[9px] bg-" + _col + "-500/20 text-" + _col + "-400 border border-" + _col + "-500/30 px-2 py-0.5 rounded-full font-semibold whitespace-nowrap\">Automatique</span>\n  </button>";
  }).join("");
  let _v2ManualMethods = [];
  try {
    const _v2r2 = await api("GET", "/payment/methods");
    const _v2all2 = Object.values(_v2r2.methods || {}).flat();
    _v2ManualMethods = _v2all2.filter(m => !m.is_automatic && m.is_active && !["leader_wallet", "internal_wallet", "internal", "wallet"].includes(m.provider));
  } catch (_v2e2) {
    _v2ManualMethods = [];
  }
  const _v2ManualBtns = _v2ManualMethods.map(m => {
  ...
  let _preWbal = 0;
  try {
    const _pwr = await api("GET", "/members/wallet");
    _preWbal = Number(_pwr.wallet?.balance || _pwr.member?.wallet_balance) || 0;
    window._memberWalletBalance = _preWbal;
  } catch (_pwe) {
    _preWbal = 0;
  }
  window._wStep4Data = {
    ...
    i: i,
    _v2Btns: _v2Btns,
    _v2ManualBtns: _v2ManualBtns,
    _preWbal: _preWbal
  };
  setTimeout(function () {
    _wizardRenderStep4(window._wStep4Data);
  }, 0);
}
function _wizardRenderStep4(_d) {
  var e = _d.e,
    t = _d.t,
    n = _d.n,
    a = _d.a,
    ...
    _preWbal = _d._preWbal;
  window._wStep4Data = _d;
  _buildPayButtons(_d);
}
function _buildPayButtons(_d) {
  var e = _d.e,
    t = _d.t,
    n = _d.n,
    a = _d.a,
    ...
    _v2ManualBtns = _d._v2ManualBtns,
    _preWbal = _d._preWbal;
  const l = !(n || a || s || r || i || cp_enabled || _v2Methods.length > 0 || true),
...
function _wizardRenderFinal() {
  ...
  var l = _b.l,
    ...
    _v2Btns = _d._v2Btns,
    _v2ManualBtns = _d._v2ManualBtns;
  _wizardShow("\n\n  " + _wizardProgress(4, 7) + "\n\n  <div class=\"p-6 space-y-5\">\n\n    <div>\n\n      <h3 class=\"text-lg font-bold text-white\">Moyen de paiement</h3>\n\n      <p class=\"text-sm text-gray-400 mt-1\">Total \xE0 envoyer : <span class=\"text-rouge-400 font-bold\">$" + e.toLocaleString("en-US", {
    minimumFractionDigits: 2
...
function wizardSelectPayment(e) {
  _wizardData.paymentMethod = e, wizardStep5();
}
async function wizardSelectPaymentV2(methodId, provider, displayName) {
  _wizardData.paymentMethod = "v2_psp";
  _wizardData.v2MethodId = methodId;
  _wizardData.v2Provider = provider;
  _wizardData.v2DisplayName = displayName;
  wizardStep5V2();
}
async function wizardSelectPaymentV2Manual(methodId, provider, displayName) {
  _wizardData.paymentMethod = "v2_manual";
  _wizardData.v2MethodId = methodId;
  _wizardData.v2Provider = provider;
  _wizardData.v2DisplayName = displayName;
  await wizardStep5V2Manual();
}
async function wizardStep5V2Manual() {
  ...
  try {
    ...
    if (_wizardData.isTopup) {
      orderResp = await api("POST", "/payment/topup/create", {
      ...
      orderResp = await api("POST", "/members/license/order", {
        payment_method: "v2_manual"
      });
      _wizardData.licenseId = orderResp.license_id || null;
      _wizardData.orderId = orderResp.license_id || null;
      orderId = _wizardData.orderId;
    } else if (_wizardData.isUpgrade) {
      orderResp = await api("POST", "/members/packages/upgrade", {
        target_package_id: _wizardData.pkgId,
        payment_method: "v2_manual"
      });
      orderId = orderResp.order_id;
      _wizardData.orderId = orderId;
    } else if (_wizardData.isPhysicalCard) {
      orderResp = await api("POST", "/members/physical-card/order", {
        payment_method: "v2_manual",
        shipping_address: _wizardData.shippingAddress || {}
      });
      orderId = orderResp.order_id;
      _wizardData.orderId = orderId;
    } else if (_wizardData.isCoursePayment) {
      orderResp = await api("POST", "/campus/course/" + _wizardData.courseId + "/order", {
        payment_method: "v2_manual"
      });
      orderId = orderResp.order_id;
      _wizardData.orderId = orderId;
      if (orderResp.existing && orderResp.status === "proof_submitted") {
        return void wizardStep7();
      }
      if (orderResp.existing) {
        return void wizardStep6();
      }
    } else {
      const _b = {
        package_id: _wizardData.pkgId,
        payment_method: "v2_manual"
      };
      if (_wizardData.addLicense) _b.add_license = true;
      orderResp = await api("POST", "/members/packages/order", _b);
      orderId = orderResp.order_id;
      _wizardData.orderId = orderId;
    }
  } catch (err) {
    if (err.existing_order_id) {
      orderId = err.existing_order_id;
      _wizardData.orderId = orderId;
    } else {
      _wizardShow("<div class=\"p-8 text-center space-y-4\"><i class=\"fas fa-exclamation-circle text-red-400 text-3xl\"></i><p class=\"text-red-400\">Erreur lors de la cr\xE9ation de commande.<br><span class=\"text-xs text-gray-500\">" + (err.error || err.message || "") + "</span></p><button onclick=\"wizardStep4()\" class=\"py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm\"><i class=\"fas fa-arrow-left mr-1\"></i>Retour</button></div>");
      return;
    }
  }
  let methodDetails = {
  ...
  _wizardShow("\n  " + _wizardProgress(5, 7) + "\n  <div class=\"p-6 space-y-5\">\n    <div>\n      <h3 class=\"text-lg font-bold text-white\">" + emoji + " " + displayName + "</h3>\n      <p class=\"text-sm text-gray-400 mt-1\">Effectuez votre paiement puis uploadez votre preuve ci-dessous.</p>\n    </div>\n    <div class=\"bg-dark-700 border border-emerald-500/30 rounded-xl p-4 space-y-2 text-sm\">\n      <div class=\"flex items-center gap-2 text-emerald-400 font-semibold mb-2\"><i class=\"fas fa-info-circle\"></i>Instructions de paiement</div>\n      " +
    (phoneNum ? "<div class=\"flex justify-between\"><span class=\"text-gray-400\">Num\xE9ro</span><span class=\"font-mono font-bold text-white\">" + phoneNum + "</span></div>" : "") + "\n      " + (accName ? "<div class=\"flex justify-between\"><span class=\"text-gray-400\">B\xE9n\xE9ficiaire</span><span class=\"font-medium text-white\">" +
    accName + "</span></div>" : "") + "\n      <div class=\"flex justify-between\"><span class=\"text-gray-400\">Montant \xE0 envoyer</span><span class=\"font-bold text-rouge-400\">$" + totalAmt.toLocaleString("en-US", {
    minimumFractionDigits: 2
...
async function _submitV2ManualProof(orderId) {
  ...
  if (!proofUrl) {
    if (errDiv) {
      errDiv.textContent = "Veuillez uploader une preuve de paiement.";
  ...
  try {
    if (_wizardData.isTopup) {
      ...
      await api("POST", "/payment/topup/submit-proof", {
        topup_id: topupId,
        proof_url: proofUrl,
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = "<i class=\"fas fa-paper-plane mr-2\"></i>Soumettre la preuve";
    }
    if (errDiv) {
      errDiv.textContent = err.error || err.message || "Erreur lors de la soumission";
      errDiv.classList.remove("hidden");
    }
  }
}
async function wizardStep5V2() {
  _wizardShow('<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Création de votre commande…</p></div>');
  const t = _wizardData.v2MethodId;
  const n = _wizardData.totalAmt;
  let orderId = null;
  let licenseId = null;
  try {
    let orderResp;
    if (_wizardData.isTopup) {
      ...
      orderResp = await api("POST", "/members/license/order", {
        payment_method: "v2_psp"
      });
      licenseId = orderResp.license_id || null;
      _wizardData.licenseId = licenseId;
      _wizardData.orderId = licenseId;
      orderId = licenseId;
    } else if (_wizardData.isUpgrade) {
      orderResp = await api("POST", "/members/packages/upgrade", {
        target_package_id: _wizardData.pkgId,
      ...
      if (_wizardData.addLicense) body.add_license = true;
      orderResp = await api("POST", "/members/packages/order", body);
      orderId = orderResp.order_id;
      _wizardData.orderId = orderId;
    }
  } catch (err) {
  ...
  _wizardShow('<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Redirection vers ${_wizardData.v2DisplayName}…</p></div>');
  try {
    const pspBody = {
      payment_method_id: t,
      amount: n,
    ...
    if (_wizardData.isTopup) pspBody.topup_id = orderId;else if (licenseId) pspBody.license_id = licenseId;else pspBody.order_id = orderId;
    const pspResp = await api("POST", "/psp/initiate", pspBody);
    ...
    _wizardShow("<div class=\"p-8 text-center space-y-4\"><i class=\"fas fa-exclamation-circle text-red-400 text-3xl\"></i><p class=\"text-red-400\">Erreur de paiement " + _wizardData.v2DisplayName + "\n.<br><span class=\"text-xs text-gray-500\">" + (pspErr.error || pspErr.message || "") + "</span></p><button onclick=\"wizardStep4()\" class=\"py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm\"><i class=\"fas fa-arrow-left mr-1\"></i>Retour</button></div>");
  }
}
async function wizardStep5() {
  const e = _gw,
    t = _wizardData.paymentMethod,
  ...
  try {
    let _apiResp;
    if (_wizardData.isTopup) {
      _apiResp = await api("POST", "/payment/topup/create", {
        legacy_method: t,
        amount: n
      });
      ...
      _apiResp = await api("POST", "/members/license/order", {
        payment_method: t || "manual"
      });
      ...
      if (_apiResp.activated) {
        if (_apiResp.new_balance !== undefined) {
          window._memberWalletBalance = _apiResp.new_balance;
          const _hb = document.getElementById('header-balance');
          if (_hb) {
            const _nb = _apiResp.new_balance;
            _hb.innerHTML = '<span class="text-gray-400">Solde : </span><span class="text-rouge-400 font-bold">$' + Number(_nb).toLocaleString('en-US', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            }) + '</span>';
          }
      ...
      _apiResp = await api("POST", "/members/packages/upgrade", {
        target_package_id: _wizardData.pkgId,
        payment_method: t || "manual"
      });
      _wizardData.orderId = _apiResp.order_id || _wizardData.orderId;
    } else if (_wizardData.isPhysicalCard) {
      _apiResp = await api("POST", "/members/physical-card/order", {
        payment_method: t || "manual",
        shipping_address: _wizardData.shippingAddress || {}
      });
      _wizardData.orderId = _apiResp.order_id || _wizardData.orderId;
    } else if (_wizardData.isCoursePayment) {
      _apiResp = await api("POST", "/campus/course/" + _wizardData.courseId + "/order", {
        payment_method: t || "manual"
      });
      _wizardData.orderId = _apiResp.order_id || _wizardData.orderId;
      if (_apiResp.existing && _apiResp.status === "proof_submitted") {
        return void wizardStep7();
      }
      if (_apiResp.existing) {
        return void wizardStep6();
      }
    } else {
      const _pkgBody = {
      ...
      _wizardData.addLicense && (_pkgBody.add_license = !0);
      _apiResp = await api("POST", "/members/packages/order", _pkgBody);
      _wizardData.orderId = _apiResp.order_id || _wizardData.orderId;
    }
  } catch (_err) {
    if (_err.existing_order_id) {
      const _existId = _err.existing_order_id || "";
      const _existStatus = _err.existing_status || "pending";
      if (_wizardData.licenseOnly) {
        _wizardData.licenseId = _existId;
        _wizardData.orderId = _existId;
      } else {
        _wizardData.orderId = _existId;
        if (_existStatus === "proof_submitted") {
          return void wizardStep7();
        } else {
          return void wizardStep6();
        }
  ...
  a = "stripe" === t ? "\n\n    <div class=\"bg-purple-900/20 border border-purple-500/30 rounded-xl p-4 space-y-3\">\n\n      <div class=\"flex items-center gap-3\">\n\n        <i class=\"fas fa-credit-card text-purple-400 text-2xl\"></i>\n\n        <div>\n\n          <div class=\"font-semibold\">Paiement sécurisé par carte</div>\n\n          <div class=\"text-xs text-gray-400\">Visa · Mastercard · Amex · Apple Pay · Google Pay</div>\n\n        </div>\n\n      </div>\n\n      <div id=\"stripe-payment-element\" class=\"min-h-[120px]\">\n\n        <div class=\"flex items-center justify-center py-6\"><div class=\"loader\"></div></div>\n" +
    ...
    (e.crypto_network || "USDT") + "</div></div>\n\n      </div>\n\n      <div class=\"bg-dark-700 rounded-lg px-3 py-2 font-mono text-orange-300 text-xs break-all select-all\">" + e.crypto_address + "</div>\n\n      " + (e.crypto_instructions ? "<p class=\"text-xs text-gray-400\">" +
    e.crypto_instructions + "</p>" : "") + "\n\n    </div>" : "wallet" === t ? "\n\n    <div class=\"bg-green-900/10 border border-green-500/30 rounded-xl p-4 space-y-4\">\n\n      <div class=\"flex items-center gap-3\">\n\n        <i class=\"fas fa-wallet text-green-400 text-2xl\"></i>\n\n        <div>\n\n          <div class=\"font-semibold text-white\">Paiement via Wallet LEADER</div>\n\n          <div class=\"text-xs text-gray-400\">Débit immédiat · Activation instantanée</div>\n\n        </div>\n\n      </div>\n\n      <div class=\"bg-dark-700 rounded-xl p-4 space-y-2 text-sm\">\n\n        <div " +
    "class=\"flex justify-between\"><span class=\"text-gray-400\">Montant débité</span><span class=\"text-white font-bold\">$" + n.toLocaleString("en-US", {
    minimumFractionDigits: 2
  }) + "</span></div>\n\n        <div class=\"flex justify-between\"><span class=\"text-gray-400\">Solde wallet</span><span class=\"text-green-400\">$" + (Number(window._memberWalletBalance) || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2
  }) + "</span></div>\n\n        <div class=\"flex justify-between font-semibold\"><span class=\"text-gray-400\">Solde apr\xE8s paiement</span><span class=\"text-" + ((Number(window._memberWalletBalance) || 0) - n >= 0 ? 'green' : 'red') + "-400\">$" + ((Number(window._memberWalletBalance) || 0) - n).toLocaleString("en-US", {
    minimumFractionDigits: 2
    ...
    e.instructions + "</p>\n\n      " + (e.contact_email ? "<p class=\"text-xs text-gray-400\"><i class=\"fas fa-envelope mr-1\"></i>" + e.contact_email + "</p>" : "") + "\n\n      " + (e.contact_phone ? "<p class=\"text-xs text-gray-400\"><i class=\"fas fa-phone mr-1\"></i>" +
    e.contact_phone + "</p>" : "") + "\n\n    </div>";
  const s = "paypal" === t,
    r = "stripe" === t,
    i = s || r ? '<div class="bg-green-900/10 border border-green-500/20 rounded-xl p-3 text-xs text-gray-400">\n        <i class="fas fa-bolt text-green-400 mr-1"></i>\n        <strong class="text-green-300">Activation immédiate</strong> — Votre commande sera activée automatiquement après paiement confirmé.\n       </div>' : '<div class="bg-blue-900/10 border border-blue-500/20 rounded-xl p-4 text-sm text-gray-300">\n        <i class="fas fa-info-circle text-blue-400 mr-2"></i>\n        Une fois le paiement effectué, cliquez sur <strong>"J\'ai payé"</strong> pour soumettre votre preuve.\n       </div>',
    l = s || r ? '<button onclick="wizardStep4()" class="w-full py-2.5 bg-dark-700 text-gray-400 rounded-xl hover:bg-dark-600 transition text-sm"><i class="fas fa-arrow-left mr-2"></i>Retour / Changer de méthode</button>' : '<div class="flex gap-3">\n        <button onclick="wizardStep4()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl hover:bg-dark-600 transition text-sm"><i class="fas fa-arrow-left mr-2"></i>Retour</button>\n        <button onclick="wizardStep6()" class="flex-1 py-3 bg-rouge-500 text-dark-900 font-bold rounded-xl hover:bg-rouge-500 transition text-sm">\n          <i class="fas fa-check mr-2"></i>J\'ai payé\n        </button>\n       </div>';
  if (_wizardShow("\n\n  " + _wizardProgress(5, 7) + "\n\n  <div class=\"p-6 space-y-5\">\n\n    <div>\n\n      <h3 class=\"text-lg font-bold text-white\">Effectuez votre paiement</h3>\n\n      <p class=\"text-sm text-gray-400 mt-1\">Commande cr\xE9\xE9e \xB7 R\xE9f. <span class=\"font-mono text-rouge-400\">" + (_wizardData.orderId || "").substring(0, 12) + "\u2026</span></p>\n\n    </div>\n\n    <div class=\"bg-dark-700 border border-dark-500 rounded-xl p-4 flex justify-between items-center\">\n\n      <span class=\"text-gray-400 text-sm\">Montant total</span>\n\n      <span class=\"text-rouge-400 font-bold text-2xl\">$" + n.toLocaleString("en-US", {
    minimumFractionDigits: 2
  }) + "</span>\n\n    </div>\n\n    " + a + "\n\n    " + i + "\n\n    " + l + "\n\n  </div>"), s) {
    const e = _wizardData.orderId,
      t = _wizardData.licenseId || null,
      a = n;
    _renderPayPalButtons("paypal-buttons-container", async () => {
      const n = {
        amount: a
      };
      e && (n.order_id = e), t && (n.license_id = t), n.description = "LEADER \u2014 " + (_wizardData.pkgName || "Licence Finstrategia") + " ($" + a.toFixed(2) + ")";
      const s = await api("POST", "/members/paypal/create-order", n);
      if (!s.paypal_order_id) throw new Error("Erreur création ordre PayPal");
      return s.paypal_order_id;
    }, e => {
      _wizardShow("\n\n        <div class=\"p-8 text-center space-y-5\">\n\n          <div class=\"w-20 h-20 rounded-full bg-green-500/20 flex items-center justify-center mx-auto\">\n\n            <i class=\"fas fa-check text-green-400 text-4xl\"></i>\n\n          </div>\n\n          <div>\n\n            <p class=\"text-white font-bold text-xl\">Paiement confirm\xE9 !</p>\n\n            <p class=\"text-green-400 text-sm mt-1\">" +
        (e.message || "Votre commande est activée.") + "</p>\n\n          </div>\n\n          <div class=\"bg-dark-700 rounded-xl p-4 text-left space-y-2 text-sm\">\n\n            " + (e.amount ? "<div class=\"flex justify-between\"><span class=\"text-gray-400\">Montant pay\xE9</span><span class=\"text-green-400 font-bold\">$" +
        Number(e.amount).toFixed(2) + "</span></div>" : "") + "\n\n            " + (e.paypal_capture_id ? "<div class=\"flex justify-between\"><span class=\"text-gray-400\">R\xE9f\xE9rence PayPal</span><span class=\"font-mono text-xs text-gray-300\">" + e.paypal_capture_id +
        "</span></div>" : "") + "\n\n            " + (e.order_id ? "<div class=\"flex justify-between\"><span class=\"text-gray-400\">R\xE9f. commande</span><span class=\"font-mono text-xs text-gray-300\">" + (e.order_id || "").substring(0, 16) + "\u2026</span></div>" : "") +
        "\n\n          </div>\n\n          <button onclick=\"wizardClose(); showPage('dashboard')\"\n\n            class=\"w-full py-3 bg-rouge-500 text-dark-900 font-bold rounded-xl hover:bg-rouge-500 transition\">\n\n            <i class=\"fas fa-home mr-2\"></i>Aller \xE0 mon tableau de bord\n\n          </button>\n\n        </div>"), _wizardReset();
    }, e => {
      document.getElementById("paypal-buttons-container").innerHTML = "<div class=\"bg-red-900/20 border border-red-500/30 rounded-xl p-4 text-center space-y-3\">\n\n            <p class=\"text-red-400 text-sm\"><i class=\"fas fa-exclamation-circle mr-1\"></i>" + e + "</p>\n\n            <button onclick=\"wizardStep5()\" class=\"text-xs text-gray-400 hover:text-white underline\">R\xE9essayer</button>\n\n          </div>";
    });
  }
  r && setTimeout(() => _initStripePaymentElement(n), 150);
}
let _stripe = null,
  _stripeElements = null,
  _stripePiClientSecret = null;
async function _loadStripeSdk() {
  return !!window.Stripe || new Promise(e => {
    const t = document.createElement("script");
    t.src = "https://js.stripe.com/v3/", t.onload = () => e(!0), t.onerror = () => e(!1), document.head.appendChild(t);
  });
}
async function _initStripePaymentElement(e) {
  if (!(await _loadStripeSdk())) return void (document.getElementById("stripe-payment-element").innerHTML = '<p class="text-red-400 text-sm text-center"><i class="fas fa-exclamation-circle mr-1"></i>Impossible de charger Stripe. Vérifiez votre connexion.</p>');
  const t = _wizardData.orderId,
    n = _wizardData.licenseId || null,
    a = e;
  try {
    const e = {
      amount: a
    };
    if (_wizardData.isPhysicalCard && _wizardData.orderId) {
      e.physical_card_order_id = _wizardData.orderId;
    } else if (_wizardData.licenseOnly) {
      if (n) e.license_id = n;
    } else {
      t && (e.order_id = t);
    ...
    e.description = "LEADER \u2014 Commande " + (_wizardData.isPhysicalCard ? _wizardData.orderId : n || t || "").substring(0, 12);
    const s = await api("POST", "/members/stripe/create-payment-intent", e);
    _stripePiClientSecret = s.client_secret, window._stripePiId = s.payment_intent_id, window._stripeOrderId = t, window._stripeLicenseId = n, _stripe = Stripe(window._stripePublicKey);
    const r = {
      theme: "night",
      variables: {
        colorPrimary: "#c9a84c",
    ...
    _stripeElements = _stripe.elements({
      clientSecret: _stripePiClientSecret,
      appearance: r
    }), _stripeElements.create("payment", {
      layout: "tabs"
    }).mount("#stripe-payment-element");
  } catch (e) {
    const t = e.error || e.message || "Erreur Stripe";
    console.error("[Stripe init error]", t, e);
    const _se = document.getElementById("stripe-payment-element");
    if (_se) {
      _se.innerHTML = "<div class=\"bg-red-900/20 border border-red-500/30 rounded-xl p-4 text-center space-y-3\"><i class=\"fas fa-exclamation-circle text-red-400 text-2xl\"></i><p class=\"text-red-300 font-semibold text-sm mt-2\">" + t + "</p><button onclick=\"wizardStep4()\" class=\"py-2 px-4 bg-dark-700 text-gray-300 rounded-xl text-xs mt-2\">\u2190 Retour</button></div>";
    } else {
      _wizardShow("<div class=\"p-6 text-center space-y-4\"><div class=\"w-14 h-14 rounded-full bg-red-500/20 flex items-center justify-center mx-auto\"><i class=\"fas fa-exclamation-circle text-red-400 text-3xl\"></i></div><p class=\"text-red-300 font-semibold\">" + t + "</p><button onclick=\"wizardStep4()\" class=\"py-2 px-4 bg-dark-700 text-gray-300 rounded-xl text-sm\">\u2190 Retour</button></div>");
    }
  }
}
async function _stripeConfirmPayment() {
  const e = document.getElementById("stripe-pay-btn"),
    t = document.getElementById("stripe-error-msg");
  if (_stripe && _stripeElements) {
    e.disabled = !0, e.innerHTML = '<div class="loader-sm mx-auto"></div>', t.classList.add("hidden");
    try {
      const {
        error: n,
        paymentIntent: a
      ...
      if (n) return t.textContent = n.message || "Paiement refusé", t.classList.remove("hidden"), e.disabled = !1, void (e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer');
      if (a && "succeeded" === a.status) {
        e.innerHTML = '<div class="loader-sm mx-auto"></div>';
        const t = await api("POST", "/members/stripe/confirm-payment", {
          payment_intent_id: window._stripePiId,
          order_id: (_wizardData.isPhysicalCard ? null : window._stripeOrderId) || null,
          license_id: window._stripeLicenseId || null,
          physical_card_order_id: _wizardData.isPhysicalCard ? _wizardData.orderId || null : null
        });
        _wizardData.confirmationMsg = t.message || "Paiement confirmé et commande activée !", wizardStep7();
      } else t.textContent = "Statut inattendu: " + a?.status, t.classList.remove("hidden"), e.disabled = !1, e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer';
    } catch (n) {
      t.textContent = n.error || n.message || "Erreur lors du paiement", t.classList.remove("hidden"), e.disabled = !1, e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer';
    }
  }
}
function wizardStep6() {
  _wizardShow("\n\n  " + _wizardProgress(6, 7) + ("\n\n  <div class=\"p-6 space-y-5\">\n\n    <div>\n\n      <h3 class=\"text-lg font-bold text-white\">Preuve de paiement</h3>\n\n      <p class=\"text-sm text-gray-400 mt-1\">Uploadez une capture d'écran de votre virement ou tout justificatif de paiement.</p>\n\n    </div>\n\n\n\n    <!-- Zone upload preuve de paiement -->\n\n    <div id=\"wiz-proof-upload-zone\"></div>\n\n\n\n    <div>\n\n      <label class=\"block text-sm text-gray-400 mb-1.5\">Note (optionnel)</label>\n\n      <input id=\"wiz-proof-note\" type=\"text\" placeholder=\"Ex : virement du 15 mai 2026…\"\n" +
    ...
    window._wizProofUploadUid = createUploadZone({
      containerId: "wiz-proof-upload-zone",
      accept: "image/*,application/pdf",
      purpose: "proof",
      apiPrefix: "/api/members",
      label: 'Justificatif de paiement <span class="text-red-400">*</span>',
      onUrl: e => {
        window._wizProofUrl = e || "";
      },
      getToken: () => memberToken || localStorage.getItem("leader_member_token")
    });
  }, 50);
}
async function wizardSubmitProof() {
...
function wizardProofOnly(e) {
  _wizardReset(), _wizardData.orderId = e, wizardStep6();
}
function showProofModal(e) {
  wizardProofOnly(e);
}
...
async function renderLicense(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const t = await api("GET", "/members/license"),
      n = t.member,
      a = new Date(),
      s = n?.license_expires_at ? new Date(n.license_expires_at) : null,
      r = s ? Math.ceil((s - a) / 864e5) : null,
      i = t.license_alert_days || 7,
      _renewEarly = t.license_renew_early_days || 30,
      _price = t.license_price || 97,
      l = n?.license_active && null !== r && r <= i && r > 0,
      _canRenew = !n?.license_active || null !== r && r <= _renewEarly,
      d = n?.license_active || null !== s ? n?.license_active && !l ? "\n      <div class=\"bg-green-900/10 border border-green-500/30 rounded-xl p-5\">\n        <div class=\"flex items-center justify-between mb-4\">\n          <div class=\"flex items-center gap-3\">\n            <div class=\"w-10 h-10 rounded-xl bg-green-500/20 flex items-center justify-center\">\n              <i class=\"fas fa-check-circle text-green-400 text-lg\"></i>\n            </div>\n            <div>\n              <div class=\"font-bold text-green-300\">Licence active</div>\n              <div class=\"text-xs text-green-400/70\">Finstrategia — accès complet</div>\n" +
        ...
        r + " j</div>\n            <div class=\"text-xs text-gray-400\">Jours restants</div>\n          </div>\n          <div class=\"bg-dark-700 rounded-xl p-3 text-center\">\n            <div class=\"text-sm font-bold text-white\">" + (s ? s.toLocaleDateString("fr-FR") : "—") +
        "</div>\n            <div class=\"text-xs text-gray-400\">Date d'expiration</div>\n          </div>\n        </div>\n        " + (_canRenew ? "<p class=\"text-xs text-orange-400 mb-3\"><i class=\"fas fa-info-circle mr-1\"></i>Renouvellement anticip\xE9 disponible \u2014 il reste " +
        r + " jours. Le renouvellement s'ajoutera \xE0 la date d'expiration actuelle.</p>" : "<p class=\"text-xs text-gray-500 mb-3\">Le bouton de renouvellement sera actif " + _renewEarly + " jours avant l'expiration (dans " + (r - _renewEarly) + " jours).</p>") + "\n        <button onclick=\"" +
        (_canRenew ? "orderLicense()" : "void(0)") + "\" id=\"btn-order-license\" class=\"w-full font-bold py-3 rounded-xl transition border " + (_canRenew ? "bg-dark-700 text-white hover:bg-dark-600 border-dark-500" : "bg-dark-800 text-gray-600 cursor-not-allowed border-dark-700") +
        ...
        "licence Finstrategia — $") + fmtPrice(_price) + "/an\n        </button>\n      </div>";
    e.innerHTML = "\n    <div class=\"max-w-lg space-y-6\">\n      <div>\n        <h2 class=\"text-xl font-bold\">Ma Licence Finstrategia</h2>\n        <p class=\"text-gray-400 text-sm mt-1\">Licence annuelle ($" + fmtPrice(_price) + "/an) \u2014 \xE9ligibilit\xE9 aux commissions LEADER</p>\n      </div>\n      " + d + "\n    </div>";
  } catch (t) {
    e.innerHTML = "<div class=\"p-4 text-red-400\">Erreur : " + (t.error || t.message) + "</div>";
  }
}
async function orderLicense() {
  ...
  if (overlay) overlay.style.display = "flex";
  try {
    const [licCfg, pkgData] = await Promise.all([api("GET", "/members/license").catch(() => null), !_gw || !Object.keys(_gw).length ? api("GET", "/members/packages").catch(() => null) : Promise.resolve(null)]);
    if (licCfg && licCfg.license_price) {
      _wizardData.totalAmt = licCfg.license_price;
      _wizardData.licensePrice = licCfg.license_price;
    }
    if (pkgData) {
      _gw = _buildGw(pkgData.gateways || {});
      _adminFeeInfo = pkgData.adminFee || {
        amount: 0,
        active: false,
        paid: true
      };
      if (pkgData.licensePrice) {
        _licPriceForWizard = pkgData.licensePrice;
        _wizardData.totalAmt = pkgData.licensePrice;
        _wizardData.licensePrice = pkgData.licensePrice;
      }
    }
  } catch (e) {}
  wizardStep4();
...
async function renderProfile(e) {
  ...
  e.innerHTML = "\n  <div class=\"max-w-2xl space-y-5\">\n\n    <!-- En-t\xEAte -->\n    <div class=\"flex items-center justify-between flex-wrap gap-3\">\n      <h2 class=\"text-xl font-bold text-white flex items-center gap-2\">\n        <i class=\"fas fa-user-circle text-rouge-400\"></i> Mon Profil\n      </h2>\n      <div class=\"flex items-center gap-2\">\n        <span class=\"text-xs text-gray-500\">" + (t?.unique_id || "") + "</span>\n        " + (t?.member_status ? "<span class=\"text-xs px-2 py-0.5 rounded-full font-medium " + ({
    ...
    (t?.unique_id || "—") + "</div>\n          </div>\n          <div class=\"bg-dark-700/50 rounded-xl p-3\">\n            <div class=\"text-xs text-gray-400 mb-1 uppercase tracking-wider\">Rang actuel</div>\n            <div class=\"text-white font-medium\">" + (t?.current_rank && t.current_rank !== "none" ? t.current_rank : "—") +
...
function profFilterCountries(q) {
  ...
  var src = (typeof REG_COUNTRIES !== "undefined" ? REG_COUNTRIES : []).slice().sort(function (a, b) {
    return a.name.localeCompare(b.name, 'fr');
  });
  var filtered = q ? src.filter(function (c) {
  ...
  list.onmouseenter = function () {
    _profCountryDropdownHovered = true;
  };
...
async function saveProfile() {
  ...
  const payload = {
    first_name: document.getElementById("p-firstname")?.value.trim() || t?.first_name,
    last_name: document.getElementById("p-lastname")?.value.trim() || t?.last_name,
    phone: _phoneFull || null,
    country: document.getElementById("p-country")?.value.trim() || null,
    address: document.getElementById("p-address")?.value.trim() || null,
    city: document.getElementById("p-city")?.value.trim() || null,
    postal_code: document.getElementById("p-postalcode")?.value.trim() || null,
    ...
    paypal_email: t?.paypal_email || null
  };
  ...
  try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-save mr-2"></i>Sauvegarder le profil';
    }
  }
}
async function savePin() {
  const pwd = document.getElementById("pin-pwd")?.value;
  ...
  const btn = document.querySelector('[onclick="savePin()"]');
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Modification…';
  }
  try {
    await api("PUT", "/members/me/pin", {
      current_password: pwd,
      new_pin: newPin
    });
    showToast("PIN modifié avec succès !");
    document.getElementById("pin-pwd").value = "";
    document.getElementById("pin-new").value = "";
  } catch (err) {
    showToast(err.error || "Erreur lors de la modification du PIN", "error");
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-key mr-2"></i>Modifier le PIN';
    }
  }
}
function copyProfileReferralLink() {
  const e = document.getElementById("profile-referral-input");
  e && navigator.clipboard.writeText(e.value).then(() => {
...
async function _cpStartPayment() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Création de la transaction…';
  }
  try {
    const body = {
      amount,
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-coins text-sm"></i> Payer en Crypto';
    }
    if (errEl) {
      errEl.textContent = err.error || err.message || 'Erreur CoinPayments';
      errEl.classList.remove('hidden');
    }
  }
}
...
async function _cpCheckStatus() {
  ...
  try {
    ...
    if (res.is_complete) {
      ...
      if (statusBox) {
        statusBox.className = 'bg-green-900/20 border border-green-500/30 rounded-xl p-4 text-center';
      }
      const body = {
        txn_id: txnId
      };
      ...
      if (statusBox) {
        statusBox.className = 'bg-red-900/20 border border-red-500/30 rounded-xl p-4 text-center';
      }
    } else {
      const labels = {
        0: 'En attente de paiement…',
        1: 'Paiement partiel reçu…',
        2: 'En file de traitement…',
        3: 'Paiement en cours de validation…'
      };
...
function _supportStartPolling(ticketId) {
  ...
  _supportPollingTimer = setInterval(async () => {
    ...
    if (_supportCurrentTicketId !== ticketId) {
      _supportStopPolling();
      return;
    }
    try {
...
function _supportStopPolling() {
  if (_supportPollingTimer) {
    clearInterval(_supportPollingTimer);
    _supportPollingTimer = null;
  }
}
...
async function supportLoadTickets() {
  ...
  try {
    ...
    listEl.innerHTML = "\n      <div class=\"bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden\">\n        <div class=\"divide-y divide-dark-600\">\n          " + tickets.map(t => "\n            <div class=\"px-5 py-4 hover:bg-dark-700/40 cursor-pointer transition group\" onclick=\"supportOpenTicket('" +
      ...
      t.ticket_number + "</span>\n                    " + supportStatusBadge(t.status) + "\n                    " + (t.unread_member > 0 ? "<span class=\"text-xs bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full font-medium\"><i class=\"fas fa-circle text-[8px] mr-1 animate-pulse\"></i>" +
      t.unread_member + " nouveau" + (t.unread_member > 1 ? 'x' : '') + "</span>" : '') + "\n                  </div>\n                  <p class=\"text-sm font-medium text-white mt-1 truncate\">" + t.subject + "</p>\n                  <div class=\"flex items-center gap-3 mt-1 text-xs text-gray-500\">\n                    <span><i class=\"fas fa-tag mr-1\"></i>" +
      ...
      "\n        </div>\n      </div>";
    renderPagination('support-tickets-pagination', total, _supportPage, _supportPerPage, '(function(p){_supportPage=p;supportLoadTickets()})', '(function(n,p){_supportPerPage=n;_supportPage=p;supportLoadTickets()})');
  } catch (err) {
...
async function supportCreateTicket() {
  ...
  if (!message) {
    errEl.textContent = 'Le message est obligatoire.';
    errEl.classList.remove('hidden');
    return;
  }
  btn.disabled = true;
...
async function supportOpenTicket(ticketId) {
  ...
  try {
    ...
    pc.innerHTML = "\n    <div class=\"space-y-5 max-w-4xl\">\n      <!-- En-t\xEAte -->\n      <div class=\"flex items-center gap-3 flex-wrap\">\n        <button onclick=\"_supportStopPolling();showPage('support')\" class=\"text-gray-400 hover:text-white transition\">\n          <i class=\"fas fa-arrow-left\"></i>\n        </button>\n        <div class=\"flex-1\">\n          <div class=\"flex items-center gap-2 flex-wrap\">\n            <span class=\"text-xs text-gray-500 font-mono\">#" +
      ...
      " \u2014 r\xE9pondez pour le r\xE9ouvrir si n\xE9cessaire.\n        <button onclick=\"supportReopenTicket('" + ticket.id + "')\" class=\"ml-2 text-rouge-400 hover:underline\">R\xE9ouvrir \u2192</button>\n      </div>") + "\n\n      <!-- Indicateur de polling (discret) -->\n      " +
    ...
    setTimeout(() => {
      const thread = document.getElementById('support-thread');
      if (thread) thread.lastElementChild?.scrollIntoView({
        behavior: 'smooth',
        block: 'nearest'
      });
    }, 100);
...
function supportRenderMessage(m) {
  ...
  const bubbleClass = isAdmin ? 'bg-dark-700 border border-dark-600 mr-8' : 'bg-rouge-500/10 border border-rouge-500/20 ml-8';
...
function supportShowRate(ticketId, currentRating) {
  ...
  showModal("\n    <div class=\"p-6 space-y-5\">\n      <div class=\"flex items-center justify-between\">\n        <h3 class=\"text-lg font-bold text-white\"><i class=\"fas fa-star text-rouge-400 mr-2\"></i>Évaluer votre expérience</h3>\n        <button onclick=\"closeModal()\" class=\"text-gray-500 hover:text-white text-xl leading-none\">&times;</button>\n      </div>\n\n      <!-- Intro premium -->\n      <div class=\"bg-dark-700/60 border border-dark-500 rounded-xl p-3 text-xs text-gray-400 leading-relaxed\">\n        <i class=\"fas fa-shield-halved text-rouge-400 mr-1.5\"></i>\n" +
    ...
    "onclick=\"supportSubmitRate('") + ticketId + "')\" class=\"flex-1 py-3 bg-rouge-500 hover:bg-rouge-500 text-dark-900 font-bold rounded-xl text-sm transition\">\n          <i class=\"fas fa-check mr-2\"></i>Envoyer mon avis\n        </button>\n      </div>\n    </div>");
...
async function supportSubmitRate(ticketId) {
  ...
  try {
    const result = await api('POST', "/support/tickets/" + ticketId + "/rate", {
      rating,
      comment
    });
    closeModal();
...
function kycLevelBg(lvl) {
  return ['bg-gray-500/10 border-gray-500/30', 'bg-blue-500/10 border-blue-500/30', 'bg-purple-500/10 border-purple-500/30', 'bg-rouge-500/10 border-rouge-500/25'][lvl] || 'bg-gray-500/10 border-gray-500/30';
}
...
function kycDocStatusIcon(st) {
  return {
    ...
    rejected: '<i class="fas fa-times-circle text-red-400"></i>'
  }[st] || '<i class="fas fa-question-circle text-gray-400"></i>';
}
async function renderKYC(el) {
  el.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const [cfgRes, statusRes] = await Promise.all([api('GET', '/kyc/config-public').catch(() => null), api('GET', '/kyc/status').catch(() => null)]);
...
function _kycWizardRender(docs) {
  ...
  let bodyHtml = '';
  if (step === 1) bodyHtml = _kycWizardStep1Html(docsForLevel);else if (step === 2) bodyHtml = _kycWizardStep2Html();else if (step === 3 && _kycWizard.livenessEnabled) bodyHtml = _kycWizardStepLivenessHtml();else if (step === 3 || step === 4) bodyHtml = _kycWizardStep3Html();
  const html = "\n  <div class=\"p-6 space-y-5\">\n    <!-- En-t\xEAte -->\n    <div class=\"flex items-center justify-between\">\n      <h3 class=\"text-lg font-bold text-white flex items-center gap-2\">\n        <i class=\"fas fa-shield-halved text-purple-400\"></i>\n        V\xE9rification niveau " + _kycWizard.targetLevel + "\n      </h3>\n      <button onclick=\"_kycWizardClose()\" class=\"text-gray-500 hover:text-white transition text-sm\">\n        <i class=\"fas fa-times\"></i>\n      </button>\n    </div>\n\n    <!-- Indicateur d'\xE9tapes -->\n    <div class=\"flex items-start gap-0\">" + stepsHtml + "</div>\n\n    <!-- Corps de l'\xE9tape -->\n    <div id=\"kyc-wizard-body\">" + bodyHtml + "</div>\n  </div>";
  showModal(html, true);
  if (step === 1) {
...
function _kycWizardStep1Html(docs) {
  ...
  const previewHtml = _kycWizard.docDataUrl ? "\n    <div class=\"flex items-center gap-3 bg-dark-700 border border-green-500/30 rounded-xl px-3 py-2.5 mt-2\">\n      " + (_kycWizard.docMime?.startsWith('image/') ? "<img src=\"" + _kycWizard.docDataUrl + "\" class=\"w-14 h-14 rounded-lg object-cover flex-shrink-0 border border-dark-500\">" : "<div class=\"w-14 h-14 rounded-lg bg-red-500/20 flex items-center justify-center flex-shrink-0\"><i class=\"fas fa-file-pdf text-red-400 text-xl\"></i></div>") +
  ...
  list.querySelectorAll('.kyc-doc-option').forEach(el => {
    el.addEventListener('click', () => {
      ...
      list.querySelectorAll('.kyc-doc-option').forEach(opt => {
        ...
        opt.className = opt.className.replace(/border-purple-500 bg-purple-500\/10|border-dark-500 bg-dark-700\/50 hover:border-purple-500\/50 hover:bg-dark-700/g, '').trim();
        opt.classList.add(...(sel ? ['border-purple-500', 'bg-purple-500/10'] : ['border-dark-500', 'bg-dark-700/50', 'hover:border-purple-500/50', 'hover:bg-dark-700']));
...
function _kycWizardClearDoc() {
  ...
  const body = document.getElementById('kyc-wizard-body');
  if (body) {
    body.innerHTML = _kycWizardStep1Html();
    setTimeout(_kycWizardBindDocOptions, 0);
  }
}
...
async function _kycWizardLoadDocFile(file) {
  ...
  if (!allowed.includes(file.type)) {
    if (errEl) {
      errEl.textContent = 'Format non accepté (JPG, PNG, PDF uniquement)';
      errEl.classList.remove('hidden');
    }
    return;
  }
  if (file.size > maxRaw) {
    if (errEl) {
      errEl.textContent = 'Fichier trop volumineux (max 50 Mo)';
      errEl.classList.remove('hidden');
    }
    return;
  }
  ...
  if (progressEl) {
    progressEl.textContent = 'Optimisation en cours...';
  ...
  try {
    ...
    if (progressEl) {
      progressEl.textContent = "Optimis\xE9 (" + approxKB + " Ko) \u2014 Ajustez si besoin";
    }
    ...
    if (errEl) {
      errEl.textContent = 'Erreur lors du traitement du fichier. Réessayez.';
      errEl.classList.remove('hidden');
    }
...
function _kycWizardNext1() {
  if (!_kycWizard.docTypeId) {
    ...
    if (e) {
      e.textContent = 'Choisissez d\'abord le type de document';
  ...
  if (!_kycWizard.docBase64) {
    const e = document.getElementById('kyc-wizard-err1');
    if (e) {
      e.textContent = 'Uploadez votre pièce d\'identité avant de continuer';
      e.classList.remove('hidden');
    }
    return;
  }
...
async function _kycWizardStartCamera() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<div class="loader !w-4 !h-4"></div> Activation...';
  }
  try {
    ...
    if (e) {
      e.textContent = err.name === 'NotAllowedError' ? 'Accès à la caméra refusé — utilisez le bouton "Uploader photo"' : 'Caméra non disponible — utilisez le bouton "Uploader photo"';
      e.classList.remove('hidden');
    }
    if (btn) {
      btn.disabled = false;
...
async function _kycWizardLoadSelfieFile(file) {
  ...
  if (!allowed.includes(file.type)) {
    if (errEl) {
      errEl.textContent = 'Format non accepté (JPG ou PNG uniquement)';
      errEl.classList.remove('hidden');
    }
    return;
  }
  ...
  if (file.size > maxRaw) {
    if (errEl) {
      errEl.textContent = 'Photo trop lourde (max 30 Mo)';
      errEl.classList.remove('hidden');
    }
    return;
  }
  ...
  try {
    ...
    if (errEl) {
      errEl.textContent = 'Erreur lors du traitement de la photo. Réessayez.';
      errEl.classList.remove('hidden');
    }
...
function _kycWizardNext2() {
  if (!_kycWizard.selfieBase64) {
    const e = document.getElementById('kyc-wizard-err2');
    if (e) {
      e.textContent = 'Prenez un selfie ou uploadez une photo avant de continuer';
      e.classList.remove('hidden');
    }
    return;
  }
...
function _kycWizardStepLivenessHtml() {
  ...
  const SL = {
    blink: {
      ...
      hint: 'Regardez la caméra et clignez des yeux lentement'
    },
    turn_left: {
      icon: 'fa-arrow-left',
      label: 'Tête à gauche',
      hint: 'Tournez doucement la tête vers la gauche'
    },
    turn_right: {
      icon: 'fa-arrow-right',
      label: 'Tête à droite',
      hint: 'Tournez doucement la tête vers la droite'
    },
    nod: {
      icon: 'fa-arrows-up-down',
      label: 'Hochez la tête',
      hint: 'Faites oui de haut en bas'
    },
    smile: {
      icon: 'fa-face-smile',
      label: 'Souriez',
      hint: 'Souriez naturellement à la caméra'
    }
  };
  ...
  const s = SL[steps[cur]] || {
    icon: 'fa-circle-question',
    label: steps[cur],
    hint: 'Effectuez le geste'
  };
...
function _kycWizardStopLivenessCamera() {
  if (_kycWizard.livenessStream) {
    _kycWizard.livenessStream.getTracks().forEach(function (t) {
      t.stop();
    });
...
function _kycWizardStep3Html(result) {
  if (!result) {
    return "\n      <div class=\"space-y-6 text-center py-4\">\n        <div class=\"w-20 h-20 rounded-full bg-purple-500/20 border-2 border-purple-500/40 flex items-center justify-center mx-auto animate-pulse\">\n          <i class=\"fas fa-shield-check text-4xl text-purple-400\"></i>\n        </div>\n        <div>\n          <div class=\"text-white font-bold text-lg mb-1\">Validation en cours…</div>\n          <div id=\"kyc-ai-status\" class=\"text-sm text-gray-400\">Vérification de votre document</div>\n        </div>\n\n        <!-- Barre de progression animée -->\n" +
      ...
      id: 'ai-s1',
      icon: 'fa-file-shield',
      label: 'Contrôle de conformité du document'
    }, {
      id: 'ai-s2',
      icon: 'fa-user-check',
      ...
      id: 'ai-s3',
      icon: 'fa-lock',
      label: 'Sécurisation de votre dossier'
    }, {
      id: 'ai-s4',
  ...
  try {
    ...
    if (!_kycWizard.applicationId) {
      const startRes = await api('POST', '/kyc/start', {
        target_level: _kycWizard.targetLevel
      });
    ...
    if (analysisRes.decision === 'approved') {
      ...
      if (body) body.innerHTML = _kycWizardStep3Html({
...
!function () {
  ...
  e.id = "wizard-overlay", e.className = "hidden fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4", e.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.82);z-index:9999;display:none;align-items:center;justify-content:center;padding:1rem;", e.innerHTML = '<div id="wizard-box" class="bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-lg shadow-2xl" style="max-height:92vh;overflow-y:auto;"></div>', document.body ? document.body.appendChild(e) : document.addEventListener("DOMContentLoaded", () => document.body.appendChild(e));
}();
function detectImpersonation() {
  try {
    const p = new URLSearchParams(window.location.search);
    const t = p.get("impersonate");
    if (t) {
      window.__IMPERSONATE_TOKEN__ = t;
      return t;
    }
  } catch (e) {}
  return window.__IMPERSONATE_TOKEN__ || null;
}
function showImpersonationBar(t) {
  const b = document.createElement("div");
  b.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:99999;background:#7c3aed;color:#fff;text-align:center;padding:6px 12px;font-size:12px;font-weight:600;";
  b.textContent = "Mode impersonation actif";
...
window.addEventListener("load", async () => {
  (function _bindAuthButtons() {
    ...
    if (btnLogin) btnLogin.addEventListener('click', function (e) {
      e.preventDefault();
      doLogin();
    });
    var btnRegister = document.querySelector("button[onclick='doRegister()']") || document.getElementById('reg-submit-btn');
    if (btnRegister) btnRegister.addEventListener('click', function (e) {
      e.preventDefault();
      doRegister();
    });
    var btnShowReg = document.querySelector("button[onclick='showRegister()']");
    if (btnShowReg) btnShowReg.addEventListener('click', function (e) {
      e.preventDefault();
      showRegister();
    });
  ...
  const e = detectImpersonation();
  if (e) return await initApp(), void showImpersonationBar(e);
  if (memberToken) initApp();else {
    document.getElementById("auth-screen").classList.remove("hidden");
    const e = window.__REGISTRATION__ || {};
    "M2" !== e.mode && "M3" !== e.mode || showRegister();
  }
});
async function renderEarnings(el) {
  el.innerHTML = '<div class="flex justify-center py-16"><div class="loader"></div></div>';
  try {
    const [stmtData, ordersData] = await Promise.all([api('GET', '/members/earnings/statement'), api('GET', '/members/orders').catch(() => ({
      orders: []
    }))]);
    ...
    el.innerHTML = "\n    <div class=\"space-y-6\">\n\n      <!-- En-tête premium -->\n      <div class=\"relative overflow-hidden bg-gradient-to-br from-dark-800 via-dark-800 to-emerald-900/20 border border-emerald-500/20 rounded-2xl p-6\">\n        <div class=\"absolute top-0 right-0 w-64 h-64 bg-emerald-500/5 rounded-full -translate-y-1/2 translate-x-1/3 pointer-events-none\"></div>\n        <div class=\"relative flex items-start justify-between gap-4 flex-wrap\">\n          <div>\n            <div class=\"flex items-center gap-3 mb-2\">\n" +
      ...
      (stmtData.company?.legal_name || stmtData.company?.name || stmtData.company?.network_name || 'LEADER') + "</div>\n            " + (stmtData.company?.registration_number ? "<div class=\"text-[10px] font-mono text-gray-500\">Co. No. " + stmtData.company.registration_number +
...
function _trunc(v, n) {
  return String(v || '—').substring(0, n || 40);
}
const _COMM_LABELS = {
  prime_leadership: 'Leadership Prime',
  bonus_influence: 'Influence Bonus',
  bonus_rayonnement: 'Rayonnement Bonus',
  valorisation_recommandation: 'Referral Bonus',
  fast_start: 'Fast Start Bonus',
  credit_croissance: 'Growth Credit',
  reserve_strategique: 'Strategic Reserve',
...
function _pdfDrawHeader(pdf, pw, co, member, docRef, docTitle) {
  ...
  pdf.setFontSize(10);
  pdf.setFont('helvetica', 'bold');
  pdf.text(docTitle, pw / 2, 12, {
    align: 'center'
  });
  pdf.setTextColor(...GRAY);
  pdf.setFontSize(6.5);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Ref: ' + docRef, pw - 5, 9, {
    align: 'right'
  });
  ...
  pdf.setFillColor(20, 20, 42);
  pdf.rect(0, 21, pw, 9, 'F');
  pdf.setTextColor(...ACCENT);
  pdf.setFontSize(7.5);
  pdf.setFont('helvetica', 'bold');
  pdf.text((member.first_name || '') + ' ' + (member.last_name || '') + '  ·  ' + (member.unique_id || ''), pw / 2, 27, {
    align: 'center'
  });
}
function _pdfDrawFooter(pdf, pw, ph, co, pageNum, totalPages) {
  ...
  const GOLD = _hexToRgb(co.pdf_color_primary || '#D4AF37');
  pdf.setFillColor(18, 18, 38);
  pdf.rect(0, ph - 14, pw, 14, 'F');
  pdf.setFillColor(...GOLD);
  pdf.rect(0, ph - 14, pw, 0.8, 'F');
  ...
  pdf.text(line1, pw / 2, ph - 10.5, {
    align: 'center'
  });
  if (line2) pdf.text(line2, pw / 2, ph - 7.5, {
    align: 'center'
  });
  if (line3) pdf.text(line3, pw / 2, ph - 4.5, {
    align: 'center'
  });
  pdf.setTextColor(...GOLD);
  pdf.setFontSize(6);
  pdf.setFont('helvetica', 'bold');
  pdf.text('Page ' + pageNum + ' / ' + totalPages, pw - 5, ph - 3, {
    align: 'right'
  });
}
...
const _LANGS = {
  en: {
    ...
    total: 'TOTAL',
    declText: (name, uid, period, co, regNo) => "This Monthly Earnings Statement certifies that the above-listed commissions were earned by " + name + " (Member ID: " + uid + ") during the period of " + period + ". This document is issued by " + co + (regNo ? ', registered under Company No. ' + regNo : '') + ", and constitutes an official record of income for tax declaration purposes.",
  ...
  fr: {
    ...
    para2: (name, co, regNo, memberId, title) => "Il est par la pr\xE9sente confirm\xE9 que " + name + " et " + co + " ont conclu un Accord de Partenariat aux termes duquel " + name + " a \xE9t\xE9 nomm\xE9(e) " + (title || 'Partenaire Indépendant(e)') + ". Dans le cadre dudit accord, " + name + " est \xE9ligible \xE0 percevoir des commissions en contrepartie des services fournis. Le num\xE9ro d'enregistrement de " + name + " aupr\xE8s de nos services est le " + memberId + ".",
    para3annual: (name, start, amount, year) => "Depuis la date d'adh\xE9sion (" + start + ") \xE0 ce jour, " + name + " a per\xE7u " + amount + " USD de commissions au cours de l'exercice fiscal " + year + ".",
    ...
    summaryTitle: year => "R\xC9CAPITULATIF ANNUEL \u2014 " + year,
    summaryMonthly: period => "R\xC9CAPITULATIF MENSUEL \u2014 " + period,
    fullName: 'Nom et Prénom',
    memberId: 'N° de Membre / N° d\'Enregistrement',
    status: 'Statut Membre',
    rank: 'Rang Actuel',
    totalAnnual: year => "Total Commissions Per\xE7ues (" + year + ")",
    ...
    total: 'TOTAL',
    declText: (name, uid, period, co, regNo) => "Ce Relev\xE9 de Commissions Mensuel certifie que les commissions list\xE9es ci-dessus ont \xE9t\xE9 per\xE7ues par " + name + " (N\xB0 Membre : " + uid + ") au cours de la p\xE9riode " + period + ". Ce document est \xE9mis par " + co + (regNo ? ', immatriculée sous le numéro ' + regNo : '') + ", et constitue un justificatif officiel de revenus \xE0 des fins de d\xE9claration fiscale.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) => "Nous, soussign\xE9s, certifions que " + name + " (N\xB0 Membre : " + uid + ") a per\xE7u un montant brut total de " + amount + " en commissions et primes de la part de " + co + " au cours de l'exercice fiscal " + year + ". Le total des retraits trait\xE9s au cours de cette p\xE9riode s'\xE9l\xE8ve \xE0 " + wdAmount + ". Ce Certificat de Revenus Annuel est \xE9mis \xE0 des fins de d\xE9claration fiscale et l\xE9gale.",
    authorisedBy: name => "Autoris\xE9 par : " + name,
    generated: 'Généré le :',
    page: (n, t) => "Page " + n + " / " + t,
  ...
  es: {
    ...
    para2: (name, co, regNo, memberId, title) => "Se informa por la presente que " + name + " y " + co + " han suscrito un Acuerdo de Marketing y Promoci\xF3n en virtud del cual " + name + " ha sido designado/a como Socio/a Independiente. Conforme a dicho acuerdo, " + name + " es elegible para recibir comisiones en funci\xF3n de los servicios prestados. El n\xFAmero de registro de " + name + " en nuestro sistema es " + memberId + ".",
    ...
    total: 'TOTAL',
    declText: (name, uid, period, co, regNo) => "Este Extracto Mensual de Comisiones certifica que las comisiones enumeradas fueron percibidas por " + name + " (N\xB0 Socio: " + uid + ") durante el per\xEDodo " + period + ". Este documento es emitido por " + co + (regNo ? ', registrada bajo el N° ' + regNo : '') + ", y constituye un registro oficial de ingresos a efectos fiscales.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) => "Los abajo firmantes certifican que " + name + " (N\xB0 Socio: " + uid + ") ha percibido un importe bruto total de " + amount + " en comisiones y bonificaciones de " + co + " durante el ejercicio fiscal " + year + ". El total de retiros procesados durante este per\xEDodo asciende a " + wdAmount + ". Este Certificado de Ingresos Anuales se emite a efectos de declaraci\xF3n fiscal y legal.",
    authorisedBy: name => "Autorizado por: " + name,
    generated: 'Generado:',
    page: (n, t) => "P\xE1gina " + n + " / " + t,
  ...
  de: {
    ...
    para2: (name, co, regNo, memberId, title) => "Hiermit wird best\xE4tigt, dass " + name + " und " + co + " eine Marketing- und Promotionsvereinbarung abgeschlossen haben, in deren Rahmen " + name + " als unabh\xE4ngige/r Partner/in ernannt wurde. Gem\xE4\xDF dieser Vereinbarung ist " + name + " berechtigt, Provisionen auf Basis der erbrachten Dienstleistungen zu erhalten. Die Registrierungsnummer von " + name + " in unserem System lautet " + memberId + ".",
    para3annual: (name, start, amount, year) => "Seit dem Startdatum (" + start + ") bis heute hat " + name + " im Gesch\xE4ftsjahr " + year + " Provisionen in H\xF6he von " + amount + " USD verdient.",
    ...
    summaryTitle: year => "JAHRESVERDIENST-\xDCBERSICHT \u2014 " + year,
    summaryMonthly: period => "MONATSVERDIENST-\xDCBERSICHT \u2014 " + period,
    ...
    total: 'GESAMT',
    declText: (name, uid, period, co, regNo) => "Diese monatliche Provisionsabrechnung best\xE4tigt, dass die oben aufgef\xFChrten Provisionen von " + name + " (Mitgl.-Nr.: " + uid + ") im Zeitraum " + period + " verdient wurden. Dieses Dokument wird von " + co + (regNo ? ', eingetragen unter Nr. ' + regNo : '') + " ausgestellt und stellt einen offiziellen Einkommensnachweis f\xFCr Steuererkl\xE4rungszwecke dar.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) => "Die Unterzeichnenden bescheinigen hiermit, dass " + name + " (Mitgl.-Nr.: " + uid + ") im Gesch\xE4ftsjahr " + year + " einen Bruttobetrag von " + amount + " an Provisionen und Boni von " + co + " erhalten hat. Die im gleichen Zeitraum verarbeiteten Auszahlungen belaufen sich auf " + wdAmount + ". Diese Jahreseinkommensbescheinigung wird f\xFCr steuerliche und rechtliche Zwecke ausgestellt.",
  ...
  pt: {
    ...
    para2: (name, co, regNo, memberId, title) => "Informa-se pela presente que " + name + " e " + co + " celebraram um Acordo de Marketing e Promo\xE7\xE3o pelo qual " + name + " foi nomeado/a Parceiro/a Independente. Ao abrigo do referido acordo, " + name + " tem direito a receber comiss\xF5es pelos servi\xE7os prestados. O n\xFAmero de registo de " + name + " nos nossos sistemas \xE9 " + memberId + ".",
    para3annual: (name, start, amount, year) => "Desde a data de in\xEDcio (" + start + ") at\xE9 \xE0 presente data, " + name + " recebeu " + amount + " USD em comiss\xF5es durante o exerc\xEDcio fiscal " + year + ".",
    ...
    rank: 'Nível Atual',
    totalAnnual: year => "Total de Comiss\xF5es Recebidas (" + year + ")",
    ...
    total: 'TOTAL',
    declText: (name, uid, period, co, regNo) => "Este Extrato Mensal de Comiss\xF5es certifica que as comiss\xF5es listadas foram recebidas por " + name + " (N.\xBA S\xF3cio: " + uid + ") durante o per\xEDodo " + period + ". Este documento \xE9 emitido por " + co + (regNo ? ', registada sob o N.º ' + regNo : '') + " e constitui um registo oficial de rendimentos para efeitos de declara\xE7\xE3o fiscal.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) => "Os abaixo assinados certificam que " + name + " (N.\xBA S\xF3cio: " + uid + ") recebeu um montante bruto total de " + amount + " em comiss\xF5es e b\xF3nus de " + co + " durante o exerc\xEDcio fiscal " + year + ". O total de levantamentos processados durante este per\xEDodo ascende a " + wdAmount + ". Este Certificado de Rendimentos Anuais \xE9 emitido para efeitos de declara\xE7\xE3o fiscal e legal.",
    authorisedBy: name => "Autorizado por: " + name,
    generated: 'Gerado em:',
    page: (n, t) => "P\xE1gina " + n + " / " + t,
  ...
  ar: {
    ...
    para2: (name, co, regNo, memberId, title) => "\u064A\u064F\u0634\u0639\u0631 \u0628\u0645\u0648\u062C\u0628 \u0647\u0630\u0627 \u0623\u0646 " + name + " \u0648" + co + " \u0623\u0628\u0631\u0645\u0627 \u0627\u062A\u0641\u0627\u0642\u064A\u0629 \u062A\u0633\u0648\u064A\u0642 \u0648\u062A\u0631\u0648\u064A\u062C \u062D\u064A\u062B \u0639\u064F\u064A\u0651\u0646(\u062A) " +
      name + " \u0628\u0648\u0635\u0641\u0647(\u0627) \u0634\u0631\u064A\u0643\u0627\u064B(\u0629) \u0645\u0633\u062A\u0642\u0644\u0627\u064B(\u0629). \u0628\u0645\u0648\u062C\u0628 \u0627\u0644\u0627\u062A\u0641\u0627\u0642\u064A\u0629 \u0627\u0644\u0645\u0630\u0643\u0648\u0631\u0629\u060C \u064A\u062D\u0642 \u0644\u0640" +
      name + " \u062A\u0644\u0642\u0651\u064A \u0639\u0645\u0648\u0644\u0627\u062A \u0645\u0642\u0627\u0628\u0644 \u0627\u0644\u062E\u062F\u0645\u0627\u062A \u0627\u0644\u0645\u0642\u062F\u064E\u0651\u0645\u0629. \u0631\u0642\u0645 \u062A\u0633\u062C\u064A\u0644 " + name +
      " \u0644\u062F\u064A\u0646\u0627 \u0647\u0648 " + memberId + ".",
    ...
    total: 'الإجمالي',
    declText: (name, uid, period, co, regNo) => "\u064A\u064F\u0634\u0647\u062F \u0628\u0645\u0648\u062C\u0628 \u0647\u0630\u0627 \u0627\u0644\u0643\u0634\u0641 \u0627\u0644\u0634\u0647\u0631\u064A \u0644\u0644\u0639\u0645\u0648\u0644\u0627\u062A \u0623\u0646 \u0627\u0644\u0639\u0645\u0648\u0644\u0627\u062A \u0627\u0644\u0645\u062F\u0631\u062C\u0629 \u0623\u0639\u0644\u0627\u0647 \u0642\u062F \u062D\u0642\u0642\u0647\u0627 " +
      name + " (\u0631\u0642\u0645 \u0627\u0644\u0639\u0636\u0648: " + uid + ") \u062E\u0644\u0627\u0644 \u0641\u062A\u0631\u0629 " + period + ". \u064A\u0635\u062F\u0631 \u0647\u0630\u0627 \u0627\u0644\u0645\u0633\u062A\u0646\u062F \u0639\u0646 " + co + (regNo ? ' المسجّلة برقم ' +
      regNo : '') + "\u060C \u0648\u064A\u064F\u0639\u062F\u0651 \u0633\u062C\u0644\u0627\u064B \u0631\u0633\u0645\u064A\u0627\u064B \u0644\u0644\u062F\u062E\u0644 \u0644\u0623\u063A\u0631\u0627\u0636 \u0627\u0644\u062A\u0635\u0631\u064A\u062D \u0627\u0644\u0636\u0631\u064A\u0628\u064A.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) => "\u0646\u062D\u0646 \u0627\u0644\u0645\u0648\u0642\u0651\u0639\u064A\u0646 \u0623\u062F\u0646\u0627\u0647 \u0646\u064F\u0634\u0647\u062F \u0628\u0623\u0646 " + name + " (\u0631\u0642\u0645 \u0627\u0644\u0639\u0636\u0648: " +
      uid + ") \u0642\u062F \u062D\u0642\u0651\u0642(\u062A) \u0645\u0628\u0644\u063A\u0627\u064B \u0625\u062C\u0645\u0627\u0644\u064A\u0627\u064B \u0642\u062F\u0631\u0647 " + amount + " \u0645\u0646 \u0627\u0644\u0639\u0645\u0648\u0644\u0627\u062A \u0648\u0627\u0644\u0645\u0643\u0627\u0641\u0622\u062A \u0645\u0646 " +
      co + " \u062E\u0644\u0627\u0644 \u0627\u0644\u0633\u0646\u0629 \u0627\u0644\u0645\u0627\u0644\u064A\u0629 " + year + ". \u0648\u0642\u062F \u0628\u0644\u063A \u0625\u062C\u0645\u0627\u0644\u064A \u0627\u0644\u0645\u0633\u062D\u0648\u0628\u0627\u062A \u0627\u0644\u0645\u0639\u0627\u0644\u062C\u0629 \u062E\u0644\u0627\u0644 \u0647\u0630\u0647 \u0627\u0644\u0641\u062A\u0631\u0629 " +
      wdAmount + ". \u062A\u064F\u0635\u062F\u0631 \u0647\u0630\u0647 \u0627\u0644\u0634\u0647\u0627\u062F\u0629 \u0644\u0623\u063A\u0631\u0627\u0636 \u0627\u0644\u062A\u0635\u0631\u064A\u062D \u0627\u0644\u0636\u0631\u064A\u0628\u064A \u0648\u0627\u0644\u0625\u062C\u0631\u0627\u0621\u0627\u062A \u0627\u0644\u0642\u0627\u0646\u0648\u0646\u064A\u0629.",
...
function _showLangPicker(callback) {
  ...
  Object.entries(_LANGS).forEach(([code, l]) => {
    ...
    btn.onmouseenter = () => {
      btn.style.background = '#2e3650';
      btn.style.borderColor = '#d4a847';
    };
    btn.onmouseleave = () => {
      btn.style.background = '#252b3b';
      btn.style.borderColor = '#3a4260';
    };
...
async function generateMonthlyStatement() {
  ...
  try {
    ...
    const periodLabel = data.period_label || new Date(parseInt(yr), parseInt(mo) - 1, 1).toLocaleDateString('en-GB', {
      month: 'long',
      year: 'numeric'
    });
    ...
    const _drawPage = () => {
      ...
      try {
        const _f = _stmtLogoUri.includes('image/jpeg') || _stmtLogoUri.includes('image/jpg') ? 'JPEG' : 'PNG';
        pdf.addImage(_stmtLogoUri, _f, pw / 2 - 22, 5, 44, 14);
      } catch (e) {
        pdf.setTextColor(255, 255, 255);
        pdf.setFontSize(16);
        pdf.setFont('helvetica', 'bold');
        pdf.text(co.network_name || 'LEADER', 12, 18);
      }
      pdf.setTextColor(200, 200, 215);
      pdf.setFontSize(6.5);
      pdf.setFont('helvetica', 'normal');
      const coL1 = co.legal_name || co.name || '';
      const coL2 = co.registration_number ? 'Co. No. ' + co.registration_number : '';
      const coL3 = [co.address, co.city, co.country].filter(Boolean).join(', ');
      if (coL1) pdf.text(coL1, pw - 8, 10, {
        align: 'right'
      });
      if (coL2) pdf.text(coL2, pw - 8, 15.5, {
        align: 'right'
      });
      if (coL3) pdf.text(coL3, pw - 8, 21, {
        align: 'right'
      });
    };
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.stmtTitle, pw / 2, y, {
      align: 'center'
    });
    y += 6;
    pdf.setFillColor(...NAVY);
    pdf.rect(20, y, pw - 40, 0.4, 'F');
    y += 5;
    pdf.setFillColor(245, 248, 255);
    pdf.roundedRect(14, y, pw - 28, 18, 2, 2, 'F');
    pdf.setDrawColor(...NAVY);
    pdf.setLineWidth(0.2);
    pdf.roundedRect(14, y, pw - 28, 18, 2, 2, 'S');
    pdf.setFillColor(...NAVY);
    pdf.roundedRect(14, y, pw - 28, 6, 2, 2, 'F');
    pdf.rect(14, y + 3, pw - 28, 3, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    ...
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'bold');
    pdf.text(periodLabel, pw / 2, y + 14, {
      align: 'center'
    });
    y += 22;
    pdf.setFillColor(250, 251, 254);
    pdf.roundedRect(14, y, pw - 28, 14, 2, 2, 'F');
    pdf.setDrawColor(220, 225, 240);
    pdf.setLineWidth(0.2);
    pdf.roundedRect(14, y, pw - 28, 14, 2, 2, 'S');
    const infoItems = [[L.fullName, ((m.first_name || '') + ' ' + (m.last_name || '')).trim()], [L.memberId, m.unique_id || 'N/A'], [L.rank, m.current_rank || 'N/A']];
    const iw = (pw - 30) / 3;
    infoItems.forEach(([lbl, val], i) => {
      ...
      pdf.setFontSize(7.5);
      pdf.setFont('helvetica', 'bold');
      pdf.text(_trunc(val, 22), ix, y + 11);
    });
    y += 18;
    pdf.setFillColor(13, 27, 62);
    pdf.roundedRect(14, y, pw - 28, 16, 2, 2, 'F');
    pdf.setTextColor(200, 200, 215);
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'normal');
    pdf.text(L.totalComm, pw / 2, y + 5.5, {
      align: 'center'
    });
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(16);
    pdf.setFont('helvetica', 'bold');
    pdf.text(_fmtMoney(totGross) + ' USD', pw / 2, y + 13, {
      align: 'center'
    });
    y += 20;
    ...
    for (const c of comms) {
      ...
      pdf.setFontSize(5.5);
      pdf.setFont('helvetica', 'normal');
      pdf.text(_fmtDate(c.created_at), 16, y + 3.5);
      pdf.setTextColor(...NAVY);
      pdf.setFontSize(5.5);
      pdf.text(_trunc(_COMM_LABELS[c.type] || c.type, 22), 38, y + 3.5);
      pdf.setTextColor(...GRAY);
      pdf.text(_trunc(c.description || '—', 36), 82, y + 3.5);
      pdf.setTextColor(...sc);
      pdf.setFont('helvetica', 'bold');
      pdf.text((c.status || '—').toUpperCase(), 142, y + 3.5);
      pdf.setTextColor(...NAVY);
      pdf.setFont('helvetica', 'bold');
      pdf.text(_fmtMoney(c.amount), 192, y + 3.5, {
        align: 'right'
      });
      y += rowH;
      rowIdx++;
    ...
    if (comms.length === 0) {
      ...
      pdf.text('No commissions recorded for this period.', pw / 2, y + 6, {
        align: 'center'
      });
      y += 14;
    }
    ...
    pdf.setFillColor(...NAVY);
    pdf.roundedRect(14, y, (pw - 30) / 2, 7, 2, 2, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.summaryByType, 18, y + 4.7);
    y += 9;
    const summary = data.summary || {};
    for (const [type, amount] of Object.entries(summary)) {
      pdf.setFillColor(248, 250, 255);
      pdf.rect(14, y - 1, (pw - 30) / 2, 6, 'F');
      pdf.setDrawColor(220, 228, 245);
      pdf.setLineWidth(0.1);
      pdf.rect(14, y - 1, (pw - 30) / 2, 6, 'S');
      pdf.setTextColor(...NAVY);
      pdf.setFontSize(6);
      pdf.setFont('helvetica', 'normal');
      pdf.text(_COMM_LABELS[type] || type, 17, y + 3);
      pdf.setTextColor(...BORD);
      pdf.setFont('helvetica', 'bold');
      pdf.text(_fmtMoney(amount), (pw - 30) / 2 + 11, y + 3, {
        align: 'right'
      });
      y += 6.5;
    }
    y += 2;
    pdf.setFillColor(...NAVY);
    pdf.roundedRect(14, y, (pw - 30) / 2, 9, 2, 2, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.total, 18, y + 6);
    pdf.text(_fmtMoney(totGross), (pw - 30) / 2 + 11, y + 6, {
      align: 'right'
    });
    y += 16;
    if (y + 32 > ph - 22) {
    ...
    pdf.roundedRect(14, y, pw - 28, 30, 3, 3, 'S');
    pdf.setTextColor(...NAVY);
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.officialDecl, pw / 2, y + 6, {
      align: 'center'
    });
    pdf.setTextColor(...GRAY);
    pdf.setFontSize(6.2);
...
async function generateAnnualCertificate() {
  ...
  try {
    ...
    const LGRAY = [160, 160, 170];
    const GREEN = [34, 139, 34];
    const DBLUE = [230, 238, 255];
    const totalAnnual = data.totals?.gross || 0;
    const totalWd = data.totals?.withdrawn || 0;
    _letterDrawHeader(pdf, pw, co, new Date().toLocaleDateString('en-GB', {
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    }));
    let y = 40;
    pdf.setFillColor(245, 248, 255);
    pdf.roundedRect(14, y, pw - 28, 22, 3, 3, 'F');
    pdf.setDrawColor(...NAVY);
    pdf.setLineWidth(0.4);
    pdf.roundedRect(14, y, pw - 28, 22, 3, 3, 'S');
    pdf.setFillColor(...BORD);
    pdf.roundedRect(14, y, pw - 28, 2.5, 3, 3, 'F');
    pdf.rect(14, y + 1.5, pw - 28, 1, 'F');
    pdf.setTextColor(...NAVY);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.annualCertTitle, pw / 2, y + 12, {
      align: 'center'
    });
    pdf.setTextColor(...BORD);
    pdf.setFontSize(9);
    pdf.setFont('helvetica', 'normal');
    pdf.text(L.fiscalYear(data.year), pw / 2, y + 19, {
      align: 'center'
    });
    y += 28;
    pdf.setFillColor(245, 248, 255);
    ...
    const infoData = [[L.fullName, (m.first_name || '') + ' ' + (m.last_name || '')], [L.memberId, m.unique_id || 'N/A'], ['Email', m.email || 'N/A'], ['Country', m.country || 'N/A'], [L.rank, m.current_rank || 'N/A'], [L.status, m.member_status || 'N/A']];
    const colW = (pw - 32) / 3;
    infoData.forEach(([lbl, val], i) => {
      const col = i % 3,
        row = Math.floor(i / 3);
      const ix = 16 + col * (colW + 2),
        iy = y + 11 + row * 9;
      pdf.setTextColor(...GRAY);
      pdf.setFontSize(5.5);
      pdf.setFont('helvetica', 'normal');
      ...
      pdf.setFontSize(7);
      pdf.setFont('helvetica', 'bold');
      pdf.text(_trunc(val, 26), ix, iy + 5);
    });
    y += 36;
    const kpis = [{
      l: L.totalAnnual(data.year),
      v: _fmtMoney(totalAnnual),
      c: NAVY
    }, {
      l: 'Total Withdrawn',
      v: _fmtMoney(totalWd),
      ...
      l: 'Balance',
      v: _fmtMoney(totalAnnual - totalWd),
      c: [34, 139, 34]
    }, {
      l: 'Fiscal Year',
      v: String(data.year),
    ...
    kpis.forEach((k, i) => {
      const kx = 14 + i * (kw + 2);
      pdf.setFillColor(245, 248, 255);
      pdf.roundedRect(kx, y, kw, 18, 2, 2, 'F');
      pdf.setDrawColor(...k.c);
      pdf.setLineWidth(0.4);
      pdf.roundedRect(kx, y, kw, 18, 2, 2, 'S');
      pdf.setFillColor(...k.c);
      pdf.roundedRect(kx, y, kw, 2.5, 1, 1, 'F');
      pdf.setTextColor(...GRAY);
      pdf.setFontSize(5.5);
      pdf.setFont('helvetica', 'normal');
      const ll = pdf.splitTextToSize(k.l, kw - 4);
      pdf.text(ll, kx + kw / 2, y + 8, {
        align: 'center'
      });
      pdf.setTextColor(...k.c);
      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'bold');
      pdf.text(k.v, kx + kw / 2, y + 15, {
        align: 'center'
      });
    });
    y += 24;
    ...
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.earningsByType, 18, y + 4.7);
    y += 9;
    let yTypeStart = y;
    for (const [type, amount] of Object.entries(data.commissions_by_type || {})) {
      pdf.setFillColor(248, 250, 255);
      pdf.roundedRect(14, y - 1, (pw - 30) / 2, 6.5, 1, 1, 'F');
      pdf.setDrawColor(220, 228, 245);
      pdf.setLineWidth(0.1);
      pdf.roundedRect(14, y - 1, (pw - 30) / 2, 6.5, 1, 1, 'S');
      pdf.setTextColor(...NAVY);
      pdf.setFontSize(6);
      pdf.setFont('helvetica', 'normal');
      pdf.text(_COMM_LABELS[type] || type, 17, y + 3.5);
      pdf.setTextColor(...BORD);
      pdf.setFont('helvetica', 'bold');
      pdf.text(_fmtMoney(amount), (pw - 30) / 2 + 11, y + 3.5, {
        align: 'right'
      });
      y += 7;
    }
    ...
    const xR = (pw - 30) / 2 + 16;
    pdf.setFillColor(...BORD);
    pdf.roundedRect(xR, yR, (pw - 30) / 2, 7, 2, 2, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.monthlyBreakdown, xR + 4, yR + 4.7);
    yR += 9;
    ...
    for (let mo = 1; mo <= 12; mo++) {
      ...
      pdf.setFontSize(6);
      pdf.setFont('helvetica', 'normal');
      pdf.text(months[mo - 1] + ' ' + data.year, xR + 4, yR + 3.5);
      ...
      pdf.text(val > 0 ? _fmtMoney(val) : '—', xR + (pw - 30) / 2 - 4, yR + 3.5, {
        align: 'right'
      });
      yR += 6.5;
    }
    ...
    if (y + 40 > ph - 22) {
      _letterDrawFooter(pdf, pw, ph, co);
      pdf.addPage();
      _letterDrawHeader(pdf, pw, co, '');
      y = 40;
    }
    pdf.setFillColor(245, 248, 255);
    pdf.roundedRect(14, y, pw - 28, 38, 3, 3, 'F');
    pdf.setDrawColor(...NAVY);
    pdf.setLineWidth(0.4);
    pdf.roundedRect(14, y, pw - 28, 38, 3, 3, 'S');
    pdf.setDrawColor(...BORD);
    pdf.setLineWidth(0.2);
    pdf.roundedRect(16, y + 2, pw - 32, 34, 2, 2, 'S');
    pdf.setTextColor(...NAVY);
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.officialCert, pw / 2, y + 10, {
      align: 'center'
    });
    pdf.setTextColor(...GRAY);
    pdf.setFontSize(6.5);
    ...
    if (co.director) {
      ...
      pdf.setFontSize(6);
      pdf.setFont('helvetica', 'bold');
      pdf.text(L.authorisedBy(co.director), pw / 2, y + 35, {
        align: 'center'
      });
    }
    const totalPages = pdf.internal.getNumberOfPages();
...
async function generatePurchaseReceipt() {
  ...
  try {
    ...
    if (!orderId) {
      showToast('Sélectionnez une commande', 'error');
      return;
    }
    ...
    try {
      ...
      pdf.setFontSize(16);
      pdf.setFont('helvetica', 'bold');
      pdf.text(co.trade_name || co.network_name || 'LEADER', pw / 2, 18, {
        align: 'center'
      });
    }
    pdf.setTextColor(200, 200, 215);
    ...
    if (rcptLegal) pdf.text(rcptLegal, pw - 8, 11, {
      align: 'right'
    });
    if (rcptReg) pdf.text(rcptReg, pw - 8, 16, {
      align: 'right'
    });
    if (rcptAddr) pdf.text(rcptAddr, pw - 8, 21, {
      align: 'right'
    });
    let y = 36;
    pdf.setTextColor(...NAVY);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text('REÇU OFFICIEL D\'ACHAT', pw / 2, y + 7, {
      align: 'center'
    });
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(...GRAY);
    pdf.text('Official Purchase Receipt', pw / 2, y + 13, {
      align: 'center'
    });
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'normal');
    ...
    pdf.text('Date : ' + _fmtDate(o.created_at || new Date()), pw - 14, y + 7, {
      align: 'right'
    });
    pdf.setDrawColor(...BORD);
    ...
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'normal');
    pdf.text('MONTANT TOTAL / AMOUNT PAID', pw / 2, y + 8, {
      align: 'center'
    });
    pdf.setTextColor(...NAVY);
    pdf.setFontSize(24);
    pdf.setFont('helvetica', 'bold');
    pdf.text(_fmtMoney(o.amount_usd), pw / 2, y + 21, {
      align: 'center'
    });
    y += 34;
    ...
    const orderRows = [['Package / Produit', o.package_name || 'N/A'], ['Référence', o.id ? o.id.substring(0, 16).toUpperCase() : 'N/A'], ['Date commande', _fmtDate(o.created_at)], ['Méthode paiement', o.payment_method || 'N/A'], ['Valeur BV', (o.bv_value || 0) + ' BV']];
    let yi1 = y + 11;
    ...
    const buyerRows = [['Nom complet', (m.first_name || '') + ' ' + (m.last_name || '')], ['ID Membre', m.unique_id || 'N/A'], ['Email', m.email || 'N/A'], ['Pays', m.country || 'N/A'], ['Statut', co.member_title || 'Partenaire Indépendant(e)']];
    let yi2 = y + 11;
    ...
    const sellerCols = [[['Nom commercial', co.trade_name || co.network_name || 'LEADER'], ['Raison sociale', co.legal_name || 'WILL BE COACHING LLP'], ['N° entreprise', co.registration_number || 'OC445365']], [['Adresse', co.address || '72 Halliwick Road'], ['Ville / Code', [co.city, co.postal_code].filter(Boolean).join(' ') || 'London, N10 1AB'], ['Pays', co.country || 'England']], [['Email', co.email || 'N/A'], ['Site web', co.website || 'N/A'], ['Directeur', co.director || 'N/A']]];
    const sColW = (pw - 32) / 3;
    ...
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'bold');
    pdf.text('NOTE LÉGALE / LEGAL NOTE', pw / 2, y + 7, {
      align: 'center'
    });
    pdf.setTextColor(...GRAY);
    pdf.setFontSize(6);
    ...
    const legalNote = "Ce re\xE7u officiel certifie que " + memberFull + " (ID: " + (m.unique_id || 'N/A') + "), " + memberTitle + ", a effectu\xE9 l'achat du \"" + (o.package_name || 'Package') + "\" pour un montant de " + _fmtMoney(o.amount_usd) + " le " + _fmtDate(o.created_at) + ". Ce document est \xE9mis par " + tradeName + " (" + legalName + ", Co. No. " + regNo + ") et peut \xEAtre utilis\xE9 comme justificatif d'investissement \xE0 des fins fiscales et l\xE9gales. / This receipt certifies that " + memberFull + " purchased \"" + (o.package_name || 'Package') + "\" for " + _fmtMoney(o.amount_usd) + " on " + _fmtDate(o.created_at) + ", issued by " + tradeName + " (" + legalName + ", Co. No. " + regNo + ").";
    ...
    pdf.setFontSize(5);
    pdf.setFont('helvetica', 'normal');
    pdf.text('Document officiel', pw - 20, ph - 8, {
      align: 'center'
    });
    pdf.text(data.doc_ref || '', pw - 20, ph - 5, {
      align: 'center'
    });
...
function _letterDrawHeader(pdf, pw, co, dateStr) {
  ...
  if (coLine1) pdf.text(coLine1, pw - 8, 10, {
    align: 'right'
  });
  if (coLine2) pdf.text(coLine2, pw - 8, 15.5, {
    align: 'right'
  });
  if (coLine3) pdf.text(coLine3, pw - 8, 21, {
    align: 'right'
  });
  pdf.setTextColor(...GRAY);
  pdf.setFontSize(9);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Dated: ' + dateStr, pw - 8, 37, {
    align: 'right'
  });
}
function _letterDrawFooter(pdf, pw, ph, co) {
  ...
  pdf.text(line1, pw / 2, ph - 12, {
    align: 'center'
  });
  if (line2) pdf.text(line2, pw / 2, ph - 8.5, {
    align: 'center'
  });
  if (line3) pdf.text(line3, pw / 2, ph - 5, {
    align: 'center'
  });
}
async function generateOfficialLetterAnnual() {
  ...
  try {
    ...
    const startDateStr = m.created_at ? new Date(m.created_at).toLocaleDateString('en-GB', {
      month: 'long',
      year: 'numeric'
    }) : 'registration date';
    ...
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'normal');
    pdf.text(data.doc_ref || 'LTR-' + year + '-' + (m.unique_id || ''), pw - 14, y - 2, {
      align: 'right'
    });
    y += 2;
    ...
    const paragraphs = [L.para1(fullName, today), L.para2(fullName, companyName, regNo, memberRef, co.member_title), L.para3annual(fullName, startDateStr, _fmtMoney(totalEarned), year), L.para4()];
    pdf.setTextColor(...BLACK);
    pdf.setFontSize(9.5);
    pdf.setFont('helvetica', 'normal');
    ...
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.summaryTitle(year), pw / 2, y + 5.5, {
      align: 'center'
    });
    y += 10;
    const recapRows = [[L.fullName, fullName], [L.memberId, memberRef], [L.status, m.member_status || 'N/A'], [L.rank, m.current_rank && m.current_rank !== 'none' ? m.current_rank : 'N/A'], [L.totalAnnual(year), _fmtMoney(totalEarned) + ' USD']];
    const rowH2 = 5.8;
...
async function generateOfficialLetterMonthly() {
  ...
  try {
    ...
    pdf.setFontSize(13);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.letterTitleMonthly, pw / 2, y, {
      align: 'center'
    });
    y += 8;
    pdf.setFillColor(...NAVY);
    ...
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'normal');
    pdf.text(data.doc_ref ? data.doc_ref.replace('STMT-', 'LTR-') : 'LTR-' + period + '-' + (m.unique_id || ''), pw - 14, y - 2, {
      align: 'right'
    });
    y += 2;
    const fullName = ((m.first_name || '') + ' ' + (m.last_name || '')).toUpperCase().trim();
    ...
    const regNo = co.registration_number || '';
    const memberRef = m.unique_id || 'N/A';
    const paragraphs = [L.para1(fullName, today), L.para2(fullName, companyName, regNo, memberRef, co.member_title), L.para3monthly(fullName, periodLabel, _fmtMoney(totalMonth)), L.para4()];
    pdf.setTextColor(...BLACK);
    pdf.setFontSize(9.5);
    pdf.setFont('helvetica', 'normal');
    for (const para of paragraphs) {
    ...
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(L.summaryMonthly(periodLabel.toUpperCase()), pw / 2, y + 5.5, {
      align: 'center'
    });
    y += 10;
    const summary = data.summary || {};
    const summaryEntries = Object.entries(summary);
    const recapRows = [[L.fullName, fullName], [L.memberId, memberRef], [L.periodCovered, periodLabel], ...summaryEntries.map(([type, amount]) => [_COMM_LABELS[type] || type, _fmtMoney(amount) + ' USD']), [L.totalComm, _fmtMoney(totalMonth) + ' USD']];
    const rowH2 = 5.6;
    recapRows.forEach(([lbl, val], i) => {
      const rowY = y + i * rowH2;
...
async function handleProfilePhotoUpload(input) {
  ...
  if (!file.type.startsWith('image/')) {
    showToast('Format non accepté — utilisez une image (JPG, PNG, WebP…)', 'error');
    return;
  }
  ...
  if (statusEl) {
    statusEl.textContent = 'Optimisation en cours...';
  ...
  try {
    ...
    const approxKB = Math.round(finalDataUrl.length * 0.75 / 1024);
    if (statusEl) {
      statusEl.textContent = 'Photo optimisée (' + approxKB + ' Ko)';
    }
...
function _buildMandateBlock(cardData, m) {
  ...
  if (!mandate.enabled) {
    return '<div class="bg-dark-700/50 rounded-xl p-4 border border-dark-500">' + '<label class="flex items-start gap-3 cursor-pointer">' + '<input type="checkbox" id="prof-vdi" class="mt-0.5 accent-gold-500 w-4 h-4 flex-shrink-0" ' + (m && m.vdi_consent ? 'checked' : '') +
      ' onchange="if(this.checked){api(\'PUT\',\'/members/profile\',{vdi_consent:1}).catch(()=>{})}">' + '<span class="text-sm text-gray-300"><strong class="text-white">Autorisation VDI</strong> — Je déclare exercer mon activité de Vendeur à Domicile Indépendant(e) en conformité avec la réglementation en vigueur et les exigences légales du marketing de réseau.' +
      (m && m.vdi_consent_at ? '<span class="block text-xs text-gray-500 mt-1">Accepté le ' + new Date(m.vdi_consent_at).toLocaleDateString('fr-FR') + '</span>' : '') + '</span></label></div>';
  }
  ...
  var bodyHtml = bodyRaw.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\*\*(.+?)\*\*/g, '<strong class="text-white">$1</strong>').replace(/\[DATE\]/g, signedAt ? new Date(signedAt).toLocaleDateString('fr-FR') : new Date().toLocaleDateString('fr-FR')).replace(/\n/g, '<br>');
  return '<div class="bg-dark-700/50 rounded-xl p-4 border ' + (signed ? 'border-indigo-500/40' : 'border-dark-500') + '">' + '<div class="flex items-start gap-3">' + '<input type="checkbox" id="prof-mandate" class="mt-0.5 accent-indigo-500 w-4 h-4 flex-shrink-0 cursor-pointer" ' + (signed ? 'checked disabled' : '') + ' onchange="if(this.checked)signMandate(this)">' + '<div class="flex-1 min-w-0">' + '<span class="text-sm ' + (signed ? 'text-indigo-300' : 'text-gray-300') + '"><strong class="' + (signed ? 'text-indigo-200' : 'text-white') + '">' + title + '</strong></span>' + (signed ? '<span class="block text-xs text-indigo-400 mt-1"><i class="fas fa-check-circle mr-1"></i>Signé électroniquement le ' + new Date(signedAt).toLocaleDateString('fr-FR', {
    day: '2-digit',
    month: 'long',
    year: 'numeric'
  }) + '</span>' : '<span class="block text-xs text-gray-500 mt-1">Cochez pour signer électroniquement</span>') + '<button type="button" onclick="toggleMandateText()" class="mt-2 text-xs text-indigo-400 hover:text-indigo-300 underline underline-offset-2 flex items-center gap-1">' + '<i id="mandate-chevron" class="fas fa-chevron-right text-[10px] transition-transform"></i>' + '<span id="mandate-toggle-label">Voir le texte complet</span></button>' + '<div id="mandate-full-text" class="hidden mt-3 p-3 bg-dark-800 border border-dark-600 rounded-lg text-xs text-gray-300 leading-relaxed max-h-64 overflow-y-auto">' + bodyHtml + '</div>' + '</div></div></div>';
}
function _buildCardButtonsBlock(cardData) {
  ...
  var virtualBtn = '<div class="relative" id="virtual-card-menu-wrap">' + '<button onclick="_showVirtualCardMenu()" class="flex items-center gap-2 bg-rouge-500 text-dark-900 font-bold px-5 py-2.5 rounded-xl hover:bg-rouge-500 transition text-sm">' + '<i class="fas fa-id-card"></i> Carte virtuelle <i class="fas fa-chevron-down text-xs ml-1"></i></button>' +
    '<div id="virtual-card-dropdown" class="hidden absolute left-0 top-full mt-1 z-50 bg-dark-800 border border-dark-600 rounded-xl shadow-xl min-w-[160px] overflow-hidden">' + '<button onclick="downloadCardPNG();_hideVirtualCardMenu()" class="flex items-center gap-2 w-full px-4 py-3 text-sm text-gray-300 hover:bg-dark-700 hover:text-white transition"><i class="fas fa-image text-rouge-400"></i> Télécharger PNG</button>' +
    '<button onclick="downloadCardPDF();_hideVirtualCardMenu()" class="flex items-center gap-2 w-full px-4 py-3 text-sm text-gray-300 hover:bg-dark-700 hover:text-white transition"><i class="fas fa-file-pdf text-red-400"></i> Télécharger PDF</button>' + '</div></div>';
  var physicalBtn = '';
  if (physEnabled) {
    if (physOrder && ['pending', 'paid', 'processing', 'shipped'].includes(physOrder.status)) {
      var sl = {
        pending: 'En attente de paiement',
        paid: 'Payée — en traitement',
        processing: 'En fabrication',
        shipped: 'Expédiée'
      };
      physicalBtn = '<div class="flex items-center gap-2 border border-amber-500/40 text-amber-400 px-5 py-2.5 rounded-xl text-sm">' + '<i class="fas fa-credit-card"></i><div><span class="font-bold block text-xs">Carte physique</span>' + '<span class="text-xs text-amber-300/70">' + (sl[physOrder.status] || physOrder.status) + '</span></div></div>';
    } else {
      physicalBtn = '<button onclick="wizardPhysicalCard()" class="flex items-center gap-2 border border-amber-500/50 text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 font-bold px-5 py-2.5 rounded-xl transition text-sm">' + '<i class="fas fa-credit-card"></i> Carte physique</button>';
    }
  }
  var revokeBtn = '<button onclick="revokeCard()" id="btn-revoke" class="flex items-center gap-2 border border-red-500/40 text-red-400 px-5 py-2.5 rounded-xl hover:bg-red-900/20 transition text-sm ml-auto">' + '<i class="fas fa-ban"></i> Révoquer</button>';
...
function toggleMandateText() {
  ...
  if (!nowHidden) {
    ...
    label.textContent = 'Masquer le texte';
  } else {
    chev.style.transform = '';
    label.textContent = 'Voir le texte complet';
  }
}
...
async function wizardPhysicalCard() {
  ...
  if (overlay) overlay.style.display = 'flex';
  try {
    var results = await Promise.all([api('GET', '/members/card').catch(function () {
      return null;
    }), !_gw || !Object.keys(_gw).length ? api('GET', '/members/packages').catch(function () {
      return null;
...
function _wizardPhysCardStep1() {
  ...
  _wizardShow('<div class="px-6 pt-5 pb-3 border-b border-dark-600">' + '<div class="flex items-center justify-between mb-2">' + '<span class="text-xs font-semibold text-amber-400 uppercase tracking-wider"><i class="fas fa-credit-card mr-1.5"></i>Carte physique — Livraison</span>' +
    '<button onclick="wizardClose()" class="text-gray-400 hover:text-white w-7 h-7 flex items-center justify-center rounded-lg hover:bg-dark-700 text-lg">×</button>' + '</div>' + '<div class="w-full bg-dark-700 rounded-full h-1.5"><div class="bg-amber-500 h-1.5 rounded-full" style="width:33%"></div></div>' +
    '</div>' + '<div class="p-6 space-y-4">' + '<div><h3 class="text-lg font-bold text-white">Informations de livraison</h3>' + '<p class="text-sm text-gray-400 mt-1">Votre carte sera expédiée à cette adresse sous 7–14 jours ouvrés.</p></div>' + '<div class="grid grid-cols-1 gap-3">' +
    '<div><label class="text-xs text-gray-400 block mb-1">Nom complet <span class="text-red-400">*</span></label>' + '<input id="phys-full-name" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="Prénom Nom"></div>' +
    '<div><label class="text-xs text-gray-400 block mb-1">Adresse <span class="text-red-400">*</span></label>' + '<input id="phys-address" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="Numéro et nom de rue"></div>' +
    '<div class="grid grid-cols-2 gap-3">' + '<div><label class="text-xs text-gray-400 block mb-1">Ville <span class="text-red-400">*</span></label>' + '<input id="phys-city" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="Paris"></div>' +
    '<div><label class="text-xs text-gray-400 block mb-1">Code postal <span class="text-red-400">*</span></label>' + '<input id="phys-postal" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="75001"></div>' +
    '</div>' + '<div><label class="text-xs text-gray-400 block mb-1">Pays <span class="text-red-400">*</span></label>' + '<input id="phys-country" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="France"></div>' +
    '<div><label class="text-xs text-gray-400 block mb-1">Téléphone <span class="text-red-400">*</span></label>' + '<input id="phys-phone" class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-400" placeholder="+33612345678"></div>' +
    '</div>' + '<div id="phys-addr-error" class="hidden bg-red-900/20 border border-red-500/30 rounded-xl p-3 text-red-400 text-sm"></div>' + '<div class="flex items-center justify-between bg-dark-700/50 rounded-xl px-4 py-3 border border-dark-600">' + '<div class="flex items-center gap-2 text-gray-400 text-sm"><i class="fas fa-credit-card text-amber-400"></i> Carte physique LEADER</div>' +
    '<span class="text-amber-400 font-bold">' + curr + ' ' + price.toFixed(2) + '</span>' + '</div>' + '<div class="flex gap-3">' + '<button onclick="wizardClose()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl hover:bg-dark-600 transition text-sm">Annuler</button>' +
    '<button onclick="_wizardPhysCardValidate()" class="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-dark-900 font-bold rounded-xl transition text-sm">Continuer <i class="fas fa-arrow-right ml-2"></i></button>' + '</div>' + '</div>');
}
function _wizardPhysCardValidate() {
  var fn = (document.getElementById('phys-full-name') || {
    value: ''
  }).value.trim();
  var ad = (document.getElementById('phys-address') || {
    value: ''
  }).value.trim();
  var ci = (document.getElementById('phys-city') || {
    value: ''
  }).value.trim();
  var po = (document.getElementById('phys-postal') || {
    value: ''
  }).value.trim();
  var co = (document.getElementById('phys-country') || {
    value: ''
  }).value.trim();
  var ph = (document.getElementById('phys-phone') || {
    value: ''
  }).value.trim();
...
async function renderMemberCard(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const [cardData, completeness] = await Promise.all([api('GET', '/members/card'), api('GET', '/members/profile/completeness')]);
    const {
      card,
      config
    } = cardData;
    ...
    const langOptions = LANGS.map(l => '<option value="' + l.code + '" ' + ((m && m.preferred_lang || 'fr') === l.code ? 'selected' : '') + '>' + l.label + '</option>').join('');
    ...
    const missingLabels = {
      ...
      vdi_consent: 'Mandat Finestrategia (non signé)'
    };
    const missingHtml = missingItems.length > 0 ? '<div class="bg-orange-900/20 border border-orange-500/30 rounded-xl p-4 mb-4"><p class="text-orange-300 font-medium text-sm mb-2"><i class="fas fa-exclamation-triangle mr-1"></i>Profil incomplet :</p><ul class="list-disc list-inside text-orange-400 text-xs space-y-1">' + missingItems.map(k => '<li>' + (missingLabels[k] || k) + '</li>').join('') + '</ul></div>' : '';
    const photoSrc = m && m.profile_photo ? m.profile_photo : '';
    const enrichedSection = '<div class="bg-dark-800 rounded-2xl border border-dark-600 p-6 space-y-4">' + '<h3 class="font-semibold text-white flex items-center gap-2 pb-2 border-b border-dark-600"><i class="fas fa-user-circle text-rouge-400 text-sm"></i> Profil enrichi<span class="text-xs text-gray-500 font-normal ml-2">Requis pour la carte</span></h3>' +
      missingHtml + '<div><label class="form-label">Photo de profil <span class="text-red-400">*</span></label>' + '<div class="flex items-center gap-4">' + '<div class="relative w-20 h-20 rounded-full overflow-hidden bg-dark-700 border-2 border-dark-500 flex-shrink-0">' +
      ...
      '</div>' + '<div><input type="file" id="prof-photo-input" accept="image/*" class="hidden" onchange="handleProfilePhotoUpload(this)">' + '<button onclick="document.getElementById(\'prof-photo-input\').click()" class="text-sm bg-dark-700 border border-dark-500 text-gray-300 px-4 py-2 rounded-xl hover:bg-dark-600 transition"><i class="fas fa-upload mr-1"></i>Choisir une photo</button>' +
      '<p class="text-xs text-gray-500 mt-1">Toute image · max 50 Mo (compressée automatiquement)</p><p id="prof-photo-status" class="text-xs text-rouge-400 mt-1 hidden"></p></div></div></div>' + '<div><label class="form-label">Ville de résidence <span class="text-red-400">*</span></label>' +
      '<input id="p-city" class="form-input" value="' + (m && m.city ? m.city : '') + '" placeholder="Paris, London, Dakar\u2026"></div>' + '<div><label class="form-label">Langue préférée <span class="text-red-400">*</span></label>' + '<select id="prof-lang" class="form-input">' +
      langOptions + '</select></div>' + '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">' + '<div><label class="form-label"><i class=\"fas fa-brands fa-whatsapp text-green-400 mr-1\"></i>Numéro WhatsApp</label>' + '<input id="prof-whatsapp" class="form-input" value="' +
      (m && m.whatsapp_number ? m.whatsapp_number : '') + '" placeholder="+33612345678 (avec indicatif)"></div>' + '<div><label class="form-label"><i class=\"fas fa-comment text-blue-400 mr-1\"></i>ID Messenger Facebook</label>' + '<input id="prof-messenger" class="form-input" value="' +
      (m && m.messenger_id ? m.messenger_id : '') + '" placeholder="votre.nom.facebook"></div>' + '</div>' + _buildMandateBlock(cardData, m) + '<button onclick="saveProfileEnriched()" class="bg-rouge-500 text-dark-900 font-bold px-6 py-2.5 rounded-xl hover:bg-rouge-500 transition flex items-center gap-2"><i class="fas fa-save"></i> Sauvegarder le profil</button>' +
      '</div>';
    let cardSection = '';
    if (!licenseActive) {
      ...
      cardSection = '<div class="bg-dark-800 rounded-2xl border border-rouge-500/25 p-6 space-y-4">' + '<h3 class="font-semibold text-white flex items-center gap-2 pb-2 border-b border-dark-600"><i class="fas fa-id-badge text-rouge-400 text-sm"></i> Ma Carte de Membre' +
        '<span class="ml-auto text-xs px-2 py-1 rounded-full font-medium ' + (card.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400') + '">' + (card.status === 'active' ? 'Active' : 'Révoquée') + '</span></h3>' + '<div class="flex justify-center"><canvas id="card-canvas" width="600" height="360" style="max-width:100%;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.4);"></canvas></div>' +
        '<div class="bg-dark-700/50 rounded-xl p-3"><p class="text-xs text-gray-400 mb-1">Lien de vérification publique</p>' + '<div class="flex gap-2 items-center"><span class="text-xs text-rouge-400 font-mono truncate flex-1">' + verifyUrl + '</span>' + '<button onclick="navigator.clipboard.writeText(\'' +
        verifyUrl + '\').then(()=>showToast(\'Lien copié\',\'success\'))" class="text-xs bg-dark-600 px-3 py-1.5 rounded-lg hover:bg-dark-500 text-gray-300 transition flex-shrink-0"><i class="fas fa-copy"></i></button></div></div>' + _buildCardButtonsBlock(cardData) + '<div class="pt-2 border-t border-dark-600"><p class="text-xs text-gray-400 mb-2">Changer de design</p><div class="flex gap-2">' +
      ...
      cardSection = '<div class="bg-dark-800 rounded-2xl border border-dark-600 p-6 space-y-4">' + '<h3 class="font-semibold text-white flex items-center gap-2 pb-2 border-b border-dark-600"><i class="fas fa-id-badge text-rouge-400 text-sm"></i> Demander ma Carte de Membre</h3>' +
        (!profileComplete ? '<div class="bg-orange-900/20 border border-orange-500/30 rounded-xl p-4"><p class="text-orange-300 text-sm"><i class="fas fa-info-circle mr-1"></i>Complétez votre profil ci-dessus avant de demander votre carte.</p></div>' : '') + '<p class="text-sm text-gray-400">Votre carte digitale certifie votre statut d\'<strong class="text-white">Ambassadeur</strong> LEADER. Elle contient un QR code permettant de vérifier son authenticité en temps réel.</p>' +
        '<div><p class="form-label mb-2">Choisissez votre design</p><div class="grid grid-cols-2 gap-3">' + (config.design_a_enabled ? '<label class="cursor-pointer"><input type="radio" name="card-design" value="A" checked class="hidden peer"><div class="border-2 border-dark-500 peer-checked:border-rouge-500 rounded-xl p-4 text-center transition"><div class="w-full h-12 rounded-lg mb-2" style="background:linear-gradient(135deg,' +
        config.design_a_color1 + ',' + config.design_a_color2 + ',' + config.design_a_color3 + ')"></div><p class="text-sm font-medium text-white">Design A</p><p class="text-xs text-gray-400">Prestige Orange</p></div></label>' : '') + (config.design_b_enabled ? '<label class="cursor-pointer"><input type="radio" name="card-design" value="B" ' +
        (!config.design_a_enabled ? 'checked' : '') + ' class="hidden peer"><div class="border-2 border-dark-500 peer-checked:border-blue-400 rounded-xl p-4 text-center transition"><div class="w-full h-12 rounded-lg mb-2" style="background:linear-gradient(135deg,#0D1B2A,#1B2A4A,#8B6914)"></div><p class="text-sm font-medium text-white">Design B</p><p class="text-xs text-gray-400">Prestige Navy</p></div></label>' : '') +
        '</div></div>' + '<button onclick="orderCard(document.querySelector(\'[name=card-design]:checked\')?.value||\'A\')" id="btn-order-card" ' + (!profileComplete ? 'disabled title="Complétez votre profil"' : '') + ' class="w-full bg-rouge-500 text-dark-900 font-bold py-3 rounded-xl hover:bg-rouge-500 transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><i class="fas fa-id-badge"></i>' +
        (config.price > 0 ? 'Obtenir ma carte — ' + config.price + ' ' + config.currency : 'Obtenir ma carte gratuitement') + '</button><p class="text-xs text-gray-500 text-center">' + (config.legal_mention || '') + '</p></div>';
    }
    e.innerHTML = '<div class="max-w-2xl space-y-6"><div class="flex items-center justify-between"><h2 class="text-xl font-bold text-white flex items-center gap-2"><i class="fas fa-id-badge text-rouge-400"></i> Carte de Membre</h2></div>' + enrichedSection + cardSection + '</div>';
...
async function orderCard(design) {
  ...
  try {
    await api('POST', '/members/card/order', {
      design
    });
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-id-badge"></i> Obtenir ma carte';
    }
  }
}
...
async function _drawCardDesignA(ctx, W, H, member, config, qrImg, logoImg) {
  ...
  ov.addColorStop(0, 'rgba(255,255,255,0.08)');
  ...
  if (qrImg) {
    ...
    ctx.save();
    ctx.fillStyle = '#fff';
    _roundRect(ctx, W - qs - 22, H - qs - 22, qs, qs, 8);
    ctx.fill();
  ...
  if (member.profile_photo) {
    try {
      ...
      ctx.save();
      ctx.beginPath();
      ctx.arc(26 + ps / 2, H - 22 - ps / 2, ps / 2, 0, Math.PI * 2);
      ctx.clip();
      ctx.drawImage(pImg, 26, H - 22 - ps, ps, ps);
      ctx.restore();
...
async function _drawCardDesignB(ctx, W, H, member, config, qrImg, logoImg) {
  ...
  if (logoImg) {
    ...
    gText.addColorStop(0, '#D4AF37');
    gText.addColorStop(1, '#F5E06A');
    ctx.fillStyle = gText;
...
async function _renderCardCanvas(canvasId, card, config, member) {
  ...
  if (config.logo_uri) {
    try {
      logoImg = await _loadImg(config.logo_uri);
    } catch (_e) {}
  }
  if (card.design === 'B') {
...
async function renderReserveStrategique(el) {
  el.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    var d = await api('GET', '/members/reserve-strategique');
    var entries = d.entries || [];
    var active = entries.filter(function (e) {
      return e.status === 'locked';
    });
    var available = entries.filter(function (e) {
      return e.status === 'released' || e.status === 'unlocked';
    });
    var fmtD = function (v) {
    ...
    var today = new Date();
    var daysUntil = function (ds) {
      if (!ds) return null;
      return Math.ceil((new Date(ds).getTime() - today.getTime()) / 86400000);
    };
    var totalActive = d.total_locked || 0;
    var totalAvailable = d.total_released || 0;
    var totalPrelevement = d.total_prelevement || 0;
    var totalAbondement = d.total_abondement || 0;
    var aboRate = Math.round((d.abondement_rate || 0) * 100);
    var nextUnlock = d.next_unlock_date;
    var daysNext = daysUntil(nextUnlock);
    var withdrawalEnabled = d.withdrawal_enabled !== false;
    var rsPct = d.rs_pct || 10;
    var firstActive = active.length > 0 ? active[active.length - 1] : null;
    var progressPct = function () {
      if (!firstActive) return 0;
      var start = new Date(firstActive.created_at).getTime();
      var end = new Date(firstActive.locked_until).getTime();
      var now = today.getTime();
      if (end <= start) return 100;
      return Math.min(100, Math.max(0, Math.round((now - start) / (end - start) * 100)));
    }();
    var html = '<div class="space-y-6">';
    html += "<div class=\"flex items-center gap-3 mb-2\"><button onclick=\"(window._prevPage?showPage(window._prevPage):showPage('wallet'))\" class=\"text-gray-500 hover:text-white transition-colors p-1 rounded-lg hover:bg-dark-700\"><i class=\"fas fa-arrow-left text-sm\"></i></button><div><h1 class=\"text-xl font-bold text-white flex items-center gap-2\"><i class=\"fas fa-vault text-cyan-400\"></i> Réserve Stratégique</h1><p class=\"text-gray-400 text-xs mt-0.5\">Votre épargne constituée avec le soutien de LEADER</p><" + "/div></div>";
    html += '<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">';
    html += '<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Votre épargne constituée</span><div class="w-8 h-8 rounded-lg bg-cyan-500/15 flex items-center justify-center"><i class="fas fa-piggy-bank text-cyan-400 text-sm"></i></div></div><div class="text-2xl font-bold text-cyan-400">' + fmt$(totalActive) + '</div><div class="text-xs text-gray-500 mt-1">' + active.length + ' dépôt' + (active.length > 1 ? 's en cours' : ' en cours') + '</div></div>';
    html += '<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Soutien LEADER</span><div class="w-8 h-8 rounded-lg bg-emerald-500/15 flex items-center justify-center"><i class="fas fa-seedling text-emerald-400 text-sm"></i></div></div><div class="text-2xl font-bold text-emerald-400">' + fmt$(totalAbondement) + '</div><div class="text-xs text-gray-500 mt-1">Abondement compagnie (+' + aboRate + '%)</div></div>';
    html += '<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Disponibilité prévue</span><div class="w-8 h-8 rounded-lg bg-orange-500/15 flex items-center justify-center"><i class="fas fa-calendar-check text-orange-400 text-sm"></i></div></div>' + (nextUnlock ? '<div class="text-lg font-bold text-orange-400">' + fmtD(nextUnlock) + '</div><div class="text-xs text-gray-500 mt-1">' + (daysNext !== null ? 'dans ' + daysNext + ' jours' : '—') + '</div>' : ' <div class="text-lg font-bold text-gray-500">—</div><div class="text-xs text-gray-500 mt-1">Aucun dépôt actif</div>') + '</div>';
    html += '</div>';
    if (totalActive > 0) {
      var totalProg = totalPrelevement + totalAbondement;
      var progPct = totalProg > 0 ? Math.round(totalAbondement / totalProg * 100) : 0;
      html += '<div class="stat-card border-cyan-500/20 space-y-3"><div class="flex items-center justify-between mb-1"><span class="text-sm font-semibold text-white">Récapitulatif de votre épargne</span></div><div class="grid grid-cols-3 gap-3 text-center"><div class="bg-dark-700/50 rounded-xl p-3"><div class="text-xs text-gray-400 mb-1">Votre contribution</div><div class="text-base font-bold text-cyan-400">' +
        fmt$(totalPrelevement) + '</div><div class="text-xs text-gray-500">prélevé sur votre prime</div></div><div class="bg-dark-700/50 rounded-xl p-3"><div class="text-xs text-gray-400 mb-1">Soutien de la compagnie</div><div class="text-base font-bold text-emerald-400">+' +
        fmt$(totalAbondement) + '</div><div class="text-xs text-gray-500">abondement garanti</div></div><div class="bg-dark-700/50 rounded-xl p-3 border border-cyan-500/20"><div class="text-xs text-gray-400 mb-1">Total constitué</div><div class="text-base font-bold text-white">' +
        fmt$(totalActive) + '</div><div class="text-xs text-gray-500">disponible à l\'échéance</div></div></div><div class="flex justify-between text-xs text-gray-400 mb-1 mt-2"><span>Part abondement dans votre épargne</span><span class="text-emerald-400 font-medium">' +
        progPct + '%</span></div><div class="h-2 bg-dark-700 rounded-full overflow-hidden"><div class="h-2 rounded-full" style="width:' + progPct + '%;background:linear-gradient(90deg,#10b981,#059669)"></div></div><div class="flex justify-between text-xs text-gray-600 mt-1"><span>Votre mise</span><span>Abondement compagnie</span></div></div>';
    }
    if (withdrawalEnabled && totalAvailable > 0) {
      html += '<div class="stat-card border-cyan-500/20"><h3 class="font-semibold text-white mb-2 flex items-center gap-2"><i class="fas fa-arrow-right-from-bracket text-cyan-400"></i> Transférer vers mon portefeuille</h3><p class="text-xs text-gray-400 mb-4">Vos fonds disponibles peuvent être transférés immédiatement dans votre portefeuille.</p><div class="flex flex-col sm:flex-row gap-3"><div class="flex-1"><label class="text-xs text-gray-400 block mb-1">Montant à transférer (max ' +
        fmt$(totalAvailable) + ')</label><input id="rs-withdraw-amount" type="number" min="1" max="' + totalAvailable.toFixed(2) + '" step="0.01" placeholder="Ex : 500" class="w-full bg-dark-700 border border-dark-600 rounded-xl px-4 py-2.5 text-white text-sm focus:border-cyan-500/60 focus:outline-none" /></div><button onclick="submitRSWithdrawal(' +
        totalAvailable + ')" class="bg-cyan-600 hover:bg-cyan-500 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-2 justify-center sm:self-end"><i class="fas fa-wallet"></i> Transférer</button></div><div id="rs-withdraw-msg" class="mt-3 text-sm hidden"></div></div>';
    } else if (totalAvailable === 0 && totalActive > 0 && withdrawalEnabled) {
      html += '<div class="bg-dark-700/60 border border-cyan-500/20 rounded-xl p-4 text-sm"><div class="flex items-start gap-3"><i class="fas fa-hourglass-half text-cyan-400 mt-0.5 text-base"></i><div><p class="text-white font-semibold mb-1">Votre épargne est en cours de maturation</p><p class="text-gray-400">Elle sera disponible le <span class="text-cyan-400 font-medium">' + (nextUnlock ? fmtD(nextUnlock) : '—') + '</span>' + (daysNext !== null ? ' — dans <span class="text-white font-medium">' + daysNext + ' jours</span>' : '') + '.</p><p class="text-gray-500 text-xs mt-2">À cette date, vous pourrez transférer librement votre épargne et l\'abondement vers votre portefeuille.</p></div></div></div>';
    } else if (totalActive > 0 && !withdrawalEnabled) {
      html += '<div class="bg-yellow-900/20 border border-yellow-500/20 rounded-xl p-4 text-sm text-yellow-400"><i class="fas fa-info-circle mr-2"></i>Les transferts de Réserve Stratégique sont momentanément suspendus.</div>';
    }
    if (active.length > 0) {
      html += '<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-piggy-bank text-cyan-400"></i> Dépôts en cours (' + active.length + ')</h3><div class="space-y-3">' + active.map(function (e) {
        var days = daysUntil(e.locked_until);
        var abo = Math.round((e.abondement_rate || 0) * 100);
        var partPrel = e.part_prelevement || 0;
        var partAbo = e.part_abondement || 0;
        var total = e.amount || 0;
        var createdTs = new Date(e.created_at).getTime();
        var lockedTs = new Date(e.locked_until).getTime();
        var elapsed = Math.max(0, today.getTime() - createdTs);
        var totalDur = Math.max(1, lockedTs - createdTs);
        var pct = Math.min(100, Math.round(elapsed / totalDur * 100));
        return '<div class="bg-dark-700/60 border border-dark-600 rounded-xl p-4"><div class="flex items-start justify-between mb-3"><div><div class="text-sm font-semibold text-white">Période ' + (e.period || '—') + '</div><div class="text-xs text-gray-400 mt-0.5">Constitué le ' +
          fmtD(e.created_at) + '</div></div><div class="text-right"><div class="text-lg font-bold text-cyan-400">' + fmt$(total) + '</div><div class="text-xs text-gray-500">total constitué</div></div></div><div class="grid grid-cols-3 gap-2 mb-3"><div class="bg-dark-800/60 rounded-lg p-2 text-center"><div class="text-xs text-gray-500 mb-0.5">Votre contribution</div><div class="text-sm font-semibold text-cyan-300">' +
          fmt$(partPrel) + '</div><div class="text-xs text-gray-600">' + rsPct + '% de votre prime</div></div><div class="bg-dark-800/60 rounded-lg p-2 text-center"><div class="text-xs text-gray-500 mb-0.5">Soutien LEADER</div><div class="text-sm font-semibold text-emerald-400">+' +
          fmt$(partAbo) + '</div><div class="text-xs text-gray-600">abondement +' + abo + '%</div></div><div class="bg-dark-800/60 rounded-lg p-2 text-center border border-cyan-500/15"><div class="text-xs text-gray-500 mb-0.5">Total à l\'échéance</div><div class="text-sm font-bold text-white">' +
          fmt$(total) + '</div><div class="text-xs text-gray-600">garanti</div></div></div><div class="flex justify-between text-xs text-gray-400 mb-1"><span>Progression vers la disponibilité</span><span class="text-cyan-400 font-medium">' + pct + '%</span></div><div class="h-1.5 bg-dark-600 rounded-full overflow-hidden"><div class="h-1.5 rounded-full" style="width:' +
          pct + '%;background:linear-gradient(90deg,#06b6d4,#0e7490)"></div></div><div class="flex justify-between text-xs text-gray-500 mt-1.5"><span>Déposé le ' + fmtD(e.created_at) + '</span><span>Disponible le <span class="text-orange-400 font-medium">' + fmtD(e.locked_until) +
          '</span>' + (days !== null ? ' (' + days + ' j)' : '') + '</span></div></div>';
      }).join('') + '</div></div>';
    }
    var hist = entries.filter(function (e) {
      return e.status !== 'locked';
    });
    if (hist.length > 0) {
      html += '<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-history text-gray-400"></i> Historique</h3><div class="space-y-1">' + hist.map(function (e) {
        var isAvail = e.status === 'released' || e.status === 'unlocked';
        var isWithdrawn = e.status === 'withdrawn';
        var isCan = e.status === 'cancelled';
        var color = isAvail ? 'text-green-400' : isWithdrawn ? 'text-cyan-400' : isCan ? 'text-gray-500' : 'text-gray-400';
        var icon = isAvail ? 'fa-check-circle' : isWithdrawn ? 'fa-arrow-right-from-bracket' : isCan ? 'fa-ban' : 'fa-circle';
        var label = isAvail ? 'Disponible' : isWithdrawn ? 'Transféré' : isCan ? 'Remplacé' : e.status;
        var partPrel = e.part_prelevement || 0;
        var partAbo = e.part_abondement || 0;
        return '<div class="bg-dark-700/30 rounded-xl p-3 mb-1"><div class="flex items-center justify-between"><div class="flex items-center gap-3"><i class="fas ' + icon + ' ' + color + ' text-sm"></i><div><div class="text-sm text-white">Période ' + (e.period || '—') +
          '</div><div class="text-xs text-gray-500">' + fmtD(isAvail ? e.unlocked_at : e.created_at) + '</div></div></div><div class="text-right"><div class="text-sm font-semibold ' + color + '">' + fmt$(e.amount) + '</div><div class="text-xs text-gray-500">' + label +
          '</div></div></div>' + (partPrel > 0 && !isCan ? '<div class="flex gap-4 mt-2 pt-2 border-t border-dark-600 text-xs text-gray-500"><span>Votre part : <span class="text-gray-300">' + fmt$(partPrel) + '</span></span><span>Soutien LEADER : <span class="text-emerald-400">+' +
          fmt$(partAbo) + '</span></span></div>' : '') + '</div>';
      }).join('') + '</div></div>';
    }
    if (entries.length === 0) {
      html += '<div class="text-center py-12"><div class="w-16 h-16 rounded-full bg-cyan-500/10 flex items-center justify-center mx-auto mb-4"><i class="fas fa-vault text-cyan-500/50 text-2xl"></i></div><p class="text-gray-400 font-medium">Aucune réserve stratégique pour le moment</p><p class="text-gray-600 text-sm mt-1">Vos primes de leadership alimenteront automatiquement votre réserve dès le rang Mentor.</p></div>';
    }
    html += '</div>';
    el.innerHTML = html;
  } catch (err) {
    el.innerHTML = '<div class="text-red-400 text-center py-12"><i class="fas fa-exclamation-triangle text-2xl mb-3 block"></i>' + (err.message || err.error || 'Erreur inconnue') + '</div>';
  }
}
async function submitRSWithdrawal(maxAmount) {
  var inp = document.getElementById('rs-withdraw-amount');
  var msg = document.getElementById('rs-withdraw-msg');
  var amount = parseFloat(inp ? inp.value : 0);
  if (!inp || !msg) return;
  if (!amount || amount <= 0) {
    msg.textContent = 'Veuillez saisir un montant valide.';
    msg.className = 'mt-3 text-sm text-red-400';
    msg.classList.remove('hidden');
    return;
  }
  if (amount > maxAmount) {
    msg.textContent = 'Montant supérieur au solde disponible (' + fmt$(maxAmount) + ').';
    msg.className = 'mt-3 text-sm text-red-400';
    msg.classList.remove('hidden');
    return;
  }
  msg.textContent = 'Transfert en cours...';
  msg.className = 'mt-3 text-sm text-gray-400';
  msg.classList.remove('hidden');
  try {
    var r = await api('POST', '/members/reserve-strategique/withdraw', {
      amount: amount
    });
    showToast(r.message || 'Transfert effectué avec succès !', 'success');
    showPage('reserve-strategique');
  } catch (e) {
    msg.textContent = e.error || e.message || 'Erreur lors du transfert.';
    msg.className = 'mt-3 text-sm text-red-400';
    msg.classList.remove('hidden');
  }
}
async function renderServices(el) {
  ...
  let services = [],
    ...
    activePkg = null;
  try {
    const [svcRes, cfgRes] = await Promise.all([api('GET', '/members/services'), api('GET', '/landing/config')]);
  ...
  el.innerHTML = "\n    <div>\n      <!-- Hero -->\n      <div class=\"svc-hero\">\n        <div style=\"position:relative;z-index:1\">\n          <div style=\"font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(121,30,21,0.9);margin-bottom:8px\">\n            <i class=\"fas fa-th-large mr-2\"></i>Espace Services\n          </div>\n          <h1 style=\"font-size:clamp(22px,4vw,32px);font-weight:900;color:#fff;line-height:1.2;margin-bottom:10px\">\n            " +
    ...
    accessibleCount + " service" + (accessibleCount > 1 ? 's' : '') + " accessible" + (accessibleCount > 1 ? 's' : '') + "</span>\n            </div>\n            " + (activePkg ? "\n            <div style=\"display:flex;align-items:center;gap:6px;font-size:12px\">\n              <i class=\"fas fa-box\" style=\"color:rgba(121,30,21,0.8);font-size:10px\"></i>\n              <span style=\"color:rgba(245,240,232,0.6)\">Package : <span style=\"color:#fff;font-weight:700\">" +
...
function svcFilter(search, cat) {
  ...
  grid.innerHTML = filtered.length ? filtered.map(window._svcBuildCard).join('') : "<div style=\"grid-column:1/-1;text-align:center;padding:48px;color:rgba(245,240,232,0.3)\">\n         <i class=\"fas fa-search\" style=\"font-size:32px;margin-bottom:12px;display:block;opacity:.4\"></i>\n         Aucun service ne correspond \xE0 votre recherche\n       </div>";
}
async function renderReservoir(e) {
  loading("page-content");
  try {
    const t = await api("GET", "/members/holding-tank"),
      n = !0 === t.license_active,
      a = n && t.my_unique_id ? "" + window.location.origin + "/register/" + t.my_unique_id : "",
      s = t.pending || [],
      r = t.placed || [];
    e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n      <div class=\"flex items-center justify-between flex-wrap gap-2\">\n\n        <h2 class=\"text-xl font-bold\">R\xE9servoir \xE0 placement</h2>\n\n        <div class=\"flex items-center gap-2\">\n\n          <span class=\"bg-orange-500/20 text-orange-400 text-xs px-3 py-1 rounded-full font-medium\">\n\n            <i class=\"fas fa-clock mr-1\"></i>" +
      ...
      s.map(e => {
      const t = null !== e.days_remaining && void 0 !== e.days_remaining ? e.days_remaining : null,
      ...
      return "\n\n          <div class=\"p-4 flex items-center gap-3 flex-wrap sm:flex-nowrap " + (a ? "bg-red-900/10" : "") + "\">\n\n            <div class=\"w-10 h-10 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0\">\n\n              " +
        ...
        "\n\n            </div>\n\n            <div class=\"flex items-center gap-3 flex-shrink-0\">\n\n              <div class=\"text-right\">\n\n                " + (null !== t ? "\n\n                  <div class=\"" + n + " font-bold text-base\">" + Math.max(0, t) +
    ...
    e.innerHTML = "<div class=\"p-4 text-red-400\"><i class=\"fas fa-exclamation-triangle mr-2\"></i>Erreur : " + (t.error || t.message || "Impossible de charger le réservoir") + "</div>";
  }
}
function copyReservoirLink() {
  const e = document.getElementById("reservoir-link-input");
  e && navigator.clipboard.writeText(e.value).then(() => showToast("Lien de parrainage copié !")).catch(() => {
    e.select(), document.execCommand("copy"), showToast("Lien copié !");
  });
}
let _placementMemberId = null,
  _placementMemberName = null,
  _placementMemberUid = null;
function svgCollectPlacement(e, t, n, a, s, r, i, l, d, o, c) {
  if (!e) {
    if (null !== d && null !== c) {
      const e = d - 77,
        t = o + 108 + 40,
        n = "L" === c ? "Gauche" : "Droite",
        s = "L" === c ? "#3B82F6" : "#10B981";
      a.push("<path d=\"" + svgBezier(d, o + 108, d, t) + "\" stroke=\"#374151\" stroke-width=\"1.5\" fill=\"none\" stroke-dasharray=\"5,4\"/>");
      const x = (i || "").replace(/'/g, ""),
        p = (l || "").replace(/'/g, "");
      r.push("\n\n        <g class=\"svg-slot-place\" onclick=\"confirmPlacement('" + x + "','" + p + "','" + c + "')\" style=\"cursor:pointer\">\n\n          <rect x=\"" + e + "\" y=\"" + t + "\" width=\"154\" height=\"108\" rx=\"12\"\n\n                fill=\"#111827\" stroke=\"" +
        ...
        (e + 77) + "\" y=\"" + (t + 54) + "\" text-anchor=\"middle\" fill=\"" + s + "dd\" font-size=\"10\" font-weight=\"700\" font-family=\"sans-serif\">Spot libre \u2014 " + n + "</text>\n\n          <text x=\"" + (e + 77) + "\" y=\"" + (t + 70) + "\" text-anchor=\"middle\" fill=\"" +
        s + "88\" font-size=\"9\" font-family=\"sans-serif\">Cliquer pour placer ici</text>\n\n          <!-- Zone hover invisible pour agrandir la cible de clic -->\n\n          <rect x=\"" + e + "\" y=\"" + t + "\" width=\"154\" height=\"108\" rx=\"12\" fill=\"transparent\" class=\"svg-slot-hover-zone\"/>\n\n        </g>");
    }
    return;
  }
  const x = e._x + 77,
  ...
  if (null !== d && a.push("<path d=\"" + svgBezier(d, o + 108, x, p) + "\" stroke=\"#374151\" stroke-width=\"1.5\" fill=\"none\"/>"), null !== c) {
    ...
    s.push("<text x=\"" + x + "\" y=\"" + (p - 14) + "\" text-anchor=\"middle\" fill=\"" + e + "\" font-size=\"9\" font-weight=\"700\" font-family=\"sans-serif\">" + ("L" === c ? "G" : "D") + "</text>");
  }
  const m = e.in_holding_tank,
    ...
    g = m ? "#FB923C" : u[e.current_rank] || "#374151",
    f = m ? "#431407" : "#1F2937",
    b = ("" + (e.first_name || "?")[0] + (e.last_name || "?")[0]).toUpperCase(),
    v = {
      captain: "Capt.",
      commander: "Cmdr.",
      admiral: "Adm.",
  ...
  s.push("\n\n    <g class=\"svg-node-member\">\n\n      <rect x=\"" + e._x + "\" y=\"" + p + "\" width=\"154\" height=\"108\" rx=\"12\"\n\n            fill=\"" + f + "\" stroke=\"" + g + "\" stroke-width=\"" + (m ? 2 : 1.5) + "\"/>\n\n      <circle cx=\"" + x + "\" cy=\"" +
    (p + 28) + "\" r=\"18\" fill=\"#111827\" stroke=\"" + g + "\" stroke-width=\"1.5\"/>\n\n      <text x=\"" + x + "\" y=\"" + (p + 33) + "\" text-anchor=\"middle\" fill=\"" + y + "\" font-size=\"11\" font-weight=\"700\" font-family=\"sans-serif\">" + b + "</text>\n\n      <text x=\"" +
    x + "\" y=\"" + (p + 56) + "\" text-anchor=\"middle\" fill=\"#F9FAFB\" font-size=\"10\" font-weight=\"600\" font-family=\"sans-serif\">" + (e.first_name + " " + e.last_name).substring(0, 18) + "</text>\n\n      <text x=\"" + x + "\" y=\"" + (p + 68) + "\" text-anchor=\"middle\" fill=\"#9CA3AF\" font-size=\"8.5\" font-family=\"monospace\">" +
    e.unique_id + "</text>\n\n      <rect x=\"" + (x - 28) + "\" y=\"" + (p + 73) + "\" width=\"56\" height=\"13\" rx=\"6\" fill=\"" + y + "22\"/>\n\n      <text x=\"" + x + "\" y=\"" + (p + 83) + "\" text-anchor=\"middle\" fill=\"" + y + "\" font-size=\"8\" font-weight=\"700\" font-family=\"sans-serif\">" +
    v + "</text>\n\n      <text x=\"" + x + "\" y=\"" + (p + 93) + "\" text-anchor=\"middle\" fill=\"#6B7280\" font-size=\"8\" font-family=\"sans-serif\">" + (e.member_status || "") + "</text>\n\n      <text x=\"" + (e._x + 8) + "\" y=\"" + (p + 102) + "\" fill=\"#60A5FA\" font-size=\"8\" font-family=\"sans-serif\">G:" +
    fmtBV(e.left_bv_monthly) + "</text>\n\n      <text x=\"" + (e._x + 154 - 8) + "\" y=\"" + (p + 102) + "\" text-anchor=\"end\" fill=\"#34D399\" font-size=\"8\" font-family=\"sans-serif\">D:" + fmtBV(e.right_bv_monthly) + "</text>\n\n      " + (m ? "<text x=\"" + x +
    "\" y=\"" + (p + 8) + "\" text-anchor=\"middle\" fill=\"#FB923C\" font-size=\"7.5\" font-weight=\"700\" font-family=\"sans-serif\">HOLDING</text>" : "") + "\n\n    </g>"), t < n && (svgCollectPlacement(e.left, t + 1, n, a, s, r, e.id, e.unique_id, x, p, "L"), svgCollectPlacement(e.right, t +
    1, n, a, s, r, e.id, e.unique_id, x, p, "R"));
}
function renderPlacementSVG(e) {
  const t = Math.min(treeMaxDepth(e), 5),
    n = svgAssignPositions(e, 0, 0),
    a = 188 * (t + 1) + 60,
    s = [],
...
let _placeTree = null,
  ...
  _placeFocus = null,
  _placeNavStack = [];
function placeNavigateTo(e) {
  const t = findNodeById(_placeTree, e);
  t && (_placeNavStack.push(_placeFocus), _placeFocus = t, renderPlacementClassicInner());
}
function placeGoBack() {
  0 !== _placeNavStack.length && (_placeFocus = _placeNavStack.pop(), renderPlacementClassicInner());
}
function placeGoHome() {
  _placeNavStack = [], _placeFocus = _placeRoot, renderPlacementClassicInner();
}
function renderPlacementNode(e, t, n = !1) {
  if (!e) return "";
  const a = e.in_holding_tank ? '<div class="text-[9px] text-orange-400 mt-0.5"><i class="fas fa-clock mr-0.5"></i>Holding</div>' : "",
    s = "tree-node-card\n    " + (n ? "ring-2 ring-gold-500/40 bg-dark-700 scale-105 shadow-lg shadow-black/40 cursor-default" : "cursor-pointer hover:border-rouge-500/60 hover:bg-dark-700 transition-all"),
    r = "\n\n    <div class=\"w-9 h-9 rounded-full bg-dark-900 flex items-center justify-center mx-auto mb-2 text-xs font-bold " + ({
      captain: "text-yellow-400",
      commander: "text-purple-400",
      admiral: "text-blue-400",
      ...
      "\n\n  ",
    i = n ? "<div class=\"" + s + "\">" + r + "</div>" : "<div class=\"" + s + "\" onclick=\"placeNavigateTo('" + e.id + "')\">" + r + "</div>";
  if (t >= 3) return "<div class=\"tree-node\">" + i + "</div>";
  const l = (e, t, n) => {
...
function renderPlacementClassicInner() {
  const e = document.getElementById("placement-tree-container");
  if (!e || !_placeFocus) return;
  const t = _placeFocus.id === _placeRoot.id,
    n = _placeNavStack.length > 0,
    a = _placeNavStack.map((e, t) => "\n\n    <span class=\"text-gray-500 text-xs\">" + (0 === t ? '<i class="fas fa-home text-[10px]"></i>' : "") + " " + e.first_name + "</span>\n\n    <span class=\"text-gray-600 text-xs\">\u203A</span>").join("");
...
async function openPlacementModal(e, t, n) {
  _placementMemberId = e, _placementMemberName = t, _placementMemberUid = n;
  const a = document.getElementById("placement-modal"),
    s = document.getElementById("placement-modal-subtitle"),
    r = document.getElementById("placement-tree-container");
  s.textContent = "Choisissez o\xF9 placer : " + t + " (" + n + ")", r.innerHTML = '<div class="flex justify-center py-8"><div class="loader"></div></div>', a.classList.remove("hidden"), a.classList.add("flex");
  try {
    if (_fullTree) return _placeTree = _fullTree, _placeRoot = _fullTree, _placeFocus = _fullTree, _placeNavStack = [], void renderPlacementClassicInner();
    const e = await api("GET", "/members/tree?depth=12");
    if (!e.tree) return void (r.innerHTML = '\n        <div class="text-center py-12">\n          <i class="fas fa-tree text-gray-600 text-4xl mb-4"></i>\n          <p class="text-gray-400">Vous n\'êtes pas encore placé dans l\'arbre binaire.</p>\n          <p class="text-gray-500 text-sm mt-2">Vous devez être placé pour pouvoir placer vos filleuls.</p>\n        </div>');
    _placeTree = e.tree, _placeRoot = e.tree, _placeFocus = e.tree, _placeNavStack = [], renderPlacementClassicInner();
  } catch (e) {
    r.innerHTML = "<p class=\"text-red-400 text-center py-8\">Erreur : " + (e.error || e.message) + "</p>";
  }
}
function closePlacementModal() {
  const e = document.getElementById("placement-modal");
  e.classList.add("hidden"), e.classList.remove("flex"), _placementMemberId = null;
}
async function confirmPlacement(e, t, n) {
  if (!_placementMemberId) return;
  const a = "L" === n ? "jambe GAUCHE" : "jambe DROITE",
    s = document.getElementById("placement-tree-container"),
    r = document.createElement("div");
  r.id = "placement-confirm-banner", r.className = "sticky top-0 z-10 bg-orange-900/90 border border-orange-400/50 rounded-xl p-4 mb-4 flex items-center justify-between gap-4 backdrop-blur", r.innerHTML = "\n\n    <div class=\"flex items-center gap-3\">\n\n      <i class=\"fas fa-question-circle text-orange-400 text-xl flex-shrink-0\"></i>\n\n      <div>\n\n        <div class=\"font-bold text-orange-200 text-sm\">Confirmer le placement ?</div>\n\n        <div class=\"text-orange-300/80 text-xs mt-0.5\">\n\n          <strong>" +
    _placementMemberName + "</strong> (" + _placementMemberUid + ") \u2192\n\n          sous <strong>" + t + "</strong> \u2014 " + a + ("\n\n        </div>\n\n        <div class=\"text-orange-400/60 text-xs mt-0.5\">[!] Cette action est irréversible.</div>\n\n      </div>\n\n    </div>\n\n    <div class=\"flex gap-2 flex-shrink-0\">\n\n      <button id=\"btn-cancel-place\" class=\"px-4 py-2 bg-dark-700 text-gray-300 rounded-xl text-sm hover:bg-dark-600 transition\">Annuler</button>\n\n      <button id=\"btn-confirm-place\" class=\"px-4 py-2 bg-orange-500 text-white font-bold rounded-xl text-sm hover:bg-orange-400 transition\">\n\n        <i class=\"fas fa-check mr-1\"></i>Confirmer\n" +
    "\n      </button>\n\n    </div>"), s.insertBefore(r, s.firstChild), s.scrollTop = 0;
  const i = s.querySelectorAll("button:not(#btn-confirm-place):not(#btn-cancel-place)");
  i.forEach(e => {
    e.disabled = !0, e.style.opacity = "0.3";
  }), document.getElementById("btn-cancel-place").onclick = () => {
    r.remove(), i.forEach(e => {
      e.disabled = !1, e.style.opacity = "1";
    });
  }, document.getElementById("btn-confirm-place").onclick = async () => {
    const t = document.getElementById("btn-confirm-place");
    if (!t) return;
    t.disabled = !0, t.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>En cours…';
    const a = _placementMemberId,
      s = _placementMemberName;
    if (!a) return void showToast("Erreur interne : aucun filleul sélectionné", "error");
    let l = null;
    try {
      l = await api("POST", "/members/holding-tank/place", {
        member_id: a,
        parent_id: e,
        position: n
      });
    } catch (e) {
      return r.isConnected && r.remove(), i.forEach(e => {
        e.disabled = !1, e.style.opacity = "1";
      }), void showToast("\xC9chec du placement : " + (e && (e.error || e.message) || "Erreur serveur lors du placement"), "error");
    }
    closePlacementModal(), showToast(" " + s + " plac\xE9(e) avec succ\xE8s dans l'arbre !");
    try {
      const e = document.getElementById("page-content");
      e && (await renderReservoir(e));
    } catch (e) {
      showPage("reservoir");
    }
  };
}
const MKTG_MEMBER_TYPE_META = {
    pdf: {
      icon: "fa-file-pdf",
      color: "red",
      label: "PDF",
  ...
  MKTG_MEMBER_TAG = {
    gold: "bg-yellow-900/40 text-yellow-300 border-yellow-600/30",
  ...
  MKTG_CAT_PAL = {
    gold: {
      ...
      activeText: "text-dark-900",
      gradient: "from-yellow-900/30 to-dark-800"
    },
    blue: {
      bg: "bg-blue-900/20",
      ...
      gradient: "from-blue-900/30 to-dark-800"
    },
    purple: {
      bg: "bg-purple-900/20",
      ...
      activeText: "text-white",
      gradient: "from-purple-900/30 to-dark-800"
    },
    pink: {
      bg: "bg-pink-900/20",
      ...
      activeText: "text-white",
      gradient: "from-pink-900/30 to-dark-800"
    },
    red: {
      bg: "bg-red-900/20",
      ...
      activeBg: "bg-red-500",
      activeText: "text-white",
      gradient: "from-red-900/30 to-dark-800"
    },
    green: {
      bg: "bg-green-900/20",
      ...
      activeText: "text-white",
      gradient: "from-green-900/30 to-dark-800"
    }
  };
function mktgCatPal(e) {
  return MKTG_CAT_PAL[e] || MKTG_CAT_PAL.gold;
}
function mktgMemberCard(e) {
  ...
  return "\n\n  <div class=\"bg-dark-800 rounded-2xl border border-dark-600 hover:border-dark-500 transition-all duration-200 overflow-hidden flex flex-col group hover:shadow-lg hover:shadow-black/30\">\n\n    <!-- Placeholder / header color\xE9 -->\n\n    " + (e.thumbnail_url ? "\n\n    <div class=\"relative overflow-hidden h-32\">\n\n      <img src=\"" +
    ...
    n.bg + " " + n.text + " " + n.border + " font-medium\">\n\n          <i class=\"fas " + (e.category_icon || "fa-tag") + " text-[10px]\"></i>" + e.category_name + "\n\n        </span>" : "") + "\n\n        " + (e.file_size ? "<span class=\"text-xs text-gray-500\">" +
    e.file_size + "</span>" : "") + "\n\n        " + (e.is_downloadable ? '<span class="text-xs text-green-500/70"><i class="fas fa-download mr-0.5 text-[10px]"></i>DL</span>' : "") + "\n\n        " + (e.views_count > 0 ? "<span class=\"text-xs text-gray-600 ml-auto\"><i class=\"fas fa-eye mr-0.5 text-[10px]\"></i>" +
    e.views_count + "</span>" : "") + "\n\n      </div>\n\n\n\n      <!-- Boutons -->\n\n      <div class=\"mt-auto flex gap-2\">\n\n        " + ("pdf" === e.type || e.url.includes(".pdf") ? "\n\n        <button onclick=\"openMktgViewer('" + e.id + "','" + encodeURIComponent(e.url) +
    ...
    e.url + "\" target=\"_blank\" onclick=\"trackMktgView('" + e.id + "')\"\n\n          class=\"flex-1 text-xs bg-blue-600/20 text-blue-400 border border-blue-600/30 px-3 py-2 rounded-xl hover:bg-blue-600/30 transition flex items-center justify-center gap-1.5 font-medium\">\n\n          <i class=\"fas fa-external-link-alt\"></i> Ouvrir\n\n        </a>") +
...
function toggleQR() {
  const e = document.getElementById("qr-panel");
  e && (e.classList.toggle("hidden"), e.classList.toggle("flex"));
}
function trackMktgView(e) {
  api("POST", "/members/marketing/" + e + "/view").catch(() => {});
}
function openMktgViewer(e, t, n) {
  trackMktgView(e);
  ...
  var body;
  if (isImg) {
    body = '<div class="flex-1 bg-gray-900 flex items-center justify-center p-4 overflow-auto">' + '<img src="' + a + '" alt="' + title + '" class="max-w-full max-h-full object-contain rounded-lg shadow-2xl">' + '</div>';
  } else if (isVid) {
    body = '<div class="flex-1 bg-black flex items-center justify-center">' + '<video src="' + a + '" controls class="max-w-full" style="max-height:calc(85vh - 56px)"></video>' + '</div>';
  } else if (isAud) {
    body = '<div class="flex-1 bg-gray-900 flex flex-col items-center justify-center gap-6 p-8">' + '<div class="w-20 h-20 rounded-full bg-yellow-500/20 border border-yellow-500/30 flex items-center justify-center">' + '<i class="fas fa-music text-yellow-400 text-3xl"></i></div>' + '<p class="font-semibold text-white text-center">' + title + '</p>' + '<audio src="' + a + '" controls class="w-full max-w-md"></audio>' + '</div>';
  } else if (isDoc) {
    body = '<div class="flex-1 bg-gray-900 relative">' + '<iframe src="https://docs.google.com/gview?url=' + encodeURIComponent(a) + '&embedded=true"' + ' class="w-full h-full border-0" allowfullscreen></iframe>' + '</div>';
  } else {
    body = '<div class="flex-1 bg-gray-900 relative flex flex-col">' + '<iframe src="' + a + '" class="w-full h-full border-0 flex-1" style="min-height:0"></iframe>' + '</div>';
  }
  var html = '<div class="flex flex-col" style="height:85vh;min-width:min(90vw,900px);">' + '<div class="flex items-center justify-between px-5 py-3 border-b border-dark-600 bg-dark-800 flex-shrink-0">' + '<div class="flex items-center gap-2 min-w-0">' + '<i class="fas ' +
    icon + ' ' + icol + ' flex-shrink-0"></i>' + '<span class="font-semibold text-white text-sm truncate">' + title + '</span>' + '</div>' + '<div class="flex items-center gap-2 flex-shrink-0 ml-3">' + '<a href="' + a + '" target="_blank" class="text-xs bg-blue-600/20 text-blue-400 border border-blue-600/30 px-3 py-1.5 rounded-lg hover:bg-blue-600/30 transition flex items-center gap-1.5">' +
    '<i class="fas fa-external-link-alt"></i> Nouvelle fen&#234;tre' + '</a>' + '<a href="' + a + '" download target="_blank" class="text-xs bg-green-600/20 text-green-400 border border-green-600/30 px-3 py-1.5 rounded-lg hover:bg-green-600/30 transition flex items-center gap-1.5">' +
    '<i class="fas fa-download"></i> T&#233;l&#233;charger' + '</a>' + '<button onclick="closeModal()" class="text-gray-400 hover:text-white ml-2"><i class="fas fa-times"></i></button>' + '</div>' + '</div>' + body + '</div>';
  showModal(html, !0);
}
function copyReferralLink() {
  const e = document.getElementById("referral-link-input");
  e && navigator.clipboard.writeText(e.value).then(() => {
...
async function renderCCWallet(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    window._ccWdFiles = [];
    const t = await api("GET", "/members/cc-wallet"),
      n = t.balance || {
        total: 0,
        available: 0,
        held: 0
      ...
      _ = window._ccWalletCfg = cfg,
      i = e => "$" + (e || 0).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, " "),
      l = e => e ? new Date(e).toLocaleDateString("fr-FR", {
        day: "2-digit",
        month: "short",
        year: "numeric"
      }) : "—",
      d = e => ({
        pending: '<span class="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full">En attente</span>',
        approved: '<span class="text-xs bg-green-500/20  text-green-400  px-2 py-0.5 rounded-full">✓ Approuvé</span>',
        rejected: '<span class="text-xs bg-red-500/20    text-red-400    px-2 py-0.5 rounded-full">✗ Refusé</span>'
      })[e] || "<span class=\"text-xs bg-gray-500/20 text-gray-400 px-2 py-0.5 rounded-full\">" + e + "</span>",
      o = e => ({
        held: '<span class="text-xs bg-blue-500/20   text-blue-400   px-2 py-0.5 rounded-full">En attente</span>',
        available: '<span class="text-xs bg-green-500/20  text-green-400  px-2 py-0.5 rounded-full">Disponible</span>',
        expired: '<span class="text-xs bg-gray-500/20   text-gray-400   px-2 py-0.5 rounded-full">Expiré</span>',
        used: '<span class="text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full">✓ Utilisé</span>'
      })[e] || e,
    ...
    e.innerHTML = "\n\n    <div class=\"space-y-6\">\n\n\n\n      <!-- En-tête -->\n\n      <div class=\"flex items-center justify-between flex-wrap gap-3\">\n\n        <div>\n\n          <button onclick=\"showPage('wallet')\" class=\"flex items-center gap-1.5 text-xs text-gray-400 hover:text-white transition mb-2\"><i class=\"fas fa-arrow-left text-[10px] mr-1\"></i>Retour au wallet</button>\n\n          <h2 class=\"text-xl font-bold text-white flex items-center gap-2\">\n\n            <i class=\"fas fa-seedling text-green-400\"></i>\n\n            Crédit de Croissance\n" +
      ...
      "\n\n               " + (cfg.fields?.custom3?.visible === true || cfg.fields?.custom3?.visible === '1' ? "<div>\n\n                 <label class=\"text-xs text-gray-400 block mb-1\">" + (cfg.fields.custom3.label || 'Information complémentaire 3') + (cfg.fields.custom3.required ? '<span class=\\"text-red-400 ml-0.5\\">*</span>' : '') +
      ...
      const t = e.remaining || e.amount - e.used_amount || 0,
        ...
        x = d ? "border-red-500/40" : c ? "border-yellow-500/40" : "available" === e.status ? "border-green-500/30" : "held" === e.status ? "border-blue-500/30" : "border-dark-600",
      ...
      e.description + "</td>\n\n                <td class=\"py-2 text-center\">" + d(e.status) + "</td>\n\n                <td class=\"py-2 pl-4 text-xs text-gray-400\">" + (e.admin_note || "—") + "</td>\n\n                <td class=\"py-2 text-center\">" + (e.justificatif_url ? '<a href="' +
      e.justificatif_url + '" download="' + (e.justificatif_name || 'justif') + '" class="inline-flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs"><i class="fas fa-' + (e.justificatif_url.startsWith('data:image/') ? 'fa-image' : e.justificatif_url.startsWith('data:application/pdf') ? 'fa-file-pdf' : 'fa-file') +
      '"></i><span>Voir</span></a>' : '<span class="text-gray-600 text-xs">—</span>') + "</td>\n\n              </tr>").join("") + "\n\n            </tbody>\n\n          </table>\n\n        </div>\n\n      </div>" : "") + "\n\n\n\n      <!-- Journal des transactions CC -->\n\n      <div class=\"stat-card space-y-3\">\n\n        <h3 class=\"font-semibold text-white flex items-center gap-2\">\n\n          <i class=\"fas fa-clock-rotate-left text-gray-400 text-sm\"></i>\n\n          Journal des op\xE9rations\n\n        </h3>\n\n        " +
    ...
    e.innerHTML = "<div class=\"p-4 text-red-400 text-sm\"><i class=\"fas fa-triangle-exclamation mr-2\"></i>Erreur : " + (t?.error || t?.message || "Impossible de charger le wallet CC") + "</div>";
  }
}
function _ccAddJustif(inp) {
  if (!window._ccWdFiles) window._ccWdFiles = [];
  var files = Array.from(inp.files || []);
  if (!files.length) return;
  var oversized = files.filter(function (f) {
    return f.size > 4 * 1024 * 1024;
  });
  if (oversized.length) {
    alert("Fichier trop volumineux : " + oversized.map(function (f) {
      return f.name + " (" + (f.size / 1024 / 1024).toFixed(1) + " MB)";
    }).join(", ") + "\nLimite : 4 MB par fichier.");
    inp.value = "";
    return;
  }
  var list = document.getElementById("cc-wd-files-list");
  files.forEach(function (f) {
    var i = window._ccWdFiles.push(f) - 1;
    var ext = (f.name.split(".").pop() || ".").toLowerCase();
    var isPdf = f.type.indexOf("pdf") >= 0 || ext === "pdf";
    var isImg = f.type.indexOf("image") >= 0 || ["jpg", "jpeg", "png", "gif", "webp"].indexOf(ext) >= 0;
    var icon = isPdf ? "fa-file-pdf text-red-400" : isImg ? "fa-image text-blue-400" : "fa-file text-gray-400";
    var sz = f.size > 1048576 ? (f.size / 1048576).toFixed(1) + " MB" : (f.size / 1024).toFixed(0) + " KB";
    var tl = isPdf ? "PDF" : isImg ? "Image" : "Fichier";
    var card = document.createElement("div");
    card.className = "flex items-center gap-3 bg-green-900/20 border border-green-500/30 rounded-xl px-4 py-2.5";
    var iconEl = document.createElement("div");
    iconEl.className = "w-9 h-9 rounded-lg bg-green-500/15 border border-green-500/25 flex items-center justify-center flex-shrink-0";
    iconEl.innerHTML = "<i class=\"fas " + icon + " text-base\"></i>";
    var info = document.createElement("div");
    info.className = "flex-1 min-w-0";
    info.innerHTML = "<div class=\"text-sm text-white font-medium truncate\">" + f.name + "</div><div class=\"text-xs text-gray-500\">" + tl + " · " + sz + "</div>";
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "ml-auto w-7 h-7 rounded-full bg-dark-600 hover:bg-red-900/40 border border-dark-500 flex items-center justify-center flex-shrink-0";
    btn.innerHTML = "<i class=\"fas fa-times text-gray-400 hover:text-red-400 text-xs\"></i>";
    (function (idx, c) {
      btn.onclick = function () {
        window._ccWdFiles.splice(idx, 1);
        c.remove();
      };
    })(i, card);
...
async function submitCCWithdrawal() {
  const elAmt = document.getElementById("cc-wd-amount");
  const elDesc = document.getElementById("cc-wd-desc");
  const elErr = document.getElementById("cc-wd-error");
  const elBtn = document.getElementById("cc-wd-btn");
  const files = window._ccWdFiles || [];
  if (!elAmt) return;
  const cfg = window._ccWalletCfg || {};
  const fields = cfg.fields || {};
  const amount = parseFloat(elAmt.value);
  const description = elDesc ? elDesc.value.trim() : "";
  elErr.classList.add("hidden");
  if (!amount || amount < (cfg.min_amount || 10)) return elErr.textContent = "Montant minimum : " + (cfg.min_amount || 10) + "$", void elErr.classList.remove("hidden");
  if (fields.description?.visible !== false && fields.description?.required !== false) {
    const minC = cfg.desc_min_chars || 10;
    if (!description || description.length < minC) return elErr.textContent = (fields.description?.label || "Motif de la demande") + " requis (" + minC + " caractères minimum)", void elErr.classList.remove("hidden");
  }
  if (fields.justificatif?.visible !== false && fields.justificatif?.required) {
    if (!files.length) return elErr.textContent = (fields.justificatif?.label || "Justificatif") + " obligatoire", void elErr.classList.remove("hidden");
  }
  const elC1 = document.getElementById("cc-wd-custom1");
  if ((fields.custom1?.visible === true || fields.custom1?.visible === "1") && fields.custom1?.required) {
    if (!elC1 || !elC1.value.trim()) return elErr.textContent = (fields.custom1?.label || "Champ 1") + " obligatoire", void elErr.classList.remove("hidden");
  }
  const elC2 = document.getElementById("cc-wd-custom2");
  if ((fields.custom2?.visible === true || fields.custom2?.visible === "1") && fields.custom2?.required) {
    if (!elC2 || !elC2.value.trim()) return elErr.textContent = (fields.custom2?.label || "Champ 2") + " obligatoire", void elErr.classList.remove("hidden");
  }
  const elC3 = document.getElementById("cc-wd-custom3");
  if ((fields.custom3?.visible === true || fields.custom3?.visible === "1") && fields.custom3?.required) {
    if (!elC3 || !elC3.value.trim()) return elErr.textContent = (fields.custom3?.label || "Champ 3") + " obligatoire", void elErr.classList.remove("hidden");
  }
  elBtn.disabled = true;
  elBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Envoi...';
  try {
    const fd = new FormData();
    fd.append("amount", String(amount));
    fd.append("description", description);
    files.forEach(function (f, i) {
      fd.append("justificatif_" + i, f);
    });
    if (elC1 && elC1.value.trim()) fd.append("custom1", elC1.value.trim());
    if (elC2 && elC2.value.trim()) fd.append("custom2", elC2.value.trim());
    if (elC3 && elC3.value.trim()) fd.append("custom3", elC3.value.trim());
    const res = await fetch("/api/members/cc-withdrawal", {
      method: "POST",
      headers: {
    ...
    showToast("Demande soumise avec succès — traitement sous " + (cfg.processing_time || "48h") + " ✓", "success");
    showPage("cc-wallet");
  } catch (e) {
    elErr.textContent = e?.error || "Erreur lors de l'envoi";
    elErr.classList.remove("hidden");
    elBtn.disabled = false;
    elBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Soumettre la demande';
  }
}
async function renderNotifications(e, page, perPage) {
  page = page || window._notifPage || 1;
  perPage = perPage || window._notifPerPage || 25;
  window._notifPage = page;
  window._notifPerPage = perPage;
  try {
    const t = await api("GET", "/members/notifications?page=" + page + "&per_page=" + perPage);
    await api("PUT", "/members/notifications/read-all"), document.getElementById("notif-badge").classList.add("hidden"), document.getElementById("header-notif-badge").classList.add("hidden"), e.innerHTML = "\n\n    <div class=\"space-y-4\">\n\n      <h2 class=\"text-xl font-bold\">Notifications</h2>\n\n      <div class=\"space-y-2\">\n\n        " +
      ...
      (e.is_read ? "text-gray-300" : "text-white") + "\">" + e.title + "</div>\n\n              <div class=\"text-xs text-gray-400 mt-0.5\">" + e.message + "</div>\n\n              <div class=\"text-xs text-gray-600 mt-1\">" + fmtDatetime(e.created_at) + "</div>\n\n            </div>\n\n          </div>\n\n        </div>").join("") || '<p class="text-gray-400 text-center py-8">Aucune notification</p>') +
      "\n\n      </div>\n\n    </div><div id=\"notif-pagination\" class=\"py-3 border-t border-dark-600\"></div>";
    renderPagination("notif-pagination", t.total || 0, page, perPage, "(function(p){renderNotifications(document.getElementById('page-content'),p,window._notifPerPage)})", "(function(pp,p){renderNotifications(document.getElementById('page-content'),p,pp)})");
  } catch (t) {
