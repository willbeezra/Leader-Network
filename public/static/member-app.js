...
async function openWalletDetail(type) {
  ...
  if (!modal) {
    ...
    modal.innerHTML =
      '<div class="absolute inset-0 bg-black/70 backdrop-blur-sm" onclick="document.getElementById(\'wallet-detail-modal\').remove()"></div><div ' +
      'class="relative bg-dark-800 border border-dark-600 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[85vh] flex flex-col"><div class="flex ' +
      'items-center justify-between p-4 border-b border-dark-600"><h3 id="wdm-title" class="font-bold text-white text-lg"></h3><button ' +
      'onclick="document.getElementById(\'wallet-detail-modal\').remove()" class="text-gray-500 ' +
      'hover:text-white text-xl leading-none">&times;</button></div><div id="wdm-body" class="overflow-y-auto flex-1 p-4"></div></div>';
    ...
    if (type === "pending") {
      body.innerHTML =
        ...
        rows
          .map((e) => {
            ...
            const sourceHtml = e.source_name
              ? '<span class="text-gray-300"> \xB7 ' +
                e.source_name +
            ...
            const releaseHtml =
              e.status === "pending_release"
                ? '<span class="text-yellow-400">Lib\xE9ration le ' + fmtD(e.eligible_date) + "</span>"
                : e.status === "active"
            ...
            return (
              ...
              (e.days_paid || 0) +
              "j vers\xE9s</span><span>" +
              daysLeft +
              "j restants</span></div></div>"
            );
          })
          .join("") +
        "</div>";
      ...
      body.innerHTML =
        ...
        rows
          .map(
            (tx) =>
              '<div class="flex items-center justify-between py-2 border-b border-dark-700"><div class="flex-1 min-w-0"><div class="text-sm text-white truncate">' +
              (tx.type_label || tx.label || tx.description || tx.transaction_type || "Transaction") +
              '</div><div class="text-xs text-gray-500">' +
              fmtD(tx.created_at) +
              '</div></div><div class="text-right ml-3">' +
              fmtA(tx.amount) +
              "</div></div>"
          )
          .join("") +
...
function showToast(e, t = "success") {
  ...
  ((a.textContent = e),
    (a.className = "px-6 py-3 rounded-xl shadow-lg text-sm font-medium " + ("success" === t ? "bg-green-600 text-white" : "bg-red-600 text-white")),
...
function renderPagination(containerId, total, page, perPage, onPageChange, onPerPageChange) {
  ...
  window["_cbPp_" + cid] =
    typeof onPerPageChange === "function"
      ? onPerPageChange
      : new Function("pp", "p", onPerPageChange.replace(/^\s*\(function\s*\([^)]*\)\s*\{/, "").replace(/\}\s*\)\s*$/, ""));
  ...
  var selectHtml =
    ...
    "</select>";
  var info =
    '<span class="text-xs text-gray-400">' + ((page - 1) * perPage + 1) + "-" + Math.min(page * perPage, total) + " sur " + total + "</span>";
  var buttons = "";
  var WINDOW = 2;
...
function showModal(e, t = !1) {
  ...
  n &&
    a &&
    ((a.className =
      "bg-dark-800 rounded-2xl shadow-2xl border border-dark-600 w-full " +
      (t ? "max-w-4xl" : "max-w-lg") +
...
function showLogin() {
  (document.getElementById("login-form").classList.remove("hidden"),
    ...
    document.getElementById("pin-confirmation").classList.add("hidden"));
}
...
async function initRegistrationMode() {
  ...
  if ("M1" === t)
    return (
      (document.getElementById("reg-title").textContent = "Créer un compte"),
      void document.getElementById("reg-sponsor-field").classList.remove("hidden")
    );
  if ("M2" === t) {
    ((document.getElementById("reg-title").textContent = "Inscription — Placement direct"),
      document.getElementById("reg-sponsor-field").classList.add("hidden"));
    ...
    try {
      const s = e.parent || e.sponsor,
        r = (
          await axios.get(
            "/api/auth/validate-slot?sponsor=" +
              encodeURIComponent(e.sponsor) +
              "&parent=" +
              encodeURIComponent(s) +
              "&position=" +
              encodeURIComponent(e.position)
          )
        ).data;
      if (r.valid) {
        ((document.getElementById("reg-sponsor").value = r.sponsor_id),
          (document.getElementById("reg-binary-parent").value = r.parent_id),
          n.classList.remove("hidden"));
        const t =
          r.parent_unique_id !== r.sponsor_unique_id
            ? "Parrain\xE9(e) par " +
              r.sponsor_name +
              " \u2014 plac\xE9(e) sous " +
              r.parent_name +
              ...
              r.parent_unique_id +
              "), jambe " +
              ("L" === e.position ? "GAUCHE" : "DROITE")
            : "Vous serez plac\xE9(e) c\xF4t\xE9 " +
              ("L" === e.position ? "GAUCHE" : "DROITE") +
              " de " +
              r.sponsor_name +
              " (" +
  ...
  if ("M3" === t) {
    ((document.getElementById("reg-title").textContent = "Inscription — Via lien parrain"),
      document.getElementById("reg-sponsor-field").classList.add("hidden"));
    ...
    if (s)
      try {
        ...
        e.found
          ? ((document.getElementById("reg-sponsor").value = e.id),
            t.classList.remove("hidden"),
            (document.getElementById("sponsor-banner-text").textContent = "Parrain\xE9(e) par : " + e.name + " (" + e.unique_id + ")"))
          : "no_license" === e.reason
            ? (n.classList.remove("hidden"),
              (document.getElementById("slot-error-text").textContent =
                e.message || "Ce lien de parrainage est inactif (licence du parrain expirée ou non activée)."),
              (a.disabled = !0))
            : (t.classList.remove("hidden"),
              (document.getElementById("sponsor-banner-text").textContent = "Inscription sans parrain (lien introuvable)"));
      } catch (e) {
...
async function doRegister() {
  ...
  for (const fk of reqFields) {
    if (fieldMap[fk] && !fieldMap[fk].val)
      return ((c.textContent = "Le champ «" + fieldMap[fk].label + "» est obligatoire."), void c.classList.remove("hidden"));
  }
  const m = {
    first_name: n,
    last_name: a,
  ...
  if (
    "M2" === t &&
    ((m.registration_method = "M2"),
    (m.sponsor_id = document.getElementById("reg-sponsor").value.trim()),
    (m.binary_parent_id = document.getElementById("reg-binary-parent").value.trim()),
    (m.binary_position = e.position),
    !m.binary_parent_id || !m.sponsor_id)
  )
    return ((c.textContent = "Slot invalide — rechargez la page"), void c.classList.remove("hidden"));
  if ("M3" === t) {
  ...
  try {
    ...
    ((c.textContent = e.error || "Erreur lors de l'inscription"),
      c.classList.remove("hidden"),
      (p.disabled = !1),
      (x.textContent = "Créer mon compte"));
  }
}
function showPinConfirmation(e) {
  (document.getElementById("register-form").classList.add("hidden"),
    document.getElementById("login-form").classList.add("hidden"),
    document.getElementById("pin-confirmation").classList.remove("hidden"));
  const t =
    {
...
function doLogout() {
  (localStorage.removeItem("leader_member_token"),
    (memberToken = null),
    (currentMember = null),
...
function updateSidebar() {
  ...
  ((n.textContent = e.member_status),
    ...
    (document.getElementById("sidebar-rank").textContent = "none" === e.current_rank ? "—" : e.current_rank));
}
async function loadNotifCount() {
  try {
    ...
    e > 0
      ? ((t.textContent = e), t.classList.remove("hidden"), (n.textContent = e), n.classList.remove("hidden"))
      ...
      ((t.recentCommissions || [])
        .map((e) => {
          ...
          return (
            ...
            '">' +
            (isCredit ? "+" : "−") +
            fmt$(Math.abs(e.amount ?? 0)) +
            '</td>\n\n                <td class="text-gray-500">' +
            fmtDate(e.created_at) +
            "</td>\n\n              </tr>"
          );
        })
        .join("") || '<tr><td colspan="4" class="text-center text-gray-500 py-8">Aucune opération pour le moment</td></tr>') +
      "</tbody>\n\n          </table>\n\n        </div>\n\n      </div>\n\n    </div>";
  } catch (t) {
...
function switchTeamTab(e) {
  (["directs", "downline"].forEach((t) => {
    ...
    const s = t === e;
    ((n.className = s
      ? "px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-dark-600 text-white"
      : "px-4 py-2 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white"),
      a.classList.toggle("hidden", !s));
  }),
...
async function loadTeam(e = "", page, perPage) {
  ...
  if (t) {
    ...
    try {
      ...
      t.innerHTML =
        '\n\n      <div class="overflow-x-auto">\n\n        <table class="data-table">\n\n          <thead><tr>\n\n            <th>ID</th><th>Nom</th><th ' +
        'class="text-blue-400"><i class="fas fa-arrow-left mr-1"></i>BV Gauche</th><th class="text-green-400"><i class="fas fa-arrow-right mr-1"></i>BV Droite<' +
        '/th><th>Statut</th><th>Rang</th>\n\n            <th title="Nombre de filleuls directs">Filleuls</th><th>Inscription</th>\n\n          </tr></thead>\n\n      ' +
        "    <tbody>\n\n            " +
        ((n.members || [])
          .map(
            (e) =>
        ...
        ('\n\n          </tbody>\n\n        </table>\n\n      </div>\n\n      <div class="px-6 py-3 text-sm text-gray-500 border-t border-dark-600 flex items-center ' +
          'justify-between gap-4"><span>') +
        (n.total || 0) +
        ' filleul(s) direct(s)</span><div id="team-pagination"></div></div>';
      renderPagination(
        "team-pagination",
        n.total || 0,
        page,
        perPage,
        "(function(p){loadTeam(window._teamSearch,p,window._teamPerPage)})",
        "(function(pp,p){loadTeam(window._teamSearch,p,pp)})"
      );
    } catch (e) {
      ...
      try {
        ...
        if (!s.tree || 0 === s.tree.length)
          return (
            (_downlineCache =
              ...
              '        <p class="text-gray-400 font-medium">Votre réseau de parrainage est vide</p>\n        <p class="text-gray-600 text-sm mt-1">Les personnes que ' +
              "vous parrainez apparaîtront ici, ainsi que leurs propres filleuls.</p>\n      </div>"),
            void (t.innerHTML = _downlineCache)
          );
        function n(e, t) {
          const s = 24 * t,
            r = 0 === t,
          ...
          return (
            ...
            (d
              ? '\n\n        <div id="' +
                o +
                '-children" class="border-l border-dark-600 ml-4"\n\n             style="padding-left:0;margin-top:2px">\n\n          ' +
        ...
        const r =
          ...
          i =
            ...
            'class="flex items-center gap-2">\n\n          <i class="fas fa-sitemap text-indigo-400"></i>\n\n          <span class="font-semibold text-white">Arbre de ' +
            'parrainage</span>\n\n          <span class="text-xs text-gray-500">— qui a parrainé qui dans votre réseau</span>\n\n        </div>\n\n        <button ' +
            'onclick="loadDownline(true)" class="text-xs text-gray-400 hover:text-white transition px-2 py-1 rounded bg-dark-700 border border-dark-600">\n' +
...
function svgAssignPositions(e, t, n) {
  ...
  return (
    (e._x = n + a - 95),
    (e._y = 188 * t),
    e.left && svgAssignPositions(e.left, t + 1, n),
    e.right && svgAssignPositions(e.right, t + 1, n + a),
    s
  );
}
    ...
    if (null !== i && null !== d) {
      ...
      r.push(
        [
          ...
          '" y="' +
          (t + 72) +
          '" text-anchor="middle" fill="#4B5563" font-size="9" font-family="sans-serif">Cliquer pour s\'inscrire</text>\n\n        </a>'
      );
    }
    return;
  }
...
function svgPanZoomWrapper(e, t, n = "") {
  return (
    ...
    ('\',1.25)" style="width:32px;height:32px;background:#1F2937;border:1px solid #374151;border-radius:8px;color:#9CA3AF;font-size:16px;cursor:pointer;displ' +
      'ay:flex;align-items:center;justify-content:center;" title="Zoom +">+</button>\n\n      <button onclick="svgZoom(\'') +
    t +
    ('\',0.8)"  style="width:32px;height:32px;background:#1F2937;border:1px solid #374151;border-radius:8px;color:#9CA3AF;font-size:16px;cursor:pointer;displ' +
      'ay:flex;align-items:center;justify-content:center;" title="Zoom −">−</button>\n\n      <button onclick="svgReset(\'') +
    t +
    ('\')"     style="width:32px;height:32px;background:#1F2937;border:1px solid #374151;border-radius:8px;color:#9CA3AF;font-size:13px;cursor:pointer;displa' +
      'y:flex;align-items:center;justify-content:center;" title="Réinitialiser">Haut</button>\n\n    </div>\n\n    <div ' +
      'style="position:absolute;bottom:12px;left:12px;font-size:10px;color:#6B7280;pointer-events:none;"><i class="fas fa-hand-paper" ' +
      'style="margin-right:4px"></i>Glisser · Molette / boutons</div>\n\n  </div>')
  );
}
function svgInitPanZoom(e) {
  ...
  (t.addEventListener(
    ...
    t.addEventListener(
      ...
      (e) => {
        1 === e.touches.length &&
          ((a.tx += e.touches[0].clientX - d), (a.ty += e.touches[0].clientY - o), (d = e.touches[0].clientX), (o = e.touches[0].clientY), s());
      },
      {
        passive: !0
      }
    ...
    t.addEventListener(
      ...
      (e) => {
        ...
        const n = t.getBoundingClientRect(),
          r = e.clientX - n.left,
          i = e.clientY - n.top,
          l = e.deltaY > 0 ? 0.88 : 1.14,
          d = Math.min(3, Math.max(0.15, a.scale * l));
...
function svgZoom(e, t) {
  ...
  const s = document.getElementById(e + "-canvas"),
    ...
    i = r.width / 2,
    l = r.height / 2,
    d = Math.min(3, Math.max(0.15, n.scale * t));
  ((n.tx = i - (i - n.tx) * (d / n.scale)),
    (n.ty = l - (l - n.ty) * (d / n.scale)),
    (n.scale = d),
    (s.style.transform = "translate(" + n.tx + "px," + n.ty + "px) scale(" + n.scale + ")"));
}
function svgReset(e) {
  ...
  if (0 === n.length)
    return (
      (t.innerHTML =
        ...
        'Aucun résultat pour <strong class="text-gray-400 ml-1">"' +
        e +
        '"</strong>\n\n      </div>'),
...
function initTreeSearch() {
  ...
  e &&
    t &&
    (e.addEventListener("input", (e) => renderTreeSearchResults(e.target.value)),
    e.addEventListener("focus", (e) => {
      e.target.value && renderTreeSearchResults(e.target.value);
    }),
    e.addEventListener("blur", () => setTimeout(() => t.classList.add("hidden"), 150)),
...
function renderClassicNode(e, t, n = !1) {
  ...
  const o = (e, t) => {
    ...
    return (
      ...
      ('" target="_blank"\n\n         class="tree-node-card empty group cursor-pointer hover:border-rouge-500/60 hover:bg-rouge-500/5 transition-all">\n\n        <' +
        ...
        'justify-center mx-auto mb-2 transition-colors">\n\n          <i class="fas fa-plus text-gray-600 group-hover:text-rouge-400 text-xs transition-colors"><' +
  ...
  return (
    ...
    ('\n\n    <div class="tree-connector"></div>\n\n    <div class="tree-children">\n\n      <div class="flex flex-col items-center">\n\n        <div ' +
      'class="text-[10px] text-blue-400 mb-1 font-medium">Gauche</div>\n\n        ') +
...
function renderClassicTreeInner() {
  ...
  const t = _classicFocus.id === _classicRoot.id,
    ...
    a =
      _classicNavStack.length > 0
        ? _classicNavStack
            .map(
              (e, t) =>
                '\n\n        <span class="text-gray-500 text-xs">' +
                (0 === t ? '<i class="fas fa-home text-[10px]"></i>' : "") +
                " " +
                e.first_name +
...
function _updateTreeToggleBtn(e) {
  ...
  t &&
    ("classic" === e
      ? ((t.innerHTML = '<i class="fas fa-diagram-project mr-1.5"></i>Vue SVG'), (t.title = "Passer en vue SVG (pan/zoom)"))
      : ((t.innerHTML = '<i class="fas fa-sitemap mr-1.5"></i>Vue Classique'), (t.title = "Passer en vue classique (navigation par clic)")));
}
function _renderTreeWithMode(e) {
  if (_fullTree)
    if ("classic" === e) (_classicFocus || ((_classicFocus = _classicRoot), (_classicNavStack = [])), renderClassicTreeInner());
    else {
      const e = document.getElementById("member-tree-wrap");
      if (!e) return;
      e.innerHTML = "";
...
async function renderTree(e) {
  ...
  ((e.innerHTML =
    ...
    _updateTreeToggleBtn(t));
  try {
    const e = await api("GET", "/members/tree?depth=12");
    if (!e.tree)
      return void (document.getElementById("member-tree-wrap").innerHTML =
        '\n        <div class="bg-dark-800 rounded-2xl border border-dark-600 p-10 text-center text-gray-400">\n          <i class="fas fa-tree text-4xl ' +
        "text-gray-600 mb-4 block\"></i>\n          Vous n'êtes pas encore placé dans l'arbre binaire.\n        </div>");
    ((_fullTree = e.tree), (_classicRoot = e.tree), (_classicFocus = e.tree), (_classicNavStack = []), initTreeSearch(), _renderTreeWithMode(t));
  } catch (e) {
    document.getElementById("member-tree-wrap").innerHTML =
      '<div class="bg-dark-800 rounded-2xl border border-dark-600 p-6 text-red-400">Erreur : ' + (e.error || e.message) + "</div>";
  }
}
async function renderBV(e, page, perPage) {
  ...
  e.innerHTML =
    '<div class="space-y-4"><h2 class="text-xl font-bold">Journal BV</h2><div class="bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden"><div ' +
    'id="bv-table"></div></div></div>';
  try {
    ...
    var rows =
      (d.logs || [])
        .map(function (l) {
          var src = l.first_name
            ? '<div class="font-medium text-sm text-white">' +
              l.first_name +
              " " +
              l.last_name +
              '</div><div class="text-xs text-rouge-400/70 font-mono">' +
              l.source_unique_id +
              "</div>"
    ...
    renderPagination(
      ...
      page,
      perPage,
      "(function(p){renderBV(document.getElementById('page-content'),p,window._bvPerPage)})",
      "(function(pp,p){renderBV(document.getElementById('page-content'),p,pp)})"
    );
  } catch (err) {
...
async function renderCommissions(e, page, perPage) {
  ...
  try {
    ...
    let html =
      ...
      'font-bold text-white">Mes Commissions</h1>\n          <p class="text-gray-400 text-sm mt-0.5">Suivi complet de vos bonus et leur libération</p>\n        ' +
      '</div>\n        <button onclick="showPage(\'commissions-faq\')" class="flex items-center gap-2 text-xs text-blue-400 hover:text-blue-300 ' +
      'transition-colors border border-blue-500/30 rounded-lg px-3 py-1.5">\n          <i class="fas fa-question-circle"></i> Comprendre mes commissions\n' +
      "        </button>\n      </div>";
    if (activePWE.length > 0) {
      ...
      if (pendingPWE.length > 0) {
        html +=
          ...
          'class="fas fa-hourglass-half"></i> En attente de libération (' +
          pendingPWE.length +
          ')\n          </h2>\n          <div class="space-y-4">\n            ' +
      ...
      if (inProgressPWE.length > 0) {
        html +=
          ...
          'fa-unlock"></i> En cours de libération (' +
      ...
      if (completedPWE.length > 0) {
        ...
        html +=
          ...
          'class="fas fa-check-circle"></i> Entièrement libérées (' +
          completedPWE.length +
          ')\n          </h2>\n          <div class="space-y-4">\n            ' +
          last5.map((p) => pweCard(p)).join("") +
          "\n          </div>\n          " +
          (completedPWE.length > 5
            ? '<p class="text-xs text-gray-500 mt-2 text-center">' +
              (completedPWE.length - 5) +
              " autres commissions termin\xE9es dans l'historique</p>"
            : "") +
          "\n        </div>";
      }
    } else {
    ...
    html +=
      ...
      "/i> Historique de toutes mes commissions\n      </h2>";
    ...
    e.innerHTML =
      '<div class="stat-card text-center py-10 text-red-400"><i class="fas fa-exclamation-circle text-3xl mb-3"></i><p>Erreur de chargement des commissions.<' +
      "/p></div>";
    console.error(err);
  }
}
function renderCommissionsFAQ(e) {
  e.innerHTML =
    ...
    ('            <span class="text-xs font-bold px-2 py-1 rounded-full border bg-orange-500/20 text-orange-300 border-orange-500/30 whitespace-nowrap">\n    ' +
      ...
      'gap-3 p-3 rounded-lg bg-dark-700/60">\n            <span class="text-xs font-bold px-2 py-1 rounded-full border bg-blue-500/20 text-blue-300 ' +
      'border-blue-500/30 whitespace-nowrap">\n') +
    ('              <i class="fas fa-droplet mr-1"></i>En cours de libération\n            </span>\n            <div class="text-xs text-gray-400">\n           ' +
      ...
      'items-center gap-3 p-3 rounded-lg bg-dark-700/60">\n            <span class="text-xs font-bold px-2 py-1 rounded-full border bg-emerald-500/20 ' +
      'text-emerald-300 border-emerald-500/30 whitespace-nowrap">\n              <i class="fas fa-check-circle mr-1"></i>Entièrement libéré\n') +
    ('            </span>\n            <div class="text-xs text-gray-400">\n              100% du montant est dans votre portefeuille disponible. Retrait ' +
      "possible.\n            </div>\n          </div>\n\n        </div>\n      </div>\n    </div>\n\n    <!-- FAQ Questions fréquentes -->\n    <div " +
      'class="rounded-xl border border-dark-500/60 overflow-hidden bg-dark-800/60">\n      <div class="p-5">\n        <div class="flex items-center gap-2 mb-4">' +
    ...
    [
      {
        ...
        a:
          "Le système applique un délai de sécurité de 14 jours pour prévenir toute fraude ou annulation de package. Après " +
          "ce délai, la libération commence dès " +
          "le 1er du mois suivant."
      },
      {
        q: "Puis-je retirer avant la fin de la libération ?",
        a:
          "Vous pouvez retirer uniquement ce qui est dans votre portefeuille principal (solde disponible). Les montants encore en cours de libération (pending) " +
          "ne sont pas retirables."
      },
      {
        q: "Qu'est-ce que le 'rattrapage' ?",
        a:
          "Lorsque la date éligible de votre commission est après le début du mois, tous les jours manqués sont versés en une seule fois lors du premier " +
          "paiement. C'est un versement plus important que les jours suivants."
      },
      {
      ...
      },
      {
        ...
        a:
          "Non. Le reset BV efface uniquement les volumes mensuels utilisés pour calculer de nouvelles commissions. Les fiches de libération déjà créées " +
          "continuent leur versement normalement."
      },
      {
        q: "Comment savoir quand ma commission sera 100% libérée ?",
        a:
          "Chaque fiche affiche la date de fin et le nombre de jours restants. La date de fin = date de début + nombre de jours du mois. Vous recevrez aussi une " +
          "notification '✅ Commission entièrement libérée'."
      }
    ]
      .map(
        (item, i) =>
    ...
    ('\n\n        </div>\n      </div>\n    </div>\n\n    <!-- Bouton retour -->\n    <div class="text-center">\n      <button onclick="showPage(\'commissions\')"\n    ' +
      '          class="px-6 py-2.5 rounded-xl bg-rouge-500/15 border border-rouge-500/30 text-rouge-400 text-sm font-medium hover:bg-rouge-500/20 ' +
...
async function showCommissionCalendar(pweId) {
  ...
  modal.innerHTML =
    '<div class="bg-dark-800 rounded-xl border border-dark-500 p-6 max-w-lg w-full"><div class="flex items-center justify-center py-8"><div class="loader"><' +
    "/div></div></div>";
  try {
    ...
    modal.innerHTML =
      ...
      'onclick="document.getElementById(\'calendar-modal\').remove()" class="mt-3 text-xs text-gray-500 hover:text-gray-300">Fermer</button>\n    </div>';
  }
}
async function renderWallet(e) {
  ...
  try {
    ...
    const o = _wdCfg.next_allowed_date
        ? new Date(_wdCfg.next_allowed_date + "T12:00:00").toLocaleDateString("fr-FR", {
            weekday: "long",
            day: "numeric",
            month: "long"
          })
        : _wdCfg.allowed_days_labels && _wdCfg.allowed_days_labels.length
    ...
    e.innerHTML =
      ...
      ('\n\n\n\n      <!-- Raccourcis -->\n\n      <div class="flex gap-3">\n\n        <button onclick="showPage(\'transactions\')" class="flex-1 bg-dark-800 border ' +
        ...
        'text-sm text-gray-300 hover:bg-dark-700 transition text-center">\n' +
...
async function renderWithdraw(el, page, perPage) {
  ...
  try {
    ...
    const nextAllowedStr = wdConfig.next_allowed_date
      ? new Date(wdConfig.next_allowed_date + "T12:00:00").toLocaleDateString("fr-FR", {
          weekday: "long",
          day: "numeric",
          month: "long"
        })
    ...
    const wdStatusBadge = (s) =>
      ({
        pending:
          '<span class="inline-flex items-center gap-1 text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full"><i class="fas fa-clock text-[10px]"></i>' +
          "En attente</span>",
        processing:
          '<span class="inline-flex items-center gap-1 text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full"><i class="fas fa-spinner fa-spin ' +
          'text-[10px]"></i>En cours</span>',
        completed:
          '<span class="inline-flex items-center gap-1 text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full"><i class="fas fa-check text-[10px]"></i>' +
          "Confirmé</span>",
        rejected:
          '<span class="inline-flex items-center gap-1 text-xs bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full"><i class="fas fa-times text-[10px]"></i>' +
          "Rejeté</span>"
    ...
    el.innerHTML = [
      ...
      '\n        </div>\n\n        <!-- Preview récapitulatif dynamique -->\n        <div id="wd-preview" class="hidden bg-dark-700 border border-dark-500 ' +
        ...
        'class="text-green-400 font-medium">$0.00 <span class="text-xs text-gray-600">(offerts)</span></span></div>\n' +
        ('          <div class="h-px bg-dark-500 my-1"></div>\n          <div class="flex justify-between text-sm font-bold"><span class="text-gray-300">Vous ' +
          'recevrez</span><span id="prev-net" class="text-rouge-400 text-base">—</span></div>\n          <div class="flex justify-between text-xs text-gray-500 ' +
    ...
    el.innerHTML =
      '<div class="p-4 text-red-400 flex items-center gap-2"><i class="fas fa-triangle-exclamation"></i>Erreur : ' +
  ...
  showModal(
    [
      ...
      ('</span>\n      </div>\n      <div class="flex justify-between items-center px-4 py-3">\n        <span class="text-sm text-gray-400">Délai estimé</span>\n  ' +
        '      <span class="text-sm text-gray-300">24-48h</span>\n      </div>\n    </div>\n    <div class="bg-yellow-900/20 border border-yellow-500/30 ' +
        'rounded-xl p-3">\n      <p class="text-xs text-yellow-400"><i class="fas fa-triangle-exclamation mr-1"></i>Cette demande sera vérifiée manuellement par ' +
        "notre équipe avant paiement. Vous serez notifié(e) dès validation.</p>\n" +
        ('    </div>\n    <div id="wd-confirm-error" class="hidden text-red-400 text-xs bg-red-900/20 rounded-lg p-2"></div>\n    <div class="flex gap-3">\n      <' +
          'button onclick="closeModal()" class="flex-1 bg-dark-600 text-gray-300 hover:bg-dark-500 font-medium py-2.5 rounded-xl text-sm transition">Annuler<' +
          '/button>\n      <button id="wd-confirm-btn" onclick="doWithdraw()" class="flex-1 bg-rouge-500 text-dark-900 hover:bg-rouge-500 font-bold py-2.5 ' +
          'rounded-xl text-sm transition flex items-center justify-center gap-2"><i class="fas fa-check"></i>Valider</button>\n') +
        "    </div>\n  </div>")
  );
}
...
async function confirmConvertDreamiles(e) {
  ...
  try {
    ...
    (closeModal(),
      showToast("" + t.toLocaleString() + " Dreamiles convertis \u2192 " + fmt$(e.usd_credited) + " cr\xE9dit\xE9s dans votre wallet !"),
      showPage("wallet"));
  } catch (e) {
...
function _buildSourceSection(e) {
  ...
  if ("bonus_influence" === t) {
    ...
    (t &&
      (r =
        '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-300 text-xs font-semibold">\n\n                      <' +
        'i class="fas fa-layer-group text-[10px]"></i> Niveau ' +
        t +
        "\n\n                    </span>"),
      n &&
        (i =
          '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 text-xs font-medium">\n\n                    ' +
          n +
          "%\n\n                  </span>"),
      n &&
        e.amount &&
        (l =
          '\n\n        <div class="flex justify-between items-center py-1.5 border-t border-dark-600 mt-2">\n\n          <span class="text-xs text-gray-500">Prime de ' +
          'base (source)</span>\n\n          <span class="text-xs font-semibold text-gray-200">' +
          fmt$(Math.abs(e.amount) / (n / 100)) +
          "</span>\n\n        </div>"));
  }
  "bonus_rayonnement" === t &&
    ((i =
      '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 text-xs font-medium">\n                  10%\n         ' +
      "       </span>"),
    e.amount) &&
    (l =
      '\n\n        <div class="flex justify-between items-center py-1.5 border-t border-dark-600 mt-2">\n\n          <span class="text-xs text-gray-500">Prime de ' +
      'base (filleul)</span>\n\n          <span class="text-xs font-semibold text-gray-200">' +
      fmt$(Math.abs(e.amount) / 0.1) +
      "</span>\n\n        </div>");
  const d = n || "—";
  return (
    [
...
async function renderTransactions(e, page, perPage) {
  ...
  try {
    ...
    const statusBadgeTx = function (s) {
      ...
      return (
        '<span class="text-[10px] px-1.5 py-0.5 rounded-full ' + (map[s] || "bg-gray-500/15 text-gray-400") + '">' + (lbl[s] || s || "—") + "</span>"
      );
    };
    let rows = "";
    for (const row of t) {
      ...
      rows +=
        ...
        "</td>" +
        '<td class="text-right font-medium text-white text-sm whitespace-nowrap">' +
        balAfter +
        '</td><td class="text-center"><i class="fas fa-chevron-right text-gray-600 group-hover:text-gray-400 text-[10px] transition-colors"></i></td></tr>';
    }
    const emptyState =
      '<div class="bg-dark-800 border border-dark-600 rounded-2xl p-12 text-center"><div class="w-16 h-16 rounded-full bg-dark-700 flex items-center ' +
      'justify-center mx-auto mb-4"><i class="fas fa-receipt text-gray-600 text-2xl"></i></div><h3 class="text-gray-400 font-medium mb-2">Aucune transaction ' +
      'pour le moment</h3><p class="text-gray-600 text-sm">Vos bonus, achats de packages, licences et retraits apparaîtront ici.</p></div>';
    const tableHtml =
      0 === t.length
        ? emptyState
        : '<div class="text-xs text-gray-500 flex items-center gap-2 mb-2"><i class="fas fa-hand-pointer text-gray-600"></i>Cliquez sur une ligne pour voir le ' +
          'détail</div><div class="bg-dark-800 border border-dark-600 rounded-2xl overflow-hidden"><div class="overflow-x-auto"><table class="data-table w-full"><' +
          'thead><tr><th>Date</th><th>Type</th><th>Description</th><th>Statut</th><th class="text-right">Montant</th><th class="text-right">Solde après</th><th ' +
    ...
    renderPagination(
      ...
      page,
      perPage,
      "(function(p){renderTransactions(document.getElementById(\'page-content\'),p,window._txPerPage)})",
      "(function(pp,p){renderTransactions(document.getElementById(\'page-content\'),p,pp)})"
    );
  } catch (err) {
    e.innerHTML =
      '<div class="p-4 text-red-400"><i class="fas fa-exclamation-circle mr-2"></i>Erreur : ' +
      (err.error || err.message || String(err) || "Erreur inconnue") +
      "</div>";
  }
}
async function _showTopupModal() {
  ...
  if (!_gw || !Object.keys(_gw).length) {
    _wizardShow(
      '<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Chargement des moyens de paiement…</p><' +
        "/div>"
    );
    try {
      ...
      _wizardShow(
        '<div class="p-8 text-center space-y-4"><i class="fas fa-exclamation-circle text-red-400 text-3xl"></i><p class="text-red-400">Impossible de charger ' +
          'les modes de paiement.<br><span class="text-xs text-gray-500">' +
          (e.error || e.message || "") +
          '</span></p><button onclick="wizardClose()" class="py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm">Fermer</button></div>'
      );
      return;
    }
  }
  _wizardShow(
    '\n  <div class="px-6 pt-5 pb-3 border-b border-dark-600">\n    <div class="flex items-center justify-between mb-2">\n      <span class="text-xs ' +
      'font-semibold text-green-400 uppercase tracking-wider">Recharger mon Wallet</span>\n      <button onclick="wizardClose()" class="text-gray-400 ' +
      'hover:text-white w-7 h-7 flex items-center justify-center rounded-lg hover:bg-dark-700 text-lg">×</button>\n    </div>\n  </div>\n  <div class="p-6 ' +
      ...
      ('      <p class="text-sm text-gray-400 mt-1">Saisissez le montant que vous souhaitez ajouter à votre wallet.</p>\n    </div>\n    <div>\n      <label ' +
        'class="block text-sm text-gray-400 mb-2">Montant ($)</label>\n      <div class="relative">\n        <span class="absolute left-4 top-1/2 ' +
        '-translate-y-1/2 text-green-400 font-bold text-lg">$</span>\n        <input id="topup-amount-input" type="number" min="10" step="1" placeholder="Ex : ' +
        '100"\n          class="w-full bg-dark-700 border border-dark-500 rounded-xl pl-10 pr-4 py-3 text-white text-lg font-bold focus:border-green-400 ' +
        'focus:outline-none"\n') +
      ('          oninput="document.getElementById(\'topup-amount-err\').classList.add(\'hidden\')">\n      </div>\n      <div id="topup-amount-err" class="hidden ' +
        'mt-2 text-red-400 text-xs"></div>\n    </div>\n    <div class="grid grid-cols-4 gap-2">\n      ') +
      [50, 100, 200, 500]
        .map(
          (v) =>
            "<button onclick=\"document.getElementById('topup-amount-input').value=" +
            v +
            ('" class="py-2.5 bg-dark-700 hover:bg-green-900/30 border border-dark-500 hover:border-green-500/50 rounded-xl text-sm text-gray-300 ' +
              'hover:text-green-300 font-medium transition">$') +
            v +
            "</button>"
        )
        .join("") +
      ('\n    </div>\n    <div class="flex gap-3">\n      <button onclick="wizardClose()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl ' +
        'hover:bg-dark-600 transition text-sm">Annuler</button>\n      <button onclick="_topupConfirmAmount()" class="flex-1 py-3 bg-green-600 text-white ' +
        'font-bold rounded-xl hover:bg-green-500 transition text-sm">\n        <i class="fas fa-arrow-right mr-2"></i>Choisir le paiement\n      </button>\n    <' +
        "/div>\n  </div>")
  );
}
...
async function _showTransferModal() {
  ...
  const fee =
    transferCfg.fee_type === "percent"
      ? "" + transferCfg.fee_value + "%"
      : transferCfg.fee_value > 0
    ...
    [
      ...
      '</span></p>\n        </div>\n      </div>\n      <button onclick="closeModal()" class="text-gray-400 hover:text-white w-8 h-8 flex items-center ' +
        ...
        ("          oninput=\"this.value=this.value.toUpperCase();document.getElementById('tr-recipient-info').innerHTML=''\">\n        <button " +
          'onclick="_transferLookup()" class="px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-medium transition">\n          <i ' +
      ...
      ('</p>\n    </div>\n\n    <div>\n      <label class="block text-sm text-gray-400 mb-1.5">PIN de sécurité <span class="text-red-400">*</span></label>\n      <' +
        'input id="tr-pin" type="password" maxlength="6" placeholder="Votre PIN (4-6 chiffres)"\n        class="w-full bg-dark-700 border border-dark-500 ' +
        'rounded-xl px-4 py-3 text-white focus:border-blue-400 focus:outline-none text-sm tracking-widest">\n    </div>\n\n    <div>\n      <label class="block ' +
        'text-sm text-gray-400 mb-1.5">Note (optionnel)</label>\n      <input id="tr-note" type="text" placeholder="Motif du transfert…"\n' +
        ('        class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-blue-400 focus:outline-none ' +
          'text-sm">\n    </div>\n\n    <div id="tr-error" class="hidden bg-red-900/20 border border-red-500/30 rounded-xl p-3 text-red-400 text-sm"></div>\n\n    <' +
          'div class="flex gap-3">\n      <button onclick="closeModal()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl hover:bg-dark-600 transition ' +
          'text-sm">Annuler</button>\n      <button onclick="_doTransfer()" id="tr-submit-btn" class="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl ' +
...
async function _transferLookup() {
  ...
  try {
    ...
    if (infoEl)
      infoEl.innerHTML =
        '<div class="flex items-center gap-2 mt-1 bg-green-900/20 border border-green-500/30 rounded-lg px-3 py-2"><i class="fas fa-check-circle text-green-400 ' +
        'text-sm"></i><span class="text-green-300 text-sm font-medium">' +
        r.recipient.name +
        ' <span class="text-gray-400 font-normal">(' +
        r.recipient.unique_id +
        ")</span></span></div>";
  } catch (e) {
    if (infoEl)
      infoEl.innerHTML = '<p class="text-red-400 text-xs mt-1"><i class="fas fa-times-circle mr-1"></i>' + (e.error || "Membre introuvable") + "</p>";
  }
}
async function _doTransfer() {
  ...
  try {
    ...
    showToast(
      "$" +
        Number(r.amount_sent || amount).toLocaleString("en-US", {
          minimumFractionDigits: 2
        }) +
        " envoy\xE9s avec succ\xE8s !",
      "success"
    );
    setTimeout(() => {
...
async function _walletPayOrder() {
  ...
  try {
    ...
    if (r.new_balance !== undefined) {
      ...
      if (hb) {
        hb.innerHTML =
          '<span class="text-gray-400">Solde : </span><span class="text-rouge-400 font-bold">$' +
          Number(r.new_balance).toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }) +
          "</span>";
...
window._wizardOpenForCourse = function (courseId, title, priceUsd) {
  ...
  if (!document.getElementById("wizard-overlay")) {
    ...
    ov.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.82);z-index:9999;display:none;align-items:center;justify-content:center;padding:1rem;";
    ov.innerHTML =
      '<div id="wizard-box" class="bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-lg shadow-2xl" style="max-height:92vh;overflow-y:auto;"></div>';
  ...
  if (box)
    box.innerHTML =
      '<div style="padding:2.5rem;display:flex;flex-direction:column;align-items:center;gap:1rem;"><div class="loader"></div><p ' +
      'style="color:#9ca3af;font-size:.875rem;">Chargement du paiement…</p></div>';
  ...
  return (
    ...
    new Promise((e) => {
      ...
      ((t.src =
        ...
        _ppClientId +
        "&currency=USD&components=buttons&enable-funding=card,paylater&disable-funding=venmo"),
        (t.onload = () => {
...
async function _renderPayPalButtons(e, t, n, a) {
  (await _loadPayPalSdk()) && "undefined" != typeof paypal
    ? paypal
        .Buttons({
          style: {
            layout: "vertical",
          ...
          onApprove: async (t) => {
            document.getElementById(e).innerHTML =
              '<div class="flex flex-col items-center gap-3 py-4"><div class="loader"></div><p class="text-gray-400 text-sm">Confirmation du paiement…</p></div>';
            try {
          ...
          onCancel: () => {
            document.getElementById(e).innerHTML =
              '<div class="text-yellow-400 text-sm text-center py-2"><i class="fas fa-times-circle mr-1"></i>Paiement annulé. Cliquez sur « Retour » pour ' +
              "recommencer.</div>";
          }
        })
        .render("#" + e)
    : (document.getElementById(e).innerHTML =
        '<div class="text-red-400 text-sm text-center py-2"><i class="fas fa-exclamation-triangle mr-1"></i>SDK PayPal indisponible. Rechargez ou utilisez une ' +
        "autre méthode.</div>");
}
function _buildGw(e) {
  return {
    ...
    (() => {
      ...
      if (e.payment_mode === "broker") {
        ...
        if (_brReg && _brReg.status === "activated") {
          return (
            '<div class="w-full text-center py-3.5 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-sm font-semibold flex items-center ' +
            'justify-center gap-2"><i class="fas fa-check-circle"></i>Package actif — BV déployés</div>'
          );
        }
        if (_brReg && (_brReg.status === "deposited" || _brReg.status === "registered")) {
          ...
          return (
            '<div class="w-full text-center py-3.5 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-300 text-sm font-semibold flex items-center ' +
            'justify-center gap-2"><i class="fas fa-clock"></i>' +
            _brLabel +
            "</div>"
          );
        }
        return (
          "<button onclick=\"brokerRedirect('" +
          e.id +
          "','" +
          encodeURIComponent(e.name) +
          ('\',event)" class="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-500 hover:to-orange-400 text-white font-bold py-3.5 ' +
            'rounded-xl transition-all duration-200 text-sm shadow-lg flex items-center justify-center gap-2"><i class="fas fa-external-link-alt"></i>' +
            "S&#39;inscrire chez Triomarkets</button>")
        );
      }
      if (sameOwned) {
        return (
          '<div class="w-full text-center py-3.5 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-sm font-semibold flex items-center ' +
          'justify-center gap-2"><i class="fas fa-check-circle"></i>Package actif</div>'
        );
      }
      if (superiorInCat) {
        return (
          '<div class="w-full text-center py-3.5 rounded-xl bg-gray-700/50 border border-gray-600/40 text-gray-500 text-sm font-semibold flex items-center ' +
          'justify-center gap-2 cursor-not-allowed select-none"><i class="fas fa-lock mr-1"></i>Niveau d\'abord atteint — Upgrade requis</div>'
        );
      }
      if (activeOrder) {
        return (
          ...
          (')" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl transition-all duration-200 text-sm shadow-lg flex ' +
            'items-center justify-center gap-2"><i class="fas fa-arrow-up"></i>Upgrader depuis ') +
          (activeOrder.package_name || "mon package") +
          "</button>"
        );
      }
      return (
        ...
        (' text-white font-bold py-3.5 rounded-xl transition-all duration-200 text-sm shadow-lg flex items-center justify-center gap-2"><i class="fas ' +
          'fa-shopping-cart"></i>') +
        (s ? "S'abonner maintenant" : "Acquérir ce package") +
        "</button>"
      );
    })() +
    "\n\n    </div>\n\n  </div>"
...
async function renderPackages(e) {
  e.innerHTML = '<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';
  try {
    const [t, _bsRes] = await Promise.all([
      api("GET", "/members/packages"),
      api("GET", "/broker/status").catch(() => ({
        registrations: []
      }))
    ]);
    window._brokerRegs = _bsRes.registrations || [];
    ((_gw = _buildGw(t.gateways || {})),
      (_adminFeeInfo = t.adminFee || {
        amount: 0,
        active: !1,
        paid: !0
      }));
    ...
    e.innerHTML =
      ...
      'class="text-gray-400 text-sm mt-1">Sélectionnez une catégorie pour découvrir les offres.</p>\n\n      </div>\n\n      <div class="grid grid-cols-1 ' +
...
function showPackageCategory(e) {
  const { pkgByCategory: t, categoryMeta: n } = window._pkgData || {};
  if (!t || !t[e]) return;
  const a = t[e],
    s = n[e],
    r = CAT_THEME[e] || CAT_THEME._none,
  ...
  (d.classList.remove("hidden"),
    (d.innerHTML =
      ...
      ('\n\n  </div>\n\n\n\n  <!-- Retour bas -->\n\n  <div class="mt-10 text-center">\n\n    <button onclick="backToCategories()"\n\n      class="inline-flex ' +
        'items-center gap-2 text-sm text-gray-400 hover:text-white border border-dark-600 hover:border-dark-400 px-5 py-2.5 rounded-xl transition-all">\n\n      <' +
        'i class="fas fa-th-large"></i>\n\n      Voir toutes les catégories\n\n    </button>\n\n  </div>')));
}
function backToCategories() {
  (document.getElementById("pkg-view-categories").classList.remove("hidden"),
    document.getElementById("pkg-view-detail").classList.add("hidden"),
    window.scrollTo({
      top: 0,
      behavior: "smooth"
    }));
...
function renderMyOrders(e) {
  ...
  return (
    '\n\n  <div class="bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden">\n\n    <div class="px-6 py-4 border-b border-dark-600 flex items-center ' +
...
function wizardStart(e, t, n, a, s, r, i) {
  (_wizardReset(),
    (_wizardData.pkgId = e),
    (_wizardData.pkgName = decodeURIComponent(t)),
    (_wizardData.price = Number(n)),
    (_wizardData.bv = Number(a)),
    (_wizardData.isUpgrade = !!s),
    (_wizardData.currentPrice = Number(r || 0)),
    (_wizardData.currentBV = Number(i || 0)),
    (_wizardData.diffPrice = _wizardData.isUpgrade ? Math.max(0, _wizardData.price - _wizardData.currentPrice) : _wizardData.price),
    (_wizardData.diffBV = _wizardData.isUpgrade ? Math.max(0, _wizardData.bv - _wizardData.currentBV) : _wizardData.bv),
    (_wizardData.addLicense = !1),
    (_wizardData.licensePrice = typeof _licPriceForWizard !== "undefined" ? _licPriceForWizard : 97));
    ...
    [
      ...
      (d
        ? '\n\n      <div class="bg-orange-900/20 border border-orange-500/30 rounded-lg p-3 text-sm text-orange-300">\n\n        <i class="fas fa-info-circle mr-2"><' +
          ...
          o.toFixed(2) +
          "</strong>" +
          ("once" === l ? " (1ère commande uniquement)" : " (facturés à chaque achat)") +
          "\n\n      </div>"
        : '\n      <div class="bg-green-900/10 border border-green-500/20 rounded-lg p-3 text-sm text-green-400">\n        <i class="fas fa-check-circle mr-2"></i>' +
          "Frais d'administration : déjà réglés\n      </div>") +
      ('\n\n    </div>\n\n    <div class="flex gap-3">\n\n      <button onclick="wizardClose()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl ' +
        'hover:bg-dark-600 transition text-sm">Annuler</button>\n\n      <button onclick="wizardStep2()" class="flex-1 py-3 bg-rouge-500 text-dark-900 font-bold ' +
        'rounded-xl hover:bg-rouge-500 transition text-sm">\n\n        Continuer <i class="fas fa-arrow-right ml-2"></i>\n\n      </button>\n\n    </div>\n\n  </div>')
  );
}
...
function wizardStep3() {
  ...
  _wizardShow(
    [
      ...
      _wizardData.diffBV +
      " BV" +
      (_wizardData.isUpgrade ? " (différentiel)" : "") +
      ('</span>\n\n      </div>\n\n    </div>\n\n    <div class="flex gap-3">\n\n      <button onclick="wizardStep2()" class="flex-1 py-3 bg-dark-700 text-gray-300 ' +
        'rounded-xl hover:bg-dark-600 transition text-sm"><i class="fas fa-arrow-left mr-2"></i>Retour</button>\n\n      <button onclick="wizardStep4()" ' +
        'class="flex-1 py-3 bg-rouge-500 text-dark-900 font-bold rounded-xl hover:bg-rouge-500 transition text-sm">\n\n        Choisir le paiement <i class="fas ' +
        'fa-arrow-right ml-2"></i>\n\n      </button>\n\n    </div>\n\n  <' +
        "/div>")
  );
}
async function wizardStep4() {
  ...
  if (!_gw || !Object.keys(_gw).length) {
    ...
    try {
      ...
      ((_gw = _buildGw(e.gateways || {})),
        (_adminFeeInfo = e.adminFee || {
          amount: 0,
          active: !1,
          paid: !0
        }));
      ...
      return void _wizardShow(
        '<div class="p-8 text-center space-y-4">\n\n        <i class="fas fa-exclamation-circle text-red-400 text-3xl"></i>\n\n        <p class="text-red-400">' +
          'Impossible de charger les modes de paiement.<br><span class="text-xs text-gray-500">' +
          (e.error || e.message || "") +
  ...
  try {
    ...
    _v2Methods = _v2all.filter(
      (m) =>
        m.is_automatic &&
        m.is_active &&
        !["stripe", "paypal", "coinpayments", "wallet", "internal_wallet", "internal", "leader_wallet"].includes(m.provider)
    );
  } catch (_v2e) {
    _v2Methods = [];
  }
  const _v2Btns = _v2Methods
    .map((m) => {
  ...
  try {
    ...
    _v2ManualMethods = _v2all2.filter(
      (m) => !m.is_automatic && m.is_active && !["leader_wallet", "internal_wallet", "internal", "wallet"].includes(m.provider)
    );
  } catch (_v2e2) {
    _v2ManualMethods = [];
  }
  const _v2ManualBtns = _v2ManualMethods
    .map((m) => {
      ...
      return (
        ...
        ('</div>\n\n    </div>\n\n    <span class="ml-auto text-[9px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded-full ' +
          'font-semibold whitespace-nowrap">Mobile Money</span>\n\n  </button>')
      );
    })
    .join("");
  let _preWbal = 0;
...
function _buildPayButtons(_d) {
  ...
  const l = !(n || a || s || r || i || cp_enabled || _v2Methods.length > 0 || true),
    d = i
      ? '\n  <button onclick="wizardSelectPayment(\'stripe\')"\n    class="w-full border-2 border-purple-500/40 rounded-xl p-4 text-left hover:border-purple-400 ' +
        'transition flex items-center gap-4 bg-purple-900/10">\n    <div class="flex gap-1.5 w-8 justify-center">\n      <i class="fas fa-credit-card ' +
        'text-purple-400 text-xl"></i>\n    </div>\n    <div>\n      <div class="font-bold text-white">Carte bancaire <span class="text-purple-400">/ Apple Pay / ' +
        'Google Pay</span></div>\n      <div class="text-xs text-gray-400">Paiement immédiat · Visa, Mastercard, Amex, Apple Pay, Google Pay</div>\n' +
        ('    </div>\n    <span class="ml-auto text-[9px] bg-green-500/20 text-green-400 border border-green-500/30 px-2 py-0.5 rounded-full font-semibold ' +
          'whitespace-nowrap">Recommandé</span>\n  </button>')
      : "",
    cp = cp_enabled
      ? '\n  <button onclick="wizardSelectPayment(\'coinpayments\')"\n    class="w-full border-2 border-orange-500/40 rounded-xl p-4 text-left ' +
        'hover:border-orange-400 transition flex items-center gap-4 bg-orange-900/10">\n    <div class="flex gap-1.5 w-8 justify-center">\n      <i class="fas ' +
        'fa-coins text-orange-400 text-xl"></i>\n    </div>\n    <div>\n      <div class="font-bold text-white">Cryptomonnaie <span class="text-orange-400">via ' +
        'CoinPayments</span></div>\n      <div class="text-xs text-gray-400">Bitcoin, Ethereum, USDT TRC20, Litecoin et plus</div>\n' +
        ('    </div>\n    <span class="ml-auto text-[9px] bg-orange-500/20 text-orange-400 border border-orange-500/30 px-2 py-0.5 rounded-full font-semibold ' +
          'whitespace-nowrap">Crypto</span>\n  </button>')
      : "",
    o = n
      ? '\n  <button onclick="wizardSelectPayment(\'paypal\')"\n    class="w-full border-2 border-blue-500/40 rounded-xl p-4 text-left hover:border-blue-400 ' +
        'transition flex items-center gap-4 bg-blue-900/10">\n    <div class="flex gap-1.5 w-8 justify-center">\n      <i class="fab fa-paypal text-blue-400 ' +
        'text-xl"></i>\n      <i class="far fa-credit-card text-blue-300 text-xl"></i>\n    </div>\n    <div>\n      <div class="font-bold text-white">PayPal <span ' +
        'class="text-blue-400">/ Carte bancaire</span></div>\n      <div class="text-xs text-gray-400">Paiement immédiat et sécurisé · Visa, Mastercard, PayPal<' +
        "/div>\n" +
        "    </div>\n  </button>"
      : "",
    c = a
      ? '\n\n  <button onclick="wizardSelectPayment(\'bank\')"\n\n    class="w-full border-2 border-dark-500 rounded-xl p-4 text-left hover:border-green-400 ' +
        ...
        'text-white">Virement bancaire</div><div class="text-xs text-gray-400">2–3 jours ouvrables · ' +
        (t.bank_beneficiary || t.bank_name) +
        "</div></div>\n\n  </button>"
      : "",
    x = s
      ? '\n\n  <button onclick="wizardSelectPayment(\'crypto\')"\n\n    class="w-full border-2 border-dark-500 rounded-xl p-4 text-left hover:border-orange-400 ' +
        'transition flex items-center gap-4">\n\n    <i class="fab fa-bitcoin text-orange-400 text-2xl w-8 text-center"></i>\n\n    <div><div class="font-bold ' +
        'text-white">Crypto (' +
        (t.crypto_network || "USDT") +
        ')</div><div class="text-xs text-gray-400">R\xE9seau : ' +
        (t.crypto_network || "TRC20") +
        "</div></div>\n\n  </button>"
      : "",
    p = r
      ? '\n  <button onclick="wizardSelectPayment(\'manual\')"\n    class="w-full border-2 border-dark-500 rounded-xl p-4 text-left hover:border-gray-400 ' +
        'transition flex items-center gap-4">\n    <i class="fas fa-info-circle text-gray-400 text-2xl w-8 text-center"></i>\n    <div><div class="font-bold ' +
        'text-white">Autre méthode</div><div class="text-xs text-gray-400">Voir instructions</div></div>\n  </button>'
      : "";
  window._wPmtSimple = {
    l: l,
    d: d,
...
function _buildWalletBtn() {
  ...
  if (_wbal > 0) {
    w = _canW
      ? '\n\n  <button onclick="wizardSelectPayment(\'wallet\')"\n\n    class="w-full border-2 border-green-500/40 rounded-xl p-4 text-left hover:border-green-400 ' +
        'transition flex items-center gap-4 bg-green-900/10">\n\n    <i class="fas fa-wallet text-green-400 text-2xl w-8 text-center"></i>\n\n    <div ' +
        'class="flex-1">\n\n      <div class="font-bold text-white">Wallet LEADER</div>\n\n      <div class="text-xs text-gray-400">Solde : <span ' +
        'class="text-green-400 font-semibold">$' +
        _wbal.toLocaleString("en-US", {
          minimumFractionDigits: 2
        }) +
        ('</span></div>\n\n    </div>\n\n    <span class="ml-auto text-[9px] bg-green-500/20 text-green-400 border border-green-500/30 px-2 py-0.5 rounded-full ' +
          'font-semibold">Immédiat</span>\n\n  </button>')
      : '\n\n  <div class="w-full border-2 border-dark-600 rounded-xl p-4 flex items-center gap-4 opacity-50 cursor-not-allowed">\n\n    <i class="fas fa-wallet ' +
        'text-gray-500 text-2xl w-8 text-center"></i>\n\n    <div>\n\n      <div class="font-bold text-gray-400">Wallet LEADER</div>\n\n      <div class="text-xs ' +
        'text-gray-500">Solde insuffisant : $' +
        _wbal.toLocaleString("en-US", {
          minimumFractionDigits: 2
        }) +
        " / $" +
...
async function wizardStep5V2Manual() {
  ...
  _wizardShow(
    '<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Création de votre commande\u2026</p></div>'
  );
  let orderId = null;
  try {
    let orderResp;
    ...
    if (err.existing_order_id) {
      ...
      _wizardShow(
        '<div class="p-8 text-center space-y-4"><i class="fas fa-exclamation-circle text-red-400 text-3xl"></i><p class="text-red-400">Erreur lors de la ' +
          'création de commande.<br><span class="text-xs text-gray-500">' +
          (err.error || err.message || "") +
          ('</span></p><button onclick="wizardStep4()" class="py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm"><i class="fas fa-arrow-left mr-1"></i>Retour<' +
            "/button></div>")
      );
      return;
    }
    ...
    if (_wizardData.isTopup) {
      ...
      _wizardShow(
        '\n      <div class="p-8 text-center space-y-5">\n        <div class="w-20 h-20 rounded-full bg-green-500/20 flex items-center justify-center mx-auto">\n  ' +
          ...
          'Recharge soumise !</p>\n          <p class="text-green-400 text-sm mt-1">Votre preuve de paiement a été envoyée. La recharge sera validée sous 24–48h.<' +
          '/p>\n          <p class="text-gray-500 text-xs mt-2">' +
          "Réf. : " +
          (topupId || "").substring(0, 16) +
          ('…</p>\n        </div>\n        <button onclick="wizardClose()" class="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl ' +
      ...
      _wizardShow(
        '\n      <div class="p-8 text-center space-y-5">\n        <div class="w-20 h-20 rounded-full bg-amber-500/20 flex items-center justify-center mx-auto">\n  ' +
          ...
          ('Commande enregistrée !</p>\n          <p class="text-amber-400 text-sm mt-1">Votre preuve a été envoyée. Votre carte sera ' +
            "fabriquée et expédiée après ") +
          "validation sous 24–48h.</p>\n          <p " +
          'class="text-gray-500 text-xs mt-2">Réf. commande : ' +
          (orderId || "").substring(0, 16) +
          ('…</p>\n        </div>\n        <button onclick="wizardClose();showPage(\'member-card\')" class="w-full py-3 bg-amber-500 text-dark-900 font-bold ' +
      ...
      _wizardShow(
        '\n      <div class="p-8 text-center space-y-5">\n        <div class="w-20 h-20 rounded-full bg-emerald-500/20 flex items-center justify-center mx-auto">\n' +
          ...
          ('Preuve soumise !</p>\n          <p class="text-emerald-400 text-sm mt-1">Votre accès à la formation sera débloqué sous 24–48h ' +
            "après validation.</p>\n    ") +
          '      <p class="text-gray-500 text-xs mt-2">' +
          "Réf. : " +
          (courseOrderId || "").substring(0, 16) +
          ('…</p>\n        </div>\n        <button onclick="wizardClose()" class="w-full py-3 bg-rouge-500 text-dark-900 font-bold rounded-xl hover:bg-rouge-500 ' +
            'transition">\n          <i class="fas fa-check mr-2"></i>Fermer\n        </button>\n      </div>')
      );
      _wizardReset();
    } else {
      ...
      _wizardShow(
        '\n      <div class="p-8 text-center space-y-5">\n        <div class="w-20 h-20 rounded-full bg-emerald-500/20 flex items-center justify-center mx-auto">\n' +
          ...
          'class="text-gray-500 text-xs mt-2">Réf. commande : ' +
          (orderId || "").substring(0, 16) +
          ('…</p>\n        </div>\n        <button onclick="wizardClose();showPage(\'packages\')" class="w-full py-3 bg-rouge-500 text-dark-900 font-bold rounded-xl ' +
...
async function wizardStep5V2() {
  ...
  _wizardShow(
    '<div class="p-10 flex flex-col items-center gap-4"><div class="loader"></div><p class="text-gray-400 text-sm">Redirection vers ' +
      "${_wizardData.v2DisplayName}…</p></div>"
  );
  try {
    const pspBody = {
      payment_method_id: t,
    ...
    if (_wizardData.isTopup) pspBody.topup_id = orderId;
    else if (licenseId) pspBody.license_id = licenseId;
    else pspBody.order_id = orderId;
    ...
    _wizardShow(
      '<div class="p-8 text-center space-y-4"><i class="fas fa-exclamation-circle text-red-400 text-3xl"></i><p class="text-red-400">Erreur de paiement ' +
        _wizardData.v2DisplayName +
        '\n.<br><span class="text-xs text-gray-500">' +
        (pspErr.error || pspErr.message || "") +
        ('</span></p><button onclick="wizardStep4()" class="py-2 px-6 bg-dark-700 text-gray-300 rounded-xl text-sm"><i class="fas fa-arrow-left mr-1"></i>Retour<' +
          "/button></div>")
    );
  }
...
async function wizardStep5() {
  ...
  ((_wizardData.orderId = null),
    _wizardShow(
      '\n  <div class="p-10 flex flex-col items-center gap-4">\n    <div class="loader"></div>\n    <p class="text-gray-400 text-sm">Création de votre commande…<' +
        "/p>\n  </div>"
    ));
  try {
    let _apiResp;
      ...
      if (_apiResp.activated) {
        if (_apiResp.new_balance !== undefined) {
          ...
          if (_hb) {
            ...
            _hb.innerHTML =
              '<span class="text-gray-400">Solde : </span><span class="text-rouge-400 font-bold">$' +
              Number(_nb).toLocaleString("en-US", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              }) +
              "</span>";
        ...
        _wizardShow(
          '\n  <div class="p-8 text-center space-y-5">\n    <div class="w-20 h-20 rounded-full bg-green-500/20 flex items-center justify-center mx-auto">\n      <i ' +
            'class="fas fa-key text-green-400 text-4xl"></i>\n    </div>\n    <div>\n      <p class="text-white font-bold text-xl">Licence activée !</p>\n      <p ' +
            'class="text-green-400 text-sm mt-1">' +
            (_apiResp.message || "Votre licence Finstrategia est maintenant active.") +
            ('</p>\n    </div>\n    <button onclick="wizardClose(); renderLicense(document.getElementById(\'page-content\'))"\n      class="w-full py-3 bg-rouge-500 ' +
              'text-dark-900 font-bold rounded-xl hover:bg-rouge-500 transition">\n      <i class="fas fa-check mr-2"></i>Voir ma licence\n    </button>\n  </div>')
        );
        _wizardReset();
        return;
      }
  ...
  a =
    "stripe" === t
      ? '\n\n    <div class="bg-purple-900/20 border border-purple-500/30 rounded-xl p-4 space-y-3">\n\n      <div class="flex items-center gap-3">\n\n        <i ' +
        'class="fas fa-credit-card text-purple-400 text-2xl"></i>\n\n        <div>\n\n          <div class="font-semibold">Paiement sécurisé par carte</div>\n\n      ' +
        '    <div class="text-xs text-gray-400">Visa · Mastercard · Amex · Apple Pay · Google Pay</div>\n\n        </div>\n\n      </div>\n\n      <div ' +
        'id="stripe-payment-element" class="min-h-[120px]">\n\n        <div class="flex items-center justify-center py-6"><div class="loader"></div></div>\n' +
        ('\n      </div>\n\n      <div id="stripe-error-msg" class="hidden text-red-400 text-sm text-center"></div>\n\n      <button id="stripe-pay-btn" ' +
          'onclick="_stripeConfirmPayment()"\n\n        class="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl transition flex ' +
          'items-center justify-center gap-2">\n\n        <i class="fas fa-lock text-sm"></i> Payer $') +
        n.toLocaleString("en-US", {
          minimumFractionDigits: 2
        }) +
        ('\n\n      </button>\n\n      <p class="text-xs text-gray-500 text-center"><i class="fas fa-lock mr-1"></i>Paiement 100% sécurisé — SSL chiffré · Aucune ' +
          "donnée bancaire stockée</p>\n\n    </div>")
      : "paypal" === t
        ? '\n    <div class="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 space-y-3">\n      <div class="flex items-center gap-3">\n        <i class="fab ' +
          'fa-paypal text-blue-400 text-2xl"></i>\n        <div>\n          <div class="font-semibold">Paiement sécurisé PayPal / Carte</div>\n          <div ' +
          'class="text-xs text-gray-400">Visa · Mastercard · PayPal · Activation immédiate</div>\n        </div>\n      </div>\n      <div ' +
          'id="paypal-buttons-container" class="min-h-[80px] flex items-center justify-center">\n' +
          ...
          : "coinpayments" === t
            ...
            : "crypto" === t
              ? '\n\n    <div class="bg-orange-900/20 border border-orange-500/30 rounded-xl p-4 space-y-3">\n\n      <div class="flex items-center gap-3">\n\n        <i ' +
                ...
                (e.crypto_network || "USDT") +
                '</div></div>\n\n      </div>\n\n      <div class="bg-dark-700 rounded-lg px-3 py-2 font-mono text-orange-300 text-xs break-all select-all">' +
                e.crypto_address +
                "</div>\n\n      " +
              ...
              : "wallet" === t
                ? [
                  ...
                  ('</span></div>\n\n      </div>\n\n      <button onclick="_walletPayOrder()" id="wallet-pay-btn"\n\n        class="w-full py-3 bg-green-600 hover:bg-green-500 ' +
  ...
  const s = "paypal" === t,
    ...
    i =
      s || r
        ? '<div class="bg-green-900/10 border border-green-500/20 rounded-xl p-3 text-xs text-gray-400">\n        <i class="fas fa-bolt text-green-400 mr-1"></i>\n ' +
          ...
          "/div>"
        : '<div class="bg-blue-900/10 border border-blue-500/20 rounded-xl p-4 text-sm text-gray-300">\n        <i class="fas fa-info-circle text-blue-400 mr-2"><' +
          '/i>\n        Une fois le paiement effectué, cliquez sur <strong>"J\'ai payé"</strong> pour soumettre votre preuve.\n       </div>',
    l =
      s || r
        ? '<button onclick="wizardStep4()" class="w-full py-2.5 bg-dark-700 text-gray-400 rounded-xl hover:bg-dark-600 transition text-sm"><i class="fas ' +
          'fa-arrow-left mr-2"></i>Retour / Changer de méthode</button>'
        : '<div class="flex gap-3">\n        <button onclick="wizardStep4()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl hover:bg-dark-600 transition ' +
          'text-sm"><i class="fas fa-arrow-left mr-2"></i>Retour</button>\n        <button onclick="wizardStep6()" class="flex-1 py-3 bg-rouge-500 text-dark-900 ' +
  ...
  if (
    ...
    _renderPayPalButtons(
      ...
      async () => {
        ...
        (e && (n.order_id = e),
          t && (n.license_id = t),
          (n.description = "LEADER \u2014 " + (_wizardData.pkgName || "Licence Finstrategia") + " ($" + a.toFixed(2) + ")"));
          ...
          [
            ...
            (e.order_id
              ? '<div class="flex justify-between"><span class="text-gray-400">R\xE9f. commande</span><span class="font-mono text-xs text-gray-300">' +
                ...
                "\u2026</span></div>"
              : "") +
            ('\n\n          </div>\n\n          <button onclick="wizardClose(); showPage(\'dashboard\')"\n\n            class="w-full py-3 bg-rouge-500 text-dark-900 ' +
              'font-bold rounded-xl hover:bg-rouge-500 transition">\n\n            <i class="fas fa-home mr-2"></i>Aller à mon tableau de bord\n\n          </button>\n\n   ' +
              "     </div>")
        ),
          _wizardReset());
      },
      (e) => {
        document.getElementById("paypal-buttons-container").innerHTML =
          '<div class="bg-red-900/20 border border-red-500/30 rounded-xl p-4 text-center space-y-3">\n\n            <p class="text-red-400 text-sm"><i class="fas ' +
          'fa-exclamation-circle mr-1"></i>' +
          e +
...
async function _initStripePaymentElement(e) {
  if (!(await _loadStripeSdk()))
    return void (document.getElementById("stripe-payment-element").innerHTML =
      '<p class="text-red-400 text-sm text-center"><i class="fas fa-exclamation-circle mr-1"></i>Impossible de charger Stripe. Vérifiez votre connexion.</p>');
  ...
  try {
    ...
    ((_stripePiClientSecret = s.client_secret),
      (window._stripePiId = s.payment_intent_id),
      (window._stripeOrderId = t),
      (window._stripeLicenseId = n),
      (_stripe = Stripe(window._stripePublicKey)));
    const r = {
      theme: "night",
      variables: {
        colorPrimary: "#c9a84c",
    ...
    if (_se) {
      _se.innerHTML =
        '<div class="bg-red-900/20 border border-red-500/30 rounded-xl p-4 text-center space-y-3"><i class="fas fa-exclamation-circle text-red-400 text-2xl"><' +
        '/i><p class="text-red-300 font-semibold text-sm mt-2">' +
        t +
        '</p><button onclick="wizardStep4()" class="py-2 px-4 bg-dark-700 text-gray-300 rounded-xl text-xs mt-2">\u2190 Retour</button></div>';
    } else {
      _wizardShow(
        '<div class="p-6 text-center space-y-4"><div class="w-14 h-14 rounded-full bg-red-500/20 flex items-center justify-center mx-auto"><i class="fas ' +
          'fa-exclamation-circle text-red-400 text-3xl"></i></div><p class="text-red-300 font-semibold">' +
          t +
          '</p><button onclick="wizardStep4()" class="py-2 px-4 bg-dark-700 text-gray-300 rounded-xl text-sm">\u2190 Retour</button></div>'
      );
    }
  }
}
async function _stripeConfirmPayment() {
  ...
  if (_stripe && _stripeElements) {
    ...
    try {
      const { error: n, paymentIntent: a } = await _stripe.confirmPayment({
        elements: _stripeElements,
        redirect: "if_required"
      });
      if (n)
        return (
          (t.textContent = n.message || "Paiement refusé"),
          t.classList.remove("hidden"),
          (e.disabled = !1),
          void (e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer')
        );
      if (a && "succeeded" === a.status) {
        ...
        ((t.textContent = "Statut inattendu: " + a?.status),
          t.classList.remove("hidden"),
          (e.disabled = !1),
          (e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer'));
    } catch (n) {
      ((t.textContent = n.error || n.message || "Erreur lors du paiement"),
        t.classList.remove("hidden"),
        (e.disabled = !1),
        (e.innerHTML = '<i class="fas fa-lock text-sm"></i> Réessayer'));
    }
  ...
  (_wizardShow(
    "\n\n  " +
      ...
      ('\n\n  <div class="p-6 space-y-5">\n\n    <div>\n\n      <h3 class="text-lg font-bold text-white">Preuve de paiement</h3>\n\n      <p class="text-sm ' +
        "text-gray-400 mt-1\">Uploadez une capture d'écran de votre virement ou tout justificatif de paiement.</p>\n\n    </div>\n\n\n\n    <!-- Zone upload preuve de " +
        ...
        ('\n        class="w-full bg-dark-700 border border-dark-500 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:ring-0 focus:outline-none ' +
          'text-sm">\n\n    </div>\n\n\n\n    <div class="bg-yellow-900/10 border border-yellow-500/20 rounded-xl p-3 text-xs text-gray-400">\n\n      <i class="fas ' +
          'fa-clock text-yellow-400 mr-2"></i>\n\n      Validation sous <strong class="text-white">24–48h</strong> après réception de votre preuve.\n\n    </div>\n\n   ' +
          ' <div class="flex gap-3">\n\n      <button onclick="wizardStep5()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl hover:bg-dark-600 transition ' +
          'text-sm"><i class="fas fa-arrow-left mr-2"></i>Retour</button>\n') +
        ('\n      <button onclick="wizardSubmitProof()" id="wiz-submit-btn" class="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-500 ' +
...
async function wizardSubmitProof() {
  ...
  try {
    ...
    const isAlreadyHandled =
      errMsg.includes("proof_submitted") ||
      errMsg.includes("déjà") ||
      errMsg.includes("activée") ||
      errMsg.includes("validated") ||
...
async function renderLicense(e) {
  ...
  try {
    ...
    e.innerHTML =
      ...
      fmtPrice(_price) +
      "/an) \u2014 \xE9ligibilit\xE9 aux commissions LEADER</p>\n      </div>\n      " +
      d +
      "\n    </div>";
  } catch (t) {
    e.innerHTML = '<div class="p-4 text-red-400">Erreur : ' + (t.error || t.message) + "</div>";
  ...
  e.innerHTML =
    ...
    (referralUrl
      ? '\n        <div class="flex gap-2">\n          <input id="profile-referral-input" type="text" value="' +
        referralUrl +
        ('" readonly\n            class="flex-1 bg-dark-700 border border-dark-600 rounded-xl px-4 py-2.5 text-rouge-400 text-xs font-mono focus:outline-none ' +
          'truncate">\n          <button onclick="copyProfileReferralLink()" class="bg-rouge-500 text-dark-900 font-bold px-4 py-2.5 rounded-xl hover:bg-rouge-500 ' +
      ...
      : '\n        <div class="flex items-start gap-3 bg-orange-900/20 border border-orange-500/30 rounded-xl p-4">\n          <i class="fas fa-lock ' +
        'text-orange-400 mt-0.5 flex-shrink-0"></i>\n          <div>\n            <p class="text-orange-300 font-medium text-sm">Licence requise pour parrainer<' +
...
function profFilterCountries(q) {
  ...
  filtered.forEach(function (c) {
    ...
    html +=
      '<div class="prof-country-item flex items-center gap-3 px-4 py-2.5 hover:bg-dark-700 cursor-pointer text-sm" data-country-name="' +
      safe +
      '" data-country-code="' +
      codeEsc +
      '">';
...
async function saveProfile() {
  ...
  const _phoneFull = _phoneNumVal
    ? _phoneDialVal && !_phoneNumVal.startsWith("+") && !_phoneNumVal.startsWith(_phoneDialVal)
      ? _phoneDialVal + " " + _phoneNumVal
      : _phoneNumVal
    : null;
  const payload = {
      ...
      [
        ...
        (qrTarget
          ? [
            ...
            (res.dest_tag
              ? '<div class="text-xs text-amber-400 bg-amber-900/20 rounded-lg p-2 border border-amber-500/30">\n                <i class="fas fa-exclamation-triangle ' +
                'mr-1"></i>\n                <strong>Tag / Mémo requis :</strong> <span class="font-mono">' +
                res.dest_tag +
                "</span>\n              </div>"
              : "") +
            "\n            </div>\n          </div>\n          " +
            (res.checkout_url
              ? '<a href="' +
                res.checkout_url +
                ('" target="_blank" rel="noopener"\n            class="mt-3 flex items-center justify-center gap-2 text-xs text-orange-400 hover:text-orange-300 ' +
                  'transition">\n            <i class="fas fa-external-link-alt"></i> Ouvrir la page CoinPayments\n          </a>')
              : "") +
            "\n        </div>\n        "
          : '\n        <div class="text-center">\n          <a href="' +
            res.checkout_url +
            ('" target="_blank" rel="noopener"\n            class="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 hover:bg-orange-400 text-dark-900 font-bold ' +
        ...
        ('\n\n        <!-- Statut du paiement -->\n        <div id="cp-status-box" class="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 text-center">\n    ' +
          ...
          'id="cp-status-text">Vérification automatique en cours…</span>\n          </div>\n        </div>\n\n        <div class="flex gap-3">\n          <button ' +
          'id="cp-verify-btn" onclick="_cpManualCheck(this)" class="flex-1 py-3 bg-orange-500 hover:bg-orange-400 text-dark-900 font-bold rounded-xl transition ' +
          'text-sm">\n' +
          ('            <i class="fas fa-sync mr-2"></i>Vérifier maintenant\n          </button>\n          <button onclick="wizardStep4()" class="py-3 px-4 ' +
            'bg-dark-700 text-gray-300 rounded-xl text-sm hover:bg-dark-600 transition">\n            Retour\n          </button>\n        </div>\n        <p ' +
            'class="text-xs text-gray-600 text-center">\n          <i class="fas fa-info-circle mr-1"></i>\n          Vérification automatique toutes les 15 ' +
            'secondes. Après paiement, cliquez "Vérifier maintenant".\n        </p>\n      </div>\n') +
          "    ")
    );
    ...
    if (res.is_complete) {
      ...
      if (statusEl)
        statusEl.textContent =
          res.status_text && res.temporary ? res.status_text : labels[res.status] || "Statut : " + (res.status_text || res.status);
    }
  } catch (err) {
...
function supportPriorityBadge(p) {
  ...
  return (
    '<span class="text-xs font-medium ' + (map[p] || "text-gray-400") + '"><i class="fas fa-flag text-[10px] mr-1"></i>' + (lbl[p] || p) + "</span>"
  );
}
...
async function renderSupport(el) {
  el.innerHTML =
    '\n  <div class="space-y-6">\n    <div class="flex items-center justify-between flex-wrap gap-3">\n      <div>\n        <h2 class="text-xl font-bold ' +
    'text-white">Support Client</h2>\n        <p class="text-gray-400 text-sm mt-0.5">Posez vos questions, suivez vos tickets</p>\n      </div>\n      <button ' +
    'onclick="supportShowCreate()" class="flex items-center gap-2 bg-rouge-500 hover:bg-rouge-500 text-dark-900 font-bold px-4 py-2.5 rounded-xl transition ' +
    'text-sm">\n        <i class="fas fa-plus"></i> Nouveau ticket\n' +
    ('      </button>\n    </div>\n\n    <!-- Onglets -->\n    <div class="flex gap-1 bg-dark-800 border border-dark-600 rounded-xl p-1 w-fit">\n      <button ' +
      'id="stab-tickets" onclick="supportTab(\'tickets\')" class="px-4 py-2 rounded-lg text-sm font-medium bg-dark-700 text-white transition">Mes tickets<' +
      '/button>\n      <button id="stab-faq"     onclick="supportTab(\'faq\')"     class="px-4 py-2 rounded-lg text-sm font-medium text-gray-400 ' +
      'hover:text-white transition">FAQ</button>\n    </div>\n\n    <!-- Contenu onglets -->\n') +
    '    <div id="support-tickets-tab">\n      <!-- Filtres statut -->\n      <div class="flex flex-wrap gap-2 mb-4">\n        ' +
    ["", "open", "in_progress", "waiting_member", "resolved", "closed"]
      .map((s) => {
        const labels = {
          "": "Tous",
          open: "Ouverts",
        ...
        return (
          "<button onclick=\"supportFilterStatus('" +
          s +
          '\')" class="px-3 py-1.5 rounded-lg text-xs font-medium transition ' +
          (active ? "bg-rouge-500 text-dark-900" : "bg-dark-800 border border-dark-600 text-gray-400 hover:text-white") +
          '">' +
          labels[s] +
          "</button>"
        );
...
function supportTab(tab) {
  document.getElementById("stab-tickets").className =
    "px-4 py-2 rounded-lg text-sm font-medium transition " + (tab === "tickets" ? "bg-dark-700 text-white" : "text-gray-400 hover:text-white");
  document.getElementById("stab-faq").className =
    "px-4 py-2 rounded-lg text-sm font-medium transition " + (tab === "faq" ? "bg-dark-700 text-white" : "text-gray-400 hover:text-white");
...
async function supportLoadTickets() {
  ...
  try {
    ...
    if (tickets.length === 0) {
      listEl.innerHTML =
        '\n        <div class="bg-dark-800 rounded-2xl border border-dark-600 p-12 text-center">\n          <div class="w-14 h-14 rounded-2xl bg-blue-500/10 flex ' +
        'items-center justify-center mx-auto mb-4">\n            <i class="fas fa-headset text-blue-400 text-2xl"></i>\n          </div>\n          <p ' +
        'class="text-gray-400 text-sm">Aucun ticket' +
        (_supportStatus ? " avec ce statut" : "") +
    ...
    pc.innerHTML =
      ...
      (["waiting_member", "in_progress"].includes(ticket.status)
        ? '\n      <div class="text-center">\n        <span class="text-xs text-gray-600 inline-flex items-center gap-1.5">\n          <span class="w-1.5 h-1.5 ' +
          'rounded-full bg-emerald-500/60 animate-pulse inline-block"></span>\n          Surveillance active — actualisation automatique toutes les 4s\n        <' +
          "/span>\n      </div>"
        : "") +
      "\n    </div>";
    setTimeout(() => {
...
function supportRenderMessage(m) {
  ...
  const avatar = isAdmin
    ? '<div class="w-8 h-8 rounded-full bg-red-500/20 flex items-center justify-center flex-shrink-0"><i class="fas fa-headset text-red-400 text-xs"></i><' +
      "/div>"
    : '<div class="w-8 h-8 rounded-full bg-rouge-500/15 flex items-center justify-center flex-shrink-0"><i class="fas fa-user text-rouge-400 text-xs"></i><' +
      "/div>";
...
function supportShowRate(ticketId, currentRating) {
  ...
  showModal(
    [
      ...
      (' bg-orange-900/20 border border-orange-500/30 rounded-xl p-3 text-xs text-orange-300">\n        <i class="fas fa-user-headset mr-1.5"></i>\n        <' +
        ...
        ('          class="w-full bg-dark-700 border border-dark-600 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:ring-0 focus:outline-none ' +
          ...
          '<div class="flex gap-3">\n        <button onclick="closeModal()" class="flex-1 py-3 bg-dark-700 text-gray-300 rounded-xl text-sm hover:bg-dark-600 ' +
          'transition">Annuler</button>\n        <button ') +
        "onclick=\"supportSubmitRate('") +
      ticketId +
      ('\')" class="flex-1 py-3 bg-rouge-500 hover:bg-rouge-500 text-dark-900 font-bold rounded-xl text-sm transition">\n          <i class="fas fa-check mr-2"><' +
        "/i>Envoyer mon avis\n        </button>\n      </div>\n    </div>")
  );
  ...
  try {
    ...
    faqEl.innerHTML =
      ...
      cats
        .map(
          (c) =>
            "\n            <button onclick=\"_supportFaqCategory='" +
            c +
            '\';supportLoadFaq()" \n              class="px-3 py-1.5 rounded-lg text-xs font-medium transition ' +
      ...
      ('\n\n        <!-- CTA si pas trouvé -->\n        <div class="bg-dark-800 border border-dark-600 rounded-2xl p-5 flex items-center gap-4">\n          <div ' +
        'class="w-10 h-10 rounded-xl bg-rouge-500/10 flex items-center justify-center flex-shrink-0">\n            <i class="fas fa-question-circle ' +
        'text-rouge-400"></i>\n          </div>\n          <div class="flex-1">\n            <p class="text-sm font-medium text-white">Vous n\'avez pas trouvé ' +
        'votre réponse ?</p>\n            <p class="text-xs text-gray-400 mt-0.5">Créez un ticket et notre équipe vous répondra dans les plus brefs délais.</p>\n' +
        ('          </div>\n          <button onclick="supportShowCreate()" class="px-4 py-2 bg-rouge-500 hover:bg-rouge-500 text-dark-900 font-bold rounded-xl ' +
          'text-sm transition flex-shrink-0">\n            <i class="fas fa-plus mr-1"></i>Nouveau ticket\n          </button>\n        </div>\n      </div>'));
  } catch (err) {
...
function kycLevelBg(lvl) {
  return (
    [
      ...
      "bg-rouge-500/10 border-rouge-500/25"
    ][lvl] || "bg-gray-500/10 border-gray-500/30"
  );
}
  ...
  try {
    ...
    if (!kycEnabled) {
      el.innerHTML =
        ...
        "vérification d'identité n'est pas actif pour le moment.</p>\n      </div>";
      return;
    }
    el.innerHTML =
      [
    ...
    el.innerHTML =
      ...
      (e.error || "Erreur lors du chargement KYC") +
      "</p>\n    </div>";
  }
}
  ...
  return (
    ...
    (current < maxLevel
      ? '\n    <p class="text-xs text-gray-400 mt-4 text-center">\n      <i class="fas fa-info-circle mr-1 text-purple-400"></i>\n      Passez au niveau <strong ' +
        'class="text-white">' +
        kycLevelLabel(current + 1) +
        "</strong> pour d\xE9bloquer davantage de fonctionnalit\xE9s\n    </p>"
      : '\n    <p class="text-xs text-green-400 mt-4 text-center font-semibold">\n      <i class="fas fa-star mr-1"></i> Niveau maximum atteint — vous avez ' +
        "complété toute la vérification !\n    </p>") +
    "\n  </div>"
  );
}
      ...
      (d) =>
        ...
        (isDraft || d.status === "rejected"
          ? "<button onclick=\"kycDeleteDoc('" +
            d.id +
            ('\')"\n          class="text-[10px] text-red-400 hover:text-red-300 hover:bg-red-500/20 bg-red-500/10 px-2 py-1 rounded-lg transition flex items-center ' +
...
function _kycBuildActionSection(memberLevel, app, levels, statusRes) {
  ...
  return (
    ...
    (', null)"\n            class="w-full bg-purple-600 hover:bg-purple-500 text-white font-semibold text-sm px-4 py-3 rounded-xl transition flex ' +
      'items-center justify-center gap-2">\n      <i class="fas fa-id-card"></i>\n      Commencer la vérification niveau ') +
    nextLevel +
    ('\n    </button>\n    <p class="text-[11px] text-gray-500 text-center mt-2">\n      <i class="fas fa-lock mr-1"></i>Vos documents sont stockés de façon ' +
...
function _kycWizardRender(docs) {
  ...
  const _sl = _kycWizard.livenessEnabled
    ? ["Pièce d'identité", "Selfie", "Liveness", "Vérification IA"]
    : ["Pièce d'identité", "Selfie", "Vérification IA"];
  const stepsHtml = _sl
    .map((label, i) => {
      const n = i + 1;
      const done = n < step;
...
function _kycWizardStep1Html(docs) {
  ...
  const groupHint = groups.length
    ? '\n    <div class="bg-purple-500/10 border border-purple-500/20 rounded-xl px-3 py-2 text-xs text-purple-300">\n      <i class="fas fa-info-circle ' +
      'mr-1.5"></i>Un seul document par groupe suffit.\n    </div>'
    : "";
  ...
  const previewHtml = _kycWizard.docDataUrl
    ? '\n    <div class="flex items-center gap-3 bg-dark-700 border border-green-500/30 rounded-xl px-3 py-2.5 mt-2">\n      ' +
      (_kycWizard.docMime?.startsWith("image/")
        ...
        : '<div class="w-14 h-14 rounded-lg bg-red-500/20 flex items-center justify-center flex-shrink-0"><i class="fas fa-file-pdf text-red-400 text-xl"></i><' +
          "/div>") +
      '\n      <div class="flex-1 min-w-0">\n        <div class="text-sm text-white font-medium truncate">' +
      _kycWizard.docFileName +
      ('</div>\n        <div class="text-xs text-green-400"><i class="fas fa-check-circle mr-1"></i>Prêt</div>\n      </div>\n      <button ' +
        'onclick="_kycWizardClearDoc()" class="text-gray-500 hover:text-red-400 transition">\n        <i class="fas fa-times"></i>\n      </button>\n    </div>')
    : "";
...
function _kycWizardOpenCropEditor(dataUrl, mime, fileName) {
  ...
  body.innerHTML =
    ...
    ('          <i class="fas fa-arrow-left mr-2"></i>Rechoisir\n        </button>\n        <button onclick="_kycCropConfirm()"\n                class="flex-2 ' +
      'bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm px-6 py-2.5 rounded-xl transition flex items-center gap-2">\n          <i class="fas ' +
      'fa-check-circle"></i>Confirmer\n        </button>\n      </div>\n    </div>');
  const img = new Image();
  img.onload = () => {
    crop.img = img;
...
function _kycCropConfirm() {
  ...
  const canvas = document.getElementById("kyc-crop-canvas");
  if (!canvas) return;
  const exportedDataUrl = canvas.toDataURL("image/jpeg", 0.9);
...
function _kycWizardStep2Html() {
  ...
  return (
    ...
    (hasPhoto
      ? '\n      <!-- Preview selfie pris -->\n      <div class="relative">\n        <img src="' +
        _kycWizard.selfieDataUrl +
        ('" class="w-full max-h-96 object-cover rounded-2xl border-2 border-green-500/40">\n        <div class="absolute top-3 right-3">\n          <button ' +
          'onclick="_kycWizardClearSelfie()"\n                  class="bg-dark-900/80 hover:bg-red-500/80 text-white rounded-full w-8 h-8 flex items-center ' +
          'justify-center transition">\n            <i class="fas fa-redo text-xs"></i>\n          </button>\n        </div>\n        <div class="absolute bottom-3 ' +
          'left-3 bg-green-500/90 text-white text-xs px-2 py-1 rounded-full font-semibold">\n' +
          '          <i class="fas fa-check mr-1"></i>Photo prise\n        </div>\n      </div>')
      : '\n\n      <!-- Webcam + options -->\n      <div class="space-y-3">\n        <!-- Vue webcam -->\n        <div class="relative bg-dark-900 rounded-2xl ' +
        'overflow-hidden border-2 border-dark-500" style="min-height:320px">\n          <video id="kyc-webcam" autoplay playsinline muted\n                 ' +
        ...
        ('               class="absolute inset-0 flex flex-col items-center justify-center text-center p-4 hidden">\n            <div class="w-52 h-52 ' +
          'rounded-full border-2 border-dashed border-purple-400/70 flex items-center justify-center mb-3">\n              <i class="fas fa-user text-4xl ' +
          'text-purple-400/50"></i>\n            </div>\n            <p class="text-xs text-gray-400">Centrez votre visage dans le cercle</p>\n          </div>\n\n    ' +
          '      <!-- Placeholder avant activation -->\n          <div id="kyc-cam-placeholder" class="absolute inset-0 flex flex-col items-center justify-center ' +
          'text-center p-6">\n') +
        ('            <i class="fas fa-camera text-4xl text-gray-600 mb-3"></i>\n            <p class="text-sm text-gray-400">Activez la caméra ou uploadez une ' +
          'photo</p>\n          </div>\n        </div>\n\n        <!-- Boutons -->\n        <div class="grid grid-cols-2 gap-3">\n          <button id="kyc-cam-btn" ' +
          'onclick="_kycWizardStartCamera()"\n                  class="bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold px-4 py-2.5 rounded-xl ' +
          'transition flex items-center justify-center gap-2">\n            <i class="fas fa-camera"></i>Activer la caméra\n') +
        ('          </button>\n          <button onclick="document.getElementById(\'kyc-selfie-file\').click()"\n                  class="bg-dark-600 ' +
          'hover:bg-dark-500 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition border border-dark-400 flex items-center justify-center gap-2">\n    ' +
          '        <i class="fas fa-upload text-purple-400"></i>Uploader photo\n          </button>\n        </div>\n\n        <!-- Bouton capture (apparaît quand ' +
          'webcam active) -->\n        <button id="kyc-capture-btn" onclick="_kycWizardCapture()" class="hidden\n') +
        ("                w-full bg-green-600 hover:bg-green-500 text-white font-bold text-sm px-4 py-3 rounded-xl transition flex items-center justify-center " +
          'gap-2">\n          <i class="fas fa-camera-rotate"></i>Prendre la photo\n        </button>\n\n        <input type="file" id="kyc-selfie-file" ' +
    ...
    ('\n\n      <div id="kyc-wizard-err2" class="hidden bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 text-sm text-red-400"></div>\n\n      <!-- ' +
      'Navigation -->\n      <div class="flex gap-3">\n        <button onclick="_kycWizardBack()"\n                class="flex-1 bg-dark-600 hover:bg-dark-500 ' +
      'text-white text-sm font-medium px-4 py-2.5 rounded-xl transition border border-dark-400">\n          <i class="fas fa-arrow-left mr-2"></i>Retour\n      ' +
      '  </button>\n        <button onclick="_kycWizardNext2()"\n' +
      '                class="flex-2 ') +
    (hasPhoto ? "bg-purple-600 hover:bg-purple-500" : "bg-dark-600 cursor-not-allowed opacity-60") +
    (' text-white font-bold text-sm px-6 py-2.5 rounded-xl transition flex items-center gap-2">\n          <i class="fas fa-shield-check"></i>Valider mon KYC\n' +
      "        </button>\n      </div>\n    </div>")
  );
}
...
function _kycWizardStepLivenessHtml() {
  ...
  if (done)
    return (
      '\n    <div class="flex flex-col items-center gap-4 py-4">\n      <div class="w-20 h-20 rounded-full bg-green-500/20 border-2 border-green-500 flex ' +
      'items-center justify-center">\n        <i class="fas fa-check text-3xl text-green-400"></i>\n      </div>\n      <div class="text-center">\n        <div ' +
      'class="font-bold text-white text-lg">Liveness validée !</div>\n        <div class="text-sm text-gray-400 mt-1">Toutes les vérifications ont été ' +
      'effectuées.</div>\n      </div>\n      <button onclick="_kycWizardLivenessFinish()" class="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold ' +
...
function _kycWizardStartLivenessCamera() {
  navigator.mediaDevices
    ...
    .catch(function (err) {
      ...
      if (ph) {
        ph.innerHTML =
          '<i class="fas fa-exclamation-triangle text-2xl text-red-400 mb-2"></i><p class="text-sm text-red-400">Caméra non disponible</p>';
      }
    });
}
...
function _kycWizardStep3Html(result) {
  if (!result) {
    return (
      '\n      <div class="space-y-6 text-center py-4">\n        <div class="w-20 h-20 rounded-full bg-purple-500/20 border-2 border-purple-500/40 flex ' +
      'items-center justify-center mx-auto animate-pulse">\n          <i class="fas fa-shield-check text-4xl text-purple-400"></i>\n        </div>\n        <div>' +
      '\n          <div class="text-white font-bold text-lg mb-1">Validation en cours…</div>\n          <div id="kyc-ai-status" class="text-sm text-gray-400">' +
      "Vérification de votre document</div>\n        </div>\n\n        <!-- Barre de progression animée -->\n" +
      ('        <div class="bg-dark-700 rounded-full h-2 overflow-hidden">\n          <div id="kyc-ai-progress-bar"\n               class="h-full rounded-full ' +
        'bg-gradient-to-r from-purple-600 to-purple-400 transition-all duration-700"\n               style="width:5%"></div>\n        </div>\n\n        <!-- Étapes ' +
...
async function _kycWizardRunAnalysis() {
  ...
  try {
    ...
    if (analysisRes.decision === "approved") {
      ...
      if (body)
        body.innerHTML = _kycWizardStep3Html({
...
async function _kycWizardSubmitManually() {
  ...
  if (body)
    body.innerHTML =
      '\n    <div class="text-center py-10 space-y-4">\n      <div class="w-16 h-16 rounded-full bg-purple-500/20 border-2 border-purple-500/40 flex ' +
      'items-center justify-center mx-auto animate-pulse">\n        <i class="fas fa-shield-check text-3xl text-purple-400"></i>\n      </div>\n      <div ' +
      'class="text-white font-semibold">Finalisation de votre dossier…</div>\n      <div class="text-sm text-gray-400">Quelques instants</div>\n    </div>';
  try {
    if (!_kycWizard.applicationId) {
      const startRes = await api("POST", "/kyc/start", {
        target_level: _kycWizard.targetLevel
...
!(function () {
  ...
  ((e.id = "wizard-overlay"),
    (e.className = "hidden fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4"),
    (e.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.82);z-index:9999;display:none;align-items:center;justify-content:center;padding:1rem;"),
    (e.innerHTML =
      '<div id="wizard-box" class="bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-lg shadow-2xl" style="max-height:92vh;overflow-y:auto;"></div>'),
    document.body ? document.body.appendChild(e) : document.addEventListener("DOMContentLoaded", () => document.body.appendChild(e)));
})();
...
function showImpersonationBar(t) {
  ...
  b.style.cssText =
    "position:fixed;top:0;left:0;right:0;z-index:99999;background:#7c3aed;color:#fff;text-align:center;padding:6px 12px;font-size:12px;font-weight:600;";
  ...
  try {
    ...
    el.innerHTML =
      ...
      ('\n            class="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-2.5 rounded-xl ' +
        ...
        'class="bg-dark-800/50 border border-dark-700 rounded-xl p-4 flex items-start gap-3">\n' +
        ('        <i class="fas fa-shield-halved text-emerald-400 mt-0.5"></i>\n        <div class="text-xs text-gray-400">\n          <span class="font-semibold ' +
          'text-gray-300">Documents officiels</span> — Ces documents sont émis par \n          <span class="text-white font-medium">')) +
      (stmtData.company?.name || "la compagnie") +
      "</span>\n          " +
      (stmtData.company?.registration_number
        ? ', enregistr\xE9e sous le num\xE9ro <span class="font-mono text-emerald-400">' + stmtData.company.registration_number + "</span>"
        : "") +
      ("\n          en Angleterre et au Pays de Galles. Chaque document porte un numéro de référence unique et peut être utilisé comme justificatif officiel " +
        "auprès des autorités fiscales de votre pays.\n        </div>\n      </div>\n    </div>");
  } catch (err) {
...
function _pdfDrawFooter(pdf, pw, ph, co, pageNum, totalPages) {
  ...
  pdf.setFont("helvetica", "normal");
  const line1 =
    co.name + (co.registration_number ? "  ·  Company No. " + co.registration_number : "") + (co.vat_number ? "  ·  VAT: " + co.vat_number : "");
  ...
  if (line2)
    pdf.text(line2, pw / 2, ph - 7.5, {
      align: "center"
    });
  if (line3)
    pdf.text(line3, pw / 2, ph - 4.5, {
      align: "center"
    });
  pdf.setTextColor(...GOLD);
  pdf.setFontSize(6);
...
const _DEFAULT_LEADER_LOGO = [
  ...
  "f9H7jl1fGfTh2pajW4+laTb4+aXdK0lWxcHPdU6yQkClm/KpL4TeMMFyHBFRRMfCI8lKysvfQJYqdJFgRUQUAN2gq8BffeL8M8sqV0zQrcGRMAWpWtjnLdwL0DGKKFFPHslat33X3wyE0NtylG4W5n" +
    ...
    ("2SYYe7f02s3C6x948jeVwEWYpZpdGZvIYnT1BmZ2DiA9sQ6rMTx6kk4tDbwJCLHgPWI6yLxI8kJK+kccFMWKwj89EQr6aHh0ZkTHUks+XladFfLOdRPuV9W6pQubjvR31G4WfnjHzr5q8O8sqziZQB" +
      "IkIKkZwKmrF85pAyzWYTWGRw9yNSxpfra6lKg7J/ohIcrdlMo8YCbCe7Pv+0IkWNNbIBS0KI8EVDFmNPqq7+95taT1z1qc/sFgOCqpwf1nuxqMgGwZYRjGjNrNwut3PP4Pg95unOCidzHwTU6b1emL" +
      "RxDQiDr36JFkKaVZqzKPvpGiWIXSnaXsq7VEsZtSOAgIci5Jx0tzXesAkrg20xBmD5iEp3iWzt5T8W7yfpi/buQl7wbtB7vqhFnz1fhoioWj5eBkBe5IMHaJVX8DsBas5wh/Xw/49eBWbd/5/qvaWk" +
      "9pcfpfBjJymEzIe80CH+cDt/RHgtUwHr0KlwX4Sh7FahDVaVuR") +
    ("MFAQ3RkwEkx+FWVuyiXDg0tE/VsMboeVAn2RYE0zBFAVqV4SKlu+CyuTvCivEBp5hMJquKaTnSCrwK8pN32krLpgYOLbMhxw2h0iHrt/3bYnHyfP24zn9/XneZ81T4c3DR3D/y2LdlZs/J9BQLJupn" +
      "ZuTgR9TzynDeDR+wDuTJo6X8VQ//ezG4W9dV9jZ9gbqPTflUcoikAcZYS+jKnCqduGLIpl8sdGx2roe5os3BkxHel29uUVYQSxmcpHoIE7eXflqcGrF8+/sKz6XwePMrkCUiekYmwB6J6Av9UDYRXo" +
      "9Y8//mRI5U+C2eNORCfgZqGmwQBZftUJs+bXCnim55k0T1bj1gjnIYAggXc01jZpGEFmou6c0AiQ1bPsC4SAuIWU5ZXZUytdJFjTmHLDjzX3gIrwaFSrKQCXL1gw0zR8wmziCYRlXR5TAy8gTqTcrJ" +
      "IYYUJTB721JqQP7bwntfCngpkeGHk8Ijn0EMoqx1IqnwmwanrW") +
  ...
  "/3ifRUYLi6xVoOhJJE2nk3JHJFjTU65dVuC+x2W3HSYbfZ4GjloN8wbohmRI+LQTcVWzoGOMzuQ1TkEgKYk4JzAUrFoN4XYzudWwf713x667eqEy8ufWg+vKfu6orHPtZmHv9t3vWdo+74XNKq8ePE" +
    ...
    ("HU1S0lC9MMA9WZqwjiNZKo/nujNi4mW/1nQ5r3FbKdBXCVW+Aqd8W0ryIRF914gobmFLCUzlXOCmSLCmGbcCwwf3Rk06l4Mkk+tNioG5kIRbGNy8rVGjFN01cnXivBc6sb+tjD41aJZFnVxZM8OZBr" +
      "tvIPCNRPjaB7bt3DByMfPbgZNFqJ6BHghd4NY9tGfX6sVz/kRU/m8izPSGydiVny8JSYqdA/y0Y3r3w8pra3pHnNsuhV6j0v/NwNIfSNJ8o2jytqwdQvGLnU3kQuDmek8TSkjfLknHjqw2aNJ1l4Q0" +
      "/Se4exeNHeG1zMHsy2V/pYO+PVbl3ZYsu0dV/z6L4BYxkiUKhhhnGB3lSLCmIcES9J3oVMmlQrV6L7AtNxoNF6XY34084RMl0RmDWb8oeQ5tYoBXSJpV3FAwS4N9P5j/x0ql/C83PvLI3mchVaGnAB" +
      "HI2s3Cnh2Pb1zd1vrmJtFbDEste59jETIxQAIXAB/dPK3Sg6Mx") +
    ("OLVzsjKBvqcs5S8l6diMJh8ttke//zbh2caK42DDk/VMDkTcmuyS2xShNPTvVGl0gnWQ7Pel2eddkZBuuJmk06PuE9klisL18pTsMqG0UfYnRYI1LRF8FiyZfP8PgnJQvVAjYf84nMVz/6JF5dXP1f" +
      "E8J1bBCa4smlRCeKIS7KsB/mHdiGhVd/bzhSFVB6MH0ouhdP32Xd+6cnHr6lmJXn8ERe/iDQxb0b2E5p4HGGTa1WGNBjVjs9KFtO8mLS17EtHP5x59EdOFeWsznUcydCYpfXnkoU4dKz/FNVDJdE1P" +
      "GmxIYUUppBs+KaXOU0SS9xQwTZ7rLGlxuBfGNg3TEy5LD072g/yrNWT6pzYO5+r2eQud6ocrxqHG4ZhlzUClRcWZ2aOVYB8crKRnfnDbzneu27ZzQzfo+jwK1ANpT8ELgz8L1W5Ibtix64aBEL4wQy" +
      "UJYyCDkhW6m4q0VTnu1Hw949ic5/TqV5RCddM/Qnh/1kPICmp8") +
  ...
  "Fw3xYUlWl8DWp4KE9+cefdHkSPKm3W3M6Dg+blnE+LFSgSDol/Kyp1C0YymiCyLBiogYP6FwPRAGh+SDzSpLqtmtQTXwiYg2qbg0hFt9sHOv27bz7dc/8ORv1oNrNGK1Pl+HzZBc0z7vi02JvH0wWB" +
    ...
    ("hJa1cZbzga9EERu1R+8BQlX/RUv+IURPLJjBybLmwpK4VxETRLAUNleMZfcLclI2qrUQ4l5LSjRHghURceTEwvVAurryxDXNqssHvFVnOi1XQvilF7v0uu27fgTDvbEakWDW1uDq9nmvVfiyisweHM" +
      "fgZ0C9QTA7J5+C7KOkjdbgdDno3WvSeaugf5H3CCqUjjeTE+NWRUwMuiSbdmBPDJe8Fko9upgHj4g4kqOdE4sr2+Z0OrHVqUFJKQ0Ff9PAkDvvugd2/ahWZ9WI5MoyciU9kK5pn/suFflnE2ZXsvqz" +
      "IzbqI+qwTv3r589ZDMMXBiIOh0cFEDH94fByFo2SW17kHls1REyYUBW2hCBGsCIijuBEd5H5Toq7+RinM/ZUw/1B/SXrtj1+a42ANWjUim5QyYezXt3e+tGy6mVDwcyy9N54uypLAN8k2jJY4cXA9t" +
      "667vw9megLgAX0Ls0Kf4vW4RqM2dm3vXG7IsaJ3pyk24yCOhQh") +
    ("RrAiIsZOMGo9r94xp+ReuSf1X7Nqcva6Bx6/NR9rI70NSghqNwWvaG095pr2ed9sdnrZYMiaW8rEKThTAcn7YfXHCNao1w2AarIDs93FS5sYwMwD3mtExJET9gCICG35pQ4p1DnEhmIEKyJibORKey" +
      "C9YtG8Fwh209NpesWHtu36SI18rCrgrMAJ/OzJKkgvbz/ueWWkt6yyYp8fV73VoTSnBAMROxdgbdZ8NWLU2PA0dO4G5lOY64RS+1KOBCtiggTKaHnRQnzlebk4SaHenrE3RrAiIsaIyxcsmOkclxtc" +
      "9IFtuz7S1cC1ViPJVQ+kV54457xmSv+RqKwYCBNPrnLLqykGJsu7F86aJ7EOa4yeM96EfcVcMnFxmyLGj5UKCD49H9zMfERUwQTenooEKyJi9NZLeiAkzYPzQW9Yt33Xt7sh6c3mCzaqRy7ra7cll7" +
      "T+WbnkfqAiJw6F8RWzH8b3E2+EkspxQ6XymQCrYmPKiIiI/Tje") +
  ...
  "VEvivl5SObVGribZFKs3LFFZwCy/jF3c1gXaG/thHc6zN0jOAKcF8+xraZPH4z5FHDm52lCldOoyRT+XzbcsZG2mQkgDsiUSrIiIKXR1ukA7MkLlDyZU3YvmLq6KdZroi0Q4w4xTh0TanTG35DTXLE" +
    ...
    ("GUL2gtSgYhYqtj+ypWQumwCuVpDkBCfCFhj7bbuRY2/WtM27vtnJlZVgFpiUm4KHg6aZNV7x7qU03byVIWId1iGw0kFfqiVeh+hCCjkqxwgmD8S9inimbKxIYJbB0wInh8xZwLLxT8sWSInVYnJZ9n" +
      "IfittPrdayT34OcRZhRMRRRVdOUnoyzhS6QSvt888Vsz8S5Q8EOkqqBIMqRj52xsiIj0hOzJ5DM2WTToNsGut7yzvP+8sXLJjZ3JR+oUmlayC/KSgFqG0Q0NTMVKRtxsD8U+Gxu/IB05FgPQN9BoiJ" +
      "Xib7AwCFsqEKBuYfGPF+IyJAJItQ7ceGTIBLHWcIcpEgb0X0hCyGX5Olwn4YBcOC/49IsCIijtIpWw+6CnwtWrVm8fylTsIbUuEiJ5xZUiE1qJpZavg8KqVHEDXSSjALjl/l34+qnUFt7M3qhXPak5" +
      "JfX1Z39r4wuTcFR+UPgm9WSQaxlwJ35WQztmw4ACuTLHrVeRG4") +
    ("s4p3bb3W1STsId0fwYp7GJEVPxitWur4S0AQmWfwAmCFIMuyQH2ggDJ9KDlX8E/i7fZIsCIijhKxysfmyDVL5l9owf47Yn/QpG5GakYlJ1UMR4qSIzzN5gTxZjsHSe8HGE10p3ZT8PJFc88uOe1NVN" +
      "oGQkilwPrA4ALgc50xevUsRqovwBmzwX8YghW38Fe2Q/8jccsiDiBYwglQ+uyIJzORyYhVmhGrehixZAGcMws/hbt3QbdGghURMcHE6pL582fNabGLEN7h4CXqhEow9oWQjpdUHaSdgkNcMO7/2LYn" +
      "n+Dw9Un7bwpe0z6/S8T+hyAzBkNRitmfFZoamPHSWkozittIEchqr6ScfhKStmJ6+hZA1MQ2ZfLZ5fIam4iInE2lB8mDZLMGkaS+PgeI8c9ZIdaPNY7KiYgYB9ZnKT1bBX71yXOOu6pt3mWzW8Ivyi" +
      "r/4JCXVMxC3gHd8vSbTuBpNicA4W6A7udIL3aDGtAD6dXtraud") +
  ...
  "ksor/eQTqxrp0IpZ1Xt+DdB/EMGqkav3LWw9dYZKb0llWU6u6urcC4g3MJNzyLveTz/l3aXZeelLYfkcKdtN4N6S9QQqcjrFAqgz5Gdw35PUffQxdGNhW154PfnRpGrztmFdGlEUoQB1WLqNavN3s6" +
    ...
    ("D4LZuxF9WfaSuiFXCfhqUPt+fvbrmxQHmwMrd8BjCvOn4LPUj+6cPkELdeCfCpX0c+xvm5IhEqxpCfEZw+4yuDVelz7EItVqrS5fMPP45nLLjYnyJp8VsNcacropPNWWVafb5ppx7gZ6IPSCv7q99Y" +
      "Mllasrwcwo9k3B0X7e7API+cC3+xuOYEkYJlYrZtA0uBDjDEFeJRZeY+jzRAQI+WvqoS8QeXow3EFly6/J72TU9z6FXHfGhrcRkBe3J2bVf4S7Hz64BUkkWBERByGPBtkq8Fe3z3utg08kWdTK510c" +
      "i3RuNgHMXYp771bSyxbT0qKtn29WfWORxt6Mm37kdViInQuwNr9k0AjMKg9IPF9KnT8W4RhsqBXjRHCl4WFIPuQ1P3VElLO8rgRbn6cHXS11EhHRCOwKVMHvscTdSBWB3gOii0UnWJL3TClgZ+JGUO" +
      "yFW9spX9daIXsXuOe3zf1QInJlgJFRq6LAZYOQ/eYucO/dytDV") +
    ("i+cuQmR9k+p5+4r3fseLrA7L5PT3LVw4Tx5+eGdOdq3+z6EBzBJxK7Nnah/J5+kg0WLPXzvUWZasL1BJv0EKdZ8ezD5WTWdG3Rn5VR698jczsHHHyNqrOiBYYpnQ/NjH3h8TvbRxbZ8Ntfl8l58we0" +
      "lzOflCk8or9wULuRUszFnJ04OSWtgbUnmgF/wVba0vEZHekspJdVzM/pxS6yGUVY6Tkj8D+GFvFplrlBS35ek/GSZeUsdpXfOQJGbV72XGp1G6t2vIyUwsrZjeyG8O+oes6j4MaO3mYH0QLDMHK0rw" +
      "CgcrCiTMAwKbU+o5/25SKtbaDkhWMDpl6YP9I2RWt7f+Thn5klNZtK+4RMUSEakEdlz/8OPbVi+e+8clkS+JyKzBxiRX+5VaImjF0vOAHzZYHZZQ53VyBzMRMCzYpxtMBnPd+aTCcQWwAVOuO6cpLI" +
      "BLAukauOuJ3IEIdUCwxIFHRH5fSkObYahIStSDK4l2vsMP9X+/") +
  ...
  "ux7WVplEUlMjV5fMnz9rTktY36T6mn0+1F3tUsjTqQ3PrA5yckoiiXfuHOC2jmlALOvPeXMK4YOwdaiholfDuirao+kp22mWGvTftHTLpzLZfu7Ut8ZFi5hO6MqK2cN7lxw3e3aLfa+s+pqBEKp1Wh" +
    ...
    ("sx603JR+vVn1goFg1UiuGmFv7QKAzpiGmuqd8JA4Qvhbhvrvha7YjymiQZAkZmlPSPs/diQXNiLBimhorM2L2v+zbd4XWlR/fyBYVWLNVb1DvYEYZ63vymZHxiWZUnKVYNWfhHTzTTE1GNEgCCCpWb" +
      "rOqv1rc3I1ZrmOBCuiYdENST7+5toWp28eCFYlkqu6h9QGPyNL7/jpvJPzvY66bGqMkEDYE9T+ArCYGoxoBK8BRLHwhFWH1jLKG4ORYEVMP3LVNvetLarXDISQEtOCjWTZfZOTkiZ2VtRlU2aHsrYM" +
      "ll7C0Oa86WJMDUY0gg8XPJLM16T8V5lMdx3RRZqolCIaDuvzwc3vP3HOuSXRz1TNvGWjbwpz2ywbIYJlHQeK/7Di1TmZABr2zyWMmNzlr0IpMat+MlS3fHk0TRcjIuqIY0keoH0fnHJMPsh5zPYjev" +
      "QRDYVu0C4IV50wa74m+lURKedDkAt1lb8kogGrq/4CvlgUS302") +
    ("I+hsgJ5YhzWZ5CqFpGSW/tiqLZfGuquIBoTmUaxFmthfhpSPHkmReyRYEQ3ldpB1hEvXlJq+WFZtHwjmtViDm4MAqbc/Fyd3qw8lL8W9BecM8So+wImYfUNFynk0S6Z4oyWrw7KOKxfNXXzDg7t31H" +
      "qdxWNwVMlVraP1r606uAo2VWGDEm9yRjSeOREIhsjlcMrnoO9pxjhcPhKsiIZBd54aXN3WumaG09fsC1ao+YIGvkXFDaThS+t27PpqnS3vnVe1tW5zIkurZlaA4dJi4MuiLYMaVgA7OmPD0aMtwQHU" +
      "YeGxIPwhbH2MzHmJ0auIRoRCSJHkRBK5mJQbxxrFigQroiGQzxhMr1w896UlkQ8MBvNSoMiVgakgQ96eCmbXdIHLBxWHzQUmBh1gD4P7DKRXwa+csLRqxYhW5GuKIOcB3+qPBOtoIoAqxp5g/nVUt9" +
      "wdU4MRjQ9RCKZi7wss/Rz07WEMUaxIsCIa4hQAXLZ4cYvKwBdU") +
  ...
  "ofbWS2eovGSoeKnBrJ1AsLSkXJ4TgHq8zm7doDc+8shehHucCFKcFKcYIGrn1QhXPBnj9QnwGbkK/8eqQy+nsvFXWd1JJFcR0xa1KNZCTQbzKNZKN4ofioioP9TqmNYsnr/UIWsHg4UCpgZ9s4qm8I" +
    ...
    ("iqWXwpLZh4tiRYIVUVfoBu0Hu+p5xy9QkY9UzEIB5diXRHTQ25a9TbM/Ua9tGQ4itQFAq82/ToM9oVntU2G8y3xxzx9JBiMOS6wCJA6T3YT0Eqtueg1D/feOGNwc1zHiKB9bBbP7g8npwXT58FfOCO" +
      "aXB8Lr8+NdmJKErC+WO0GTmXkU69A3CiPBiqg3aA+EkPqbmlTnecOkYHJsgApiwpU3b906VIdtGZ7pt+Up2HUPPbRLRO51mWNZoDosMOwl715KU2+W0opRrEMaNfPgHIga4ctBKytC2v+p3B5oLGaP" +
      "mGTtMkB140aqv9w0/HXTL6lu6afS/00z+zY4yZ2CIrxfzWuxLoUzZj/XjcJIsCLqBrVxOFe0z/svzU7fWMTUYK0tw6APt67btvM76+uwLcOhsD7XFwb9TmrGuhDkT72ZqUjbrIHZpwB0R4J1sGT6fJ" +
      "aggnNm4T+CpK+2ysY3M/Sr32RRKwIxJRgx+bKpZHp85FeFFSVA") +
    ("Df0ABF+gdGFei5Us0LJ/J89RixUJVkRdoBu0A+yK1tZjErObg2FWPPk1AakGq6iFKwD6GyjNsn/Wn3FX4VQ0hCYVxSVnR902cllqXr9z4NQItwdslVU3vYyhLT8kRq0iioHwzMeGKnQJ1Y13mvlvgd" +
      "MCRbGyWiyz98LyOYeqxYpKKKJeoD0QtIVrW5yeVDULBUwN+hYV9WafuW7H45vqvC3DM7C/tknCL70VTn9kRfjYBQCd07d+KCdVtZYLiQPBLPwo4N9glU3nUNnYmxsDR4xaRRQavQBi2LXFjWKFQ0ax" +
      "IsGKKDxqqcE1S+acW3LyroFsHE7RyFVwgg6G8FhTOvT/dYN2NZjhqjVJ3Ve1e6oWBjTbg0LVYYnJWSPSstMhTWgZQbJ0mFQ5lxWv22MW/OdCsPOsuul3qPR/I3t9l2N/Q9GIiELDQ5dS3fKfZv6WPI" +
      "pVkGhrXotlHDKKFQlWRNEhAJ9ZQUlMP6WI2nCPlEIZuiYRDYG1") +
...
const _LANGS = {
  en: {
    ...
    para4: () =>
      ...
      "without any prejudice and is provided solely for the purpose of confirming the commercial relationship and earnings.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      ". This Annual Income Certificate is issued for tax declaration and legal purposes.",
  ...
  fr: {
    ...
    para2: (name, co, regNo, memberId, title) =>
      ...
      name +
      " aupr\xE8s de nos services est le " +
      memberId +
      ".",
    para3annual: (name, start, amount, year) =>
      [
    ...
    para4: () =>
      ...
      "préjudice et est fournie uniquement à des fins de confirmation de la relation commerciale et des revenus.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      ". Ce Certificat de Revenus Annuel est \xE9mis \xE0 des fins de d\xE9claration fiscale et l\xE9gale.",
  ...
  es: {
    ...
    para4: () =>
      ...
      "sin perjuicio y únicamente a efectos de confirmar la relación comercial y los ingresos.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      ". Este Certificado de Ingresos Anuales se emite a efectos de declaraci\xF3n fiscal y legal.",
  ...
  de: {
    ...
    para4: () =>
      ...
      "Vorurteile ausgestellt und dient ausschließlich der Bestätigung der Geschäftsbeziehung und der Einnahmen.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      ". Diese Jahreseinkommensbescheinigung wird f\xFCr steuerliche und rechtliche Zwecke ausgestellt.",
  ...
  pt: {
    ...
    para4: () =>
      ...
      "destina-se exclusivamente a confirmar a relação comercial e os rendimentos.",
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      ". Este Certificado de Rendimentos Anuais \xE9 emitido para efeitos de declara\xE7\xE3o fiscal e legal.",
    authorisedBy: (name) => "Autorizado por: " + name,
    generated: "Gerado em:",
    page: (n, t) => "P\xE1gina " + n + " / " + t,
  ...
  ar: {
    ...
    para3monthly: (name, period, amount) =>
      ...
      " \u0645\u0628\u0644\u063A " +
      amount +
      " \u062F\u0648\u0644\u0627\u0631 \u0623\u0645\u0631\u064A\u0643\u064A \u0645\u0646 \u0627\u0644\u0639\u0645\u0648\u0644\u0627\u062A.",
    para4: () =>
      "لأي استفسار يتعلق بمحتوى هذا " +
      "الخطاب، يُرجى التواصل معنا " +
      "على العنوان الموضّح أدناه. يصدر " +
      "هذا الخطاب دون أي إخلال، وذلك " +
      "لغرض تأكيد العلاقة التجارية " +
      "والأرباح فحسب.",
    summaryTitle: (year) =>
      "\u0645\u0644\u062E\u0635 \u0627\u0644\u0623\u0631\u0628\u0627\u062D \u0627\u0644\u0633\u0646\u0648\u064A\u0629 \u2014 " + year,
    summaryMonthly: (period) =>
      "\u0645\u0644\u062E\u0635 \u0627\u0644\u0623\u0631\u0628\u0627\u062D \u0627\u0644\u0634\u0647\u0631\u064A\u0629 \u2014 " + period,
    ...
    certText: (name, uid, amount, co, wdAmount, year) =>
      ...
      wdAmount +
      (". تُصدر هذه الشهادة لأغراض التصريح " + "الضريبي والإجراءات القانونية."),
...
function _showLangPicker(callback) {
  ...
  overlay.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:bl" +
    "ur(4px)";
  const card = document.createElement("div");
  card.style.cssText =
    "background:#1a1f2e;border:1px solid #2d3448;border-radius:16px;padding:24px;max-width:340px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.5)";
  ...
  Object.entries(_LANGS).forEach(([code, l]) => {
    ...
    btn.type = "button";
    btn.style.cssText =
      "display:flex;align-items:center;gap:10px;background:#252b3b;border:1px solid #3a4260;color:#fff;font-size:13px;font-weight:500;padding:10px " +
      "14px;border-radius:10px;cursor:pointer;transition:background 0.15s,border-color 0.15s;text-align:left;width:100%";
    btn.onmouseenter = () => {
  ...
  cancelBtn.style.cssText =
    "margin-top:12px;width:100%;background:none;border:none;color:#6b7280;font-size:11px;cursor:pointer;padding:8px;border-radius:6px;transition:color " +
    "0.15s";
...
async function generateMonthlyStatement() {
  ...
  try {
    ...
    const periodLabel =
      data.period_label ||
      new Date(parseInt(yr), parseInt(mo) - 1, 1).toLocaleDateString("en-GB", {
        month: "long",
        year: "numeric"
      });
    ...
    const _drawPage = () => {
      ...
      if (coL1)
        pdf.text(coL1, pw - 8, 10, {
          align: "right"
        });
      if (coL2)
        pdf.text(coL2, pw - 8, 15.5, {
          align: "right"
        });
      if (coL3)
        pdf.text(coL3, pw - 8, 21, {
          align: "right"
        });
    };
...
async function generatePurchaseReceipt() {
  ...
  try {
    ...
    if (rcptLegal)
      pdf.text(rcptLegal, pw - 8, 11, {
        align: "right"
      });
    if (rcptReg)
      pdf.text(rcptReg, pw - 8, 16, {
        align: "right"
      });
    if (rcptAddr)
      pdf.text(rcptAddr, pw - 8, 21, {
        align: "right"
      });
    let y = 36;
    pdf.setTextColor(...NAVY);
...
function _letterDrawHeader(pdf, pw, co, dateStr) {
  ...
  if (coLine1)
    pdf.text(coLine1, pw - 8, 10, {
      align: "right"
    });
  if (coLine2)
    pdf.text(coLine2, pw - 8, 15.5, {
      align: "right"
    });
  if (coLine3)
    pdf.text(coLine3, pw - 8, 21, {
      align: "right"
    });
  pdf.setTextColor(...GRAY);
  pdf.setFontSize(9);
...
function _letterDrawFooter(pdf, pw, ph, co) {
  ...
  const line1 = [
    ...
    co.registration_number ? "Co. No. " + co.registration_number : "",
  ...
  if (line2)
    pdf.text(line2, pw / 2, ph - 8.5, {
      align: "center"
    });
  if (line3)
    pdf.text(line3, pw / 2, ph - 5, {
      align: "center"
    });
}
...
function _buildMandateBlock(cardData, m) {
  ...
  if (!mandate.enabled) {
    return (
      ...
      '<input type="checkbox" id="prof-vdi" class="mt-0.5 accent-gold-500 w-4 h-4 flex-shrink-0" ' +
      (m && m.vdi_consent ? "checked" : "") +
      " onchange=\"if(this.checked){api('PUT','/members/profile',{vdi_consent:1}).catch(()=>{})}\">" +
      ('<span class="text-sm text-gray-300"><strong class="text-white">Autorisation VDI</strong> — Je déclare exercer mon activité de Vendeur à Domicile ' +
        "Indépendant(e) en conformité avec la réglementation en vigueur et les exigences légales du marketing de réseau.") +
      (m && m.vdi_consent_at
        ? '<span class="block text-xs text-gray-500 mt-1">Accepté le ' + new Date(m.vdi_consent_at).toLocaleDateString("fr-FR") + "</span>"
        : "") +
      "</span></label></div>"
    );
  }
...
function _buildCardButtonsBlock(cardData) {
  ...
  var physPrice = phys.price || 19.9;
  var physCurr = phys.currency || "USD";
  var physOrder = phys.current_order || null;
  var virtualBtn =
    '<div class="relative" id="virtual-card-menu-wrap">' +
    ('<button onclick="_showVirtualCardMenu()" class="flex items-center gap-2 bg-rouge-500 text-dark-900 font-bold px-5 py-2.5 rounded-xl hover:bg-rouge-500 ' +
      'transition text-sm">') +
    '<i class="fas fa-id-card"></i> Carte virtuelle <i class="fas fa-chevron-down text-xs ml-1"></i></button>' +
    ('<div id="virtual-card-dropdown" class="hidden absolute left-0 top-full mt-1 z-50 bg-dark-800 border border-dark-600 rounded-xl shadow-xl min-w-[160px] ' +
      'overflow-hidden">') +
    ('<button onclick="downloadCardPNG();_hideVirtualCardMenu()" class="flex items-center gap-2 w-full px-4 py-3 text-sm text-gray-300 hover:bg-dark-700 ' +
      'hover:text-white transition"><i class="fas fa-image text-rouge-400"></i> Télécharger PNG</button>') +
    ('<button onclick="downloadCardPDF();_hideVirtualCardMenu()" class="flex items-center gap-2 w-full px-4 py-3 text-sm text-gray-300 hover:bg-dark-700 ' +
      'hover:text-white transition"><i class="fas fa-file-pdf text-red-400"></i> Télécharger PDF</button>') +
    "</div></div>";
  var physicalBtn = "";
  if (physEnabled) {
    if (physOrder && ["pending", "paid", "processing", "shipped"].includes(physOrder.status)) {
      ...
      physicalBtn =
        '<div class="flex items-center gap-2 border border-amber-500/40 text-amber-400 px-5 py-2.5 rounded-xl text-sm">' +
        ...
        (sl[physOrder.status] || physOrder.status) +
        "</span></div></div>";
    } else {
      physicalBtn =
        '<button onclick="wizardPhysicalCard()" class="flex items-center gap-2 border border-amber-500/50 text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 ' +
        'font-bold px-5 py-2.5 rounded-xl transition text-sm">' +
        '<i class="fas fa-credit-card"></i> Carte physique</button>';
    }
  }
  var revokeBtn =
    '<button onclick="revokeCard()" id="btn-revoke" class="flex items-center gap-2 border border-red-500/40 text-red-400 px-5 py-2.5 rounded-xl ' +
    'hover:bg-red-900/20 transition text-sm ml-auto">' +
...
async function wizardPhysicalCard() {
  ...
  try {
    var results = await Promise.all([
      api("GET", "/members/card").catch(function () {
        return null;
      }),
      !_gw || !Object.keys(_gw).length
        ? api("GET", "/members/packages").catch(function () {
            return null;
          })
...
function _wizardPhysCardStep1() {
  ...
  _wizardShow(
    [
      ...
      ('<button onclick="_wizardPhysCardValidate()" class="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-dark-900 font-bold rounded-xl transition text-sm">' +
        'Continuer <i class="fas fa-arrow-right ml-2"></i></button>') +
      "</div>" +
      "</div>"
  );
}
function _wizardPhysCardValidate() {
  var fn = (
    document.getElementById("phys-full-name") || {
      value: ""
    }
  ).value.trim();
  var ad = (
    document.getElementById("phys-address") || {
      value: ""
    }
  ).value.trim();
  var ci = (
    document.getElementById("phys-city") || {
      value: ""
    }
  ).value.trim();
  var po = (
    document.getElementById("phys-postal") || {
      value: ""
    }
  ).value.trim();
  var co = (
    document.getElementById("phys-country") || {
      value: ""
    }
  ).value.trim();
  var ph = (
    document.getElementById("phys-phone") || {
      value: ""
    }
  ).value.trim();
...
async function renderMemberCard(e) {
  ...
  try {
    ...
    const langOptions = LANGS.map(
      (l) => '<option value="' + l.code + '" ' + (((m && m.preferred_lang) || "fr") === l.code ? "selected" : "") + ">" + l.label + "</option>"
    ).join("");
    ...
    const missingHtml =
      missingItems.length > 0
        ? '<div class="bg-orange-900/20 border border-orange-500/30 rounded-xl p-4 mb-4"><p class="text-orange-300 font-medium text-sm mb-2"><i class="fas ' +
          'fa-exclamation-triangle mr-1"></i>Profil incomplet :</p><ul class="list-disc list-inside text-orange-400 text-xs space-y-1">' +
          missingItems.map((k) => "<li>" + (missingLabels[k] || k) + "</li>").join("") +
          "</ul></div>"
        : "";
    ...
    const enrichedSection =
      ...
      ('<button onclick="saveProfileEnriched()" class="bg-rouge-500 text-dark-900 font-bold px-6 py-2.5 rounded-xl hover:bg-rouge-500 transition flex ' +
        'items-center gap-2"><i class="fas fa-save"></i> Sauvegarder le profil</button>') +
      "</div>";
    let cardSection = "";
    if (!licenseActive) {
      cardSection =
        '<div class="bg-dark-800 rounded-2xl border border-dark-600 p-6"><div class="flex items-start gap-3 bg-orange-900/20 border border-orange-500/30 ' +
        'rounded-xl p-4"><i class="fas fa-lock text-orange-400 mt-0.5"></i><div><p class="text-orange-300 font-medium">Licence inactive</p><p ' +
        'class="text-orange-400/70 text-sm mt-1">Vous devez avoir une licence active pour obtenir votre carte de membre.</p><button ' +
        'onclick="showPage(\'license\')" class="mt-2 text-xs bg-rouge-500 text-dark-900 font-bold px-3 py-1.5 ' +
        'rounded-lg hover:bg-rouge-500 transition"><i class="fas fa-key mr-1"></i>Activer ma licence</button></div></div></div>';
    } else if (card) {
      ...
      cardSection =
        ...
        (config.design_a_enabled
          ? '<button onclick="orderCard(\'A\')" class="text-sm px-4 py-2 rounded-xl border ' +
            ...
            ' transition">Design A — Prestige</button>'
          : "") +
        (config.design_b_enabled
          ? '<button onclick="orderCard(\'B\')" class="text-sm px-4 py-2 rounded-xl border ' +
            (card.design === "B" ? "border-blue-400 bg-blue-500/10 text-blue-300" : "border-dark-500 text-gray-300 hover:border-blue-400/50") +
            ' transition">Design B — Navy</button>'
          : "") +
        "</div></div></div>";
    } else {
      cardSection =
        ...
        (config.legal_mention || "") +
        "</p></div>";
    }
    e.innerHTML =
      '<div class="max-w-2xl space-y-6"><div class="flex items-center justify-between"><h2 class="text-xl font-bold text-white flex items-center gap-2"><i ' +
      'class="fas fa-id-badge text-rouge-400"></i> Carte de Membre</h2></div>' +
      enrichedSection +
      cardSection +
      "</div>";
...
async function renderReserveStrategique(el) {
  ...
  try {
    ...
    html +=
      '<div class="flex items-center gap-3 mb-2"><button onclick="(window._prevPage?showPage(window._prevPage):showPage(\'wallet\'))" class="text-gray-500 ' +
      'hover:text-white transition-colors p-1 rounded-lg hover:bg-dark-700"><i class="fas fa-arrow-left text-sm"></i></button><div><h1 class="text-xl ' +
      'font-bold text-white flex items-center gap-2"><i class="fas fa-vault text-cyan-400"></i> Réserve Stratégique</h1><p class="text-gray-400 text-xs ' +
    ...
    html +=
      ...
      '</div><div class="text-2xl font-bold text-emerald-400">' +
      fmt$(totalAbondement) +
      '</div><div class="text-xs text-gray-500 mt-1">Abondement compagnie (+' +
      aboRate +
      "%)</div></div>";
    html +=
      '<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Disponibilité ' +
      'prévue</span><div class="w-8 h-8 rounded-lg bg-orange-500/15 flex items-center justify-center"><i class="fas fa-calendar-check text-orange-400 ' +
      'text-sm"></i></div></div>' +
      (nextUnlock
        ? '<div class="text-lg font-bold text-orange-400">' +
          fmtD(nextUnlock) +
          '</div><div class="text-xs text-gray-500 mt-1">' +
          (daysNext !== null ? "dans " + daysNext + " jours" : "—") +
          "</div>"
        : ' <div class="text-lg font-bold text-gray-500">—</div><div class="text-xs text-gray-500 mt-1">Aucun dépôt actif</div>') +
      "</div>";
    html += "</div>";
    if (totalActive > 0) {
      ...
      html +=
        ...
        progPct +
        ('%;background:linear-gradient(90deg,#10b981,#059669)"></div></div><div class="flex justify-between text-xs text-gray-600 mt-1"><span>Votre mise</span><' +
          "span>Abondement compagnie</span></div></div>");
    }
    if (withdrawalEnabled && totalAvailable > 0) {
      ...
      html +=
        '<div class="bg-dark-700/60 border border-cyan-500/20 rounded-xl p-4 text-sm"><div class="flex items-start gap-3"><i class="fas fa-hourglass-half ' +
        'text-cyan-400 mt-0.5 text-base"></i><div><p class="text-white font-semibold mb-1">Votre épargne est en cours de maturation</p><p class="text-gray-400">' +
        'Elle sera disponible le <span class="text-cyan-400 font-medium">' +
        (nextUnlock ? fmtD(nextUnlock) : "—") +
        "</span>" +
        (daysNext !== null ? ' — dans <span class="text-white font-medium">' + daysNext + " jours</span>" : "") +
        ('.</p><p class="text-gray-500 text-xs mt-2">À cette date, vous pourrez transférer librement votre épargne et l\'abondement vers votre portefeuille.</p><' +
          "/div></div></div>");
    } else if (totalActive > 0 && !withdrawalEnabled) {
      html +=
        '<div class="bg-yellow-900/20 border border-yellow-500/20 rounded-xl p-4 text-sm text-yellow-400"><i class="fas fa-info-circle mr-2"></i>Les transferts ' +
        "de Réserve Stratégique sont momentanément suspendus.</div>";
    }
    if (active.length > 0) {
      html +=
        '<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-piggy-bank text-cyan-400"></i> Dépôts en cours (' +
        active.length +
    ...
    if (hist.length > 0) {
      html +=
        '<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-history text-gray-400"></i> Historique</h3><div ' +
        'class="space-y-1">' +
        hist
          .map(function (e) {
    ...
    if (entries.length === 0) {
      html +=
        '<div class="text-center py-12"><div class="w-16 h-16 rounded-full bg-cyan-500/10 flex items-center justify-center mx-auto mb-4"><i class="fas fa-vault ' +
        'text-cyan-500/50 text-2xl"></i></div><p class="text-gray-400 font-medium">Aucune réserve stratégique pour le moment</p><p class="text-gray-600 text-sm ' +
        'mt-1">Vos primes de leadership alimenteront automatiquement votre réserve dès le rang Mentor.</p></div>';
    }
    html += "</div>";
    el.innerHTML = html;
  } catch (err) {
    el.innerHTML =
      '<div class="text-red-400 text-center py-12"><i class="fas fa-exclamation-triangle text-2xl mb-3 block"></i>' +
      (err.message || err.error || "Erreur inconnue") +
      "</div>";
  }
}
...
async function renderServices(el) {
  ...
  try {
    ...
    el.innerHTML =
      '<div class="p-8 text-center text-red-400"><i class="fas fa-exclamation-triangle mr-2"></i>Impossible de charger les services</div>';
    return;
  }
    ...
    e.innerHTML =
      ...
      (0 === s.length
        ? '\n        <div class="p-8 text-center">\n          <i class="fas fa-inbox text-gray-600 text-3xl mb-3"></i>\n          <p class="text-gray-400 text-sm">' +
          'Aucun filleul en attente de placement.</p>\n          <p class="text-gray-500 text-xs mt-1">Partagez votre lien de parrainage pour inviter des membres.<' +
          "/p>\n        </div>"
        : '\n\n        <div class="divide-y divide-dark-600">\n\n          ' +
          s
            .map((e) => {
      ...
      (r.length > 0
        ? '\n\n      <div class="bg-dark-800 rounded-2xl border border-dark-600 overflow-hidden">\n\n        <div class="px-5 py-4 border-b border-dark-600">\n\n       ' +
          ...
          "\n\n        </div>\n\n      </div>"
        : "") +
      ('\n\n    </div>\n\n\n\n    <!-- ═══ MODALE PLACEMENT DANS L\'ARBRE ═══ -->\n\n    <div id="placement-modal" class="fixed inset-0 z-50 hidden items-center ' +
        'justify-center p-4" style="background:rgba(0,0,0,0.85);">\n\n      <div class="bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-4xl ' +
        'max-h-[90vh] flex flex-col shadow-2xl">\n\n        <!-- En-tête -->\n\n        <div class="flex items-center justify-between px-6 py-4 border-b ' +
        'border-dark-600 flex-shrink-0">\n\n          <div>\n\n            <h3 class="font-bold text-white text-lg flex items-center gap-2">\n' +
        ('\n              <i class="fas fa-sitemap text-orange-400"></i>\n\n              Placement dans l\'arbre\n\n            </h3>\n\n            <p ' +
          'id="placement-modal-subtitle" class="text-sm text-gray-400 mt-0.5"></p>\n\n          </div>\n\n          <button onclick="closePlacementModal()" ' +
          'class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-dark-700 text-gray-400 hover:text-white transition">\n\n            <i class="fas ' +
          'fa-times"></i>\n\n          </button>\n\n        </div>\n\n        <!-- Instruction -->\n') +
        ('\n        <div class="px-6 py-3 bg-orange-900/20 border-b border-orange-500/20 flex-shrink-0">\n\n          <p class="text-orange-300 text-sm flex ' +
          'items-center gap-2">\n\n            <i class="fas fa-hand-pointer text-orange-400"></i>\n\n            Cliquez sur un <strong>spot libre</strong> (fond ' +
          "pointillé) pour y placer votre filleul.\n\n            Les spots occupés ne sont pas cliquables.\n\n          </p>\n\n        </div>\n\n        <!-- Arbre " +
        ...
        ('\n          <div class="flex justify-center py-8"><div class="loader"></div></div>\n\n        </div>\n\n        <!-- Pied -->\n\n        <div class="px-6 ' +
          'py-4 border-t border-dark-600 flex justify-end flex-shrink-0">\n\n          <button onclick="closePlacementModal()" class="px-5 py-2 bg-dark-700 ' +
          'hover:bg-dark-600 text-gray-300 rounded-xl text-sm transition">\n\n            Annuler\n\n          </button>\n\n        </div>\n\n      </div>\n\n    </div>'));
  } catch (t) {
    e.innerHTML =
      '<div class="p-4 text-red-400"><i class="fas fa-exclamation-triangle mr-2"></i>Erreur : ' +
      (t.error || t.message || "Impossible de charger le réservoir") +
      "</div>";
  }
}
function copyReservoirLink() {
  ...
  e &&
    navigator.clipboard
      .writeText(e.value)
      .then(() => showToast("Lien de parrainage copié !"))
      .catch(() => {
...
function svgCollectPlacement(e, t, n, a, s, r, i, l, d, o, c) {
    ...
    if (null !== d && null !== c) {
      ...
      r.push(
        [
          ...
          '" y="' +
          t +
          '" width="154" height="108" rx="12" fill="transparent" class="svg-slot-hover-zone"/>\n\n        </g>'
      );
    }
    return;
  }
  ...
  (s.push(
    ...
    t < n &&
      (svgCollectPlacement(e.left, t + 1, n, a, s, r, e.id, e.unique_id, x, p, "L"),
      svgCollectPlacement(e.right, t + 1, n, a, s, r, e.id, e.unique_id, x, p, "R")));
}
...
function renderPlacementNode(e, t, n = !1) {
  ...
  const a = e.in_holding_tank ? '<div class="text-[9px] text-orange-400 mt-0.5"><i class="fas fa-clock mr-0.5"></i>Holding</div>' : "",
    ...
    r =
      ...
      "</span>\n\n    </div>\n\n    " +
      (n
        ? '<div class="text-[9px] text-rouge-500/70 mt-1 font-medium">Vue actuelle</div>'
        : '<div class="text-[9px] text-gray-600 mt-1"><i class="fas fa-hand-pointer mr-0.5"></i>Cliquer pour descendre</div>') +
      "\n\n  ",
...
async function openPlacementModal(e, t, n) {
  ...
  ((s.textContent = "Choisissez o\xF9 placer : " + t + " (" + n + ")"),
    (r.innerHTML = '<div class="flex justify-center py-8"><div class="loader"></div></div>'),
    a.classList.remove("hidden"),
    a.classList.add("flex"));
  try {
    if (_fullTree)
      return (
        (_placeTree = _fullTree),
        (_placeRoot = _fullTree),
        (_placeFocus = _fullTree),
        (_placeNavStack = []),
        void renderPlacementClassicInner()
      );
    const e = await api("GET", "/members/tree?depth=12");
    if (!e.tree)
      return void (r.innerHTML =
        '\n        <div class="text-center py-12">\n          <i class="fas fa-tree text-gray-600 text-4xl mb-4"></i>\n          <p class="text-gray-400">Vous ' +
        "n'êtes pas encore placé dans l'arbre binaire.</p>\n          <p class=\"text-gray-500 text-sm mt-2\">Vous devez être placé pour pouvoir placer vos " +
        "filleuls.</p>\n        </div>");
  ...
  ((r.id = "placement-confirm-banner"),
    ...
    (r.innerHTML =
      ...
      ('\n\n        </div>\n\n        <div class="text-orange-400/60 text-xs mt-0.5">[!] Cette action est irréversible.</div>\n\n      </div>\n\n    </div>\n\n    <div ' +
        ...
        'transition">Annuler</button>\n\n      <button id="btn-confirm-place" class="px-4 py-2 bg-orange-500 text-white font-bold rounded-xl text-sm ' +
...
function mktgMemberCard(e) {
  ...
  return (
    ...
    (e.is_downloadable
      ? '\n\n        <a href="' +
        e.url +
        ('" download target="_blank"\n\n          class="text-xs bg-green-600/20 text-green-400 border border-green-600/30 px-3 py-2 rounded-xl ' +
          'hover:bg-green-600/30 transition flex items-center justify-center gap-1.5"\n\n          title="Télécharger">\n\n          <i class="fas fa-download"></i>\n\n' +
          "        </a>")
      : "") +
    "\n\n      </div>\n\n    </div>\n\n  </div>"
  );
      ...
      (e.innerHTML = [
        ...
        n
          ? [
            ...
            ('"\n\n            class="flex items-center gap-1.5 bg-gray-500/20 text-gray-400 border border-gray-500/30 text-xs px-2.5 py-1.5 rounded-lg ' +
              'hover:bg-gray-500/30 transition">\n\n            <i class="fas fa-envelope"></i><span class="hidden sm:inline">Email</span>\n\n          </a>\n\n          <' +
              'button onclick="toggleQR()" class="flex items-center gap-1.5 bg-dark-600 text-gray-400 border border-dark-500 text-xs px-2.5 py-1.5 rounded-lg ' +
            ...
            encodeURIComponent(n) +
            ('"\n\n            alt="QR Code" class="rounded-xl border border-dark-600 w-20 h-20 flex-shrink-0">\n\n          <p class="text-xs text-gray-400">Faites ' +
              "scanner ce QR pour que votre filleul s'inscrive via votre lien.</p>\n\n        </div>")
          : '\n        <div class="bg-orange-900/20 border border-orange-500/30 rounded-xl p-3 flex items-center gap-3">\n          <i class="fas fa-lock ' +
            'text-orange-400 flex-shrink-0"></i>\n          <div class="flex-1 min-w-0">\n            <p class="text-orange-300 font-medium text-sm">Licence requise<' +
...
function openMktgViewer(e, t, n) {
  ...
  if (isImg) {
    body =
      '<div class="flex-1 bg-gray-900 flex items-center justify-center p-4 overflow-auto">' +
      '<img src="' +
      a +
      '" alt="' +
      title +
    ...
    body =
      ...
      '<video src="' +
      a +
      '" controls class="max-w-full" style="max-height:calc(85vh - 56px)"></video>' +
      "</div>";
  } else if (isAud) {
    body =
      [
    ...
    body =
      ...
      '<iframe src="' +
      a +
      '" class="w-full h-full border-0 flex-1" style="min-height:0"></iframe>' +
      "</div>";
  }
  var html =
    [
...
async function renderCCWallet(e) {
  ...
  try {
    ...
    e.innerHTML =
              ...
              (e) =>
                ...
                (e.justificatif_url
                  ? '<a href="' +
                    e.justificatif_url +
                    '" download="' +
                    (e.justificatif_name || "justif") +
                    '" class="inline-flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs"><i class="fas fa-' +
                    (e.justificatif_url.startsWith("data:image/")
                      ? "fa-image"
                      : e.justificatif_url.startsWith("data:application/pdf")
                        ? "fa-file-pdf"
                        : "fa-file") +
                    '"></i><span>Voir</span></a>'
                  : '<span class="text-gray-600 text-xs">—</span>') +
      ...
      (0 === a.length
        ? '<p class="text-gray-500 text-sm text-center py-4">Aucune opération pour le moment</p>'
        : '<div class="overflow-x-auto">\n\n               <table class="w-full text-sm">\n\n                 <thead>\n\n                   <tr class="text-xs ' +
          'text-gray-500 border-b border-dark-600">\n\n                     <th class="pb-2 text-left">Date</th>\n\n                     <th class="pb-2 text-right">' +
    ...
    e.innerHTML =
      '<div class="p-4 text-red-400 text-sm"><i class="fas fa-triangle-exclamation mr-2"></i>Erreur : ' +
      (t?.error || t?.message || "Impossible de charger le wallet CC") +
      "</div>";
  }
}
function _ccAddJustif(inp) {
  ...
  files.forEach(function (f) {
    ...
    info.innerHTML =
      '<div class="text-sm text-white font-medium truncate">' + f.name + '</div><div class="text-xs text-gray-500">' + tl + " · " + sz + "</div>";
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className =
      "ml-auto w-7 h-7 rounded-full bg-dark-600 hover:bg-red-900/40 border border-dark-500 flex items-center justify-center flex-shrink-0";
...
async function submitCCWithdrawal() {
  ...
  if (!amount || amount < (cfg.min_amount || 10))
    return ((elErr.textContent = "Montant minimum : " + (cfg.min_amount || 10) + "$"), void elErr.classList.remove("hidden"));
  if (fields.description?.visible !== false && fields.description?.required !== false) {
    const minC = cfg.desc_min_chars || 10;
    if (!description || description.length < minC)
      return (
        (elErr.textContent = (fields.description?.label || "Motif de la demande") + " requis (" + minC + " caractères minimum)"),
        void elErr.classList.remove("hidden")
      );
  }
  if (fields.justificatif?.visible !== false && fields.justificatif?.required) {
    if (!files.length)
      return ((elErr.textContent = (fields.justificatif?.label || "Justificatif") + " obligatoire"), void elErr.classList.remove("hidden"));
  }
  ...
  if ((fields.custom1?.visible === true || fields.custom1?.visible === "1") && fields.custom1?.required) {
    if (!elC1 || !elC1.value.trim())
      return ((elErr.textContent = (fields.custom1?.label || "Champ 1") + " obligatoire"), void elErr.classList.remove("hidden"));
  }
  ...
  if ((fields.custom2?.visible === true || fields.custom2?.visible === "1") && fields.custom2?.required) {
    if (!elC2 || !elC2.value.trim())
      return ((elErr.textContent = (fields.custom2?.label || "Champ 2") + " obligatoire"), void elErr.classList.remove("hidden"));
  }
  ...
  if ((fields.custom3?.visible === true || fields.custom3?.visible === "1") && fields.custom3?.required) {
    if (!elC3 || !elC3.value.trim())
      return ((elErr.textContent = (fields.custom3?.label || "Champ 3") + " obligatoire"), void elErr.classList.remove("hidden"));
  }
  elBtn.disabled = true;
...
async function renderNotifications(e, page, perPage) {
  ...
  try {
    ...
    (await api("PUT", "/members/notifications/read-all"),
      ...
      document.getElementById("header-notif-badge").classList.add("hidden"),
      (e.innerHTML =
    ...
    renderPagination(
      ...
      page,
      perPage,
      "(function(p){renderNotifications(document.getElementById('page-content'),p,window._notifPerPage)})",
      "(function(pp,p){renderNotifications(document.getElementById('page-content'),p,pp)})"
    );
  } catch (t) {
