...
async function wizardStep5V2Manual(){
  ...
  <div class="p-6 space-y-5">
    ...
    <div class="bg-dark-700 border border-emerald-500/30 rounded-xl p-4 space-y-2 text-sm">
      ...
      <p class="text-xs text-gray-500 pt-2 border-t border-dark-600">R\u00e9f\u00e9rence : votre identifiant unique LEADER Network</p>
    </div>
    <div>
  ...
  <div class="p-8 text-center space-y-5">
    ...
    <button onclick="wizardClose(); renderLicense(document.getElementById('page-content'))"
      ...
      <div class="bg-red-900/20 border border-red-500/30 rounded-xl p-5">
        ...
        <div class="bg-red-900/30 border border-red-500/20 rounded-xl p-3 mb-4">
          <p class="text-sm text-red-300"><i class="fas fa-exclamation-triangle mr-2"></i>Votre licence est expirée. Les commissions sont stoppées. Renouvelez pour réactiver vos droits LEADER Network.</p>
        </div>
      ...
      <div class="bg-dark-700 border border-dark-500 rounded-xl p-5">
        ...
        <div class="bg-rouge-500/10 border border-rouge-500/20 rounded-xl p-4 mb-4">
          ...
          <p class="text-sm text-gray-400">Accès complet à la plateforme Finstrategia + éligibilité aux commissions LEADER Network pendant 12 mois.</p>
        </div>
    ...
    <div class="max-w-lg space-y-6">
      <div>
        ...
        <p class="text-gray-400 text-sm mt-1">Licence annuelle ($${fmtPrice(_price)}/an) — éligibilité aux commissions LEADER Network</p>
      </div>
      ${d}
...
async function _cpStartPayment() {
  ...
  try {
    const body = { amount, coin, currency: 'USD', description: 'LEADER Network — Paiement' };
  ...
  showModal(`
    <div class="p-6 space-y-5">
      ...
      <div class="bg-dark-700/60 border border-dark-500 rounded-xl p-3 text-xs text-gray-400 leading-relaxed">
        ...
        Chez LEADER Network, l'excellence du support est une priorité absolue.
...
async function wizardPhysicalCard() {
  ...
  try {
    ...
    _wizardData.physCardDesc     = phys.desc     || 'Carte de membre physique LEADER Network';
    ...
    _wizardData.physCardDesc     = 'Carte de membre physique LEADER Network';
...
function _wizardPhysCardStep1() {
  ...
  _wizardShow(
    ...
    + '<div class="flex items-center gap-2 text-gray-400 text-sm"><i class="fas fa-credit-card text-amber-400"></i> Carte physique LEADER Network</div>'
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
async function renderReserveStrategique(el){el.innerHTML='<div class="flex items-center justify-center py-16"><div class="loader"></div></div>';try{var d=await api('GET','/members/reserve-strategique');var entries=d.entries||[];var locked=entries.filter(function(e){return e.status==='locked';});var released=entries.filter(function(e){return e.status==='released'||e.status==='unlocked';});var fmtD=function(v){return v?new Date(v).toLocaleDateString('fr-FR',{day:'2-digit',month:'short',year:'numeric'}):'—';};var today=new Date();var daysUntil=function(ds){if(!ds)return null;return Math.ceil((new Date(ds).getTime()-today.getTime())/86400000);};var totalLocked=d.total_locked||0;var totalReleased=d.total_released||0;var totalWithAbo=d.total_with_abondement||0;var gain=d.gain_abondement||0;var aboRate=Math.round((d.abondement_rate||0)*100);var nextUnlock=d.next_unlock_date;var daysNext=daysUntil(nextUnlock);var withdrawalEnabled=d.withdrawal_enabled!==false;var firstLocked=locked.length>0?locked[locked.length-1]:null;var progressPct=(function(){if(!firstLocked)return 0;var start=new Date(firstLocked.created_at).getTime();var end=new Date(firstLocked.locked_until).getTime();var now=today.getTime();if(end<=start)return 100;return Math.min(100,Math.max(0,Math.round((now-start)/(end-start)*100)));})();var html='<div class="space-y-6">';html+='<div class="flex items-center gap-3 mb-2"><button onclick="showPage(\'wallet\')" class="text-gray-500 hover:text-white transition-colors p-1 rounded-lg hover:bg-dark-700"><i class="fas fa-arrow-left text-sm"></i></button><div><h1 class="text-xl font-bold text-white flex items-center gap-2"><i class="fas fa-vault text-cyan-400"></i> Réserve Stratégique</h1><p class="text-gray-400 text-xs mt-0.5">Votre épargne bloquée avec abondement garanti</p></div></div>';html+='<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">';html+='<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Total bloqué</span><div class="w-8 h-8 rounded-lg bg-cyan-500/15 flex items-center justify-center"><i class="fas fa-lock text-cyan-400 text-sm"></i></div></div><div class="text-2xl font-bold text-cyan-400">'+fmt$(totalLocked)+'</div><div class="text-xs text-gray-500 mt-1">'+locked.length+' dépôt'+(locked.length>1?'s':'')+'</div></div>';html+='<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Abondement</span><div class="w-8 h-8 rounded-lg bg-emerald-500/15 flex items-center justify-center"><i class="fas fa-seedling text-emerald-400 text-sm"></i></div></div><div class="text-2xl font-bold text-emerald-400">+'+aboRate+'%</div><div class="text-xs text-gray-500 mt-1">Gain échéance : <span class="text-emerald-400 font-semibold">+'+fmt$(gain)+'</span></div></div>';html+='<div class="stat-card"><div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-400 uppercase tracking-wider">Prochaine libération</span><div class="w-8 h-8 rounded-lg bg-orange-500/15 flex items-center justify-center"><i class="fas fa-unlock text-orange-400 text-sm"></i></div></div>'+(nextUnlock?'<div class="text-lg font-bold text-orange-400">'+fmtD(nextUnlock)+'</div><div class="text-xs text-gray-500 mt-1">'+(daysNext!==null?daysNext+' jours restants':'—')+'</div>':'<div class="text-lg font-bold text-gray-500">—</div><div class="text-xs text-gray-500 mt-1">Aucun dépôt actif</div>')+'</div></div>';if(totalLocked>0){html+='<div class="stat-card border-cyan-500/20"><div class="flex items-center justify-between mb-3"><span class="text-sm font-semibold text-white">Valeur totale à l\'échéance</span><span class="text-xl font-bold text-cyan-300">'+fmt$(totalWithAbo)+'</span></div><div class="flex justify-between text-xs text-gray-400 mb-1"><span>Mise initiale : <span class="text-white font-medium">'+fmt$(totalLocked)+'</span></span><span>Progression : <span class="text-cyan-400 font-medium">'+progressPct+'%</span></span></div><div class="h-2 bg-dark-700 rounded-full overflow-hidden"><div class="h-2 rounded-full transition-all" style="width:'+progressPct+'%;background:linear-gradient(90deg,#06b6d4,#0e7490)"></div></div><div class="flex justify-between text-xs text-gray-600 mt-1"><span>Créé le '+(firstLocked?fmtD(firstLocked.created_at):'—')+'</span><span>Échéance '+(nextUnlock?fmtD(nextUnlock):'—')+'</span></div></div>';}if(withdrawalEnabled&&totalReleased>0){html+='<div class="stat-card border-cyan-500/20"><h3 class="font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-arrow-right-from-bracket text-cyan-400"></i> Retirer vers mon portefeuille</h3><p class="text-xs text-gray-400 mb-4">Le montant retiré sera crédité immédiatement dans votre portefeuille disponible.</p><div class="flex flex-col sm:flex-row gap-3"><div class="flex-1"><label class="text-xs text-gray-400 block mb-1">Montant (max '+fmt$(totalReleased)+')</label><input id="rs-withdraw-amount" type="number" min="1" max="'+totalReleased.toFixed(2)+'" step="0.01" placeholder="Ex : 500" class="w-full bg-dark-700 border border-dark-600 rounded-xl px-4 py-2.5 text-white text-sm focus:border-cyan-500/60 focus:outline-none" /></div><button onclick="submitRSWithdrawal('+totalReleased+')" class="bg-cyan-600 hover:bg-cyan-500 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-2 justify-center sm:self-end"><i class="fas fa-wallet"></i> Retirer</button></div><div id="rs-withdraw-msg" class="mt-3 text-sm hidden"></div></div>';}else if(totalReleased===0&&totalLocked>0&&withdrawalEnabled){html+='<div class="bg-dark-700/60 border border-cyan-500/20 rounded-xl p-4 text-sm"><div class="flex items-start gap-3"><i class="fas fa-lock text-cyan-400 mt-0.5"></i><div><p class="text-white font-semibold mb-1">Fonds encore bloqués</p><p class="text-gray-400">Vos fonds sont bloqués jusqu\'au <span class="text-cyan-400 font-medium">'+(nextUnlock?fmtD(nextUnlock):'—')+'</span>. Le retrait sera disponible une fois la période écoulée.</p></div></div></div>';}else if(totalLocked>0&&!withdrawalEnabled){html+='<div class="bg-yellow-900/20 border border-yellow-500/20 rounded-xl p-4 text-sm text-yellow-400"><i class="fas fa-info-circle mr-2"></i>Les retraits de Réserve Stratégique sont temporairement désactivés.</div>';}if(locked.length>0){html+='<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-lock text-cyan-400"></i> Dépôts en cours ('+locked.length+')</h3><div class="space-y-3">'+locked.map(function(e){var days=daysUntil(e.locked_until);var abo=Math.round((e.abondement_rate||0)*100);var valueEnd=(e.amount||0)*(1+(e.abondement_rate||0));var createdTs=new Date(e.created_at).getTime();var lockedTs=new Date(e.locked_until).getTime();var elapsed=Math.max(0,today.getTime()-createdTs);var total=Math.max(1,lockedTs-createdTs);var pct=Math.min(100,Math.round(elapsed/total*100));return'<div class="bg-dark-700/60 border border-dark-600 rounded-xl p-4"><div class="flex items-start justify-between mb-3"><div><div class="text-sm font-semibold text-white">Période '+(e.period||'—')+'</div><div class="text-xs text-gray-400 mt-0.5">Créé le '+fmtD(e.created_at)+'</div></div><div class="text-right"><div class="text-lg font-bold text-cyan-400">'+fmt$(e.amount)+'</div><div class="text-xs text-emerald-400">→ '+fmt$(valueEnd)+' (+'+abo+'%)</div></div></div><div class="flex justify-between text-xs text-gray-400 mb-1"><span>Progression vers l\'échéance</span><span class="text-cyan-400 font-medium">'+pct+'%</span></div><div class="h-1.5 bg-dark-600 rounded-full overflow-hidden"><div class="h-1.5 rounded-full" style="width:'+pct+'%;background:linear-gradient(90deg,#06b6d4,#0e7490)"></div></div><div class="flex justify-between text-xs text-gray-500 mt-1"><span>Déblocage le <span class="text-orange-400 font-medium">'+fmtD(e.locked_until)+'</span></span><span>'+(days!==null?days+' j restants':'')+'</span></div></div>';}).join('')+'</div></div>';}var hist=entries.filter(function(e){return e.status!=='locked';});if(hist.length>0){html+='<div><h3 class="text-sm font-semibold text-white mb-3 flex items-center gap-2"><i class="fas fa-history text-gray-400"></i> Historique</h3><div class="space-y-1">'+hist.map(function(e){var isRel=e.status==='released'||e.status==='unlocked';var isCan=e.status==='cancelled';var color=isRel?'text-green-400':isCan?'text-red-400':'text-gray-400';var icon=isRel?'fa-check-circle':isCan?'fa-times-circle':'fa-circle';var label=isRel?'Libéré':isCan?'Annulé':e.status;return'<div class="flex items-center justify-between py-2.5 border-b border-dark-700"><div class="flex items-center gap-3"><i class="fas '+icon+' '+color+' text-sm"></i><div><div class="text-sm text-white">Période '+(e.period||'—')+'</div><div class="text-xs text-gray-500">'+fmtD(isRel?e.unlocked_at:e.created_at)+'</div></div></div><div class="text-right"><div class="text-sm font-semibold '+color+'">'+fmt$(e.amount)+'</div><div class="text-xs text-gray-500">'+label+'</div></div></div>';}).join('')+'</div></div>';}if(entries.length===0){html+='<div class="text-center py-12"><div class="w-16 h-16 rounded-full bg-cyan-500/10 flex items-center justify-center mx-auto mb-4"><i class="fas fa-vault text-cyan-500/50 text-2xl"></i></div><p class="text-gray-400 font-medium">Aucune réserve stratégique</p><p class="text-gray-600 text-sm mt-1">Vos commissions alimenteront automatiquement votre réserve.</p></div>';}html+='</div>';el.innerHTML=html;}catch(err){el.innerHTML='<div class="text-red-400 text-center py-12"><i class="fas fa-exclamation-triangle text-2xl mb-3 block"></i>'+(err.message||err.error||'Erreur inconnue')+'</div>';}}
async function submitRSWithdrawal(maxAmount){var inp=document.getElementById('rs-withdraw-amount');var msg=document.getElementById('rs-withdraw-msg');var amount=parseFloat(inp?inp.value:0);if(!inp||!msg)return;if(!amount||amount<=0){msg.textContent='Veuillez saisir un montant valide.';msg.className='mt-3 text-sm text-red-400';msg.classList.remove('hidden');return;}if(amount>maxAmount){msg.textContent='Montant supérieur au solde disponible ('+fmt$(maxAmount)+').';msg.className='mt-3 text-sm text-red-400';msg.classList.remove('hidden');return;}msg.textContent='Traitement en cours...';msg.className='mt-3 text-sm text-gray-400';msg.classList.remove('hidden');try{var r=await api('POST','/members/reserve-strategique/withdraw',{amount:amount});showToast(r.message||'Retrait effectué avec succès !','success');showPage('reserve-strategique');}catch(e){msg.textContent=e.error||e.message||'Erreur lors du retrait.';msg.className='mt-3 text-sm text-red-400';msg.classList.remove('hidden');}}
async function renderServices(el) {
  ...
  try {
    ...
    services = svcRes.data || svcRes.services || [];
  ...
  const pageTitle    = cfg.services_title    || 'Nos Services';
  const pageSubtitle = cfg.services_subtitle || 'Accédez à tous vos outils et plateformes LEADER depuis un seul endroit.';
  ...
  if (!document.getElementById('services-page-styles')) {
    ...
    styleEl.textContent = `
      ...
      .svc-logo-zone {
        ...
        position: relative;
        overflow: hidden;
        /* Fond sombre uniforme — OBLIGATOIRE pour mix-blend-mode:screen */
        background: #02072C !important;
      }
      /* Halo coloré derrière le logo (extrait de la couleur accent du service) */
      ...
      /* Ligne de séparation subtile en bas de la zone logo */
      .svc-logo-zone::after {
        content: '';
      ...
      .svc-logo-img {
        ...
        /* mix-blend-mode:screen efface les rectangles blancs/clairs sur fond sombre */
        mix-blend-mode: screen;
        filter: brightness(1.08) contrast(1.05) saturate(1.1);
      }
      .svc-card:hover .svc-logo-img {
        transform: scale(1.1);
        filter: brightness(1.15) contrast(1.08) saturate(1.15);
      }

      /* Logo fallback lettre */
      .svc-logo-letter {
      ...
      .svc-card:hover .svc-logo-letter { transform: scale(1.1); }
      ...
      .svc-desc {
        font-size: 13px; color: rgba(245,240,232,0.52);
      ...
      .svc-cta-active {
        ...
        color: #fff;
        box-shadow: 0 4px 16px rgba(121,30,21,0.4);
      }
      .svc-cta-active:hover { opacity: .92; transform: scale(1.01); }
      .svc-cta-soon {
      ...
      /* ── Badge overlay "Bientôt" ── */
      .svc-badge-soon {
        position: absolute; top: 12px; right: 12px;
        background: rgba(121,30,21,0.85);
        color: #fff; font-size: 10px; font-weight: 700;
        padding: 4px 10px; border-radius: 999px;
        backdrop-filter: blur(8px);
        animation: pulse-rouge 2.2s ease-in-out infinite;
        z-index: 2;
      }
      ...
      /* ── Transition filtre ── */
      .svc-card { animation: svcReveal .35s ease forwards; opacity: 0; }
      @keyframes svcReveal {
        from { opacity: 0; transform: translateY(16px) scale(0.97); }
        to   { opacity: 1; transform: translateY(0) scale(1); }
      }
      .svc-card.unavailable { animation: svcReveal .35s ease forwards; }
    `;
    document.head.appendChild(styleEl);
  }

  // Couleur par catégorie (palette complète)
  const catColors = {
    'formation':       { bg:'rgba(99,102,241,0.15)',  text:'#818CF8', dot:'#6366F1',  accent:'rgba(99,102,241,0.22)' },
    'financement':     { bg:'rgba(16,185,129,0.15)',  text:'#34D399', dot:'#10B981',  accent:'rgba(16,185,129,0.22)' },
    'finance':         { bg:'rgba(16,185,129,0.15)',  text:'#34D399', dot:'#10B981',  accent:'rgba(16,185,129,0.22)' },
    'marketing':       { bg:'rgba(249,115,22,0.15)',  text:'#FB923C', dot:'#F97316',  accent:'rgba(249,115,22,0.22)'  },
    'technologie':     { bg:'rgba(6,182,212,0.15)',   text:'#22D3EE', dot:'#06B6D4',  accent:'rgba(6,182,212,0.22)'   },
    'communauté':      { bg:'rgba(167,139,250,0.15)', text:'#C4B5FD', dot:'#A78BFA',  accent:'rgba(167,139,250,0.22)' },
    'coaching':        { bg:'rgba(251,191,36,0.15)',  text:'#FCD34D', dot:'#F59E0B',  accent:'rgba(251,191,36,0.22)'  },
    'lifestyle':       { bg:'rgba(236,72,153,0.15)',  text:'#F472B6', dot:'#EC4899',  accent:'rgba(236,72,153,0.22)'  },
    'evenements':      { bg:'rgba(245,158,11,0.15)',  text:'#FBB524', dot:'#F59E0B',  accent:'rgba(245,158,11,0.22)'  },
    'élite':           { bg:'rgba(212,168,67,0.15)',  text:'#DDB957', dot:'#C8A84B',  accent:'rgba(212,168,67,0.25)'  },
    'elite':           { bg:'rgba(212,168,67,0.15)',  text:'#DDB957', dot:'#C8A84B',  accent:'rgba(212,168,67,0.25)'  },
    'entrepreneuriat': { bg:'rgba(239,68,68,0.15)',   text:'#F87171', dot:'#EF4444',  accent:'rgba(239,68,68,0.22)'   },
    'marketplace':     { bg:'rgba(20,184,166,0.15)',  text:'#2DD4BF', dot:'#14B8A6',  accent:'rgba(20,184,166,0.22)'  },
    'immobilier':      { bg:'rgba(168,85,247,0.15)',  text:'#C084FC', dot:'#A855F7',  accent:'rgba(168,85,247,0.22)'  },
    'général':         { bg:'rgba(121,30,21,0.15)',   text:'#C03020', dot:'#791E15',  accent:'rgba(121,30,21,0.22)'   },
  };
  function getCatColor(cat) {
    return catColors[(cat||'général').toLowerCase()] || catColors['général'];
  }

  // Convertit une bg_color (potentiellement blanche/claire) en halo pour la zone logo
  function getAccentFromBg(bgColor) {
    if (!bgColor || bgColor === 'transparent') return 'rgba(121,30,21,0.18)';
    // Si couleur claire (blanc, crème, gris clair) → utiliser rouge LEADER
    const lc = bgColor.toLowerCase().replace(/\s/g,'');
    if (lc === '#fff' || lc === '#ffffff' || lc === 'white' ||
        lc === '#f5f0e8' || lc === '#f0f0f0' || lc === '#eeeeee' ||
        lc === '#e5e7eb' || lc === '#fafafa' || lc === '#f9fafb') {
      return 'rgba(121,30,21,0.2)';
    }
    // Sinon extraire et utiliser la couleur d'origine comme halo
    return bgColor.startsWith('#') ? hexToHalo(bgColor) : bgColor.replace(')', ',0.22)').replace('rgb(', 'rgba(');
  }

  function hexToHalo(hex) {
    hex = hex.replace('#','');
    ...
    if (lum > 0.65) return 'rgba(121,30,21,0.2)'; // couleur trop claire → rouge LEADER
  ...
  function getFiltered() {
    return services.filter(s => {
      const matchCat  = activeCat === 'Tous' || (s.category||'général') === activeCat;
  ...
  function renderLogo(s) {
    const src = s.logo_data_uri || s.logo_url;
    if (src) {
      return `<img src="${src}" alt="${s.name}" class="svc-logo-img" loading="lazy">`;
    }
    return `<div class="svc-logo-letter">${(s.name||'?').charAt(0).toUpperCase()}</div>`;
  }

  function renderCard(s, delay) {
    const isActive = s.status === 'active';
    const cat      = s.category || 'général';
    const cc       = getCatColor(cat);
    const bgColor  = s.bg_color || '#02072C';
    const cta      = isActive
      ? `<a href="${s.url}" target="_blank" rel="noopener noreferrer" class="svc-cta svc-cta-active" onclick="event.stopPropagation()">
           <i class="fas fa-arrow-up-right-from-square"></i> Accéder
         </a>`
      : `<button class="svc-cta svc-cta-soon" disabled>
           <i class="fas fa-clock"></i> Bientôt disponible
         </button>`;

    return `
      <div class="svc-card${isActive ? '' : ' unavailable'}"
           style="animation-delay:${delay * 60}ms"
           ${isActive ? `onclick="window.open('${s.url}','_blank','noopener')"` : ''}>
        ${!isActive ? '<span class="svc-badge-soon"><i class="fas fa-hourglass-half mr-1"></i>Bientôt</span>' : ''}
        <div class="svc-logo-zone" style="--svc-accent:${cc.accent||getAccentFromBg(bgColor)}">
          ${renderLogo(s)}
        </div>
        <div class="svc-body">
          <div class="svc-name">${s.name}</div>
          <div class="svc-cat" style="background:${cc.bg};color:${cc.text}">
            <span style="width:6px;height:6px;border-radius:50%;background:${cc.dot};flex-shrink:0"></span>
  ...
  function renderGrid() {
    ...
    if (!filtered.length) {
      gridEl.innerHTML = `
        <div class="svc-empty">
          <i class="fas fa-search" style="font-size:40px;margin-bottom:16px;opacity:.3"></i>
          <p style="font-size:15px;font-weight:600;color:rgba(255,255,255,0.4)">Aucun service trouvé</p>
          <p style="font-size:13px;margin-top:6px">Essayez un autre filtre ou terme de recherche</p>
        </div>`;
      return;
    }
    gridEl.innerHTML = filtered.map((s,i) => renderCard(s, i)).join('');
  }

  function renderPills() {
    return cats.map(cat => {
      const count = cat === 'Tous' ? services.length : (catCounts[cat]||0);
      return `<button class="svc-pill${activeCat===cat?' active':''}"
                onclick="window._svcSetCat('${cat}')">${cat}<span class="svc-pill-count">${count}</span></button>`;
    }).join('');
  }
  ...
  el.innerHTML = `
    <div class="space-y-0">
      ...
      <div class="svc-hero">
        <div style="position:relative;z-index:1">
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
            <div style="width:44px;height:44px;border-radius:14px;background:linear-gradient(135deg,#791E15,#5A1510);
                        display:flex;align-items:center;justify-content:center;flex-shrink:0;
                        box-shadow:0 8px 24px rgba(121,30,21,0.4)">
              <i class="fas fa-th-large" style="color:#fff;font-size:18px"></i>
            </div>
            <div>
              <h1 style="font-size:26px;font-weight:900;color:#fff;line-height:1.2">${pageTitle}</h1>
              <p style="font-size:13px;color:rgba(245,240,232,0.5);margin-top:2px">${pageSubtitle}</p>
            </div>
          </div>

          <!-- Stats rapides -->
          <div style="display:flex;align-items:center;gap:16px;margin-top:20px;flex-wrap:wrap">
            <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:rgba(245,240,232,0.6)">
              <span style="width:8px;height:8px;border-radius:50%;background:#10B981;box-shadow:0 0 6px #10B981"></span>
              <span><strong style="color:#fff">${activeCount}</strong> service${activeCount>1?'s':''} actif${activeCount>1?'s':''}</span>
            </div>
            <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:rgba(245,240,232,0.6)">
              <span style="width:8px;height:8px;border-radius:50%;background:#791E15"></span>
              <span><strong style="color:#fff">${services.length - activeCount}</strong> à venir</span>
            </div>
            <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:rgba(245,240,232,0.6)">
              <i class="fas fa-layer-group" style="color:#791E15;font-size:11px"></i>
              <span><strong style="color:#fff">${cats.length - 1}</strong> catégorie${(cats.length-1)>1?'s':''}</span>
            </div>
          </div>
          ...
          <div style="margin-top:20px;display:flex;align-items:center;gap:10px">
            <div style="position:relative;flex:1;max-width:340px">
              ...
              <input id="svc-search" type="text" placeholder="Rechercher un service…"
                class="svc-search" style="padding-left:36px"
                oninput="window._svcSearch(this.value)">
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- ── Pills catégories ── -->
      <div id="svc-pills" class="svc-pills">${renderPills()}</div>
  ...
  // Interactions
  window._svcSetCat = function(cat) {
    activeCat = cat;
    document.getElementById('svc-pills').innerHTML = renderPills();
    renderGrid();
  };
  window._svcSearch = function(q) {
  ...
  <div class="max-w-2xl space-y-5">
    ...
    <div id="ptab-panel-payment" class="hidden">
      <div class="bg-dark-800 rounded-2xl border border-dark-600 p-6 space-y-4" id="payout-section">
        ...
        <button onclick="savePayoutInfo()" id="payout-save-btn"
          class="bg-dark-700 hover:bg-dark-600 text-white border border-dark-500 font-bold px-6 py-2.5 rounded-xl transition text-sm">
          <i class="fas fa-save mr-2"></i>Sauvegarder mes coordonnées
        </button>
      </div>
...
async function loadPayoutInfo() {
  try {
    const res = await apiMember('GET', '/payout-info');
    const p = res.payout || {};
...
async function savePayoutInfo(btn) {
  ...
  try {
    await apiMember('PUT', '/payout-info', payload);
    window._memberPayoutInfo = { ...window._memberPayoutInfo, ...payload };
    showToast('Coordonnées de paiement sauvegardées !', 'success');
  } catch(e) {
    showToast(e.error || 'Erreur lors de la sauvegarde', 'error');
...
// État pagination tickets
_supportPage = 1, _supportPerPage = 10, _supportStatus = '';
_supportCurrentTicketId = null;
_supportFaqCategory = '';
