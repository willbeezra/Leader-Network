// Script de correction complète du flux d'achat de licence
// Corrige tous les bugs identifiés dans l'analyse
'use strict';
const fs = require('fs');
...
// ══════════════════════════════════════════════════════════════════════════════
// FIX #1 : renderLicense() → async function
// Sans async, le await interne est évalué comme une variable undefined → crash
// silencieux → le bouton semble ne pas répondre / pas de contenu rendu
// ══════════════════════════════════════════════════════════════════════════════
replace(
  'function renderLicense(e){e.innerHTML=\'<div class="flex items-center justify-center py-16"><div class="loader"></div></div>\';try{const t=await api',
  'async function renderLicense(e){e.innerHTML=\'<div class="flex items-center justify-center py-16"><div class="loader"></div></div>\';try{const t=await api',
  'renderLicense → async'
);
...
replace(
  ...
  // NEW : ouvre le wizard IMMÉDIATEMENT avec un loader, charge les données en arrière-plan
  `async function orderLicense(){
  ...
  try{
    ...
    if(licCfg?.license_price){_wizardData.totalAmt=licCfg.license_price;_wizardData.licensePrice=licCfg.license_price;}
    if(pkgData){_gw=_buildGw(pkgData.gateways||{});_adminFeeInfo=pkgData.adminFee||{amount:0,active:false,paid:true};_licPriceForWizard=pkgData.licensePrice||_wizardData.totalAmt;_wizardData.totalAmt=_licPriceForWizard;_wizardData.licensePrice=_licPriceForWizard;}
  }catch(e){}
...
// ══════════════════════════════════════════════════════════════════════════════
replace(
  '<button onclick="wizardStep3()" class="text-sm text-gray-500 hover:text-gray-300 transition"><i class="fas fa-arrow-left mr-1"></i>',
  '<button onclick="_wizardData.licenseOnly?wizardClose():wizardStep3()" class="text-sm text-gray-500 hover:text-gray-300 transition"><i class="fas fa-arrow-left mr-1"></i>',
...
//   - dans licenseOnly : _wizardData.orderId = e.order_id (undefined car e = réponse licence)
//   - après le try : e redevient _gw mais c'est fragile
// Après : renommer la variable interne en '_apiResp' pour clarté absolue
// ══════════════════════════════════════════════════════════════════════════════
replace(
  ...
  // NEW — variable interne _apiResp, pas de conflit avec const e=_gw
...
// ══════════════════════════════════════════════════════════════════════════════
// FIX #5 : wizardStep5() catch — gérer existing_order_id pour licence aussi
// Avant : existing_order_id check → affiche bouton "Soumettre preuve" qui appelle
//   wizardProofOnly() qui hardcode les packages. Pour licenseOnly, il faut
//   appeler le bon endpoint.
// Après : si licenseOnly + existing_order_id → stocker licenseId et continuer
//   normalement (réutiliser la licence existante en pending)
// ══════════════════════════════════════════════════════════════════════════════
replace(
...
console.log(`\n✅ ${changes} correction(s) appliquée(s)`);
